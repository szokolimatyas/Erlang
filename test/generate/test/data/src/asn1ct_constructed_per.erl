-file("asn1ct_constructed_per.erl", 1).
...
-file("asn1ct_constructed_per.erl", 33).

-import(asn1ct_gen, [emit/1, get_record_name_prefix/1]).

-type(type_name()::any()).

-spec(gen_encode_set(Gen,TypeName,#type{}) -> ok when Gen::#gen{},TypeName::type_name()).

gen_encode_set(Gen,TypeName,D) ->
    gen_encode_constructed(Gen,TypeName,D).

-spec(gen_encode_sequence(Gen,TypeName,#type{}) -> ok when Gen::#gen{},TypeName::type_name()).
...
gen_encode_constructed_imm(Gen,Typename,#type{} = D) ->
    ...
    ExternalImm ++ MatchImm ++ ExtImm ++ ObjSetImm ++ asn1ct_imm:enc_append([ImmSetExt] ++ ImmOptionals ++ ImmBody).
...
enc_extensions(#gen{pack = map},Value,ExtPos,NumExt,Aligned) ->
    Vars = [{var,lists:concat(["Input@", Pos])} || Pos <- lists:seq(ExtPos,ExtPos + NumExt - 1)],
    Undefined = atom_to_list(asn1__MISSING_IN_MAP),
...
enc_table(Gen,#simpletableattributes{objectsetname = ObjectSet,c_name = AttrN,c_index = N,usedclassfield = UniqueFieldName,uniqueclassfield = UniqueFieldName,valueindex = ValueIndex0},_) ->
    {Module,ObjSetName} = ObjectSet,
    #typedef{typespec = #'ObjectSet'{gen = MustGen}} = asn1_db:dbget(Module,ObjSetName),
    case MustGen of
        true->
...
enc_map_match(#gen{pack = map},Cs0) ->
    ...
    M = [[quote_atom(Name), ":=", lists:concat([Var0, "@", Order])] || #'ComponentType'{prop = mandatory,name = Name,textual_order = Order} <- Cs],
    Mand = case M of
        []->
            [];
        [_| _]->
    ...
    Os0 = [{Name,Order} || #'ComponentType'{prop = Prop,name = Name,textual_order = Order} <- Cs,Prop =/= mandatory],
    {var,Val} = make_var(val),
    F = fun ({Name,Order})->
        Var = lists:concat([Var0, "@", Order]),
        P0 = ["case ", Val, " of\n  #{", quote_atom(Name), ":=", Var, "_0} -> ", Var, "_0;\n  _ -> ", atom_to_list(asn1__MISSING_IN_MAP), "\nend"],
        P = lists:flatten(P0),
        {assign,{var,Var},P} end,
    Os = [(F(O)) || O <- Os0],
    Mand ++ Os.
...
gen_encode_extaddgroup(#gen{pack = record},CompList) ->
    case extgroup_pos_and_length(CompList) of
        ...
        {extgrouppos,ExtGroupPosLenList}->
            _ = [(gen_encode_eag_record(G)) || G <- ExtGroupPosLenList],
            ok
    end;
...
test_for_mandatory([Mand| _],Group) ->
    emit([{next,val}, " = case ", {curr,val}, " of", nl, "#{", quote_atom(Mand), ":=_} -> ", {curr,val}, "#{", {asis,Group}, "=>", {curr,val}, "};", nl, "#{} -> ", {curr,val}, nl, "end,", nl]),
    asn1ct_name:new(val);
...
gen_encode_eag_record({ActualPos,VirtualPos,Len}) ->
    ...
    emit([{next,val}, " = case ", Expr, " of", nl, "false -> setelement(", {asis,ActualPos + 1}, ", ", {curr,val}, ", asn1_NOVALUE);", nl, "true -> setelement(", {asis,ActualPos + 1}, ", ", {curr,val}, ", {extaddgroup,", Elements, "})", nl, "end,", nl]),
    asn1ct_name:new(val).
...
gen_dec_constructed_imm(Erule,Typename,#type{} = D) ->
    {CompList,TableConsInfo} = case D#type.def of
    ...
    ObjSetInfo = case TableConsInfo of
        #simpletableattributes{objectsetname = ObjectSet,c_name = AttrN,usedclassfield = UniqueFieldName,uniqueclassfield = UniqueFieldName,valueindex = ValIndex}->
    ...
    EmitComp = gen_dec_components_call(Erule,Typename,CompList,DecObjInf,Ext,length(Optionals)),
    EmitObjSets = gen_dec_objsets_fun(Erule,ObjSetInfo),
    EmitPack = fun (_)->
        gen_dec_pack(Erule,Typename,CompList) end,
    ...
    fun ({AccTerm,AccBytes})->
        ...
        case {AccTerm,AccBytes} of
            ...
            {[{ObjSet,LeadingAttr,Term}],ListOfOpenTypes}->
                ...
                _ = [begin gen_dec_open_type(Gen,ValueMatch,ObjSet,LeadingAttr,T),
                emit([com, nl]) end || T <- ListOfOpenTypes],
                ok
        end end.
...
gen_dec_do_pack(#gen{pack = map},_,CompList0) ->
    ...
    Zipped0 = zip_components(CompList,asn1ct_name:all(term)),
    Zipped = textual_order(Zipped0),
    PF = fun ({#'ComponentType'{prop = 'OPTIONAL'},_})->
        false;({_,_})->
        true end,
    {Mandatory,Optional} = lists:partition(PF,Zipped),
    L = [[{asis,Name}, "=>", {var,Var}] || {#'ComponentType'{name = Name},Var} <- Mandatory],
    emit([{curr,res}, " = #{", lists:join(",",L), "}"]),
...
gen_dec_map_optional([{#'ComponentType'{name = Name},Var}| T]) ->
    asn1ct_name:new(res),
    emit([com, nl, {curr,res}, " = case ", {var,Var}, " of", nl, "  asn1_NOVALUE -> ", {prev,res}, ";", nl, "  _ -> ", {prev,res}, "#{", {asis,Name}, "=>", {var,Var}, "}", nl, "end"]),
...
gen_dec_open_type(Erule,Val,{Xmod,Xtype},LeadingAttr,{_,{Name,RestFieldNames},Term,TmpTerm,Prop}) ->
    ...
    Key = erlang:md5(term_to_binary({decode,ObjSet,RestFieldNames,Prop,Extensible})),
    Gen = fun (_Fd,N)->
        dec_objset_optional(N,Prop),
        dec_objset(Erule,N,ObjSet,RestFieldNames,Typename),
        dec_objset_default(N,Name,LeadingAttr,Extensible) end,
...
dec_objset_optional(_N,mandatory) ->
    ok.
...
dec_objset_default(N,C,LeadingAttr,false) ->
    emit([{asis,N}, "(Bytes, Id) ->", nl, "exit({'Type not compatible with table constraint',{{componen" "t,", {asis,C}, "},{value,Bytes},{unique_name_and_value,", {asis,LeadingAttr}, ",Id}}}).", nl, nl]);
dec_objset_default(N,_,_,true) ->
    emit([{asis,N}, "(Bytes, Id) ->", nl| case asn1ct:use_legacy_types() of
        false->
...
dec_objset_2(Erule,Obj,RestFields0,Typename) ->
    case Obj of
        ...
        #typedef{name = {constructed,bif},typespec = Type} = Def->
            ...
            Gen = fun (_Fd,Name)->
                gen_dec_obj(Erule,Name,Typename,Type) end,
...
gen_encode_choice_imm(Erule,TopType,#type{def = {'CHOICE',CompList}}) ->
    ...
    [{assign,{expr,"{ChoiceTag,ChoiceVal}"},"Val"}| asn1ct_imm:per_enc_choice({var,"ChoiceTag"},Cs,Aligned)].

gen_decode_choice(Erules,Typename,D)
...
gen_encode_sof_imm(Erule,Typename,SeqOrSetOf,#type{} = D) ->
    ...
    Imm0 = case asn1ct_gen:type(Conttype) of
        ...
        'ASN1_OPEN_TYPE'->
            asn1ct_gen_per:gen_encode_prim_imm({var,"Comp"},#type{def = 'ASN1_OPEN_TYPE'},Aligned)
    end,
    asn1ct_imm:per_enc_sof({var,"Val"},D#type.constraint,'Comp',Imm0,Aligned).
...
do_gen_decode_sof(Erules,Typename,SeqOrSetOf,D) ->
    {_SeqOrSetOf,ComponentType} = D#type.def,
    SizeConstraint = asn1ct_imm:effective_constraint(bitstring,D#type.constraint),
    ...
    Gen = fun (_Fd,Name)->
        gen_decode_sof_components(Erules,Name,Typename,SeqOrSetOf,ComponentType) end,
    F = asn1ct_func:call_gen("dec_components",Key,Gen),
    emit([",", nl, {asis,F}, "(", Num, ", ", Buf, ObjFun, ", [])"]).

is_aligned(#gen{erule = per,aligned = Aligned}) ->
    Aligned.
...
gen_decode_sof_components(Erule,Name,Typename,SeqOrSetOf,Cont) ->
    {ObjFun,ObjFun_Var} = case Cont#type.tablecinf of
        [{objfun,_}| _R]->
            {", ObjFun",", _"};
        _->
            {"",""}
    end,
    emit([{asis,Name}, "(0, Bytes", ObjFun_Var, ", Acc) ->", nl, "{lists:reverse(Acc),Bytes};", nl]),
    emit([{asis,Name}, "(Num, Bytes", ObjFun, ", Acc) ->", nl, "{Term,Remain} = "]),
    Constructed_Suffix = asn1ct_gen:constructed_suffix(SeqOrSetOf,Cont#type.def),
    ...
    case asn1ct_gen:type(Conttype) of
        ...
        #'Externaltypereference'{} = Etype->
            ...
            emit([com, nl]);
        'ASN1_OPEN_TYPE'->
            asn1ct_gen_per:gen_dec_prim(Erule,#type{def = 'ASN1_OPEN_TYPE'},"Bytes"),
            emit([com, nl]);
        _->
...
extgroup_pos_and_length(CompList)
    when is_list(CompList)->
    {extgrouppos,[]};
...
create_optionality_table(Cs) ->
    IsOptional = fun ('OPTIONAL')->
        true;({'DEFAULT',_})->
        true;(_)->
        false end,
    OptionalsElNum = [TO || #'ComponentType'{prop = O,textual_order = TO} <- Cs,IsOptional(O)],
    {Table,_} = lists:mapfoldl(fun (X,Num)->
        {{Num,X},Num + 1} end,1,lists:sort(OptionalsElNum)),
    Table.
...
add_textual_order1(Cs,NumIn) ->
    lists:mapfoldl(fun (C = #'ComponentType'{},Num)->
        {C#'ComponentType'{textual_order = Num},Num + 1};(OtherMarker,Num)->
        {OtherMarker,Num} end,NumIn,Cs).
...
gen_enc_components_call1(Gen,TopType,[C| Rest],DynamicEnc,Ext) ->
    #'ComponentType'{name = Cname,typespec = Type,prop = Prop,textual_order = Num} = C,
    InnerType = asn1ct_gen:get_inner(Type#type.def),
...
def_values(#type{def = #'Externaltypereference'{module = Mod,type = Type}},Def) ->
    ...
    def_values(T,Def);
def_values(#type{def = {'BIT STRING',[]}},Bs)
    ...
    case asn1ct:use_legacy_types() of
        ...
        true->
            ...
            IntBs = lists:foldl(fun (B,A)->
                A bsl 1 bor B end,0,lists:reverse(ListBs)),
            Sz = bit_size(Bs),
...
def_values(#type{def = {'BIT STRING',[_| _] = Ns}},List)
    when is_list(List)->
                ...
                'ASN1_OPEN_TYPE'->
                    case Type#type.def of
                        #'ObjectClassFieldType'{type = OpenType}->
                            asn1ct_gen_per:gen_encode_prim_imm(Element,#type{def = OpenType},Aligned);
                        _->
                            asn1ct_gen_per:gen_encode_prim_imm(Element,Type,Aligned)
                    end;
                {constructed,bif}->
...
enc_var_type_call(Erule,Name,RestFieldNames,#type{tablecinf = TCI},Fun,Val) ->
...
enc_objset_imm(Erule,TypeName,Component,ObjSet,RestFieldNames,Extensible) ->
    Aligned = is_aligned(Erule),
    E = {error,fun ()->
...
enc_obj(Erule,Obj,TypeName,RestFieldNames0,Aligned) ->
    ...
    case Obj of
        #typedef{name = {constructed,bif},typespec = Type} = Def->
            ...
            Gen = fun (_Fd,Name)->
                gen_enc_obj(Erule,Name,TypeName,Type) end,
...
gen_dec_components_call(Gen,TopType,{Root1,ExtList,Root2} = CL,DecInfObj,Ext,NumberOfOptionals) ->
    ...
    Init = {ignore,fun (_)->
        {[],[]} end},
    {EmitRoot,Tpos} = gen_dec_comp_calls(Root1 ++ Root2,Gen,TopType,OptTable,DecInfObj,noext,NumberOfOptionals,1,[]),
    EmitGetExt = gen_dec_get_extension(Gen),
    {extgrouppos,ExtGroupPosLen} = extgroup_pos_and_length(CL),
    NewExtList = wrap_extensionAdditionGroups(ExtList,ExtGroupPosLen),
    {EmitExts,_} = gen_dec_comp_calls(NewExtList,Gen,TopType,OptTable,DecInfObj,Ext,NumberOfOptionals,Tpos,[]),
...
gen_dec_components_call(Erule,TopType,CompList,DecInfObj,Ext,NumberOfOptionals) ->
    OptTable = create_optionality_table(CompList),
    Init = {ignore,fun (_)->
        {[],[]} end},
    {Cs,_} = gen_dec_comp_calls(CompList,Erule,TopType,OptTable,DecInfObj,Ext,NumberOfOptionals,1,[]),
    [Init| Cs].

gen_dec_get_extension(Erule) ->
    Imm0 = asn1ct_imm:per_dec_extension_map(is_aligned(Erule)),
    E = fun (Imm,St)->
        emit([nl, "%% Extensions", nl, "{Extensions,", {next,bytes}, "} = ", "case Ext of", nl, "0 -> {<<>>,", {curr,bytes}, "};", nl, "1 ->", nl]),
        BytesVar = asn1ct_gen:mk_var(asn1ct_name:curr(bytes)),
...
gen_dec_comp_calls([C| Cs],Erule,TopType,OptTable,DecInfObj,Ext,NumberOfOptionals,Tpos,Acc) ->
    L = gen_dec_comp_call(C,Erule,TopType,Tpos,OptTable,DecInfObj,Ext,NumberOfOptionals),
    gen_dec_comp_calls(Cs,Erule,TopType,OptTable,DecInfObj,Ext,NumberOfOptionals,Tpos + 1,[L| Acc]);
gen_dec_comp_calls([],_,_,_,_,_,_,Tpos,Acc) ->
    {lists:append(lists:reverse(Acc)),Tpos}.

gen_dec_comp_call(Comp,Gen,TopType,Tpos,OptTable,DecInfObj,Ext,NumberOfOptionals) ->
    #'ComponentType'{name = Cname,typespec = Type,prop = Prop,textual_order = TextPos} = Comp,
    Pos = case Ext of
        noext->
            Tpos;
        {ext,Epos,_Enum}->
    ...
    Comment = fun (St)->
        ...
        St end,
    Preamble = case {InnerType,is_mandatory_predef_tab_c(Ext,Prop,DecInfObj)} of
        {{typefield,_},true}->
            fun (St)->
        ...
        _->
            case Type of
                #type{def = #'SEQUENCE'{extaddgroup = GroupNum,components = CompList}}
    ...
    {Pre,Post} = comp_call_pre_post(Gen,Ext,Prop,Pos,Type,TextPos,OptTable,NumberOfOptionals,Ext),
...
comp_call_pre_post(_Gen,noext,mandatory,_,_,_,_,_,_) ->
    {[],[]};
comp_call_pre_post(_Gen,noext,Prop,_,Type,TextPos,OptTable,NumOptionals,Ext) ->
...
comp_call_pre_post(Gen,{ext,_,_},Prop,Pos,Type,_,_,_,Ext) ->
    {[fun (St)->
        ...
        St end],[extadd_group_fun(Gen,Prop,Type,Ext)]}.

extadd_group_fun(#gen{pack = record},Prop,Type,Ext) ->
    fun (St)->
        emit([";", nl, "_  ->", nl, "{"]),
        case Type of
            #type{def = #'SEQUENCE'{extaddgroup = Number2,components = ExtGroupCompList2}}
...
extadd_group_fun(#gen{pack = map},Prop,Type,Ext) ->
    fun (St)->
        emit([";", nl, "_  ->", nl, "{"]),
        case Type of
            #type{def = #'SEQUENCE'{extaddgroup = Number2,components = Comp}}
                when is_integer(Number2)->
...
is_mandatory_predef_tab_c(noext,mandatory,{"got objfun through args","ObjFun"}) ->
    true;
...
dec_map_extaddgroup_no_val(Ext,Type,Comp) ->
    L0 = [(dec_map_extaddgroup_no_val_1(N,P,Ext,Type)) || #'ComponentType'{name = N,prop = P} <- Comp],
    L = [E || E <- L0,E =/= []],
    emit(["#{", lists:join(",",L), "}"]).
...
gen_dec_line_imm(Erule,TopType,Comp,DecInfObj,Pre) ->
    ...
    Post = fun ({SaveBytes,Finish})->
        {AccTerm,AccBytes} = Finish(),
        #'ComponentType'{name = Cname} = Comp,
        case DecInfObj of
            {Cname,ObjSet}->
                ObjSetRef = case ObjSet of
                    {deep,OSName,_,_}->
                        OSName;
                    _->
                        ObjSet
                end,
                {AccTerm ++ [{ObjSetRef,Cname,asn1ct_gen:mk_var(asn1ct_name:curr(term))}],AccBytes ++ SaveBytes};
            _->
                {AccTerm,AccBytes ++ SaveBytes}
        end end,
    [Pre, Decode, {safe,Post}].

gen_dec_line_open_type(Erule,{ext,Ep,_},Pos)
    when Pos >= Ep->
...
gen_dec_line_open_type(_,_,_) ->
    {safe,fun (St)->
        {asn1ct_gen:mk_var(asn1ct_name:curr(bytes)),fun ()->
            St end} end}.

gen_dec_line_special(Erule,{typefield,_},_TopType,Comp,DecInfObj) ->
    #'ComponentType'{name = Cname,typespec = Type,prop = Prop} = Comp,
    fun ({_BytesVar,PrevSt})->
        case DecInfObj of
            false->
                {Name,RestFieldNames} = (Type#type.def)#'ObjectClassFieldType'.fieldname,
                Imm = asn1ct_imm:per_dec_open_type(is_aligned(Erule)),
                BytesVar = asn1ct_gen:mk_var(asn1ct_name:curr(bytes)),
            ...
            {"got objfun through args","ObjFun"}->
                if Prop =:= mandatory ->
                    ...
                    emit([nl, "    {", {curr,tmpterm}, ", ", {curr,tmpbytes}, "} ="]) end,
                {Name,RestFieldNames} = (Type#type.def)#'ObjectClassFieldType'.fieldname,
                Imm = asn1ct_imm:per_dec_open_type(is_aligned(Erule)),
                BytesVar = asn1ct_gen:mk_var(asn1ct_name:curr(bytes)),
                asn1ct_imm:dec_code_gen(Imm,BytesVar),
            ...
            _->
                ...
                {[{Cname,RefedFieldName,asn1ct_gen:mk_var(asn1ct_name:curr(term)),asn1ct_gen:mk_var(asn1ct_name:curr(tmpterm)),Prop}],PrevSt}
        end end;
    ...
    case gen_dec_line_other(Gen,Atype,TopType,Comp) of
        Fun
            ...
            fun ({BytesVar,PrevSt})->
                ...
                {[],PrevSt} end;
        Imm0->
            {imm,Imm0,fun (Imm,{BytesVar,PrevSt})->
                asn1ct_imm:dec_code_gen(Imm,BytesVar),
                gen_dec_line_dec_inf(Gen,Comp,DecInfObj),
                {[],PrevSt} end}
    end.
...
gen_dec_line_other(Erule,Atype,TopType,Comp) ->
    ...
    case asn1ct_gen:type(Atype) of
        #'Externaltypereference'{} = Etype->
            fun (BytesVar)->
                asn1ct_gen_per:gen_dec_external(Etype,BytesVar) end;
        {primitive,bif}->
        ...
        {constructed,bif}->
            ...
            case Type#type.tablecinf of
                [{objfun,_}| _R]->
                    fun (BytesVar)->
                        emit([{asis,DecFunc}, "(", BytesVar, ", ObjFun)"]) end;
                _->
                    fun (BytesVar)->
                        emit([{asis,DecFunc}, "(", BytesVar, ")"]) end
            end
    end.

gen_enc_choice(Erule,TopType,{Root,Exts},Ext) ->
    Constr = choice_constraint(Root),
    gen_enc_choices(Root,Erule,TopType,0,Constr,Ext) ++ gen_enc_choices(Exts,Erule,TopType,0,ext,Ext);
gen_enc_choice(Erule,TopType,{Root,Exts,[]},Ext) ->
    gen_enc_choice(Erule,TopType,{Root,Exts},Ext);
gen_enc_choice(Erule,TopType,Root,Ext)
    when is_list(Root)->
    Constr = choice_constraint(Root),
...
gen_enc_choices([H| T],Erule,TopType,Pos,Constr,Ext) ->
    ...
    Aligned = is_aligned(Erule),
    EncObj = case asn1ct_gen:get_constraint(Type#type.constraint,componentrelation) of
        no->
            case Type#type.tablecinf of
                [{objfun,_}| _]->
                    {"got objfun through args",{var,"ObjFun"}};
    ...
    Tag = case {Ext,Constr} of
        ...
        {{ext,_,_},ext}->
            [{put_bits,1,1,[1]}| asn1ct_imm:per_enc_small_number(Pos,Aligned)];
        {{ext,_,_},_}->
            [{put_bits,0,1,[1]}| asn1ct_imm:per_enc_integer(Pos,Constr,Aligned)]
    end,
...
flat_complist({Rl1,El,Rl2}) ->
    Rl1 ++ El ++ Rl2;
flat_complist({Rl,El}) ->
    Rl ++ El;
flat_complist(CompList) ->
    CompList.

wrap_extensionAdditionGroups(ExtCompList,ExtGroupPosLen) ->
    wrap_eags(ExtCompList,ExtGroupPosLen,0,0).

wrap_eags([{'ExtensionAdditionGroup',_Number}| T0],[{ActualPos,_,_}| Gs],_ExtAddGroupDiff,ExtGroupNum) ->
    {ExtGroupCompList,['ExtensionAdditionGroupEnd'| T]} = lists:splitwith(fun (#'ComponentType'{})->
        true;(_)->
        false end,T0),
    Name = list_to_atom(lists:concat(["ExtAddGroup", ExtGroupNum + 1])),
    Seq = #type{def = #'SEQUENCE'{extaddgroup = ExtGroupNum + 1,components = ExtGroupCompList}},
    Comp = #'ComponentType'{name = Name,typespec = Seq,textual_order = ActualPos,prop = 'OPTIONAL'},
    [Comp| wrap_eags(T,Gs,length(ExtGroupCompList) - 1,ExtGroupNum + 1)];
wrap_eags([#'ComponentType'{textual_order = Tord} = H| T],ExtAddGrpLenPos,ExtAddGroupDiff,ExtGroupNum)
