...
parse(File0,Tokens0) ->
    try do_parse(Tokens0) of 
        {ok,#module{}} = Result->
            ...
            throw:{asn1_error,Fun}
                when is_function(Fun,0)->
                handle_parse_error(File0,Fun());
            throw:{asn1_error,{parse_error,Tokens}}->
                handle_parse_error(File0,Tokens) after clean_process_dictionary() end.
...
parse_ModuleDefinition([{typereference,L1,ModuleIdentifier}| Rest0]) ->
    ...
    {_DefinitiveIdentifier,Rest02} = case Rest0 of
        [{'{',_}| _Rest01]->
    ...
    {TagDefault,Rest2} = case Rest of
        [{'EXPLICIT',_L3}, {'TAGS',_L4}| Rest1]->
            put(tagdefault,'EXPLICIT'),
            {'EXPLICIT',Rest1};
        [{'IMPLICIT',_L3}, {'TAGS',_L4}| Rest1]->
            put(tagdefault,'IMPLICIT'),
            {'IMPLICIT',Rest1};
        [{'AUTOMATIC',_L3}, {'TAGS',_L4}| Rest1]->
            put(tagdefault,'AUTOMATIC'),
            {'AUTOMATIC',Rest1};
        Rest1->
            put(tagdefault,'EXPLICIT'),
            {'EXPLICIT',Rest1}
    end,
    {ExtensionDefault,Rest3} = case Rest2 of
        [{'EXTENSIBILITY',_L5}, {'IMPLIED',_L6}| Rest21]->
            put(extensiondefault,'IMPLIED'),
            {'IMPLIED',Rest21};
        _->
            put(extensiondefault,undefined),
            {undefined,Rest2}
    end,
    case Rest3 of
        [{'::=',_L7}, {'BEGIN',_L8}| Rest4]->
            ...
            {#module{pos = L1,name = ModuleIdentifier,defid = [],tagdefault = TagDefault,extensiondefault = ExtensionDefault,exports = Exports,imports = {imports,Imports}},Rest6};
        _->
            parse_error(Rest3)
    end;
...
parse_Reference([Tref1 = {typereference,_,_}, {'.',_}, Tref2 = {typereference,_,_}, {'{',_L2}, {'}',_L3}| Rest]) ->
...
parse_SymbolsFromModuleList(Tokens,Acc) ->
    ...
    try parse_SymbolsFromModule(Rest) of 
        {Sl,_Rest2}
            when is_record(Sl,'SymbolsFromModule')->
            parse_SymbolsFromModuleList(Rest,[SymbolsFromModule| Acc])
        catch
            throw:{asn1_error,_}->
                {lists:reverse(Acc,[SymbolsFromModule]),Rest} end.

parse_SymbolsFromModule(Tokens) ->
    SetRefModuleName = fun (N)->
        fun (X)
            when is_record(X,'Externaltypereference')->
            X#'Externaltypereference'{module = N};(X)
            when is_record(X,'Externalvaluereference')->
            X#'Externalvaluereference'{module = N} end end,
    {SymbolList,Rest} = parse_SymbolList(Tokens),
    case Rest of
        [{'FROM',_L1}, {typereference,_,Name} = Tref| [#identifier{}, {',',_}| _] = Rest2]->
            NewSymbolList = lists:map(SetRefModuleName(Name),SymbolList),
            {#'SymbolsFromModule'{symbols = NewSymbolList,module = tref2Exttref(Tref)},Rest2};
        [{'FROM',_L1}, {typereference,_,Name} = Tref| [#identifier{}, {'FROM',_}| _] = Rest2]->
            NewSymbolList = lists:map(SetRefModuleName(Name),SymbolList),
            {#'SymbolsFromModule'{symbols = NewSymbolList,module = tref2Exttref(Tref)},Rest2};
        ...
        [{'FROM',_L1}, {typereference,_,Name} = Tref| [{'{',_}| _] = Rest2]->
            ...
            NewSymbolList = lists:map(SetRefModuleName(Name),SymbolList),
            {#'SymbolsFromModule'{symbols = NewSymbolList,module = tref2Exttref(Tref)},Rest3};
...
parse_Assignment([{typereference,L1,Name}, {'::=',_}| Tokens0]) ->
    Flist = [{type,fun parse_Type/1}, {class,fun parse_ObjectClass/1}],
    case parse_or_tag(Tokens0,Flist) of
        {{type,Type},Tokens}->
            {#typedef{pos = L1,name = Name,typespec = Type},Tokens};
        {{class,Type},Tokens}->
            {#classdef{pos = L1,name = Name,module = resolve_module(Type),typespec = Type},Tokens}
    end;
parse_Assignment([{typereference,_,_}, {'{',_}| _] = Tokens) ->
    Flist = [fun parse_ParameterizedTypeAssignment/1, fun parse_ParameterizedValueSetTypeAssignment/1, fun parse_ParameterizedObjectClassAssignment/1],
    parse_or(Tokens,Flist);
parse_Assignment([{typereference,_,_}| _] = Tokens) ->
    Flist = [fun parse_ObjectSetAssignment/1, fun parse_ValueSetTypeAssignment/1],
    parse_or(Tokens,Flist);
parse_Assignment([#identifier{}, {'{',_}| _] = Tokens) ->
    Flist = [fun parse_ParameterizedValueAssignment/1, fun parse_ParameterizedObjectAssignment/1],
    parse_or(Tokens,Flist);
parse_Assignment([#identifier{}| _] = Tokens) ->
    Flist = [fun parse_ValueAssignment/1, fun parse_ObjectAssignment/1],
    parse_or(Tokens,Flist);
...
parse_or(Tokens,[Fun| Funs],ErrList)
    ...
    try Fun(Tokens) of 
        {_,Rest} = Result
            ...
            throw:{asn1_error,Error}->
                parse_or(Tokens,Funs,[Error| ErrList]) end;
parse_or(_Tokens,[],ErrList) ->
    throw({asn1_error,fun ()->
        prioritize_error(ErrList) end}).

parse_or_tag(Tokens,Flist) ->
    parse_or_tag(Tokens,Flist,[]).

parse_or_tag(Tokens,[{Tag,Fun}| Funs],ErrList)
    when is_function(Fun,1)->
    try Fun(Tokens) of 
        {Parsed,Rest}
            when is_list(Rest)->
            ...
            throw:{asn1_error,Error}->
                parse_or_tag(Tokens,Funs,[Error| ErrList]) end;
...
parse_Type(Tokens) ->
    ...
    case Rest5 of
        [{'(',_}| _]->
            ...
            {Type#type{tag = Tag2,constraint = merge_constraints(Constraints)},Rest6};
        [_| _]->
...
parse_BuiltinType([{'CHOICE',_}, {'{',_}| Rest]) ->
    ...
    case Rest2 of
        [{'}',_}| Rest3]->
            NeedExt =  not lists:keymember('EXTENSIONMARK',1,L0) andalso get(extensiondefault) =:= 'IMPLIED',
            L = case NeedExt of
                true->
                    L0 ++ [#'EXTENSIONMARK'{}];
                false->
...
parse_BuiltinType([{'SEQUENCE',_}, {'{',_}, {'}',_}| Rest]) ->
    {#type{def = #'SEQUENCE'{components = []}},Rest};
...
parse_BuiltinType([{'SEQUENCE',_}, {'{',_}, {'...',Line}, {'!',_}| Rest]) ->
    ...
    case Rest2 of
        [{'}',_}| Rest3]->
            {#type{def = #'SEQUENCE'{components = [#'EXTENSIONMARK'{pos = Line,val = ExceptionIdentification}]}},Rest3};
        _->
...
parse_BuiltinType([{'SEQUENCE',_}, {'{',_}| Rest]) ->
    ...
    case Rest2 of
        [{'}',_}| Rest3]->
            ComponentTypeLists2 = case {[Ext || Ext = #'EXTENSIONMARK'{} <- ComponentTypeLists],get(extensiondefault)} of
                {[],'IMPLIED'}->
                    ComponentTypeLists ++ [#'EXTENSIONMARK'{}];
                _->
                    ComponentTypeLists
            end,
            {#type{def = #'SEQUENCE'{components = ComponentTypeLists2}},Rest3};
        _->
            parse_error(Rest2)
    end;
parse_BuiltinType([{'SEQUENCE',_}, {'OF',_}| [#identifier{}, {'<',_}| _] = Tokens0]) ->
...
parse_BuiltinType([{'SET',_}, {'{',_}, {'...',Line}, {'!',_}| Rest]) ->
    ...
    case Rest2 of
        [{'}',_}| Rest3]->
            {#type{def = #'SET'{components = [#'EXTENSIONMARK'{pos = Line,val = ExceptionIdentification}]}},Rest3};
        _->
            {ComponentTypeLists,Rest3} = parse_ComponentTypeLists2(Rest2,[#'EXTENSIONMARK'{pos = Line}]),
            case Rest3 of
                [{'}',_}| Rest4]->
...
parse_BuiltinType([{'SET',_}, {'{',_}| Rest]) ->
    ...
    case Rest2 of
        [{'}',_}| Rest3]->
            ...
            {#type{def = #'SET'{components = ComponentTypeLists2}},Rest3};
        _->
            parse_error(Rest2)
    end;
parse_BuiltinType([{'SET',_}, {'OF',_}| [#identifier{}, {'<',_}| _] = Tokens0]) ->
    {Type,Tokens} = parse_SelectionType(Tokens0),
...
parse_TypeWithConstraint([{'SEQUENCE',_}| [{'(',_}| _] = Rest0]) ->
    ...
    {#type{def = {'SEQUENCE OF',Type},constraint = merge_constraints([Constraint])},Rest5};
parse_TypeWithConstraint([{'SEQUENCE',_}, {'SIZE',_}| [{'(',_}| _] = Rest0]) ->
    {Constraint,Rest2} = parse_Constraint(Rest0),
    #constraint{c = C} = Constraint,
    Constraint2 = Constraint#constraint{c = {element_set,{'SizeConstraint',C},none}},
    Rest4 = case Rest2 of
        [{'OF',_}, #identifier{}| Rest3]->
            Rest3;
        [{'OF',_}| Rest3]->
...
parse_TypeWithConstraint([{'SET',_}| [{'(',_}| _] = Rest0]) ->
    ...
    {Type,Rest5} = parse_Type(Rest4),
    {#type{def = {'SET OF',Type},constraint = merge_constraints([Constraint])},Rest5};
parse_TypeWithConstraint([{'SET',_}, {'SIZE',_}| [{'(',_}| _] = Rest0]) ->
    ...
    {Type,Rest5} = parse_Type(Rest4),
    {#type{def = {'SET OF',Type},constraint = merge_constraints([Constraint2])},Rest5};
parse_TypeWithConstraint(Tokens) ->
    parse_error(Tokens).

parse_ReferencedType(Tokens) ->
    Flist = [fun parse_ParameterizedType/1, fun parse_DefinedType/1, fun parse_SelectionType/1, fun parse_TypeFromObject/1],
    parse_or(Tokens,Flist).

parse_DefinedType([{typereference,L1,Module}, {'.',_}, {typereference,_,TypeName}| Tokens]) ->
    {#type{def = #'Externaltypereference'{pos = L1,module = Module,type = TypeName}},Tokens};
...
resolve_module(_Type,Current,undefined) ->
    Current;
resolve_module(Type,Current,Imports) ->
    case [Mod || #'SymbolsFromModule'{symbols = S,module = Mod} <- Imports,#'Externaltypereference'{type = T} <- S,Type =:= T] of
        [#'Externaltypereference'{type = Mod}| _]->
            Mod;
        []->
            Current
    end.
...
parse_ConstraintSpec(Tokens) ->
    Flist = [fun parse_GeneralConstraint/1, fun parse_SubtypeConstraint/1],
    parse_or(Tokens,Flist).
...
parse_ExceptionIdentification(Tokens) ->
    Flist = [fun parse_SignedNumber/1, fun parse_DefinedValue/1, fun parse_TypeColonValue/1],
    parse_or(Tokens,Flist).
...
parse_Elements(Tokens) ->
    Flist = [fun parse_ObjectSetElements/1, fun parse_SubtypeElements/1, fun parse_Object/1, fun parse_DefinedObjectSet/1],
    parse_or(Tokens,Flist).

parse_DefinedObjectClass([{typereference,_,ModName}, {'.',_}, {typereference,Pos,Name}| Tokens]) ->
    Ext = #'Externaltypereference'{pos = Pos,module = ModName,type = Name},
    {Ext,Tokens};
...
parse_ObjectClass(Tokens) ->
    Flist = [fun parse_ObjectClassDefn/1, fun parse_DefinedObjectClass/1],
    parse_or(Tokens,Flist).
...
parse_FieldSpec(Tokens0,Acc) ->
    Fl = case Tokens0 of
        [{valuefieldreference,_,_}| _]->
            [fun parse_FixedTypeValueFieldSpec/1, fun parse_VariableTypeValueFieldSpec/1];
        [{typefieldreference,_,_}| _]->
            [fun parse_FixedTypeValueSetFieldSpec/1, fun parse_VariableTypeValueSetFieldSpec/1, fun parse_TypeFieldSpec/1];
        [_| _]->
            parse_error(Tokens0)
    end,
...
parse_TokenOrGroupSpec(Tokens) ->
    Flist = [fun parse_RequiredToken/1, fun parse_OptionalGroup/1],
    parse_or(Tokens,Flist).
...
parse_ObjectAssignment([#identifier{pos = L1,val = ObjName}| Rest]) ->
    ...
    case Rest2 of
        [{'::=',_}| Rest3]->
            ...
            {#typedef{pos = L1,name = ObjName,typespec = #'Object'{classname = Class,def = Object}},Rest4};
        _->
            parse_error(Rest2)
    end.

parse_Object(Tokens) ->
    Flist = [fun parse_ObjectDefn/1, fun parse_DefinedObject/1],
    parse_or(Tokens,Flist).

parse_ObjectDefn(Tokens) ->
    Flist = [fun parse_DefaultSyntax/1, fun parse_DefinedSyntax/1],
    parse_or(Tokens,Flist).
...
parse_Setting(Tokens) ->
    Flist = [{type_tag,fun parse_Type/1}, {value_tag,fun parse_Value/1}, {object_tag,fun parse_Object/1}, {objectset_tag,fun parse_ObjectSet/1}],
...
parse_DefinedObjectSet([{typereference,L1,ModuleName}, {'.',_}, {typereference,L2,ObjSetName}| Rest]) ->
    {{objectset,L1,#'Externaltypereference'{pos = L2,module = ModuleName,type = ObjSetName}},Rest};
parse_DefinedObjectSet([{typereference,L1,ObjSetName}| Rest]) ->
    {{objectset,L1,#'Externaltypereference'{pos = L1,module = resolve_module(ObjSetName),type = ObjSetName}},Rest};
...
parse_ObjectSetAssignment([{typereference,L1,ObjSetName}| Rest]) ->
    ...
    case Rest2 of
        [{'::=',_}| Rest3]->
            ...
            {#typedef{pos = L1,name = ObjSetName,typespec = #'ObjectSet'{class = Class,set = ObjectSet}},Rest4};
        _->
            parse_error(Rest2)
    end.
...
parse_ObjectSetElements(Tokens) ->
    Flist = [fun parse_ObjectSetFromObjects/1, fun parse_ParameterizedObjectSet/1],
    parse_or(Tokens,Flist).
...
parse_ObjectClassFieldType(Tokens) ->
    ...
    case Rest of
        [{'.',_}| Rest2]->
            ...
            OCFT = #'ObjectClassFieldType'{classname = Class,class = Class,fieldname = FieldName},
            {#type{def = OCFT},Rest3};
        _->
            parse_error(Rest)
    end.
...
parse_ReferencedObjects(Tokens) ->
    Flist = [fun parse_DefinedObject/1, fun parse_DefinedObjectSet/1, fun parse_ParameterizedObjectSet/1],
    parse_or(Tokens,Flist).
...
parse_GeneralConstraint(Tokens) ->
    Flist = [fun parse_UserDefinedConstraint/1, fun parse_TableConstraint/1, fun parse_ContentsConstraint/1],
    parse_or(Tokens,Flist).

parse_UserDefinedConstraint([{'CONSTRAINED',_}, {'BY',_}, {'{',_}, {'}',_}| Rest]) ->
    {{constrained_by,[]},Rest};
parse_UserDefinedConstraint([{'CONSTRAINED',_}, {'BY',_}, {'{',_}| Rest]) ->
...
parse_UserDefinedConstraintParameter(Tokens0,Acc) ->
    Flist = [fun parse_GovernorAndActualParameter/1, fun parse_ActualParameter/1],
...
parse_TableConstraint(Tokens) ->
    Flist = [fun parse_ComponentRelationConstraint/1, fun parse_SimpleTableConstraint/1],
    parse_or(Tokens,Flist).
...
parse_Governor(Tokens) ->
    Flist = [fun parse_Type/1, fun parse_DefinedObjectClass/1],
    parse_or(Tokens,Flist).

parse_ActualParameter(Tokens) ->
    Flist = [fun parse_Type/1, fun parse_Value/1, fun parse_ValueSet/1, fun parse_DefinedObjectClass/1, fun parse_Object/1, fun parse_ObjectSet/1],
    parse_or(Tokens,Flist).
...
parse_ParameterizedTypeAssignment([{typereference,L1,Name}| Rest]) ->
    ...
    case Rest2 of
        [{'::=',_}| Rest3]->
            ...
            {#ptypedef{pos = L1,name = Name,args = ParameterList,typespec = Type},Rest4};
        _->
            parse_error(Rest2)
    end.
...
parse_ParameterizedValueAssignment([#identifier{pos = L1,val = Name}| Rest]) ->
    ...
    case Rest3 of
        [{'::=',_}| Rest4]->
            ...
            {#pvaluedef{pos = L1,name = Name,args = ParameterList,type = Type,value = Value},Rest5};
        _->
            parse_error(Rest3)
    end.
...
parse_ParameterizedValueSetTypeAssignment([{typereference,L1,Name}| Rest]) ->
    ...
    case Rest3 of
        [{'::=',_}| Rest4]->
            ...
            {#pvaluesetdef{pos = L1,name = Name,args = ParameterList,type = Type,valueset = ValueSet},Rest5};
        _->
            parse_error(Rest3)
    end.
...
parse_ParameterizedObjectClassAssignment([{typereference,L1,Name}| Rest]) ->
    ...
    case Rest2 of
        [{'::=',_}| Rest3]->
            ...
            {#ptypedef{pos = L1,name = Name,args = ParameterList,typespec = Class},Rest4};
        _->
            parse_error(Rest2)
    end.
...
parse_ParameterizedObjectAssignment([#identifier{pos = L1,val = Name}| Rest]) ->
    ...
    case Rest3 of
        [{'::=',_}| Rest4]->
            ...
            {#pobjectdef{pos = L1,name = Name,args = ParameterList,class = Class,def = Object},Rest5};
        _->
            parse_error(Rest3)
    end.
...
parse_Parameter(Tokens) ->
    Flist = [fun parse_ParamGovAndRef/1, fun parse_Reference/1],
    parse_or(Tokens,Flist).
...
parse_ParamGovernor(Tokens) ->
    Flist = [fun parse_Governor/1, fun parse_Reference/1],
    parse_or(Tokens,Flist).

parse_SimpleDefinedType([{typereference,L1,ModuleName}, {'.',_}, {typereference,_,TypeName}| Rest]) ->
    {#'Externaltypereference'{pos = L1,module = ModuleName,type = TypeName},Rest};
parse_SimpleDefinedType([Tref = {typereference,_,_}| Rest]) ->
    {tref2Exttref(Tref),Rest};
parse_SimpleDefinedType(Tokens) ->
    parse_error(Tokens).

parse_SimpleDefinedValue([{typereference,L1,ModuleName}, {'.',_}, #identifier{val = Value}| Rest]) ->
    {{simpledefinedvalue,#'Externalvaluereference'{pos = L1,module = ModuleName,value = Value}},Rest};
...
not_allowed_word(Name) ->
    lists:member(Name,["BIT", "BOOLEAN", "CHARACTER", "CHOICE", "EMBEDDED", "END", "ENUMERATED", "EXTERNAL", "FALSE", "INSTANCE", "INTEGER", "INTERSECTION", "MINUS-INFINITY", "NULL", "OBJECT", "OCTET", "PLUS-INFINITY", "REAL", "SEQUENCE", "SET", "TRUE", "UNION"]).
...
parse_ExtensionAndException([{'...',L}| Tokens0]) ->
    {[#'EXTENSIONMARK'{pos = L}],case Tokens0 of
        [{'!',_}| Tokens1]->
...
parse_ExtensionAdditionAlternativesList([{',',_}| Tokens1] = Tokens0,Acc) ->
    try parse_ExtensionAdditionAlternative(Tokens1) of 
        {ExtAddAlt,Tokens2}->
            ...
            throw:{asn1_error,_}->
                {lists:append(lists:reverse(Acc)),Tokens0} end;
...
parse_ComponentTypeList([{',',_}| [#identifier{}| _] = Tokens0],Acc)
    when Acc =/= []->
    {ComponentType,Tokens} = parse_ComponentType(Tokens0),
    parse_ComponentTypeList(Tokens,[ComponentType| Acc]);
parse_ComponentTypeList([{',',_}| [{'COMPONENTS',_}, {'OF',_}| _] = Tokens0],Acc)
    when Acc =/= []->
    {ComponentType,Tokens} = parse_ComponentType(Tokens0),
    parse_ComponentTypeList(Tokens,[ComponentType| Acc]);
...
parse_ExtensionAdditionGroup2(Tokens,Num) ->
    ...
    case Rest of
        [{']',_}, {']',_}| Rest2]->
            {[{'ExtensionAdditionGroup',Num}| CompTypeList] ++ ['ExtensionAdditionGroupEnd'],Rest2};
        _->
            parse_error(Rest)
    end.
...
parse_NamedNumber([#identifier{val = Name}, {'(',_}| Rest]) ->
    Flist = [fun parse_SignedNumber/1, fun parse_DefinedValue/1],
...
parse_Tag([{'[',_}| Rest]) ->
    ...
    {ClassNumber,Rest3} = case Rest2 of
        [{number,_,Num}| Rest21]->
            {Num,Rest21};
        _->
...
parse_Value(Tokens) ->
    Flist = [fun parse_BuiltinValue/1, fun parse_ValueFromObject/1, fun parse_DefinedValue/1],
    parse_or(Tokens,Flist).
...
parse_BuiltinValue(Tokens = [{'{',_}| _Rest]) ->
    Flist = [fun parse_SequenceOfValue/1, fun parse_SequenceValue/1, fun parse_ObjectIdentifierValue/1],
    parse_or(Tokens,Flist);
...
parse_DefinedValue(Tokens) ->
    Flist = [fun parse_ParameterizedValue/1, fun parse_DefinedValue2/1],
    parse_or(Tokens,Flist).

parse_DefinedValue2([{typereference,L1,Tname}, {'.',_}, #identifier{val = Idname}| Rest]) ->
...
parse_ValueSetTypeAssignment([{typereference,L1,Name}| Rest]) ->
    ...
    case Rest2 of
        [{'::=',_}| Rest3]->
            ...
            {#valuedef{pos = L1,name = Name,type = Type,value = ValueSet,module = get(asn1_module)},Rest4};
        _->
            parse_error(Rest2)
    end.
...
parse_ValueAssignment([#identifier{pos = L1,val = IdName}| Rest]) ->
    ...
    case Rest2 of
        [{'::=',_}| Rest3]->
            {Value,Rest4} = parse_Value(Rest3),
            {#valuedef{pos = L1,name = IdName,type = Type,value = Value,module = get(asn1_module)},Rest4};
        _->
            parse_error(Rest2)
    end.
...
parse_SubtypeElements(Tokens) ->
    Flist = [fun parse_ContainedSubtype/1, fun parse_Value/1, fun parse_MIN/1, fun parse_Type/1],
...
parse_UpperEndpoint(Lt,Tokens) ->
    Flist = [fun parse_MAX/1, fun parse_Value/1],
    case parse_or(Tokens,Flist) of
        {Value,Rest2}
            when Lt =:= lt->
            {{lt,Value},Rest2};
...
tref2Exttref(#typereference{pos = Pos,val = Name}) ->
    #'Externaltypereference'{pos = Pos,module = resolve_module(Name),type = Name}.

tref2Exttref(Pos,Name) ->
    #'Externaltypereference'{pos = Pos,module = resolve_module(Name),type = Name}.

identifier2Extvalueref(#identifier{pos = Pos,val = Name}) ->
    #'Externalvaluereference'{pos = Pos,module = resolve_module(Name),value = Name}.
