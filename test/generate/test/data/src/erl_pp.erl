...
-export([form/1, form/2, attribute/1, attribute/2, function/1, function/2, guard/1, guard/2, exprs/1, exprs/2, exprs/3, expr/1, expr/2, expr/3, expr/4]).

-import(lists, [append/1, foldr/3, map/2, mapfoldl/3, reverse/1, reverse/2]).

-import(io_lib, [write/1, format/2]).

-import(erl_parse, [inop_prec/1, preop_prec/1, func_prec/0, max_prec/0, type_inop_prec/1, type_preop_prec/1]).
...
-record(pp, {value_fun,singleton_atom_type_fun,string_fun,char_fun,linewidth = 72,indent = 4}).
...
state(Options)
    ...
    State = case encoding(Options) of
        latin1->
            latin1_state(Quote);
        unicode->
            unicode_state(Quote)
...
latin1_state(Quote) ->
    ...
    ValueFun = fun (V)->
        io_lib_pretty:print(V,Options) end,
    SingletonFun = case Quote of
        true->
            fun (A)->
                io_lib:write_string_as_latin1(atom_to_list(A),$\') end;
        false->
            ValueFun
    end,
    #pp{value_fun = ValueFun,singleton_atom_type_fun = SingletonFun,string_fun = fun io_lib:write_string_as_latin1/1,char_fun = fun io_lib:write_char_as_latin1/1}.
...
unicode_state(Quote) ->
    ...
    SingletonFun = case Quote of
        true->
            fun (A)->
                io_lib:write_string(atom_to_list(A),$\') end;
        false->
            ValueFun
    end,
    #pp{value_fun = ValueFun,singleton_atom_type_fun = SingletonFun,string_fun = fun io_lib:write_string/1,char_fun = fun io_lib:write_char/1}.
...
lattribute(optional_callbacks,Falist,Opts) ->
    try attrib(optional_callbacks,falist(Falist))
            ...
            _:_->
                attr(optional_callbacks,[abstract(Falist,Opts)]) end;
...
typeattr(Tag,{TypeName,Type,Args},_Opts) ->
    {first,leaf("-" ++ atom_to_list(Tag) ++ " "),typed(call({atom,a0(),TypeName},Args,0,options(none)),Type)}.
...
specattr(SpecKind,{FuncSpec,TypeSpecs}) ->
    ...
    {first,leaf(lists:concat(["-", SpecKind, " "])),{list,[{first,Func,spec_clauses(TypeSpecs)}]}}.
...
constraint({type,_Line,constraint,[{atom,_,is_subtype}, [{var,_,_} = V, Type]]},_Opts) ->
...
falist([]) ->
    ['[]'];
falist(Falist) ->
    L = [begin {Name,Arity} = Fa,
    [{atom,Name}, leaf(format("/~w",[Arity]))] end || Fa <- Falist],
    [{seq,$[,$],$,,L}].

lfunction({function,_Line,Name,_Arity,Cs},Opts) ->
    Cll = nl_clauses(fun (C,H)->
        func_clause(Name,C,H) end,$;,Opts,Cs),
    [Cll, leaf(".\n")].
...
lexpr({'fun',_,{clauses,Cs},Extra},_Prec,Opts) ->
    {force_nl,fun_info(Extra),{list,[{first,'fun',fun_clauses(Cs,Opts,unnamed)}, {reserved,'end'}]}};
lexpr({named_fun,_,Name,Cs,Extra},_Prec,Opts) ->
    {force_nl,fun_info(Extra),{list,[{first,['fun', " "],fun_clauses(Cs,Opts,{named,Name})}, {reserved,'end'}]}};
...
lexpr({'try',_,Es,Scs,Ccs,As},_,Opts) ->
    {list,[if Scs =:= [] ->
        {step,'try',body(Es,Opts)};true ->
        {step,{list,[{step,'try',body(Es,Opts)}, {reserved,'of'}]},cr_clauses(Scs,Opts)} end] ++ if Ccs =:= [] ->
        [];true ->
        [{step,'catch',try_clauses(Ccs,Opts)}] end ++ if As =:= [] ->
        [];true ->
        [{step,'after',body(As,Opts)}] end ++ [{reserved,'end'}]};
...
bit_elem({bin_element,_,Expr,Sz,Types},Opts) ->
    ...
    SChars = if Sz =/= default ->
        [VChars, $:, lexpr(Sz,P,Opts)];true ->
        VChars end,
    if Types =/= default ->
        [SChars, $/| bit_elem_types(Types)];true ->
        SChars end.
...
bit_elem_type({A,B}) ->
    [lexpr(erl_parse:abstract(A),options(none)), $:, lexpr(erl_parse:abstract(B),options(none))];
bit_elem_type(T) ->
...
fun_clauses(Cs,Opts,{named,Name}) ->
    nl_clauses(fun (C,H)->
        ...
        {step,[atom_to_list(Name), Gl],Bl} end,[$;],Opts,Cs).
...
f({seq,Before,After,Sep,LItems},I0,ST,WT,PP) ->
    ...
    Sizes = BSizeL ++ SizeL,
    NSepChars = if is_list(Sep),
    Sep =/= [] ->
        max(0,length(CharsL) - 1);true ->
        0 end,
    case same_line(I0,Sizes,NSepChars,PP) of
        {yes,Size}->
            Chars = if NSepChars > 0 ->
                insert_sep(CharsL,$ );true ->
                CharsL end,
            {BCharsL ++ Chars,Size};
        no->
...
f({prefer_nl,Sep,LItems},I0,ST,WT,PP) ->
    ...
    if Sizes =:= [] ->
        {[],0};true ->
        {insert_newlines(CharsSize2L,I0,ST,PP),nsz(lists:last(Sizes),I0)} end;
...
f({ehook,HookExpr,Precedence,{Mod,Func,Eas} = ModFuncEas},I,_ST,_WT,_PP) ->
    ...
    {Chars,indentation(Chars,I)};
f(WordName,_I,_ST,WT,_PP)
    when is_atom(WordName)->
    word(WordName,WT).
...
fl(CItems,Sep0,I0,After,ST,WT,PP) ->
    F = fun ({step,Item1,Item2},S)->
        [f(Item1,I0,ST,WT,PP), f([Item2, S],incr(I0,PP#pp.indent),ST,WT,PP)];({cstep,Item1,Item2},S)->
        {_,Sz1} = CharSize1 = f(Item1,I0,ST,WT,PP),
        if is_integer(Sz1),
        Sz1 < PP#pp.indent ->
            Item2p = [leaf(" "), Item2, S],
            [consecutive(Item2p,CharSize1,I0,ST,WT,PP), {[],0}];true ->
            [CharSize1, f([Item2, S],incr(I0,PP#pp.indent),ST,WT,PP)] end;({reserved,Word},S)->
        [f([Word, S],I0,ST,WT,PP), {[],0}];(Item,S)->
        [f([Item, S],I0,ST,WT,PP), {[],0}] end,
...
consecutive(Items,CharSize1,I0,ST,WT,PP) ->
    {CharsSizes,_Length} = mapfoldl(fun (Item,Len)->
        CharsSize = f(Item,Len,ST,WT,PP),
        {CharsSize,indent(CharsSize,Len)} end,indent(CharSize1,I0),Items),
...
nonzero(CharSizes) ->
    lists:filter(fun ({_,Sz})->
        Sz =/= 0 end,CharSizes).

maybe_newlines([{Chars,Size}],[],_I,_NSepChars,_ST,_PP) ->
    {Chars,Size};
maybe_newlines(CharsSizeList,Items,I,NSepChars,ST,PP)
    when I >= 0->
...
maybe_sep1([{Chars,Size}| CharsSizeL],[Item| Items],I0,I,Sep,NSepChars,Sz0,A,PP) ->
    case classify_item(Item) of
        atomic
            when is_integer(Size)->
            Size1 = Size + 1,
            I1 = I + Size1,
            if I1 =< PP#pp.linewidth ->
                A1 = if NSepChars > 0 ->
                    [Chars, $ | A];true ->
                    [Chars| A] end,
                maybe_sep1(CharsSizeL,Items,I0,I1,Sep,NSepChars,Sz0 + Size1,A1,PP);true ->
                A1 = [Chars, Sep| A],
                maybe_sep1(CharsSizeL,Items,I0,I0 + Size,Sep,NSepChars,Size1,A1,PP) end;
        _->
            A1 = [Chars, Sep| A],
            maybe_sep1(CharsSizeL,Items,I0,PP#pp.linewidth + 1,Sep,NSepChars,0,A1,PP)
    end;
maybe_sep1(_CharsSizeL,_Items,_Io,_I,_Sep,_NSepChars,Sz,A,_PP) ->
    {lists:reverse(A),Sz}.

insert_newlines(CharsSizesL,I,ST,PP)
    when I >= 0->
...
handle_step(CharsSizesL,I,ST,PP) ->
    map(fun ([{_C1,0}, {_C2,0}])->
        {[],0};([{C1,Sz1}, {_C2,0}])->
        {C1,Sz1};([{C1,Sz1}, {C2,Sz2}])
        when Sz2 > 0->
        {insert_nl([C1, C2],I + PP#pp.indent,ST),line_size([Sz1, Sz2])} end,CharsSizesL).
...
classify_item({atom,_}) ->
    atomic;
classify_item({singleton_atom_type,_}) ->
    atomic;
classify_item(Atom)
    when is_atom(Atom)->
    atomic;
classify_item({leaf,_,_}) ->
    atomic;
classify_item(_) ->
    complex.

same_line(I0,SizeL,NSepChars,PP) ->
    try Size = lists:sum(SizeL) + NSepChars,
...
write_a_string(S,N,Len,PP) ->
    ...
    case (string:length(Sl) > Len) and (N > 5) of
        true->
            write_a_string(S,N - 1,Len,PP);
        false->
            [flat_leaf(Sl)| write_a_string(string:slice(S,string:length(SS)),Len,Len,PP)]
    end.
...
spacetab() ->
    {[_| L],_} = mapfoldl(fun (_,A)->
        {A,[$ | A]} end,[],lists:seq(0,30)),
    list_to_tuple(L).
...
spaces(N,T)
    ...
    element(N,T);
spaces(N,T) ->
    [element(30,T)| spaces(N - 30,T)].

wordtable() ->
    L = [begin {leaf,Sz,S} = leaf(W),
    {S,Sz} end || W <- [" ->", " =", "<<", ">>", "[]", "after", "begin", "case", "catch", "end", "fun", "if", "of", "receive", "try", "when", " ::", "..", " |"]],
    list_to_tuple(L).
