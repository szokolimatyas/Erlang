...
-file("asn1ct_gen.erl", 24).

-export([emit/1, open_output_file/1, close_output_file/0, get_inner/1, type/1, def_to_tag/1, prim_bif/1, list2name/1, list2rname/1, constructed_suffix/2, unify_if_string/1, get_constraint/2, insert_once/2, ct_gen_module/1, index2suffix/1, get_record_name_prefix/1, conform_value/2, named_bitstring_value/2]).

-export([pgen/3, mk_var/1, un_hyphen_var/1]).

-export([gen_encode_constructed/4, gen_decode_constructed/4]).

-spec(pgen(Outfile,Gen,Code) -> ok when Outfile::any(),Gen::#gen{},Code::#abst{}).
...
pgen_typeorval(Erules,N2nConvEnums,Code) ->
    #abst{name = Module,types = Types,values = Values,objects = Objects,objsets = ObjectSets} = Code,
    Rtmod = ct_gen_module(Erules),
...
pgen_name2numfunc(TypeNameAsAtom,Mapping,Ext)
    when is_atom(TypeNameAsAtom)->
...
pgen_num2namefunc(TypeNameAsAtom,Mapping,Ext)
    when is_atom(TypeNameAsAtom)->
...
pgen_partial_inc_dec(Rtmod,Erules,Module) ->
    case asn1ct:get_gen_state_field(inc_type_pattern) of
        undefined->
            ok;
        ConfList->
            PatternLists = lists:map(fun ({_,P})->
                P end,ConfList),
...
pgen_partial_types1(Erules,[{FuncName,[TopType| RestTypes]}| Rest]) ->
    ...
    traverse_type_structure(Erules,TypeDef,RestTypes,FuncName,TypeDef#typedef.name),
...
traverse_type_structure(Erules,#type{def = Def},[T| Ts],FuncName,TopTypeName) ->
    ...
    case InnerType of
        'SET'->
            ...
            traverse_type_structure(Erules,C#'ComponentType'.typespec,Ts,FuncName,[T| TopTypeName]);
        'SEQUENCE'->
        ...
        #'Externaltypereference'{module = M,type = TName}->
            ...
            traverse_type_structure(Erules,TypeDef,[T| Ts],FuncName,[TypeDef#typedef.name]);
        _->
            traverse_type_structure(Erules,Def,Ts,FuncName,[T| TopTypeName])
    end;
traverse_type_structure(Erules,#typedef{typespec = Def},[T| Ts],FuncName,TopTypeName) ->
...
emit_partial_incomplete_decode({FuncName,TopType,Pattern}) ->
    ...
    emit([{asis,FuncName}, "(Bytes) ->", nl, "  decode_partial_incomplete('", TopTypeName, "',Bytes,", {asis,Pattern}, ").", nl]);
...
gen_part_decode_funcs(#'Externaltypereference'{module = M,type = T},_TypeName,Data) ->
    #typedef{typespec = TS} = asn1_db:dbget(M,T),
    InnerType = case TS#type.def of
        #'ObjectClassFieldType'{type = OCFTType}->
            OCFTType;
        _->
...
gen_part_decode_funcs({constructed,bif},TypeName,{_Name,parts,Tag,_Type}) ->
    emit(["  case Data of", nl, "    L when is_list(L) ->", nl, "      'dec_", TypeName, "'(lists:map(fun(X) -> element(1, ", {call,ber,ber_decode_erlang,["X"]}, ") end, L),", {asis,Tag}, ");", nl, "    _ ->", nl, "      [Res] = 'dec_", TypeName, "'([Data],", {asis,Tag}, "),", nl, "      Res", nl, "  end"]);
gen_part_decode_funcs(WhatKind,_TypeName,{_Name,parts,_Tag,_Type}) ->
    throw({error,{asn1,{"only SEQUENCE OF/SET OF may have the partial incomplete d" "irective 'parts'.",WhatKind}}});
gen_part_decode_funcs({constructed,bif},TypeName,{_Name,undecoded,Tag,_Type}) ->
    emit(["  'dec_", TypeName, "'(Data,", {asis,Tag}, ")"]);
gen_part_decode_funcs({primitive,bif},_TypeName,{_Name,undecoded,Tag,Type}) ->
...
gen_types(Erules,Tname,{RootList,ExtList},EncDec)
    when is_list(RootList)->
...
gen_decode_constructed(Erules,Typename,InnerType,D)
    when is_record(D,type)->
    Rtmod = ct_constructed_module(Erules),
...
gen_partial_inc_decode_exports() ->
    case {asn1ct:read_config_data(partial_incomplete_decode),asn1ct:get_gen_state_field(inc_type_pattern)} of
        {undefined,_}->
            ok;
        {_,undefined}->
            ok;
...
pgen_dispatcher(Gen,Types) ->
    ...
    Call = case Gen of
        ...
        #gen{erule = jer}->
            ["?JSON_ENCODE(", {call,jer,encode_jer,[CurrMod, "list_to_existing_atom(lists:concat([typeinfo_,Type]" "))", "Data"]}, ")"];
    ...
    case Gen#gen.jer of
        true->
            ...
            JerCall = ["?JSON_ENCODE(", {call,jer,encode_jer,[CurrMod, "list_to_existing_atom(lists:concat([typeinfo_,Type]" "))", "Data"]}, ")"],
            case NoOkWrapper of
                true->
                    emit(["  ", JerCall, "."]);
                false->
                    emit(["try ", JerCall, " of", nl, "  Bytes ->", nl, "    {ok,Bytes}", nl, try_catch()])
            end,
            emit([nl, nl]);
        false->
            ok
    ...
    DecWrap = case {Gen,ReturnRest} of
        {#gen{erule = ber},false}->
    ...
    case {Gen,ReturnRest} of
        ...
        {#gen{erule = jer},false}->
            emit(["   Result = ", {call,jer,decode_jer,[CurrMod, "list_to_existing_atom(lists:concat([typeinfo_,Type" "]))", DecWrap]}, ",", nl]),
            result_line(NoOkWrapper,["Result"]);
        {#gen{erule = per},true}->
    ...
    case Gen#gen.jer of
        true->
            ...
            emit(["   Result = ", {call,jer,decode_jer,[CurrMod, "list_to_existing_atom(lists:concat([typeinfo_,Type" "]))", JerDecWrap]}, ",", nl]),
...
try_catch() ->
    ["  catch", nl, "    Class:Exception:Stk when Class =:= error; Class =:= exit ->", nl, "      case Exception of", nl, "        {error,{asn1,Reason}} ->", nl, "          {error,{asn1,{Reason,Stk}}};", nl, "        Reason ->", nl, "         {error,{asn1,{Reason,Stk}}}", nl, "      end", nl, "end."].
...
gen_info_functions(Gen) ->
    ...
    emit(["encoding_rule() -> ", {asis,Erule}, ".", nl, nl, "maps() -> ", {asis,Maps}, ".", nl, nl, "bit_string_format() -> ", {asis,asn1ct:get_bit_string_format()}, ".", nl, nl, "legacy_erlang_types() -> ", {asis,asn1ct:use_legacy_types()}, ".", nl, nl]).
...
gen_decode_partial_incomplete(#gen{erule = ber}) ->
    case {asn1ct:read_config_data(partial_incomplete_decode),asn1ct:get_gen_state_field(inc_type_pattern)} of
        ...
        _->
            EmitCaseClauses = fun ()->
                emit(["   {'EXIT',{error,Reason}} ->", nl, "      {error,Reason};", nl, "    {'EXIT',Reason} ->", nl, "      {error,{asn1,Reason}};", nl, "    Result ->", nl, "      {ok,Result}", nl, "  end"]) end,
            emit(["decode_partial_incomplete(Type,Data0,", "Pattern) ->", nl]),
            emit(["  {Data,_RestBin} =", nl, "    ", {call,ber,decode_primitive_incomplete,["Pattern", "Data0"]}, com, nl, "  case catch decode_partial_inc_disp(Type,", "Data) of", nl]),
            EmitCaseClauses(),
            emit([".", nl, nl]),
            emit(["decode_part(Type, Data0) when is_binary(Data0) ->", nl]),
            emit(["  case catch decode_inc_disp(Type,element(1, ", {call,ber,ber_decode_nif,["Data0"]}, ")) of", nl]),
            EmitCaseClauses(),
            emit([";", nl]),
...
gen_partial_inc_dispatcher([{FuncName,TopType,_Pattern}| Rest],TypePattern,Sep) ->
    ...
    emit([Sep, "decode_partial_inc_disp('", TopTypeName, "',Data) ->", nl, "  ", {asis,list_to_atom(lists:concat(["dec-inc-", FuncName2]))}, "(Data)"]),
...
gen_dispatcher(L,DispFunc,Prefix) ->
    ...
    emit([DispFunc, "(", "Type", ", _Data) -> exit({error,{asn1,{undefined_type,Type}}}).", nl, nl]).
...
pgen_info() ->
    emit(["info() ->", nl, "   case ?MODULE:module_info(attributes) of", nl, "     Attributes when is_list(Attributes) ->", nl, "       case lists:keyfind(asn1_info, 1, Attributes) of", nl, "         {_,Info} when is_list(Info) ->", nl, "           Info;", nl, "         _ ->", nl, "           []", nl, "       end;", nl, "     _ ->", nl, "       []", nl, "   end.", nl]).
...
pgen_hrl(#gen{pack = record} = Gen,Code) ->
    ...
    Ret = case pgen_hrltypes(Gen,Module,Ptypes ++ Types,0) of
        0->
            case Values of
                []->
                    0;
    ...
    case Ret of
        ...
        Y->
            ...
            close_output_file(),
            asn1ct:verbose("--~p--~n",[{generated,lists:concat([get(outfile), ".hrl"])}],Gen),
            Y
    end;
...
gen_record(Gen,TorPtype,Name,[#'ComponentType'{name = Cname,typespec = Type}| T],Num) ->
...
gen_record(Gen,TorPtype,Name,{Clist1,EClist,Clist2},Num)
    ...
    is_list(Clist2)->
    gen_record(Gen,TorPtype,Name,Clist1 ++ EClist ++ Clist2,Num);
gen_record(Gen,TorPtype,Name,[_| T],Num) ->
...
do_gen_record(Gen,Name,CL0) ->
    CL = case CL0 of
        ...
        {Root1,Ext,Root2}->
            Root1 ++ [{comment,"with exensions"}] ++ only_components(Ext) ++ [{comment,"end of extensions"}] ++ Root2;
        _
            when is_list(CL0)->
            CL0
    end,
    Prefix = get_record_name_prefix(Gen),
    emit(["-record('", Prefix, list2name(Name), "', {"] ++ do_gen_record_1(CL) ++ [nl, "}).", nl, nl]).
...
needs_trailing_comma([#'ComponentType'{}| _]) ->
    true;
needs_trailing_comma([_| T]) ->
...
gen_head(#gen{options = Options} = Gen,Mod,Hrl) ->
    ...
    emit(["%% Generated by the Erlang ASN.1 ", Name, " compiler. Version: ", asn1ct:vsn(), nl, "%% Purpose: Encoding and decoding of the types in ", Mod, ".", nl, nl, "-module('", Mod, "').", nl, "-compile(nowarn_unused_vars).", nl, "-dialyzer(no_improper_lists).", nl, "-dialyzer(no_match).", nl]),
    case Hrl of
        0->
            ok;
        _->
    ...
    emit(["-asn1_info([{vsn,'", asn1ct:vsn(), "'},", nl, "            {module,'", Mod, "'},", nl, "            {options,", io_lib:format("~p",[Options]), "}]).", nl, nl]),
    ...
    JerDefines andalso emit(["-ifdef(jsone).", nl, "-define(JSON_DECODE(Data),jsone:decode(Data)).", nl, "-define(JSON_ENCODE(Term),jsone:encode(Term)).", nl, "-else.", nl, "-define(JSON_DECODE(Data),jsx:decode(Data,[return_maps])).", nl, "-define(JSON_ENCODE(Term),jsx:encode(Term)).", nl, "-endif.", nl]).

gen_hrlhead(Mod) ->
    emit(["%% Generated by the Erlang ASN.1 compiler. Version: ", asn1ct:vsn(), nl, "%% Purpose: Erlang record definitions for each named and unn" "amed", nl, "%% SEQUENCE and SET, and macro definitions for each value", nl, "%% definition in module ", Mod, ".", nl, nl]).
...
type('ASN1_OPEN_TYPE') ->
    'ASN1_OPEN_TYPE';
type({fixedtypevaluefield,_Name,Type})
...
def_to_tag(#'ObjectClassFieldType'{type = Type}) ->
    case Type of
        T
            when is_tuple(T),
            element(1,T) == fixedtypevaluefield->
...
constructed_suffix(_,#'SEQUENCE'{pname = Ptypename})
    when Ptypename =/= false->
    {ptype,Ptypename};
constructed_suffix(_,#'SET'{pname = Ptypename})
    when Ptypename =/= false->
    {ptype,Ptypename};
