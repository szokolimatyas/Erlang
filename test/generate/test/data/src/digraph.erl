...
-record(digraph,{vtab = notable::ets:tab(),etab = notable::ets:tab(),ntab = notable::ets:tab(),cyclic = true::boolean()}).
...
edges(G,V) ->
    ets:select(G#digraph.ntab,[{{{out,V},'$1'},[],['$1']}, {{{in,V},'$1'},[],['$1']}]).
...
do_del_edges([],#digraph{}) ->
    true.

do_del_edge(E,V1,V2,G) ->
    ets:select_delete(G#digraph.ntab,[{{{in,V2},E},[],[true]}, {{{out,V1},E},[],[true]}]),
...
rm_edge_0([],_,_,#digraph{}) ->
    ok.

-spec(do_add_edge({edge(),vertex(),vertex(),label()},graph()) -> edge()|{error,add_edge_err_rsn()}).
...
-spec(acyclic_add_edge(edge(),vertex(),vertex(),label(),graph()) -> edge()|{error,{bad_edge,[vertex()]}}).

acyclic_add_edge(_E,V1,V2,_L,_G)
    when V1 =:= V2->
    {error,{bad_edge,[V1, V2]}};
acyclic_add_edge(E,V1,V2,Label,G) ->
...
one_path([V| Vs],W,Cont,Xs,Ps,Prune,G,Counter) ->
    case lists:member(V,Xs) of
        ...
        false->
            one_path(out_neighbours(G,V),W,[{Vs,Ps}| Cont],[V| Xs],[V| Ps],Prune,G,Counter + 1)
    end;
...
queue_out_neighbours(V,G,Q0) ->
    lists:foldl(fun (E,Q)->
        queue:in(E,Q) end,Q0,out_edges(G,V)).