-file("asn1ct_imm.erl", 1).

-module(asn1ct_imm).

-export([per_dec_raw_bitstring/2, per_dec_boolean/0, per_dec_enumerated/2, per_dec_enumerated/3, per_dec_extension_map/1, per_dec_integer/2, per_dec_k_m_string/3, per_dec_length/3, per_dec_named_integer/3, per_dec_octet_string/2, per_dec_open_type/1, per_dec_real/1, per_dec_restricted_string/1]).

-export([per_dec_constrained/3, per_dec_normally_small_number/1]).

-export([per_enc_bit_string/4, per_enc_legacy_bit_string/4, per_enc_boolean/2, per_enc_choice/3, per_enc_enumerated/3, per_enc_integer/3, per_enc_integer/4, per_enc_null/2, per_enc_k_m_string/4, per_enc_octet_string/3, per_enc_legacy_octet_string/3, per_enc_open_type/2, per_enc_restricted_string/3, per_enc_small_number/2]).

-export([per_enc_extension_bit/2, per_enc_extensions/4, per_enc_extensions_map/4, per_enc_optional/2]).

-export([per_enc_sof/5]).

-export([enc_absent/3, enc_append/1, enc_element/2, enc_maps_get/2, enc_comment/1]).

-export([enc_cg/2]).

-export([optimize_alignment/1, optimize_alignment/2, dec_slim_cg/2, dec_code_gen/2]).
...
per_dec_length(SingleValue,_,_Aligned)
    when is_integer(SingleValue)->
    {value,SingleValue};
per_dec_length({{Fixed,Fixed},[]},AllowZero,Aligned) ->
    bit_case(per_dec_length(Fixed,AllowZero,Aligned),per_dec_length(no,AllowZero,Aligned));
per_dec_length({{_,_} = Constr,[]},AllowZero,Aligned) ->
    bit_case(per_dec_length(Constr,AllowZero,Aligned),per_dec_length(no,AllowZero,Aligned));
per_dec_length({Lb,Ub},_AllowZero,Aligned)
    when is_integer(Lb),
    is_integer(Lb)->
...
per_dec_real(Aligned) ->
    Dec = fun (V,Buf)->
        emit(["{", {call,real_common,decode_real,[V]}, com, Buf, "}"]) end,
    {call,Dec,{get_bits,decode_unconstrained_length(true,Aligned),[8, binary, {align,Aligned}]}}.
...
per_enc_bit_string(Val,[],Constraint0,Aligned) ->
    ...
    B ++ [{call,erlang,bit_size,[Val],Bits}| per_enc_length(Val,1,Bits,Constraint,Aligned,'BIT STRING')];
per_enc_bit_string(Val0,NNL0,Constraint0,Aligned) ->
    ...
    B ++ [{'try',[bit_string_name2pos_fun(NNL,Val)],{Positions,[{call,per_common,bitstring_from_positions,[Positions| ExtraArgs]}]},[ToBs],Bs}, {call,erlang,bit_size,[Bs],Bits}| per_enc_length(Bs,1,Bits,Constraint,Aligned,'BIT STRING')].
...
per_enc_legacy_bit_string(Val0,[],Constraint0,Aligned) ->
    ...
    B ++ [{call,per_common,to_bitstring,[Val| ExtraArgs],Bs}, {call,erlang,bit_size,[Bs],Bits}| per_enc_length(Bs,1,Bits,Constraint,Aligned,'BIT STRING')];
per_enc_legacy_bit_string(Val0,NNL0,Constraint0,Aligned) ->
    ...
    B ++ [{'try',[bit_string_name2pos_fun(NNL,Val)],{Positions,[{call,per_common,bitstring_from_positions,[Positions| ExtraArgs]}]},[{call,per_common,to_named_bitstring,[Val| ExtraArgs]}],Bs}, {call,erlang,bit_size,[Bs],Bits}| per_enc_length(Bs,1,Bits,Constraint,Aligned,'BIT STRING')].

per_enc_boolean(Val0,_Aligned) ->
    {B,[Val]} = mk_vars(Val0,[]),
    B ++ build_cond([[{eq,Val,false}, {put_bits,0,1,[1]}], [{eq,Val,true}, {put_bits,1,1,[1]}], ['_', {error,{illegal_boolean,Val}}]]).
...
per_enc_integer(Val0,NNL,Constraint0,Aligned) ->
    {B,[Val]} = mk_vars(Val0,[]),
    Constraint = effective_constraint(integer,Constraint0),
    Cs = [[{eq,Val,N}| per_enc_integer_1(V,Constraint,Aligned)] || {N,V} <- NNL],
...
per_enc_open_type(Imm0,Aligned) ->
    ...
    [{list,Imm,Val}, {call,enc_mod(Aligned),complete,[Val],Bin}, {call,erlang,byte_size,[Bin],Len}| per_enc_length(Bin,8,Len,Aligned)].

per_enc_octet_string(Bin,Constraint0,Aligned) ->
    {B,[[], Len]} = mk_vars([],[len]),
    Constraint = effective_constraint(bitstring,Constraint0),
    B ++ [{call,erlang,byte_size,[Bin],Len}| per_enc_length(Bin,8,Len,Constraint,Aligned,'OCTET STRING')].

per_enc_legacy_octet_string(Val0,Constraint0,Aligned) ->
    {B,[Val, Bin, Len]} = mk_vars(Val0,[bin, len]),
    Constraint = effective_constraint(bitstring,Constraint0),
    B ++ [{call,erlang,iolist_to_binary,[Val],Bin}, {call,erlang,byte_size,[Bin],Len}| per_enc_length(Bin,8,Len,Constraint,Aligned,'OCTET STRING')].

per_enc_restricted_string(Val0,{M,F},Aligned) ->
    {B,[Val, Bin, Len]} = mk_vars(Val0,[bin, len]),
    B ++ [{call,M,F,[Val],Bin}, {call,erlang,byte_size,[Bin],Len}| per_enc_length(Bin,8,Len,Aligned)].

per_enc_small_number(Val,Aligned) ->
    build_cond([[{lt,Val,64}, {put_bits,Val,7,[1]}], ['_', {put_bits,1,1,[1]}| per_enc_unsigned(Val,Aligned)]]).

per_enc_extension_bit(Val0,_Aligned) ->
    {B,[Val]} = mk_vars(Val0,[]),
    B ++ build_cond([[{eq,Val,[]}, {put_bits,0,1,[1]}], ['_', {put_bits,1,1,[1]}]]).

per_enc_extensions(Val0,Pos0,NumBits,Aligned)
    when NumBits > 0->
    Pos = Pos0 + 1,
    ...
    B ++ [{call,per_common,extension_bitmap,[Val, Pos, Pos + NumBits],Bitmap}, {list,[{'cond',[[{eq,Bitmap,0}], ['_'| Length ++ PutBits]]}],{var,"Extensions"}}].
...
per_enc_extensions_map(Val0,Vars,Undefined,Aligned) ->
    ...
    B ++ [{assign,Bitmap,BitmapExpr}, {list,[{'cond',[[{eq,Bitmap,0}], ['_'| Length ++ PutBits]]}],{var,"Extensions"}}].

per_enc_optional(Val,DefVals)
    when is_list(DefVals)->
    Zero = {put_bits,0,1,[1]},
    One = {put_bits,1,1,[1]},
    [{'cond',[[{eq,Val,DefVal}, Zero] || DefVal <- DefVals] ++ [['_', One]]}];
...
dec_string(Sv,U,Aligned0,T)
    when is_integer(Sv)->
    Bits = U * Sv,
    Aligned = Aligned0 andalso is_aligned(T,Bits,Bits),
    {get_bits,Sv,[U, binary, {align,Aligned}]};
dec_string({{Sv,Sv},[]},U,Aligned,T) ->
    bit_case(dec_string(Sv,U,Aligned,T),dec_string(no,U,Aligned,T));
dec_string({{_,_} = C,[]},U,Aligned,T) ->
    bit_case(dec_string(C,U,Aligned,T),dec_string(no,U,Aligned,T));
...
dec_string(_,U,Aligned,_T) ->
    Al = [{align,Aligned}],
    DecRest = fun (V,Buf)->
        asn1ct_func:call(per_common,decode_fragmented,[V, Buf, U]) end,
    {'case',[{test,{get_bits,1,[1| Al]},0,{value,{get_bits,{get_bits,7,[1]},[U, binary]}}}, {test,{get_bits,1,[1| Al]},1,{test,{get_bits,1,[1]},0,{value,{get_bits,{get_bits,14,[1]},[U, binary]}}}}, {test,{get_bits,1,[1| Al]},1,{test,{get_bits,1,[1]},1,{value,{call,DecRest,{get_bits,6,[1]}}}}}]}.

per_dec_enumerated_fix_list([{V,_}| T],Tail,N) ->
    [{N,V}| per_dec_enumerated_fix_list(T,Tail,N + 1)];
per_dec_enumerated_fix_list([],Tail,_) ->
    Tail.
...
per_dec_integer_1([{'ValueRange',{Lb,'MAX'}}],Aligned)
    when is_integer(Lb)->
    per_decode_semi_constrained(Lb,Aligned);
per_dec_integer_1([{'ValueRange',{Lb,Ub}}],Aligned)
    when is_integer(Lb),
    is_integer(Ub)->
    per_dec_constrained(Lb,Ub,Aligned);
per_dec_integer_1([{{_,_} = Constr0,_}],Aligned) ->
    Constr = effective_constraint(integer,[Constr0]),
    bit_case(per_dec_integer(Constr,Aligned),per_dec_unconstrained(Aligned));
...
per_dec_constrained(Lb,Ub,true) ->
    Range = Ub - Lb + 1,
    Get = if Range =< 255 ->
        {get_bits,per_num_bits(Range),[1, unsigned]};Range == 256 ->
        {get_bits,1,[8, unsigned, {align,true}]};Range =< 65536 ->
        {get_bits,2,[8, unsigned, {align,true}]};true ->
        RangeOctLen = byte_size(binary:encode_unsigned(Range - 1)),
        {get_bits,per_dec_length({1,RangeOctLen},false,true),[8, unsigned, {align,true}]} end,
    add_lb(Lb,Get).
...
decode_unconstrained_length(AllowZero,Aligned) ->
    ...
    {'case',[{test,{get_bits,1,[1| Al]},0,{value,{get_bits,7,[1| Zero]}}}, {test,{get_bits,1,[1| Al]},1,{test,{get_bits,1,[1]},0,{value,{get_bits,14,[1| Zero]}}}}]}.
...
opt_map_1([{I,_} = Pair| T],Lb,Ub) ->
    if I =:= Lb,
    I =< Ub ->
        [Pair| opt_map_1(T,Lb + 1,Ub)];Lb < I,
    I =< Ub ->
        [Pair| opt_map_1(T,Lb,Ub)];true ->
        opt_map_1(T,Lb,Ub) end;
opt_map_1(Map,Lb,Ub) ->
    if Lb =< Ub ->
        Map;true ->
        [] end.

matched_range({get_bits,Bits0,[U| Flags]})
    when is_integer(U)->
...
convert_string(NumBits,notab,Imm)
    when NumBits < 8->
    Dec = fun (V,Buf)->
        emit(["{", {call,per_common,decode_chars,[V, NumBits]}, com, Buf, "}"]) end,
    {call,Dec,Imm};
convert_string(NumBits,notab,Imm)
    when NumBits =:= 16->
    Dec = fun (V,Buf)->
        emit(["{", {call,per_common,decode_chars_16bit,[V]}, com, Buf, "}"]) end,
    {call,Dec,Imm};
convert_string(NumBits,notab,Imm) ->
    Dec = fun (V,Buf)->
        emit(["{", {call,per_common,decode_big_chars,[V, NumBits]}, com, Buf, "}"]) end,
    {call,Dec,Imm};
convert_string(NumBits,Chars,Imm) ->
    Dec = fun (V,Buf)->
        emit(["{", {call,per_common,decode_chars,[V, NumBits, {asis,Chars}]}, com, Buf, "}"]) end,
    {call,Dec,Imm}.
        ...
        no->
            case StringType of
                ...
                'VisibleString'->
                    notab;
                'PrintableString'->
                    Chars = " '()+,-./0123456789:=?ABCDEFGHIJKLMNOPQRSTUVWX" "YZabcdefghijklmnopqrstuvwxyz",
...
char_tab_1(Chars,NumBits) ->
    ...
    if Max =< BitValMax ->
        notab;true ->
        list_to_tuple(lists:sort(Chars)) end.
...
flatten({test,{get_bits,I,U},V,E0},Buf0,St0)
    when is_integer(I)->
...
alignment([{align,false}| _]) ->
    none;
alignment([{align,true}| _]) ->
    full;
alignment([{align,Bits}| _]) ->
    Bits;
alignment([_| T]) ->
    alignment(T);
alignment([]) ->
...
is_nonbuilding({assign,_,_}) ->
    true;
is_nonbuilding({call,_,_,_,_}) ->
    true;
is_nonbuilding({comment,_}) ->
    true;
is_nonbuilding({lc,_,_,_,_}) ->
    true;
is_nonbuilding({set,_,_}) ->
    true;
is_nonbuilding({list,_,_}) ->
    true;
is_nonbuilding({sub,_,_,_}) ->
    true;
is_nonbuilding({'try',_,_,_,_}) ->
    true;
is_nonbuilding(_) ->
    false.
...
per_enc_integer_1(Val,[{{'SingleValue',[_| _] = Svs} = Constr,[]}],Aligned) ->
    ...
    Root = [begin {[],_,Put} = per_enc_constrained(Sv,Lb,Ub,Aligned),
    [{eq,Val,Sv}, {put_bits,0,1,[1]}| Put] end || Sv <- Svs],
    Cs = Root ++ [['_', {put_bits,1,1,[1]}| per_enc_unconstrained(Val,Aligned)]],
    build_cond(Cs);
per_enc_integer_1(Val0,[{{_,_} = Constr,[]}],Aligned) ->
    {Prefix,Check,Action} = per_enc_integer_2(Val0,Constr,Aligned),
    Prefix ++ build_cond([[Check, {put_bits,0,1,[1]}| Action], ['_', {put_bits,1,1,[1]}| per_enc_unconstrained(Val0,Aligned)]]);
...
per_enc_constrained(Val0,Lb,Ub,true) ->
    ...
    if Range < 256 ->
        ...
        Put = [{put_bits,Val,NumBits,[1]}],
        {Prefix,Check,Put};Range =:= 256 ->
        NumBits = 8,
        Put = [{put_bits,Val,NumBits,[1, align]}],
        {Prefix,Check,Put};Range =< 65536 ->
        Put = [{put_bits,Val,16,[1, align]}],
        {Prefix,Check,Put};true ->
        RangeOctsLen = byte_size(binary:encode_unsigned(Range - 1)),
        BitsNeeded = per_num_bits(RangeOctsLen),
        {Prefix,Check,per_enc_constrained_huge(BitsNeeded,Val)} end.

per_enc_constrained_huge(BitsNeeded,{var,VarBase} = Val) ->
    Bin = {var,VarBase ++ "@bin"},
    BinSize0 = {var,VarBase ++ "@bin_size0"},
    BinSize = {var,VarBase ++ "@bin_size"},
    [{call,binary,encode_unsigned,[Val],Bin}, {call,erlang,byte_size,[Bin],BinSize0}, {sub,BinSize0,1,BinSize}, {'cond',[['_', {put_bits,BinSize,BitsNeeded,[1]}, {put_bits,Bin,binary,[8, align]}]]}];
...
per_enc_unsigned(Val,Aligned) ->
    case is_integer(Val) of
        false->
            ...
            Bin = {var,VarBase ++ "@bin"},
            BinSize = {var,VarBase ++ "@bin_size"},
            [{call,binary,encode_unsigned,[Val],Bin}, {call,erlang,byte_size,[Bin],BinSize}| per_enc_length(Bin,8,BinSize,Aligned)];
        true->
            Bin = binary:encode_unsigned(Val),
            Len = byte_size(Bin),
...
per_enc_length(Bin,Unit,Len,Aligned) ->
    ...
    build_cond([[{lt,Len,128}, {put_bits,Len,8,U}, PutBits], [{lt,Len,16384}, {put_bits,2,2,U}, {put_bits,Len,14,[1]}, PutBits], ['_'| Al ++ [EncFragmented]]]).
...
per_enc_length(Bin,Unit0,Len,Sv,Aligned,Type)
    when is_integer(Sv)->
    NumBits = Sv * Unit0,
...
enc_length(Len,no,Aligned) ->
    U = unit(1,Aligned),
    build_cond([[{lt,Len,128}, {put_bits,Len,8,U}], [{lt,Len,16384}, {put_bits,2,2,U}, {put_bits,Len,14,[1]}]]);
...
put_bits_binary(Bin,_Unit,Aligned)
    when is_binary(Bin)->
    Sz = byte_size(Bin),
    <<Int:Sz/unit:8>> = Bin,
...
opt_length_zlb([[{lt,_,Val}| _] = H| T],Ub) ->
    if Val =< Ub ->
        opt_length_zlb(T,Ub);true ->
        [H| opt_length_zlb(T,max(Ub,Val))] end;
...
opt_length_nzlb([[{lt,Var0,Val}| _] = H| T],{Var0,_,_} = St,Ub) ->
    if Val =< Ub ->
        opt_length_nzlb(T,St,Ub);true ->
        [H| opt_length_nzlb(T,St,Val)] end;
...
per_enc_enumerated_root_1([{H,_}| T],Prefix,Val,Constr,Aligned,N) ->
    [[{eq,Val,H}| Prefix ++ per_enc_integer_1(N,Constr,Aligned)]| per_enc_enumerated_root_1(T,Prefix,Val,Constr,Aligned,N + 1)];
...
per_enc_enumerated_ext_1([{H,_}| T],Val,Aligned,N) ->
    [[{eq,Val,H}, {put_bits,1,1,[1]}| per_enc_small_number(N,Aligned)]| per_enc_enumerated_ext_1(T,Val,Aligned,N + 1)];
...
per_enc_small_length(Val0,Aligned) ->
    ...
    U = unit(1,Aligned),
    Sub ++ build_cond([[{lt,Val,64}, {put_bits,Val,7,[1]}], [{lt,Val0,128}, {put_bits,1,1,[1]}, {put_bits,Val0,8,U}], ['_', {put_bits,1,1,[1]}, {put_bits,2,2,U}, {put_bits,Val0,14,[1]}]]).

constr_min_size(no) ->
    no;
constr_min_size({{Lb,_},[]})
    when is_integer(Lb)->
    Lb;
constr_min_size({Lb,_})
    when is_integer(Lb)->
...
opt_choice(Imm) ->
    {Pb,T0} = lists:splitwith(fun ({put_bits,V,_,_})
        when is_integer(V)->
        true;(_)->
        false end,Imm),
    try {Prefix,T} = split_off_nonbuilding(T0),
...
opt_lc([{lc,[{call,erlang,iolist_to_binary,[Var],Bin}, {call,erlang,byte_size,[Bin],LenVar}, {'cond',[[{eq,LenVar,Len}, {put_bits,Bin,_,[_| Align]}]]}],Var,Val}] = Lc,LenImm) ->
    CheckImm = [{'cond',[[{eq,{expr,"length(" ++ mk_val(Var) ++ ")"},Len}], [{eq,{expr,"byte_size(" ++ mk_val(Var) ++ ")"},Len}]]}],
    Al = case Align of
        []->
            [];
        [align]->
    ...
    case Al =:= [] orelse is_end_aligned(LenImm) orelse lb_is_nonzero(LenImm) of
        false->
            Lc;
        true->
...
opt_lc([{lc,ElementImm0,V,L}] = Lc,LenImm) ->
    case enc_opt_al_1(ElementImm0,0) of
        {ElementImm,0}->
            case is_end_aligned(LenImm) orelse is_beginning_aligned(ElementImm0) andalso lb_is_nonzero(LenImm) of
                false->
                    Lc;
                true->
...
is_beginning_aligned([{'cond',Cs}]) ->
    lists:all(fun ([_| Act])->
        is_beginning_aligned(Act) end,Cs);
is_beginning_aligned([{error,_}| _]) ->
    true;
...
do_combine([{'cond',Cs0}| T],Budget0) ->
    ...
    [{'cond',Cs}];
do_combine([{put_bits,V,_,_}| _] = L,Budget)
    when is_integer(V)->
...
collect_put_bits(Imm) ->
    lists:splitwith(fun ({put_bits,V,_,_})
        when is_integer(V)->
        true;(_)->
        false end,Imm).
...
enc_opt({binary,PutBits0},St) ->
    PutBits = [{put_bits,subst(V,St),Sz,F} || {put_bits,V,Sz,F} <- PutBits0],
    NumBits = lists:foldl(fun ({put_bits,_,Bits,_},Sum)->
        Sum + Bits end,0,PutBits),
...
enc_opt({call,erlang,bit_size,[Bin],Dst} = Imm0,St0) ->
    ...
    case t_range(Type) of
        any->
            ...
            St = propagate(Dst,fun (T,S)->
                bit_size_propagate(Bin,T,S) end,St1),
            {Imm0,St};
        {Lb,Ub} = Range->
...
enc_opt({call,erlang,byte_size,[Bin],Dst} = Imm0,St0) ->
    ...
    case t_range(Type) of
        any->
            ...
            St = propagate(Dst,fun (T,S)->
                byte_size_propagate(Bin,T,S) end,St1),
            {Imm0,St};
        {Lb0,Ub0}->
            Lb = (Lb0 + 7) div 8,
            Ub = (Ub0 + 7) div 8,
...
enc_opt({call,erlang,length,[List],Dst} = Imm0,St0) ->
    St1 = propagate(Dst,fun (T,S)->
        length_propagate(List,T,S) end,St0),
    {Imm0,St1};
...
enc_opt({sub,Src0,Int,Dst},St0) ->
    ...
    St = case t_range(Type) of
        any->
            propagate(Dst,fun (T,S)->
                set_type(Src,t_add(T,Int),S) end,St0);
        {Lb,Ub}->
...
eo_eval_cond({_Op,{expr,_},_Val},_St) ->
    maybe.
...
eval_cond_range(lt,{Lb,Ub},Val) ->
    if Ub < Val ->
        ...
        false;true ->
        maybe end;
eval_cond_range(_Op,_Range,_Val) ->
    maybe.
...
update_type_info({lt,{var,_} = Var,Val},St) ->
    ...
    Type = t_meet(get_type(Var,St),Int),
    set_type(Var,Type,St);
update_type_info({eq,{var,_} = Var,Val},St)
    when is_integer(Val)->
    Int = t_integer(Val),
    Type = t_meet(get_type(Var,St),Int),
    set_type(Var,Type,St);
...
propagate({var,Var},Propagate,#ost{sym = Sym0} = St)
    when is_function(Propagate,2)->
...
t_meet_types(integer,integer) ->
    integer;
t_meet_types(any,integer) ->
    integer.
...
t_join_types([T, T]) ->
    T;
t_join_types([align, any]) ->
    any;
t_join_types([align, _]) ->
    align;
t_join_types([any, _]) ->
    any;
t_join_types([bitlist, bitstring]) ->
    any;
t_join_types([bitlist, integer]) ->
    any;
t_join_types([bitlist, iolist]) ->
    bitlist;
t_join_types([bitlist, nil]) ->
    bitlist;
t_join_types([binary, bitlist]) ->
    bitlist;
t_join_types([binary, bitstring]) ->
    bitstring;
t_join_types([binary, integer]) ->
    binary;
t_join_types([binary, iolist]) ->
    iolist;
t_join_types([binary, nil]) ->
    iolist;
t_join_types([bitstring, integer]) ->
    any;
t_join_types([bitstring, iolist]) ->
    any;
t_join_types([bitstring, nil]) ->
    any;
t_join_types([integer, _]) ->
    any;
t_join_types([iolist, nil]) ->
    iolist.
...
t__cons_type({align,_}) ->
    align;
t__cons_type({any,_}) ->
    any;
t__cons_type({binary,_}) ->
    binary;
t__cons_type({bitstring,_}) ->
    bitstring;
t__cons_type({bitlist,_}) ->
    bitstring;
t__cons_type({integer,_}) ->
    binary;
t__cons_type({iolist,_}) ->
    binary;
t__cons_type({nil,_}) ->
    binary.

t__cons_range({integer,_}) ->
    {8,8};
t__cons_range({_,Range}) ->
    Range.

t__cons_ranges({Lb1,Ub1},{Lb2,Ub2}) ->
    {Lb1 + Lb2,Ub1 + Ub2};
t__cons_ranges(any,_) ->
    any;
t__cons_ranges(_,any) ->
    any.

t__range({Lb,Ub} = Range)
    when is_integer(Lb),
    is_integer(Ub)->
    Range;
t__range(any) ->
...
bit_string_name2pos_fun(NNL,Src) ->
    {call_gen,"bit_string_name2pos_",NNL,fun (Fd,Name)->
        gen_name2pos(Fd,Name,NNL) end,[],[Src]}.
...
enc_hoist_align([],_,Acc) ->
    Acc.

enc_hoist_block(Bl) ->
    try enc_hoist_block_1(lists:reverse(Bl))
        catch
            throw:impossible->
                Bl end.
...
enc_opt_al({list,Imm0,Dst},Al) ->
    ...
    {[{list,Imm,Dst}],Al};
enc_opt_al({put_bits,V,N,[U, align]},Al0)
    when Al0 rem 8 =:= 0->
    Al = if is_integer(N) ->
        N * U;N =:= binary,
    U rem 8 =:= 0 ->
        ...
        unknown end,
    {[{put_bits,V,N,[U]}],Al};
enc_opt_al({put_bits,V,binary,[U, align]},Al0)
    when is_integer(Al0)->
    N = 8 - Al0 rem 8,
    Al = case U rem 8 of
        0->
...
enc_opt_al({put_bits,V,N0,[U, align]},Al0)
    when is_integer(N0),
    is_integer(Al0)->
    N = N0 + (8 - Al0 rem 8),
    Al = N0 * U,
    {[{put_bits,V,N,[1]}],Al};
enc_opt_al({put_bits,_,N,[U, align]} = PutBits,_)
    when is_integer(N)->
    {[PutBits],N * U};
enc_opt_al({put_bits,_,binary,[U, align]} = PutBits,_)
    when U rem 8 =:= 0->
    {[PutBits],0};
enc_opt_al({put_bits,_,N,[U]} = PutBits,Al)
    when is_integer(N),
    is_integer(Al)->
    {[PutBits],Al + N * U};
enc_opt_al({put_bits,_,binary,[U]} = PutBits,Al)
    when U rem 8 =:= 0->
    {[PutBits],Al};
...
enc_opt_hoist_align([{'cond',Cs0}, {put_bits,0,0,[1, align]}] = Imm) ->
    try Cs = [(insert_align_last(C)) || C <- Cs0],
    [{'cond',Cs}]
        catch
            throw:impossible->
                Imm end;
enc_opt_hoist_align(Imm) ->
    Imm.
...
effective_constr('SingleValue',List) ->
    SVList = lists:flatten(lists:map(fun (X)->
        element(2,X) end,List)),
...
effective_constr('ValueRange',List) ->
    LBs = lists:map(fun ({_,{Lb,_}})->
        Lb end,List),
    UBs = lists:map(fun ({_,{_,Ub}})->
        Ub end,List),
    Lb = least_Lb(LBs),
...
greatest_common_range([{_,Int}],[{_,{Lb,Ub}}])
    ...
    Int < Lb->
    [{'ValueRange',{Int,Ub}}];
greatest_common_range([{_,Int}],VR = [{_,{_Lb,_Ub}}])
    when is_integer(Int)->
    VR;
greatest_common_range([{_,L}],[{_,{Lb,Ub}}])
    when is_list(L)->
    Min = least_Lb([Lb| L]),
