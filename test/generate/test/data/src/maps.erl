...
-export([get/3, filter/2, fold/3, map/2, size/1, new/0, update_with/3, update_with/4, without/2, with/2, iterator/1, next/1]).

-export([get/2, find/2, from_list/1, is_key/2, keys/1, merge/2, put/3, remove/2, take/2, to_list/1, update/3, values/1]).

-opaque(iterator(Key,Value)::{Key,Value,iterator(Key,Value)}|none|nonempty_improper_list(integer(),#{Key => Value})).
...
-spec(to_list(Map) -> [{Key,Value}] when Map::#{Key => Value}).

to_list(Map)
    when is_map(Map)->
    to_list_internal(erts_internal:map_next(0,Map,[]));
to_list(Map) ->
    error({badmap,Map},[Map]).

to_list_internal([Iter, Map| Acc])
...
-spec(update_with(Key,Fun,Map1) -> Map2 when Map1::#{Key := Value1,_ => _},Map2::#{Key := Value2,_ => _},Fun::fun((Value1) -> Value2)).
...
update_with(Key,Fun,Map) ->
    error(error_type(Map),[Key, Fun, Map]).

-spec(update_with(Key,Fun,Init,Map1) -> Map2 when Map1::#{Key => Value1,_ => _},Map2::#{Key := Value2|Init,_ => _},Fun::fun((Value1) -> Value2)).
...
update_with(Key,Fun,Init,Map) ->
    error(error_type(Map),[Key, Fun, Init, Map]).
...
get(Key,Map,Default) ->
    error({badmap,Map},[Key, Map, Default]).
...
filter(Pred,Iterator)
    when is_function(Pred,2),
    is_tuple(Iterator) andalso tuple_size(Iterator) == 3;
    Iterator == none;
    is_integer(hd(Iterator)) andalso is_map(tl(Iterator))->
    maps:from_list(filter_1(Pred,Iterator));
filter(Pred,Map) ->
    error(error_type(Map),[Pred, Map]).
...
fold(Fun,Init,Map) ->
    error(error_type_iter(Map),[Fun, Init, Map]).
...
map(Fun,Map) ->
    error(error_type_iter(Map),[Fun, Map]).
...
error_type_iter(M)
    when is_map(M);
    is_tuple(M) andalso tuple_size(M) == 3;
    M == none;
    is_integer(hd(M)) andalso is_map(tl(M))->
    badarg;
error_type_iter(V) ->
    {badmap,V}.