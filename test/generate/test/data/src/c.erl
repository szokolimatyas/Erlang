...
-export([help/0, lc/1, c/1, c/2, c/3, nc/1, nc/2, nl/1, l/1, i/0, i/1, ni/0, y/1, y/2, lc_batch/0, lc_batch/1, i/3, pid/3, m/0, m/1, mm/0, lm/0, bt/1, q/0, h/1, h/2, h/3, ht/1, ht/2, ht/3, hcb/1, hcb/2, hcb/3, erlangrc/0, erlangrc/1, bi/1, flush/0, regs/0, uptime/0, nregs/0, pwd/0, ls/0, ls/1, cd/1, memory/1, memory/0, xm/1]).

-export([display_info/1]).

-export([appcall/4]).

-import(lists, [reverse/1, flatten/1, sublist/3, sort/1, keysort/2, max/1, min/1, foreach/2, foldl/3, flatmap/2]).
...
help() ->
    io:put_chars(<<"bt(Pid)    -- stack backtrace for a process\nc(Mod)" "     -- compile and load module or file <Mod>\ncd(D" "ir)    -- change working directory\nflush()    -- f" "lush any messages sent to the shell\nhelp()     -- " "help info\nh(M)       -- module documentation\nh(M," "F)     -- module function documentation\nh(M,F,A)  " " -- module function arity documentation\ni()       " " -- information about the system\nni()       -- inf" "ormation about the networked system\ni(X,Y,Z)   -- " "information about pid <X,Y,Z>\nl(Module)  -- load o" "r reload module\nlm()       -- load all modified mo" "dules\nlc([File]) -- compile a list of Erlang modul" "es\nls()       -- list files in the current directo" "ry\nls(Dir)    -- list files in directory <Dir>\nm(" ")        -- which modules are loaded\nm(Mod)     --" " information about module <Mod>\nmm()       -- list" " all modified modules\nmemory()   -- memory allocat" "ion information\nmemory(T)  -- memory allocation in" "formation of type <T>\nnc(File)   -- compile and lo" "ad code in <File> on all nodes\nnl(Module) -- load " "module on all nodes\npid(X,Y,Z) -- convert X,Y,Z to" " a Pid\npwd()      -- print working directory\nq() " "       -- quit - shorthand for init:stop()\nregs() " "    -- information about registered processes\nnreg" "s()    -- information about all registered processe" "s\nuptime()   -- print node uptime\nxm(M)      -- c" "ross reference check a module\ny(File)    -- genera" "te a Yecc parser\n">>).
...
c(SrcFile,NewOpts,Filter,BeamFile,Info) ->
    F = fun (Opt)->
         not is_outdir_opt(Opt) andalso Filter(Opt) end,
    Options = NewOpts ++ [{outdir,filename:dirname(BeamFile)}] ++ lists:filter(F,old_options(Info)),
...
h(Module) ->
    case code:get_doc(Module) of
        {ok,#docs_v1{format = <<"application/erlang+html">>} = Docs}->
...
format_docs(Docs) ->
    ...
    _ = paged_output(fun (Line,_)->
        format("~ts",Line),
        {1,undefined} end,undefined,Lines),
    ok.
    ...
    case file:rename(BeamFile,Backup) of
        Status
            ...
            case compile_and_load(File,Options) of
                ...
                Error->
                    _ = if Status =:= ok ->
                        file:rename(Backup,BeamFile);true ->
                        ok end,
                    Error
            end;
        Error->
...
compile_and_load(File,Opts0)
    when is_list(Opts0)->
    Opts = [report_errors, report_warnings| ensure_from(filename:extension(File),ensure_outdir(".",Opts0))],
...
ensure_outdir(Dir,Opts0) ->
    {[Opt| _],Opts} = lists:partition(fun is_outdir_opt/1,Opts0 ++ [{outdir,Dir}]),
    [Opt| Opts].

is_outdir_opt({outdir,_}) ->
    true;
is_outdir_opt(_) ->
    false.

is_from_opt(from_core) ->
    true;
is_from_opt(from_asm) ->
    true;
is_from_opt(from_beam) ->
    true;
is_from_opt(_) ->
    false.
...
src_suffix([from_core| _]) ->
    ".core";
src_suffix([from_asm| _]) ->
    ".S";
src_suffix([from_beam| _]) ->
    ".beam";
src_suffix([_| Opts]) ->
    src_suffix(Opts);
src_suffix([]) ->
    ...
    case compile:output_generated(Opts) of
        true->
            case atom_to_list(Mod) of
                ...
                _OtherMod->
                    format("** Module name '~p' does not match file nam" "e '~tp' **~n",[Mod, File]),
                    {error,badfile}
            end;
        false->
...
-spec(lc(Files) -> ok|error when Files::[File::erl_compile:cmd_line_arg()]).
...
lc(Args) ->
    case  catch split(Args,[],[]) of
        ...
        {Opts,Files}->
            ...
            foreach(fun (File)->
                compile:file(File,COpts) end,reverse(Files))
    end.
...
split_def([$=| T],Res) ->
    {d,list_to_atom(reverse(Res)),make_term(T)};
split_def([H| T],Res) ->
    split_def(T,[H| Res]);
split_def([],Res) ->
    {d,list_to_atom(reverse(Res))}.
...
i(Ps) ->
    ...
    case paged_output(fun (Pid,{R,M,H,S})->
        ...
        {2,{R + A,M + B,H + C,S + D}} end,2,{0,0,0,0},Ps) of
        {R,M,H,S}->
...
paged_output(PrintFun,CurrLine,Limit,Acc,Items)
    when CurrLine >= Limit->
    case more() of
        more->
...
all_procs() ->
    case is_alive() of
        true->
            flatmap(fun (N)->
                rpc:call(N,erlang,processes,[]) end,[node()| nodes()]);
        false->
            processes()
    end.
...
m() ->
    mformat("Module","File"),
    foreach(fun ({Mod,File})->
        mformat(Mod,File) end,sort(code:all_loaded())).
...
f_p_e(P,F) ->
    case file:path_eval(P,F) of
        ...
        {error,E = {Line,_Mod,_Term}}->
            error("file:path_eval(~tp,~tp): error on line ~p: ~ts~n",[P, F, Line, file:format_error(E)]),
            {error,E};
        {error,E}->
            error("file:path_eval(~tp,~tp): ~ts~n",[P, F, file:format_error(E)]),
            {error,E};
        Other->
            Other
    end.
...
nregs() ->
    foreach(fun (N)->
        print_node_regs(N) end,all_regs()).
...
all_regs() ->
    case is_alive() of
        true->
            [{N,rpc:call(N,erlang,registered,[])} || N <- [node()| nodes()]];
        false->
            [{node(),registered()}]
    end.

print_node_regs({N,List})
    when is_list(List)->
    ...
    foreach(fun ({Name,PI,Pid})->
        procline(Name,PI,Pid) end,Pids),
    format("~n** Registered ports on node ~w **~n",[N]),
    portformat("Name","Id","Command"),
    foreach(fun ({Name,PI,Id})->
        portline(Name,PI,Id) end,Ports).
...
procline(Name,Info,Pid) ->
    ...
    Reds = fetch(reductions,Info),
    LM = fetch(message_queue_len,Info),
    procformat(io_lib:format("~tw",[Name]),io_lib:format("~w",[Pid]),io_lib:format("~ts",[mfa_string(Call)]),integer_to_list(Reds),integer_to_list(LM)).
...
portline(Name,Info,Id) ->
    Cmd = fetch(name,Info),
    portformat(io_lib:format("~tw",[Name]),port_to_list(Id),Cmd).
...
uptime({D,{H,M,S}}) ->
    lists:flatten([[(io_lib:format("~p days, ",[D])) || D > 0], [(io_lib:format("~p hours, ",[H])) || D + H > 0], [(io_lib:format("~p minutes and ",[M])) || D + H + M > 0], io_lib:format("~p seconds",[S])]).
    ...
    try apply(M,F,Args)
            ...
            error:undef:S->
                case S of
                    [{M,F,Args,_}| _]->
                        Arity = length(Args),
                        io:format("Call to ~w:~w/~w in application ~w faile" "d.\n",[M, F, Arity, App]);
                    Stk->
