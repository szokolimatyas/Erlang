...
seq(First,Last,Inc)
    ...
    First - Inc >= Last ->
        ...
        seq_loop(N,Inc * (N - 1) + First,Inc,[]);Inc =:= 0,
    First =:= Last ->
        seq_loop(1,First,Inc,[]) end.
...
keyreplace(K,N,L,New)
    when is_integer(N),
    N > 0,
    is_tuple(New)->
    keyreplace3(K,N,L,New).

keyreplace3(Key,Pos,[Tup| Tail],New)
    when element(Pos,Tup) == Key->
    [New| Tail];
...
-spec(rkeymerge(pos_integer(),[X],[Y]) -> [R] when X::tuple(),Y::tuple(),R::tuple()).

rkeymerge(Index,T1,L2)
    when is_integer(Index),
    Index > 0->
    case L2 of
        []->
...
-spec(rukeymerge(pos_integer(),[X],[Y]) -> [X|Y] when X::tuple(),Y::tuple()).

rukeymerge(Index,T1,L2)
    when is_integer(Index),
    Index > 0->
    case L2 of
        []->
...
umerge2_1([H1| T1],T2,M,_HdM,H2)
    when H1 =< H2->
    umerge2_1(T1,T2,[H1| M],H1,H2);
umerge2_1([H1| T1],T2,M,HdM,H2)
    when H2 == HdM->
    umerge2_2(T1,T2,M,H1);
umerge2_1([H1| T1],T2,M,_HdM,H2) ->
    umerge2_2(T1,T2,[H2| M],H1);
umerge2_1([],T2,M,HdM,H2)
    when H2 == HdM->
    lists:reverse(T2,M);
...
keymerge3_1(I,[],M,_D,E2,H2,T2,E3,H3,T3)
    when E2 =< E3->
...
keymerge3_2(I,E1,H1,T1,[],M,_D,E3,H3,T3)
    when E1 =< E3->
...
rkeymerge3_1(I,[],M,_D,E2,H2,T2,E3,H3,T3)
    when E2 =< E3->
...
rkeymerge3_21(I,E1,H1,T1,E2,H2,T2,E3,H3,T3,M,_D)
    when E1 =< E3->
...
ukeymergel(I,[T1, [H2| T2], [H3| T3]| L],Acc) ->
    M = ukeymerge3_1(I,T1,Acc,[H2| H3],element(I,H2),H2,T2,[],element(I,H3),H3,T3),
...
rukeymergel(I,[[H3| T3], [H2| T2], T1| L],Acc) ->
    M = rukeymerge3_1(I,T1,Acc,[],element(I,H2),H2,T2,[],element(I,H3),H3,T3),
...
ukeymerge3_1(I,[],_D,HdM,E2,_H2,T2,M,E3,H3,T3)
    when E2 == HdM->
    ukeymerge2_1(I,T2,E3,HdM,T3,M,H3);
ukeymerge3_1(I,[],_D,_HdM,E2,H2,T2,M,E3,H3,T3)
    when E2 =< E3->
    ukeymerge2_1(I,T2,E3,E2,T3,[H2| M],H3);
ukeymerge3_1(I,[],_D,HdM,E2,H2,T2,M,E3,_H3,T3)
    when E3 == HdM->
...
ukeymerge3_2(I,E1,T1,H1,[],_HdM,_D,M,E3,H3,T3)
    when E1 =< E3->
    ukeymerge2_1(I,T1,E3,E1,T3,[H1| M],H3);
ukeymerge3_2(I,E1,T1,H1,[],HdM,_D,M,E3,_H3,T3)
    when E3 == HdM->
...
rukeymerge3_1(I,[],_D1,_D2,E2,H2,T2,M,E3,H3,T3)
    when E2 =< E3->
...
rukeymerge3_2(I,E1,H1,T1,[],_H2M,E2M,M,E3,H3,T3)
    when E1 == E2M->
    rukeymerge2_1(I,T1,E3,T3,[H1| M],H3);
rukeymerge3_2(I,E1,H1,T1,[],H2M,_E2M,M,E3,H3,T3)
    when E1 =< E3->
...
rukeymerge3_12_3(I,E1,H1,T1,E2,H2,T2,M,E3M,_H3M,[])
    when E2 == E3M->
...
rukeymerge3_21_3(I,E1,H1,T1,E2,H2,T2,M,E3M,_H3M,[])
    when E1 == E3M->
...
ukeymerge2_1(_I,[],E2,HdM,T2,M,_H2)
    when E2 == HdM->
    lists:reverse(T2,M);
...
rukeymerge2_2(_I,T1,E1,[],M,E2M,_H2M,H1)
    when E1 == E2M->
    lists:reverse(T1,[H1| M]);
