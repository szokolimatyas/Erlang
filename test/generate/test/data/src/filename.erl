...
-export([absname/1, absname/2, absname_join/2, basename/1, basename/2, dirname/1, extension/1, join/1, join/2, pathtype/1, rootname/1, rootname/2, split/1, flatten/1, nativename/1, safe_relative_path/1]).
...
win_basenameb(<<Letter,$:,Rest/binary>>)
    when Letter >= $A andalso Letter =< $Z orelse Letter >= $a andalso Letter =< $z->
    basenameb(Rest,[<<"/">>, <<"\\">>]);
win_basenameb(O) ->
    basenameb(O,[<<"/">>, <<"\\">>]).

basenameb(Bin,Sep) ->
    Parts = [X || X <- binary:split(Bin,Sep,[global]),X =/= <<>>],
    if Parts =:= [] ->
        <<>>;true ->
        lists:last(Parts) end.
...
basename1([DirSep2| Rest],Tail,DirSep2)
    when is_integer(DirSep2)->
    basename1([$/| Rest],Tail,DirSep2);
basename1([Char| Rest],Tail,DirSep2)
...
skip_prefix([L, DrvSep| Name],DrvSep)
    when L >= $A andalso L =< $Z orelse L >= $a andalso L =< $z->
    Name;
skip_prefix(Name,_) ->
    Name.
...
basename([DirSep2| Rest],Ext,Tail,DirSep2)
    when is_integer(DirSep2)->
...
dirname(Name)
    ...
    {XPart0,Dirs} = case Drivesep of
        X
            ...
            case Name of
                <<DL,X,Rest/binary>>
                    when DL >= $A andalso DL =< $Z orelse DL >= $a andalso DL =< $z->
                    {<<DL,X>>,Rest};
                _->
                    {<<>>,Name}
            end;
...
extension([Char| Rest],Result,OsType)
    when is_integer(Char)->
    extension(Rest,Result,OsType);
...
join1([[]| Rest],RelativeName,Result,OsType) ->
    join1(Rest,RelativeName,Result,OsType);
join1([Char| Rest],RelativeName,Result,OsType)
    when is_integer(Char)->
    join1(Rest,RelativeName,[Char| Result],OsType);
join1([Atom| Rest],RelativeName,Result,OsType)
    when is_atom(Atom)->
...
rootname([$.| Rest],Root,Ext,OsType) ->
    rootname(Rest,Ext ++ Root,".",OsType);
rootname([Char| Rest],Root,[],OsType)
    when is_integer(Char)->
    rootname(Rest,[Char| Root],[],OsType);
rootname([Char| Rest],Root,Ext,OsType)
    when is_integer(Char)->
...
win32_splitb(<<Letter0,$:,Slash,Rest/binary>>)
    when (Slash =:= $\\ orelse Slash =:= $/) andalso (Letter0 >= $A andalso Letter0 =< $Z orelse Letter0 >= $a andalso Letter0 =< $z)->
    Letter = fix_driveletter(Letter0),
    L = binary:split(Rest,[<<"/">>, <<"\\">>],[global]),
    [<<Letter,$:,$/>>| [X || X <- L,X =/= <<>>]];
win32_splitb(<<Letter0,$:,Rest/binary>>)
    when Letter0 >= $A andalso Letter0 =< $Z orelse Letter0 >= $a andalso Letter0 =< $z->
    Letter = fix_driveletter(Letter0),
    L = binary:split(Rest,[<<"/">>, <<"\\">>],[global]),
    [<<Letter,$:>>| [X || X <- L,X =/= <<>>]];
win32_splitb(<<Slash,Slash,Rest/binary>>)
    when Slash =:= $\\ orelse Slash =:= $/->
    L = binary:split(Rest,[<<"/">>, <<"\\">>],[global]),
    [<<"//">>| [X || X <- L,X =/= <<>>]];
...
split([Char| Rest],Comp,Components,OsType)
    when is_integer(Char)->
...
-spec(safe_relative_path(Filename) -> unsafe|SafeFilename when Filename::file:name_all(),SafeFilename::file:name_all()).
...
-spec(find_src(Beam) -> {SourceFile,Options}|{error,{ErrorReason,Module}} when Beam::Module|Filename,Filename::atom()|string(),Module::module(),SourceFile::string(),Options::[Option],Option::{i,Path::string()}|{outdir,Path::string()}|{d,atom()},ErrorReason::non_existing|preloaded|interpreted).

find_src(Mod) ->
    find_src(Mod,[]).

-spec(find_src(Beam,Rules) -> {SourceFile,Options}|{error,{ErrorReason,Module}} when Beam::Module|Filename,Filename::atom()|string(),Rules::[{BinSuffix::string(),SourceSuffix::string()}],Module::module(),SourceFile::string(),Options::[Option],Option::{i,Path::string()}|{outdir,Path::string()}|{d,atom()},ErrorReason::non_existing|preloaded|interpreted).

find_src(Mod,Rules)
    when is_atom(Mod)->
    find_src(atom_to_list(Mod),Rules);
find_src(ModOrFile,Rules)
    when is_list(ModOrFile)->
    Extension = ".erl",
    Mod = list_to_atom(basename(ModOrFile,Extension)),
    case code:which(Mod) of
        Possibly_Rel_Path
            when is_list(Possibly_Rel_Path)->
            {ok,Cwd} = file:get_cwd(),
...
filter_options(Base,[Option| Rest],Result)
    when Option =:= export_all->
    filter_options(Base,Rest,[Option| Result]);
filter_options(Base,[Option| Rest],Result)
    when Option =:= binary->
    filter_options(Base,Rest,[Option| Result]);
filter_options(Base,[Tuple| Rest],Result)
    when element(1,Tuple) =:= d->
...
-type(basedir_opts()::#{author => string()|binary(),os => windows|darwin|linux,version => string()|binary()}).
...
basedir(Type,Application)
    when is_atom(Type),
    is_list(Application) orelse is_binary(Application)->
    basedir(Type,Application,#{}).
...
basedir(Type,Application,Opts)
    ...
    case {Type,Os} of
        ...
        {Type,_}
            when Type =:= site_config orelse Type =:= site_data->
...
basedir_windows_appdata() ->
    case os:getenv("APPDATA") of
        Invalid
            when Invalid =:= false orelse Invalid =:= []->
            noappdata;
        Val->
            {ok,Val}
    end.
...
validate_list(FileName,Enc,Os) ->
    try true = validate_list(FileName,Enc,Os,0) > 0
        catch
            _:_->
                false end.
...
validate_list([],_Enc,_Os,Chars) ->
    Chars;
validate_list(C,Enc,Os,Chars)
    when is_integer(C)->
    validate_char(C,Enc,Os),
    Chars + 1;
validate_list(A,Enc,Os,Chars)
    when is_atom(A)->
...
validate_char(C,_,_)
    when C < 1->
    throw(invalid);
validate_char(C,latin1,_)
    when C > 255->
    throw(invalid);
validate_char(C,utf8,_)
    when C >= 1114112->
    throw(invalid);
validate_char(C,utf8,{win32,_})
    when C > 65535->
    throw(invalid);
