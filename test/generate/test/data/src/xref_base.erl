...
delete(State)
    when State#xref.variables =:= not_set_up->
    ok;
delete(State) ->
    Fun = fun ({X,_})->
...
info(State) ->
    ...
    No = no_sum(State,D),
    [{library_path,State#xref.library_path}, {mode,State#xref.mode}, {no_releases,NoRels}, {no_applications,NoApps}] ++ No.
...
format_error({module_mismatch,Module,ReadModule}) ->
    io_lib:format("Name of read module ~tp does not match analyzed modu" "le ~tp~n",[ReadModule, Module]);
format_error({release_clash,{Release,Dir,OldDir}}) ->
    io_lib:format("The release ~tp read from ~tp clashes with release a" "lready read from ~tp~n",[Release, Dir, OldDir]);
format_error({application_clash,{Application,Dir,OldDir}}) ->
    io_lib:format("The application ~tp read from ~tp clashes with appli" "cation already read from ~tp~n",[Application, Dir, OldDir]);
format_error({module_clash,{Module,Dir,OldDir}}) ->
    io_lib:format("The module ~tp read from ~tp clashes with module alr" "eady read from ~tp~n",[Module, Dir, OldDir]);
...
format_error({no_such_info,Term}) ->
    io_lib:format("~tp is not one of 'modules', 'applications', 'releas" "es' and 'libraries'~n",[Term]);
format_error(E) ->
...
add_appldir(ApplDir,Release,Name,OB,OV,OW,OldState) ->
    ...
    AppInfo = #xref_app{name = AppName,rel_name = Release,vsn = Vsn,dir = Dir},
...
do_add_directory(Dir,AppName,Bui,Rec,Ver,War,State) ->
    ok = is_filename(Dir),
    {FileNames,Errors,Jams,Unreadable} = xref_utils:scan_directory(Dir,Rec,[".beam"],[".jam"]),
...
do_add_modules(Files,AppName,OB,OV,OW,State0) ->
    ...
    Reader = fun (SplitName,State)->
        _Pid = read_module(SplitName,AppName,OB,OV,OW,State) end,
    N = parallelism(),
...
start_readers([SplitName| Files],ReaderFun,State,N)
    when N > 0->
...
read_module(SplitName,AppName,Builtins,Verbose,Warnings,State) ->
    ...
    Fun = fun ()->
        Me ! {xref_base,read_a_module(SplitName,AppName,Builtins,Verbose,Warnings,Mode)} end,
...
read_a_module({Dir,BaseName},AppName,Builtins,Verbose,Warnings,Mode) ->
    ...
    case abst(File,Builtins,Mode) of
        ...
        {ok,M,Data,UnresCalls0}->
            ...
            case xref_utils:file_info(File) of
                {ok,{_,_,_,Time}}->
                    XMod = #xref_mod{name = M,app_name = AppName,dir = Dir,mtime = Time,builtins = Builtins,no_unresolved = NoUnresCalls},
                    {ok,PrepMod,Bad} = prepare_module(Mode,XMod,UnresCalls,Data),
                    foreach(fun ({Tag,B})->
                        warnings(Warnings,Tag,[[File, B]]) end,Bad),
                    {ok,PrepMod};
                Error->
                    Error
            end;
    ...
    case beam_lib:chunks(File,[exports, imports, attributes]) of
        {ok,{Mod,[{exports,X0}, {imports,I0}, {attributes,At}]}}->
            ...
            I = case Builtins of
                ...
                false->
                    Fun = fun ({M,F,A})->
                         not xref_utils:is_builtin(M,F,A) end,
                    filter(Fun,I0)
            end,
            {ok,Mod,{X,I,D},[]};
        Error
...
prepare_module(_Mode = functions,XMod,Unres0,Data) ->
    ...
    {ok,{functions,XMod,[DefAt, L, X, LCallAt, XCallAt, CallAt, LC, XC, EE, ECallAt, OL1, DF1, DF_11, DF_21, DF_31],NoCalls,Unres},DBad ++ Bad};
...
finish_module({functions,XMod,List,NoCalls,Unres},S) ->
    ...
    [DefAt2, L2, X2, LCallAt2, XCallAt2, CallAt2, LC2, XC2, EE2, ECallAt2, OL2, DF2, DF_12, DF_22, DF_32] = pack(List),
    LU = range(LC2),
    LPredefined = predefined_funs(LU),
    M = XMod#xref_mod.name,
    MS = xref_utils:xset(M,atom),
    T = from_sets({MS,DefAt2,L2,X2,LCallAt2,XCallAt2,CallAt2,LC2,XC2,LU,EE2,ECallAt2,Unres,LPredefined,OL2,DF2,DF_12,DF_22,DF_32}),
...
depr_mod({Depr,Bad0},X) ->
    ...
    Fun = {external,fun ({M,F,A})->
        xref_utils:is_builtin(M,F,A) end},
    ...
    Bad = map(fun (B)->
        {depr_attr,B} end,usort(Bad2 ++ Bad0)),
...
extra_edges(CAX,CAL,Bad0,F) ->
    ...
    Bad = Bad0 ++ to_external(difference(AXC0,AXC)) ++ to_external(difference(ALC0,ALC)),
    {AXC,ALC,Bad,LPreCAt2,XPreCAt2}.

no_info(X,L,LC,XC,EE,Unres,NoCalls,NoUnresCalls) ->
    NoUnres = no_elements(Unres),
    [{no_calls,{NoCalls - NoUnresCalls,NoUnresCalls}}, {no_function_calls,{no_elements(LC),no_elements(XC) - NoUnres,NoUnres}}, {no_functions,{no_elements(L),no_elements(X)}}, {no_inter_function_calls,no_elements(EE)}].
...
inter_graph(X,L,LC,XC,CallAt) ->
    ...
    R0 = xref_utils:xset(reachable(E1,G,[]),[{tspec(func),tspec(fun_edge)}]),
    true = digraph:delete(G),
    RL = restriction(R0,XL),
    RX = relative_product1(R0,XC),
    R = union(RL,converse(RX)),
    EE0 = projection({external,fun ({Ee2,{Ee1,_L}})->
        {Ee1,Ee2} end},R),
    EE = union(Es,EE0),
    SFun = {external,fun ({Ee2,{Ee1,Ls}})->
        {{Ee1,Ls},{Ee1,Ee2}} end},
...
remove_rel(S,RelName) ->
    Rels = [RelName],
    Fun = fun ({A,XApp},L)
        when XApp#xref_app.rel_name =:= Rels->
        [A| L];(_,L)->
        L end,
...
do_set_up(S,_VerboseOpt)
    when S#xref.variables =/= not_set_up->
    {ok,S};
...
do_set_up(S)
    when S#xref.mode =:= functions->
    ModDictList = dict:to_list(S#xref.modules),
    [DefAt0, L, X0, LCallAt, XCallAt, CallAt, LC, XC, LU, EE0, ECallAt, UC, LPredefined, OL, Mod_DF, Mod_DF_1, Mod_DF_2, Mod_DF_3] = make_families(ModDictList,19),
    ...
    Fun = {external,fun ({{M1,_F1,_A1},{M2,_F2,_A2}})->
        {M1,M2} end},
    ...
    RE = range(AE2RE),
    AM = domain(F1),
    {Undef,U0,Lib,Lib_DF,Lib_DF_1,Lib_DF_2,Lib_DF_3} = make_libs(XU,F1,AM,S#xref.library_path,S#xref.libraries),
    ...
    EE_local = specification({external,fun ({{M1,_,_},{M2,_,_}})->
        M1 =:= M2 end},EE_conv),
    EE_0 = converse(union(EE_local,EE_exported)),
    EE_1 = user_family(EE_0),
    EE1 = partition_family({external,fun ({{M1,_,_},_MFA2})->
        M1 end},EE_0),
    EE = family_union(family_difference(EE0,EE0),EE1),
    IFun = fun ({Mod,EE_M},XMods)->
        IMFun = fun (XrefMod)->
            [NoCalls, NoFunctionCalls, NoFunctions, _NoInter] = XrefMod#xref_mod.info,
            NewInfo = [NoCalls, NoFunctionCalls, NoFunctions, {no_inter_function_calls,length(EE_M)}],
            XrefMod#xref_mod{info = NewInfo} end,
        dict:update(Mod,IMFun,XMods) end,
...
do_set_up(S)
    ...
    {A2R,A} = make_A2R(S#xref.applications),
    R = set(dict:fetch_keys(S#xref.releases)),
    ME = projection({external,fun ({M1,{M2,_F2,_A2}})->
        {M1,M2} end},family_to_relation(I0)),
    ME2AE = multiple_relative_product({M2A,M2A},ME),
    AE = range(ME2AE),
    AE2RE = multiple_relative_product({A2R,A2R},AE),
    RE = range(AE2RE),
    {_Undef,U0,Lib,Lib_DF,Lib_DF_1,Lib_DF_2,Lib_DF_3} = make_libs(XU,X1,AM,S#xref.library_path,S#xref.libraries),
    {B,U} = make_builtins(U0),
    X1_B = family_union(X1,B),
    DF = family_union(family_intersection(Mod_DF,X1_B),Lib_DF),
    DF_1 = family_union(family_intersection(Mod_DF_1,X1_B),Lib_DF_1),
    ...
    X = family_union(X1,Lib),
    Empty = empty_set(),
    Vs = [{'X',X}, {'U',U}, {'B',B}, {'XU',XU}, {v,V}, {e,{Empty,Empty}}, {'M',M}, {'A',A}, {'R',R}, {'AM',AM}, {'UM',UM}, {'LM',LM}, {'ME',ME}, {'AE',AE}, {'RE',RE}, {'DF',DF}, {'DF_1',DF_1}, {'DF_2',DF_2}, {'DF_3',DF_3}, {me2ae,ME2AE}, {ae,AE2RE}, {m2a,M2A}, {a2r,A2R}, {def_at,Empty}, {call_at,Empty}, {e_call_at,Empty}, {l_call_at,Empty}, {x_call_at,Empty}],
    finish_set_up(S,Vs).
...
do_finish_set_up([{Key,Value}| Vs],T) ->
    ...
    Val = #xref_var{name = Key,value = Value,vtype = predef,otype = OType,type = Type},
...
predef_fun() ->
    ...
    fun ({_M,F,A})->
        member({F,A},PredefinedFuns) end.

make_defat(Undef,DefAt0) ->
    Zero = from_term(0),
    DAL = family_projection(fun (S)->
        constant_function(S,Zero) end,Undef),
...
make_libs(XU,F,AM,LibPath,LibDict) ->
    ...
    Fs = case is_empty_set(UM) of
        true->
            [];
        false
            when LibPath =:= code_path->
            BFun = fun (M,A)->
                case xref_utils:find_beam(M) of
                    {ok,File}->
                        [File| A];
                    _->
                        A
        ...
        false->
            ...
            MFun = fun ({M,XLib})->
                ...
                xref_utils:module_filename(Dir,M) end,
...
user_family(R) ->
    partition_family({external,fun ({_MFA1,{M2,_,_}})->
        M2 end},R).

do_variables(State) ->
    Fun = fun ({Name,#xref_var{vtype = user}},{P,U})->
        {P,[Name| U]};({Name,#xref_var{vtype = predef}},A = {P,U})->
...
do_info(S,Type,E)
    when is_atom(E)->
    do_info(S,Type,[E]);
do_info(S,modules,Modules0)
    when is_list(Modules0)->
    Modules = to_external(set(Modules0)),
    XMods = find_info(Modules,S#xref.modules,no_such_module),
    map(fun (XMod)->
        mod_info(XMod) end,XMods);
do_info(S,applications,Applications)
    ...
    map(fun (AppSum)->
        app_info(AppSum,S) end,AppSums);
do_info(S,releases,Releases)
    when is_list(Releases)->
...
rel_apps_sums(AR,RRA0,S) ->
    ...
    map(fun (RelAppsSums)->
        rel_info(RelAppsSums,S) end,Sum).
...
mod_info(XMod) ->
    #xref_mod{name = M,app_name = AppName,builtins = BuiltIns,dir = Dir,info = Info} = XMod,
...
no_sum([XMod| D],C0,UC0,LC0,XC0,UFC0,L0,X0,EV0,NoM) ->
    [{no_calls,{C,UC}}, {no_function_calls,{LC,XC,UFC}}, {no_functions,{L,X}}, {no_inter_function_calls,EV}] = XMod#xref_mod.info,
    no_sum(D,C0 + C,UC0 + UC,LC0 + LC,XC0 + XC,UFC0 + UFC,L0 + L,X0 + X,EV0 + EV,NoM);
no_sum([],C,UC,LC,XC,UFC,L,X,EV,NoM) ->
    [{no_analyzed_modules,NoM}, {no_calls,{C,UC}}, {no_function_calls,{LC,XC,UFC}}, {no_functions,{L,X}}, {no_inter_function_calls,EV}].
