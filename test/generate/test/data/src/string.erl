...
-export([is_empty/1, length/1, to_graphemes/1, reverse/1, equal/2, equal/3, equal/4, slice/2, slice/3, pad/2, pad/3, pad/4, trim/1, trim/2, trim/3, chomp/1, take/2, take/3, take/4, lexemes/2, nth_lexeme/3, uppercase/1, lowercase/1, titlecase/1, casefold/1, prefix/2, split/2, split/3, replace/3, replace/4, find/2, find/3, next_codepoint/1, next_grapheme/1]).

-export([to_float/1, to_integer/1]).

-export([len/1, concat/2, chr/2, rchr/2, str/2, rstr/2, span/2, cspan/2, substr/2, substr/3, tokens/2, chars/2, chars/3]).

-export([copies/2, words/1, words/2, strip/1, strip/2, strip/3, sub_word/2, sub_word/3, left/2, left/3, right/2, right/3, sub_string/2, sub_string/3, centre/2, centre/3, join/2]).
...
-spec(list_to_float(String) -> {Float,Rest}|{error,Reason} when String::string(),Float::float(),Rest::string(),Reason::no_float|not_a_list).

list_to_float(_) ->
    erlang:nif_error(undef).

-spec(list_to_integer(String) -> {Int,Rest}|{error,Reason} when String::string(),Int::integer(),Rest::string(),Reason::no_integer|not_a_list).
...
to_number(String,Number,Rest,List,_Tail)
...
equal_norm_nocase(A,A,_Norm) ->
    true;
equal_norm_nocase(A0,B0,Norm) ->
    case {unicode_util:cp(unicode_util:casefold(unicode_util:Norm(A0))),unicode_util:cp(unicode_util:casefold(unicode_util:Norm(B0)))} of
        {[CP| A],[CP| B]}->
            equal_norm_nocase(A,B,Norm);
        {[],[]}->
            true;
...
slice_l0(<<CP1/utf8,Bin/binary>>,N)
    when N > 0->
    slice_lb(Bin,CP1,N);
slice_l0(L,N) ->
    slice_l(L,N).
...
slice_bin(<<CP2/utf8,Bin/binary>>,CP1,N)
    when CP1 < 256,
    CP2 < 256,
    CP1 =/= $\r,
    N > 0->
...
trim_t([CP1| Cont] = Cs0,_,{GCs,CPs,_} = Seps)
    when is_integer(CP1)->
...
trim_t([Bin| Cont0],N,{GCs,_,_} = Seps0)
    when is_binary(Bin)->
...
take_lc([CP1| Cont] = Str0,{GCs,CPs,_} = Seps,Acc)
    when is_integer(CP1)->
    case lists:member(CP1,CPs) of
        true->
...
take_t([CP1| Cont] = Str0,_,{GCs,CPs,_} = Seps)
    when is_integer(CP1)->
    case lists:member(CP1,CPs) of
        true->
            [GC| Str] = unicode_util:gc(Str0),
...
take_t([Bin| Cont0],N,{GCs,_,_} = Seps0)
    when is_binary(Bin)->
    <<_:N/binary,Rest/binary>> = Bin,
    Seps = search_compile(Seps0),
    case bin_search(Rest,Cont0,Seps) of
...
take_tc([CP1| [CP2| _] = Cont],_,{GCs,_,_} = Seps)
    when CP1 < 256,
    CP2 < 256,
    CP1 =/= $\r->
...
take_tc([Bin| Cont0],N,{GCs,_,_} = Seps0)
    when is_binary(Bin)->
    <<_:N/binary,Rest/binary>> = Bin,
...
split_1([CP1| Cs] = Cs0,[C| _] = Needle,_,Where,Curr,Acc)
    when is_integer(CP1)->
    case CP1 =:= C of
        true->
        ...
        {Before,[Cs0| Cont],After}->
            ...
            case Where of
                ...
                trailing->
                    ...
                    split_1([Bin| Cont],Needle,Next,Where,Curr0,[rev(Curr), After]);
                all->
                    split_1(After,Needle,0,Where,[],[rev(Curr)| Acc])
            end
    end;
split_1(Cs0,[C| _] = Needle,_,Where,Curr,Acc)
    when is_list(Cs0)->
    case unicode_util:cp(Cs0) of
        [C| Cs]->
            case prefix_1(Cs0,Needle) of
        ...
        {Before,[Cs0],After}->
            case Where of
                ...
                trailing->
                    <<_/utf8,Cs/binary>> = Cs0,
                    Next = byte_size(Bin) - byte_size(Cs),
                    split_1(Bin,Needle,Next,Where,Curr0,[btoken(Before,Curr0), After]);
                all->
...
lexemes_m([CP| _] = Cs0,{GCs,CPs,_} = Seps0,Ts)
    when is_integer(CP)->
...
lexemes_m([Bin| Cont0],{GCs,_,_} = Seps0,Ts)
    when is_binary(Bin)->
...
lexemes_m(Cs0,{GCs,_,_} = Seps0,Ts)
    when is_list(Cs0)->
    case unicode_util:gc(Cs0) of
        [C| Cs]->
            case lists:member(C,GCs) of
...
lexemes_m(Bin,{GCs,_,_} = Seps0,Ts)
    when is_binary(Bin)->
...
lexeme_pick([CP| Cs1] = Cs0,{GCs,CPs,_} = Seps,Tkn)
    when is_integer(CP)->
    case lists:member(CP,CPs) of
        true->
            [GC| Cs2] = unicode_util:gc(Cs0),
...
lexeme_pick([Bin| Cont0],Seps,Tkn)
    when is_binary(Bin)->
    case bin_search(Bin,Cont0,Seps) of
        {nomatch,_}->
...
lexeme_pick(Cs0,{GCs,CPs,_} = Seps,Tkn)
    when is_list(Cs0)->
    case unicode_util:cp(Cs0) of
        [CP| Cs]->
            case lists:member(CP,CPs) of
...
nth_lexeme_m([Bin| Cont0],{GCs,_,_} = Seps0,N)
    when is_binary(Bin)->
    case bin_search_inv(Bin,Cont0,GCs) of
        {nomatch,Cont}->
...
nth_lexeme_m(Cs0,{GCs,_,_} = Seps0,N)
    when is_list(Cs0)->
    case unicode_util:gc(Cs0) of
        [C| Cs]->
            case lists:member(C,GCs) of
...
nth_lexeme_m(Bin,{GCs,_,_} = Seps0,N)
    when is_binary(Bin)->
    Seps = search_compile(Seps0),
    case bin_search_inv(Bin,[],GCs) of
        [Cs]
...
lexeme_skip([CP| Cs1] = Cs0,{GCs,CPs,_} = Seps)
    when is_integer(CP)->
    case lists:member(CP,CPs) of
        true->
            [GC| Cs2] = unicode_util:gc(Cs0),
...
lexeme_skip(Cs0,{GCs,CPs,_} = Seps)
    when is_list(Cs0)->
    case unicode_util:cp(Cs0) of
        [CP| Cs]->
            case lists:member(CP,CPs) of
...
s_word([Char| T],Stop,Char,Index,Res)
    when Index < Stop->
    s_word(strip(T,left,Char),Stop,Char,Index + 1,Res);
s_word([_| T],Stop,Char,Index,Res)
    when Index < Stop->
...
left(String,Len,Char)
    ...
    Slen = length(String),
    if Slen > Len ->
        substr(String,1,Len);Slen < Len ->
        l_pad(String,Len - Slen,Char);Slen =:= Len ->
        String end.
...
right(String,Len,Char)
    ...
    Slen = length(String),
    if Slen > Len ->
        substr(String,Slen - Len + 1);Slen < Len ->
        r_pad(String,Len - Slen,Char);Slen =:= Len ->
        String end.
...
centre(String,Len,Char)
    ...
    Slen = length(String),
    if Slen > Len ->
        substr(String,(Slen - Len) div 2 + 1,Len);Slen < Len ->
        N = (Len - Slen) div 2,
        r_pad(l_pad(String,Len - (Slen + N),Char),N,Char);Slen =:= Len ->
        String end.
