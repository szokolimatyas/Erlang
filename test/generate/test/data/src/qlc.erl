...
-export([cursor/1, cursor/2, next_answers/1, next_answers/2, delete_cursor/1]).
...
-export([template_state/0, aux_name/3, name_suffix/2, vars/1, var_ufold/2, var_fold/3, all_selections/1]).

-dialyzer(no_improper_lists).

-record(qlc_append, {hl}).

-record(qlc_table, {trav_fun,trav_MS,pre_fun,post_fun,info_fun,format_fun,lookup_fun,parent_fun,key_equality,lu_vals,ms = no_match_spec}).

-record(qlc_sort, {h,keypos,unique,compressed,order,fs_opts,tmpdir_usage = allowed,tmpdir}).
...
-record(qlc_opt, {unique = false,cache = false,max_lookup = -1,join = any,tmpdir = "",lookup = any,max_list = 512 * 1024,tmpdir_usage = allowed}).
...
cursor(QH,Options) ->
    case {options(Options,[unique_all, cache_all, tmpdir, spawn_options, max_list_size, tmpdir_usage]),get_handle(QH)} of
        {B1,B2}
            when B1 =:= badarg;
            B2 =:= badarg->
        ...
        {[GUnique, GCache, TmpDir, SpawnOptions0, MaxList, TmpUsage],H}->
            ...
            case cursor_process(H,GUnique,GCache,TmpDir,SpawnOptions,MaxList,TmpUsage) of
                Pid
                    when is_pid(Pid)->
...
delete_cursor(#qlc_cursor{c = {_,Owner}} = C)
    when Owner =/= self()->
...
eval(QH,Options) ->
    case {options(Options,[unique_all, cache_all, tmpdir, max_list_size, tmpdir_usage]),get_handle(QH)} of
        {B1,B2}
            when B1 =:= badarg;
            B2 =:= badarg->
            error(badarg,[QH, Options]);
        {[GUnique, GCache, TmpDir, MaxList, TmpUsage],Handle}->
            try Prep = prepare_qlc(Handle,[],GUnique,GCache,TmpDir,MaxList,TmpUsage),
            case setup_qlc(Prep,#setup{parent = self()}) of
                ...
                {Objs,Post,_LocalPost}
                    when is_function(Objs)->
                    try collect(Objs)
                        after post_funs(Post) end
            end
                catch
...
fold(Fun,Acc0,QH,Options) ->
    case {options(Options,[unique_all, cache_all, tmpdir, max_list_size, tmpdir_usage]),get_handle(QH)} of
        ...
        {[GUnique, GCache, TmpDir, MaxList, TmpUsage],Handle}->
            try Prep = prepare_qlc(Handle,not_a_list,GUnique,GCache,TmpDir,MaxList,TmpUsage),
            case setup_qlc(Prep,#setup{parent = self()}) of
                {Objs,Post,_LocalPost}
                    when is_function(Objs);
                    is_list(Objs)->
                    try fold_loop(Fun,Objs,Acc0)
                        after post_funs(Post) end
            end
                catch
                    throw:Term:Stacktrace->
...
format_error({used_generator_variable,V}) ->
    io_lib:format("generated variable ~w must not be used in list expre" "ssion",[V]);
format_error(binary_generator) ->
    io_lib:format("cannot handle binary generators",[]);
format_error(too_complex_join) ->
    io_lib:format("cannot handle join of three or more generators effic" "iently",[]);
...
format_error({Line,Mod,Reason})
    ...
    io_lib:format("~p: ~ts~n",[Line, lists:flatten(Mod:format_error(Reason))]);
format_error({bad_object,FileName}) ->
    io_lib:format("the temporary file \"~ts\" holding answers is corrup" "t",[FileName]);
...
format_error({premature_eof,FileName}) ->
    io_lib:format("\"~ts\": end-of-file was encountered inside some bin" "ary term",[FileName]);
...
info(QH,Options) ->
    case {options(Options,[unique_all, cache_all, flat, format, n_elements, depth, tmpdir, max_list_size, tmpdir_usage]),get_handle(QH)} of
        {B1,B2}
            when B1 =:= badarg;
            B2 =:= badarg->
            error(badarg,[QH, Options]);
        {[GUnique, GCache, Flat, Format, NElements, Depth, TmpDir, MaxList, TmpUsage],H}->
            try Prep = prepare_qlc(H,[],GUnique,GCache,TmpDir,MaxList,TmpUsage),
...
keysort(KeyPos,QH,Options) ->
    case {is_keypos(KeyPos),options(Options,[tmpdir, order, unique, compressed, size, no_files]),get_handle(QH)} of
        {true,[TmpDir, Order, Unique, Compressed| _],H}
            when H =/= badarg->
            #qlc_handle{h = #qlc_sort{h = H,keypos = {keysort,KeyPos},unique = Unique,compressed = Compressed,order = Order,fs_opts = listify(Options),tmpdir = TmpDir}};
        _->
...
next_answers(#qlc_cursor{c = {_,Owner}} = C,NumOfAnswers)
    when Owner =/= self()->
...
-spec(parse_transform(Forms,Options) -> Forms2 when Forms::[erl_parse:abstract_form()],Forms2::[erl_parse:abstract_form()],Options::[Option],Option::type_checker|compile:option()).
...
q(#qlc_lc{} = QLC_lc,Options) ->
    case options(Options,[unique, cache, max_lookup, join, lookup]) of
        [Unique, Cache, Max, Join, Lookup]->
            Opt = #qlc_opt{unique = Unique,cache = Cache,max_lookup = Max,join = Join,lookup = Lookup},
...
sort(QH,Options) ->
    case {options(Options,[tmpdir, order, unique, compressed, size, no_files]),get_handle(QH)} of
        {B1,B2}
            when B1 =:= badarg;
            B2 =:= badarg->
            error(badarg,[QH, Options]);
        {[TD, Order, Unique, Compressed| _],H}->
            #qlc_handle{h = #qlc_sort{h = H,keypos = sort,unique = Unique,compressed = Compressed,order = Order,fs_opts = listify(Options),tmpdir = TD}}
    end.
...
string_to_handle(Str,Options,Bindings)
    when is_list(Str)->
    case options(Options,[unique, cache, max_lookup, join, lookup]) of
        badarg->
        ...
        [Unique, Cache, MaxLookup, Join, Lookup]->
            case erl_scan:string(Str,1,[text]) of
                {ok,Tokens,_}->
                    ScanRes = case erl_eval:extended_parse_exprs(Tokens) of
                        {ok,[Expr0],SBs}->
                            {ok,Expr0,SBs};
                        {ok,_ExprList,_SBs}->
                            error(badarg,[Str, Options, Bindings]);
                        ...
                        {ok,Expr,XBs}->
                            ...
                            case qlc_pt:transform_expression(Expr,Bs1) of
                                {ok,{call,_,_QlcQ,Handle}}->
                                    ...
                                    O = #qlc_opt{unique = Unique,cache = Cache,max_lookup = MaxLookup,join = Join,lookup = Lookup},
...
table(TraverseFun,Options)
    when is_function(TraverseFun)->
    case {is_function(TraverseFun,0),IsFun1 = is_function(TraverseFun,1)} of
        {false,false}->
            error(badarg,[TraverseFun, Options]);
        _->
            case options(Options,[pre_fun, post_fun, info_fun, format_fun, lookup_fun, parent_fun, key_equality]) of
                [PreFun, PostFun, InfoFun, FormatFun, LookupFun, ParentFun, KeyEquality]->
                    T = #qlc_table{trav_fun = TraverseFun,pre_fun = PreFun,post_fun = PostFun,info_fun = InfoFun,parent_fun = ParentFun,trav_MS = IsFun1,format_fun = FormatFun,lookup_fun = LookupFun,key_equality = KeyEquality},
                    #qlc_handle{h = T};
                badarg->
                    error(badarg,[TraverseFun, Options])
            end
...
-spec(transform_from_evaluator(LC,Bs) -> Return when LC::abstract_expr(),Bs::erl_eval:binding_struct(),Return::{ok,abstract_expr()}|{not_ok,{error,module(),Reason::term()}}).
...
merge_binding_structs(Bs1,Bs2) ->
    lists:foldl(fun ({N,V},Bs)->
        erl_eval:add_binding(N,V,Bs) end,Bs1,erl_eval:bindings(Bs2)).
...
options(Options0,[Key| Keys],L)
    when is_list(Options0)->
    ...
    V = case lists:keyfind(Key,1,Options) of
        ...
        {unique_all,UniqueAll}
            when is_boolean(UniqueAll)->
            {ok,UniqueAll};
        {cache_all,CacheAll}
            when is_boolean(CacheAll);
            CacheAll =:= list->
            {ok,CacheAll};
        {cache_all,ets}->
            {ok,true};
...
spawn_options(default) ->
    [link];
spawn_options(SpawnOptions) ->
    lists:delete(monitor,case lists:member(link,SpawnOptions) of
        true->
            SpawnOptions;
        false->
            [link| SpawnOptions]
...
-record(optz, {unique = false,cache = false,join_option = any,fast_join = no,opt}).

-record(qlc, {lcf,codef,qdata,init_value,optz}).

-record(simple_qlc,{p,le,line::erl_anno:anno(),init_value,
optz}).

-record(prepared, {qh,sorted = no,sort_info = [],sort_info2 = [],lu_skip_quals = [],join = {[],[]},n_objs = undefined,is_unique_objects = false,is_cached = false}).
...
cursor_process(H,GUnique,GCache,TmpDir,SpawnOptions,MaxList,TmpUsage) ->
    ...
    CF = fun ()->
        ...
        {Objs,Post,_LocalPost} = try Prep = prepare_qlc(H,not_a_list,GUnique,GCache,TmpDir,MaxList,TmpUsage),
        setup_qlc(Prep,Setup)
            catch
                Class:Reason:Stacktrace->
                    Parent ! {self(),{caught,Class,Reason,Stacktrace}},
                    exit(normal) end,
        Parent ! {self(),ok},
        wait_for_request(Parent,MonRef,Post),
        reply(Parent,MonRef,Post,Objs) end,
...
parent_fun(Pid,Parent) ->
    ...
    {TPid,{parent_fun,Fun}}->
        V = try {value,Fun()}
                ...
                Class:Reason:Stacktrace->
                    {parent_fun_caught,Class,Reason,Stacktrace} end,
        TPid ! {Parent,V},
        parent_fun(Pid,Parent);
    {Pid,{caught,throw,Error,[{qlc,throw_error,_,_}| _]}}->
        Error;
    {Pid,{caught,Class,Reason,Stacktrace}}->
        erlang:raise(Class,Reason,Stacktrace) end.
...
reply(Parent,MonRef,Post,Cont) ->
    Reply = try if is_function(Cont) ->
        Cont();true ->
        throw_error(Cont) end
        catch
            Class:Reason:Stacktrace->
                post_funs(Post),
...
wait_for_request(Parent,MonRef,Post) ->
    ...
    {'EXIT',Pid,_Reason}
        ...
        wait_for_request(Parent,MonRef,Post);
    Other->
        error_logger:error_msg("The qlc cursor ~w received an unexp" "ected message:\n~tp\n",[self(), Other]),
        wait_for_request(Parent,MonRef,Post) end.
...
abstract1({qlc,E0,Qs0,Opt},NElements,Depth,A) ->
    Qs = lists:map(fun ({generate,P,LE})->
        {generate,A,binary_to_term(P),abstract1(LE,NElements,Depth,A)};(F)->
        binary_to_term(F) end,Qs0),
    E = binary_to_term(E0),
    Os = case Opt of
        []->
            [];
...
abstract1({append,Infos},NElements,Depth,A) ->
    As = lists:foldr(fun (Info,As0)->
        {cons,A,abstract1(Info,NElements,Depth,A),As0} end,{nil,A},Infos),
    {call,A,{remote,A,{atom,A,qlc},{atom,A,append}},[As]};
abstract1({sort,Info,SortOptions},NElements,Depth,A) ->
    {call,A,{remote,A,{atom,A,qlc},{atom,A,sort}},[abstract1(Info,NElements,Depth,A), abstract_term(SortOptions,1)]};
abstract1({keysort,Info,Kp,SortOptions},NElements,Depth,A) ->
    {call,A,{remote,A,{atom,A,qlc},{atom,A,keysort}},[abstract_term(Kp,1), abstract1(Info,NElements,Depth,A), abstract_term(SortOptions,1)]};
abstract1({list,L,MS},NElements,Depth,A) ->
    {call,A,{remote,A,{atom,A,ets},{atom,A,match_spec_run}},[abstract1(L,NElements,Depth,A), {call,A,{remote,A,{atom,A,ets},{atom,A,match_spec_compile}},[abstract_term(depth(MS,Depth),1)]}]};
...
depth1(List,D)
    ...
    if D =:= 1 ->
        ['...'];true ->
        depth_list(List,D - 1) end;
depth1(Binary,D)
    when byte_size(Binary) > D - 1->
    D1 = D - 1,
        ...
        false->
            ...
            case erlang:fun_info(Fun,type) of
                {type,external}->
                    ...
                    {'fun',Line,{function,{atom,Line,Module},{atom,Line,Name},{integer,Line,Arity}}};
                {type,local}->
...
abstr_term(Map,Line)
    when is_map(Map)->
    {map,Line,[{map_field_assoc,Line,abstr_term(K,Line),abstr_term(V,Line)} || {K,V} <- maps:to_list(Map)]};
...
flatten_abstr({call,L1,{remote,L2,{atom,L3,qlc},{atom,L4,q}},[LC0| Os]},VN0,Vars,Body0) ->
    {lc,L,E,Qs0} = LC0,
    F = fun ({generate,Ln,P,LE0},{VN1,Body1})->
        {VN2,Body2,LE} = flatten_abstr(LE0,VN1,Vars,Body1),
        {{generate,Ln,P,LE},{VN2,Body2}};(Fil,VN_Body)->
        {Fil,VN_Body} end,
...
flatten_abstr(T0,VN0,Vars,Body0)
    when is_tuple(T0)->
...
le_info(#prepared{qh = #simple_qlc{le = LE,p = P,line = L,optz = Optz}},InfOpt) ->
    QVar = term_to_binary({var,L,P}),
    {qlc,QVar,[{generate,QVar,le_info(LE,InfOpt)}],opt_info(Optz)};
le_info(#prepared{qh = #qlc{codef = CodeF,qdata = Qdata,optz = Optz}},InfOpt) ->
    Code = CodeF(),
...
le_info(#prepared{qh = #qlc_table{format_fun = FormatFun,trav_MS = TravMS,ms = MS,lu_vals = LuVals}},InfOpt) ->
    {NElements,Depth} = InfOpt,
    DepthFun = depth_fun(Depth),
    case LuVals of
        _
            when FormatFun =:= undefined->
            {table,{'$MOD','$FUN',[]}};
        {Pos,Vals}->
            Formated = try FormatFun({lookup,Pos,Vals,NElements,DepthFun})
                catch
                    _:_->
                        FormatFun({lookup,Pos,Vals}) end,
            if MS =:= no_match_spec ->
                {table,Formated};true ->
                {list,{table,Formated},depth(MS,Depth)} end;
        _
            when TravMS,
            is_list(MS)->
...
qual_info([{_QNum,_GoI,_SI,{gen,#join{}}}| Qdata],Code,InfOpt) ->
    [skip| qual_info(Qdata,Code,InfOpt)];
qual_info([{QNum,_GoI,_SI,{gen,LE}}| Qdata],Code,InfOpt) ->
    [{generate,element(QNum + 1,Code),le_info(LE,InfOpt)}| qual_info(Qdata,Code,InfOpt)];
qual_info([],_Code,_InfOpt) ->
    [].

join_info(Join,QInfo,Qdata,Code) ->
    #qlc_join{kind = Kind,q1 = QNum1a,c1 = C1,q2 = QNum2a,c2 = C2,opt = Opt} = Join,
    {{JQNum,_,_,_},Rev,QNum1,QNum2,_WH1,_WH2,CsFun} = find_join_data(Qdata,QNum1a,QNum2a),
    {Cs1_0,Cs2_0,Compat} = CsFun(),
    [Cs1, Cs2] = case Compat of
        []->
            [[{C,[{V,'=:='} || V <- Vs]} || {C,Vs} <- CVs] || CVs <- [Cs1_0, Cs2_0]];
        _->
            [Cs1_0, Cs2_0]
    end,
    L = anno0(),
    ...
    {{I1,G1},{I2,G2},QInfoL} = case Kind of
        {merge,_}->
    ...
    JFil = term_to_binary({op,L,JOp,{call,L,{atom,L,element},[{integer,L,C1}, G1]},{call,L,{atom,L,element},[{integer,L,C2}, G2]}}),
...
join_merge_info(QNum,QInfo,Code,G,ExtraConstants) ->
    ...
    case {P,ExtraConstants} of
        ...
        _->
            ...
            F = list2op([{op,A,Op,abstract_term(Con),Call} || {Con,Op} <- ConstOps],'or',A),
            term_to_binary(F) end || {Col,ConstOps} <- ExtraConstants],
...
opt_info(#optz{unique = Unique,cache = Cache0,join_option = JoinOption}) ->
    Cache = if Cache0 ->
        ets;true ->
        Cache0 end,
    [{T,V} || {T,V} <- [{cache,Cache}, {unique,Unique}],V =/= default_option(T)] ++ [{T,V} || {T,V} <- [{join,JoinOption}],V =:= nested_loop].

prepare_qlc(H,InitialValue,GUnique,GCache,TmpDir,MaxList,TmpUsage) ->
    GOpt = #qlc_opt{unique = GUnique,cache = GCache,tmpdir = TmpDir,max_list = MaxList,tmpdir_usage = TmpUsage},
...
prep_le(#qlc_lc{lc = LC_fun,opt = #qlc_opt{} = Opt0} = H,GOpt) ->
    #qlc_opt{unique = GUnique,cache = GCache,tmpdir = TmpDir,max_list = MaxList,tmpdir_usage = TmpUsage} = GOpt,
    Unique = Opt0#qlc_opt.unique or GUnique,
    Cache = if  not GCache ->
        Opt0#qlc_opt.cache;true ->
        GCache end,
    Opt = Opt0#qlc_opt{unique = Unique,cache = Cache,tmpdir = TmpDir,max_list = MaxList,tmpdir_usage = TmpUsage},
...
prep_le(#qlc_table{info_fun = IF} = T,GOpt) ->
    ...
    Prep = #prepared{qh = T,sort_info = SortInfo,sorted = Sorted,is_unique_objects = IsUnique},
    Opt = if IsUnique or  not GOpt#qlc_opt.unique,
    T#qlc_table.ms =:= no_match_spec ->
        GOpt#qlc_opt{cache = false};true ->
        GOpt end,
    may_create_simple(Opt,Prep);
prep_le(#qlc_append{hl = HL},GOpt) ->
    case lists:flatmap(fun (#prepared{qh = #qlc_list{l = []}})->
        [];(#prepared{qh = #qlc_append{hl = HL1}})->
        HL1;(H)->
        [H] end,[(prep_le(H,GOpt)) || H <- HL]) of
        [] = Nil->
            short_list(Nil);
        [Prep]->
            Prep;
        PrepL->
            Cache = lists:all(fun (#prepared{is_cached = IsC})->
                IsC =/= false end,PrepL),
...
prep_le([_, _| _] = L,GOpt) ->
    ...
    Opt = if  not GOpt#qlc_opt.unique ->
        GOpt#qlc_opt{cache = false};true ->
        GOpt end,
    may_create_simple(Opt,Prep);
prep_le(L,_GOpt)
    ...
    short_list(L);
prep_le(T,_GOpt) ->
    error({unsupported_qlc_handle,#qlc_handle{h = T}}).
...
prep_qlc_lc({qlc_v1,QFun,CodeF,Qdata0,QOpt},Opt,GOpt,_H) ->
    F = fun ({_QNum,_GoI,_SI,fil} = QualData,ModGens)->
        {QualData,ModGens};({_QNum,_GoI,_SI,{gen,#join{}}} = QualData,ModGens)->
        {QualData,ModGens};({QNum,GoI,SI,{gen,LE_fun}},ModGens0)->
        Prep1 = eval_le(LE_fun,GOpt),
        {Prep,ModGens} = prep_generator(QNum,Prep1,QOpt,Opt,ModGens0),
        {{QNum,GoI,SI,{gen,Prep}},ModGens} end,
    ...
    case ModGens of
        ...
        _->
            ...
            LU_SkipQuals = lists:flatmap(fun ({QNum,_LookUp,Fs,_Prep})->
                [{QNum,Fs}] end,ModGens),
            Prep1 = Prep0#prepared{lu_skip_quals = LU_SkipQuals},
            prep_join(Prep1,QOpt,Opt)
    end;
prep_qlc_lc(_,_Opt,_GOpt,H) ->
    error({unsupported_qlc_handle,#qlc_handle{h = H}}).

prep_generator(QNum,Prep0,QOpt,Opt,ModGens) ->
    PosFun = fun (KeyEquality)->
        pos_fun(KeyEquality,QOpt,QNum) end,
...
pos_fun('=:=',QOpt,QNum) ->
    {'=:=',constants(QOpt,QNum)};
pos_fun('==',QOpt,QNum) ->
    try {'==',equal_constants(QOpt,QNum)}
        catch
            _:_->
                {'=:=',constants(QOpt,QNum)} end.

prep_gen(#qlc_table{lu_vals = LuV0,ms = MS0,trav_MS = TravMS,info_fun = IF,lookup_fun = LU_fun,key_equality = KeyEquality} = LE0,Prep0,PosFun0,{MS,Fs},Opt) ->
    PosFun = PosFun0(KeyEquality),
    {LuV,{STag,SkipFils}} = find_const_positions(IF,LU_fun,PosFun,Opt),
    LU = LuV =/= false,
    if LuV0 =/= undefined;
    MS0 =/= no_match_spec ->
        {no,[],false,Prep0};MS =/= no_match_spec,
    LU ->
        MS1 = if Fs =:= SkipFils;
        STag =:= Fs ->
            case MS of
        ...
        {replace,Fs,LU,Prep};LU ->
        Prep = Prep0#prepared{qh = LE0#qlc_table{lu_vals = LuV}},
        {skip,SkipFils,LU,Prep};TravMS,
    MS =/= no_match_spec ->
        Prep = Prep0#prepared{qh = LE0#qlc_table{ms = MS},is_unique_objects = false},
        {replace,Fs,false,may_create_simple(Opt,Prep)};true ->
        {no,[],false,Prep0} end;
...
prep_gen(#qlc_list{ms = no_match_spec} = LE0,Prep0,_PosFun,{MS,Fs},Opt)
    when MS =/= no_match_spec->
    Prep = Prep0#prepared{qh = LE0#qlc_list{ms = MS},is_cached = false},
...
may_create_simple(#qlc_opt{unique = Unique,cache = Cache} = Opt,#prepared{is_cached = IsCached,is_unique_objects = IsUnique} = Prep) ->
    if Unique and  not IsUnique;
    (Cache =/= false) and  not IsCached ->
        prep_simple_qlc('SQV',anno(1),Prep,Opt);true ->
        Prep end.

prep_simple_qlc(PVar,Line,LE,Opt) ->
    check_join_option(Opt),
    #prepared{is_cached = IsCached,sort_info = SortInfo,sorted = Sorted,is_unique_objects = IsUnique} = LE,
    #qlc_opt{unique = Unique,cache = Cache} = Opt,
    Cachez = if Unique ->
        Cache; not IsCached ->
        Cache;true ->
        false end,
    Optz = #optz{unique = Unique and  not IsUnique,cache = Cachez,opt = Opt},
    QLC = #simple_qlc{p = PVar,le = LE,line = Line,init_value = not_a_list,optz = Optz},
    #prepared{qh = QLC,is_unique_objects = IsUnique or Unique,sort_info = SortInfo,sorted = Sorted,is_cached = IsCached or (Cachez =/= false)}.

prep_sort(#qlc_sort{h = #prepared{sorted = yes} = Prep},_GOpt) ->
    Prep;
prep_sort(#qlc_sort{h = #prepared{is_unique_objects = IsUniqueObjs}} = Q,GOpt) ->
    ...
    #prepared{qh = S,is_cached = true,sort_info = SortInfo,sorted = Sorted,is_unique_objects = S#qlc_sort.unique or IsUniqueObjs}.

prep_qlc(QFun,CodeF,Qdata0,QOpt,Opt) ->
    #qlc_opt{unique = Unique,cache = Cache,join = Join} = Opt,
    Optz = #optz{unique = Unique,cache = Cache,join_option = Join,opt = Opt},
    {Qdata,SortInfo} = qlc_sort_info(Qdata0,QOpt),
    QLC = #qlc{lcf = QFun,codef = CodeF,qdata = Qdata,init_value = not_a_list,optz = Optz},
    #prepared{qh = QLC,sort_info = SortInfo,is_unique_objects = Unique,is_cached = Cache =/= false}.

qlc_sort_info(Qdata0,QOpt) ->
    F = fun ({_QNum,_GoI,_SI,fil} = Qd,Info)->
        {Qd,Info};({_QNum,_GoI,_SI,{gen,#join{}}} = Qd,Info)->
        {Qd,Info};({QNum,GoI,SI,{gen,PrepLE0}},Info)->
        PrepLE = sort_info(PrepLE0,QNum,QOpt),
        Qd = {QNum,GoI,SI,{gen,PrepLE}},
        I = [{{Column,Order},[{traverse,QNum,C}]} || {{C,Order},What} <- PrepLE#prepared.sort_info2,What =:= [],Column <- equal_template_columns(QOpt,{QNum,C})],
        {Qd,[I| Info]} end,
    {Qdata,SortInfoL} = lists:mapfoldl(F,[],Qdata0),
    SortInfo0 = [{{Pos,Ord},[template]} || Pos <- constant_columns(QOpt,0),Ord <- orders(yes)] ++ lists:append(SortInfoL),
    SortInfo = family_union(SortInfo0),
    {Qdata,SortInfo}.

sort_info(#prepared{sort_info = SI,sorted = S} = Prep,QNum,QOpt) ->
    SI1 = [{{C,Ord},[]} || S =/= no,is_integer(Sz = size_of_qualifier(QOpt,QNum)),Sz > 0,(NConstCols = size_of_constant_prefix(QOpt,QNum)) < Sz,C <- [NConstCols + 1],Ord <- orders(S)] ++ [{{Pos,Ord},[]} || Pos <- constant_columns(QOpt,QNum),Ord <- orders(yes)] ++ [{PosOrd,[]} || {PosOrd,_} <- SI],
    SI2 = lists:usort(SI1),
...
sort_unique(true,#qlc_sort{fs_opts = SortOptions,keypos = sort} = Sort) ->
    Sort#qlc_sort{unique = false,fs_opts = lists:keydelete(unique,1,lists:delete(unique,SortOptions))};
sort_unique(_,Sort) ->
    Sort.
...
short_list(L) ->
    #prepared{qh = #qlc_list{l = L},sorted = yes,is_unique_objects = true,is_cached = true}.

find_const_positions(IF,LU_fun,{KeyEquality,PosFun},#qlc_opt{max_lookup = Max,lookup = Lookup})
...
pref_join(Equal,Match,Prep,QOpt,#qlc_opt{join = JoinOpt}) ->
    JQs = [{KeyEquality,QCs} || {KeyEquality,QCsL} <- [{'==',Equal}, {'=:=',Match}],QCs <- QCsL],
    IxL = [(pref_lookup_join(KE,QCs,Prep,QOpt)) || JoinOpt =:= any orelse JoinOpt =:= lookup,{KE,QCs} <- JQs],
    ML = [(pref_merge_join(KE,QCs,Prep,QOpt)) || JoinOpt =:= any orelse JoinOpt =:= merge,{KE,QCs} <- JQs],
...
pref_lookup_join(KeyEquality,{[{Q1,C1}, {Q2,C2}],Skip},Prep,QOpt)
    ...
    Lu2 = [(pref_lookup_join2(Q2,C2,Q1,C1,Skip,QOpt,KeyEquality)) || IC1 <- Is1,IC1 =:= C1],
    Is2 = lookup_qual_data(QData,Q2,KeyEquality),
    Lu1 = [(pref_lookup_join2(Q1,C1,Q2,C2,Skip,QOpt,KeyEquality)) || IC2 <- Is2,IC2 =:= C2],
    family(Lu1 ++ Lu2);
pref_lookup_join(KE,[{_,Cs1}, {_,Cs2}] = L,Prep,QOpt)
    when is_list(Cs1),
    is_list(Cs2)->
    lists:append([(pref_lookup_join(KE,QC,Prep,QOpt)) || QC <- selections_no_skip(L)]).
...
join_indices(#prepared{qh = #qlc_table{info_fun = IF,lookup_fun = LU_fun,key_equality = KeyEquality,lu_vals = undefined}},KE)
    when is_function(LU_fun),
    KE =:= KeyEquality orelse KE =:= '=:=' andalso KeyEquality =:= undefined->
...
pref_merge_join(KE,[{_,Cs1}, {_,Cs2}] = L,Prep,QOpt)
    when is_list(Cs1),
    is_list(Cs2)->
    lists:append([(pref_merge_join(KE,QC,Prep,QOpt)) || QC <- selections_no_skip(L)]).
...
pref_merge(KE,Q1,C1,Q2,C2,Skip,Sort1,Sort2,QOpt) ->
    Col1 = {Q1,C1},
    Col2 = {Q2,C2},
    DoSort = [QC || {{_QNum,Col} = QC,SortL} <- [{Col1,Sort1}, {Col2,Sort2}],lists:keymember({Col,ascending},1,SortL) =:= false],
    J = [{{Q1,C1,Q2,C2},{merge_join,DoSort,KE,Skip}}],
    [{{Column,ascending},J} || Column <- equal_template_columns(QOpt,Col1)] ++ [{other,J}].
...
opt_le(#prepared{qh = #simple_qlc{le = LE0,optz = Optz0} = QLC} = Prep0,GenNum) ->
    case LE0 of
        #prepared{qh = #simple_qlc{p = LE_Pvar,le = LE2,optz = Optz2}}->
            ...
            Optz = Optz0#optz{cache = Cachez,unique = Optz0#optz.unique or Optz2#optz.unique},
            PVar = if LE_Pvar =:= 'SQV' ->
                QLC#simple_qlc.p;true ->
                LE_Pvar end,
            Prep = Prep0#prepared{qh = QLC#simple_qlc{p = PVar,le = LE2,optz = Optz}},
            opt_le(Prep,GenNum);
        _->
...
opt_le(#prepared{qh = #qlc{},lu_skip_quals = LU_SkipQuals0} = Prep0,GenNum) ->
    ...
    {LU_QNum,Join,JoinSkipFs,DoSort} = opt_join(Prep0#prepared.join,JoinOption,Qdata0,Opt,LU_SkipQuals0),
    {LU_Skip,LU_SkipQuals} = lists:partition(fun ({QNum,_Fs})->
        QNum =:= LU_QNum end,LU_SkipQuals0),
    LU_SkipFs = lists:flatmap(fun ({_QNum,Fs})->
        Fs end,LU_SkipQuals),
    Qdata1 = if LU_Skip =:= [] ->
        Qdata0;true ->
        activate_join_lookup_filter(LU_QNum,Qdata0) end,
    Qdata2 = skip_lookup_filters(Qdata1,LU_SkipFs ++ JoinSkipFs),
    F = fun ({QNum,GoI,SI,{gen,#prepared{} = PrepLE}},GenNum1)->
        NewPrepLE = maybe_sort(PrepLE,QNum,DoSort,Opt),
        {{QNum,GoI,SI,{gen,opt_le(NewPrepLE,GenNum1)}},GenNum1 + 1};(Qd,GenNum1)->
        {Qd,GenNum1} end,
...
no_cache_of_first_generator(Optz,GenNum)
    when GenNum > 1->
    Optz;
...
maybe_sort(LE,QNum,DoSort,Opt) ->
    case lists:keyfind(QNum,1,DoSort) of
        {QNum,Col}->
            ...
            Sort = #qlc_sort{h = LE,keypos = {keysort,Col},unique = false,compressed = [],order = ascending,fs_opts = SortOpts,tmpdir_usage = TmpUsage,tmpdir = TmpDir},
...
opt_join(Join,JoinOption,Qdata,Opt,LU_SkipQuals) ->
    ...
    case Ix of
        [{{Q1,C1,Q2,C2},Skip,KE,LU_fun}| _]->
            J = #qlc_join{kind = {lookup,KE,LU_fun},q1 = Q1,c1 = C1,q2 = Q2,c2 = C2,opt = Opt},
            {Q2,J,Skip,[]};
        []->
            M = opt_join_merge(M0),
            case M of
                [{{Q1,C1,Q2,C2},{merge_join,DoSort,KE,Skip}}| _]->
                    J = #qlc_join{kind = {merge,KE},opt = Opt,q1 = Q1,c1 = C1,q2 = Q2,c2 = C2},
...
opt_join_lu([{{_Q1,_C1,Q2,_C2} = J,[{lookup_join,_KEols,JKE,Skip0}| _]}| LJ],Qdata,LU_SkipQuals) ->
    {Q2,_,_,{gen,Prep2}} = lists:keyfind(Q2,1,Qdata),
    #qlc_table{ms = MS,key_equality = KE,lookup_fun = LU_fun} = Prep2#prepared.qh,
    case MS =/= no_match_spec andalso lists:keymember(Q2,1,LU_SkipQuals) =:= false of
        true->
...
opt_join_merge(M) ->
    L = [{-length(DoSort),length(Skip),{QCs,{merge_join,DoSort,KE,Skip}}} || {_KpOrder_or_other,MJ} <- M,{QCs,{merge_join,DoSort,KE,Skip0}} <- MJ,Skip <- [skip_if_possible(KE,'==',Skip0)]],
...
setup_le(#prepared{qh = #qlc{lcf = QFun,qdata = Qdata,init_value = V,optz = Optz}},Post0,Setup) ->
    {GoTo,FirstState,Post,LocalPost} = setup_quals(Qdata,Post0,Setup,Optz),
    Objs = fun ()->
        QFun(FirstState,V,GoTo) end,
    unique_cache(Objs,Post,LocalPost,Optz);
...
setup_le(#prepared{qh = #qlc_append{hl = PrepL}},Post0,Setup) ->
    F = fun (Prep,{Post1,LPost1})->
        ...
        {Objs,{Post2,LPost1 ++ LPost2}} end,
    {ObjsL,{Post,LocalPost}} = lists:mapfoldl(F,{Post0,[]},PrepL),
    {fun ()->
        append_loop(ObjsL,0) end,Post,LocalPost};
setup_le(#prepared{qh = #qlc_sort{h = Prep,keypos = Kp,unique = Unique,compressed = Compressed,order = Order,fs_opts = SortOptions0,tmpdir_usage = TmpUsage,tmpdir = TmpDir}},Post0,Setup) ->
    SortOptions = sort_options_global_tmp(SortOptions0,TmpDir),
    LF = fun (Objs)->
        sort_list(Objs,Order,Unique,Kp,SortOptions,Post0) end,
...
setup_quals(Qdata,Post0,Setup,Optz) ->
    ...
    {GoTo2,FirstState,Post,LocalPost1} = case Optz#optz.fast_join of
        #qlc_join{kind = {merge,_KE},c1 = C1,c2 = C2,opt = Opt} = MJ->
            MF = fun (_Rev,{H1,WH1},{H2,WH2})->
                fun ()->
                    merge_join(WH1(H1),C1,WH2(H2),C2,Opt) end end,
        ...
        #qlc_join{kind = {lookup,_KE,LuF},c1 = C1,c2 = C2} = LJ->
            LF = fun (Rev,{H1,WH1},{H2,WH2})->
                {H,W} = if Rev ->
                    ...
                    {H1,WH1} end,
                fun ()->
                    lookup_join(W(H),C1,LuF,C2,Rev) end end,
...
setup_quals(GenLoopS,[{_QNum,GoI,_SI,fil}| Qdata],Gs,P,LP,Setup) ->
    setup_quals(GenLoopS,Qdata,[{GoI,[GenLoopS, next_state(Qdata)]}| Gs],P,LP,Setup);
setup_quals(GenLoopS,[{_QNum,GoI,_SI,{gen,#join{}}}| Qdata],Gs,P,LP,Setup) ->
    setup_quals(GenLoopS,Qdata,[{GoI,[-1, -1, -1]}| Gs],P,LP,Setup);
setup_quals(GenLoopS,[{_QNum,GoI,SI,{gen,LE}}| Qdata],Gs,P,LP,Setup) ->
    {V,NP,LP1} = setup_le(LE,P,Setup),
    setup_quals(SI + 1,Qdata,[{GoI,[GenLoopS, next_state(Qdata), V]}| Gs],NP,LP ++ LP1,Setup);
...
setup_join(J,Qdata,GoTo0,FirstState0,JoinFun,Post0) ->
    ...
    {{_QN,JGoI,JSI,_},Rev,QNum1,QNum2,WH1,WH2,_CsFun} = find_join_data(Qdata,QNum1a,QNum2a),
    [{GoI1,SI1}] = [{GoI,SI} || {QNum,GoI,SI,_} <- Qdata,QNum =:= QNum1],
    [{GoI2,SI2}] = [{GoI,SI} || {QNum,GoI,SI,_} <- Qdata,QNum =:= QNum2],
    [H1] = [H || {GoI,[_Back, _Forth, H]} <- GoTo0,GoI =:= GoI1],
    [{BackH2,H2}] = [{Back,H} || {GoI,[Back, _Forth, H]} <- GoTo0,GoI =:= GoI2],
    ...
    GoTo1 = lists:map(fun ({GoI,_})
        when GoI =:= JGoI->
        {JGoI,[JBack, JForth, H]};({GoI,_})
        when GoI =:= GoI1;
        GoI =:= GoI2->
        {GoI,[-1, -1, -1]};(Go)->
        Go end,GoTo0),
    GoTo = lists:map(fun (S)
        when S =:= SI1->
        JSI;(S)
        when S =:= SI2->
        next_after(Qdata,S,QNum2);(S)
        when S =:= SI1 + 1->
        JSI + 1;(S)
        when S =:= SI2 + 1,
        SI1 + 1 =:= BackH2->
        ...
        when S =:= SI2 + 1->
        BackH2;(S)->
        S end,flat_goto(GoTo1)),
    FirstState = if SI1 =:= FirstState0 ->
        JSI;true ->
        FirstState0 end,
...
next_state([{_,_,_,{gen,#join{}}}| Qdata]) ->
    next_state(Qdata);
...
find_join_data(Qdata,QNum1,QNum2) ->
    [QRev] = [{Q,Rev,QN1,QN2,H1,H2,CsF} || {_QN,_GoI,_SI,{gen,#join{q1 = QN1,q2 = QN2,wh1 = H1,wh2 = H2,cs_fun = CsF}}} = Q <- Qdata,if QN1 =:= QNum1,
    QN2 =:= QNum2 ->
         not (Rev = false);QN1 =:= QNum2,
    QN2 =:= QNum1 ->
        Rev = true;true ->
        Rev = false end],
    QRev.

table_handle(#qlc_table{trav_fun = TraverseFun,trav_MS = TravMS,pre_fun = PreFun,lookup_fun = LuF,parent_fun = ParentFun,lu_vals = LuVals,ms = MS},Post,Setup) ->
    #setup{parent = Parent} = Setup,
    ParentValue = if ParentFun =:= undefined ->
        undefined;Parent =:= self() ->
        try ParentFun()
            catch
                Class:Reason:Stacktrace->
                    post_funs(Post),
                    erlang:raise(Class,Reason,Stacktrace) end;true ->
    ...
    StopFun = if Parent =:= self() ->
        undefined;true ->
        Cursor = #qlc_cursor{c = {self(),Parent}},
        fun ()->
            delete_cursor(Cursor) end end,
...
file_loop(Bin0,Fd_FName,Ts0,TF) ->
    case try file_loop2(Bin0,Ts0)
        catch
            _:_->
                {_Fd,FileName} = Fd_FName,
                error({bad_object,FileName}) end of
        {terms,<<Size:4/unit:8,B/bytes>> = Bin,[]}->
            file_loop_read(Bin,Size - byte_size(B) + 4,Fd_FName,TF);
        {terms,<<Size:4/unit:8,_/bytes>> = Bin,Ts}->
            C = fun ()->
                file_loop_read(Bin,Size + 4,Fd_FName,TF) end,
            TF(Ts,C);
        {terms,B,Ts}->
            C = fun ()->
                file_loop_read(B,64 * 1024,Fd_FName,TF) end,
            TF(Ts,C);
        Error->
            Error
    end.
        ...
        {ok,Bin}->
            case B of
                <<Size:4/unit:8,Tl/bytes>>
                    ...
                    Foo = fun ([T],Fun)->
                        [T| Fun] end,
                    case file_loop(list_to_binary([B, B1]),Fd_FName,[],Foo) of
                        [T| Fun]->
...
sort_cursor_input(H,NoObjects) ->
    fun (close)->
        ok;(read)->
        sort_cursor_input_read(H,NoObjects) end.

sort_cursor_list_output(TmpDir,Z,Unique) ->
    fun (close)->
        {terms,[]};({value,NoObjects})->
        fun (BTerms)
            when Unique;
            length(BTerms) =:= NoObjects->
            fun (close)->
                {terms,BTerms};(BTerms1)->
                sort_cursor_file(BTerms ++ BTerms1,TmpDir,Z) end;(BTerms)->
            sort_cursor_file(BTerms,TmpDir,Z) end end.
...
unique_cache(L,Post,LocalPost,Optz)
    when is_list(L)->
...
unique_cache(H,Post,LocalPost,#optz{unique = true,cache = true}) ->
    ...
    {fun ()->
        ucache(H,UT,MT,LocalPost) end,[P1, P2| Post],[L1, L2]};
unique_cache(H,Post,LocalPost,#optz{unique = false,cache = list} = Optz) ->
    Ref = make_ref(),
    F = del_lcache(Ref),
    #qlc_opt{tmpdir = TmpDir,max_list = MaxList,tmpdir_usage = TmpUsage} = Optz#optz.opt,
    {fun ()->
        lcache(H,Ref,LocalPost,TmpDir,MaxList,TmpUsage) end,[F| Post],[F]};
unique_cache(H,Post0,LocalPost0,#optz{unique = true,cache = list} = Optz) ->
    #qlc_opt{tmpdir = TmpDir,max_list = MaxList,tmpdir_usage = TmpUsage} = Optz#optz.opt,
    Size = if MaxList >= 1 bsl 31 ->
        1 bsl 31 - 1;MaxList =:= 0 ->
        1;true ->
        MaxList end,
    ...
    LF1 = fun (Objs)->
        lists:ukeysort(1,Objs) end,
    FF1 = fun (Objs)->
        file_sort_handle(Objs,{keysort,1},USortOptions,TmpDir,[],Post0,LocalPost0) end,
    {UH,Post1,LocalPost1} = sort_handle(tag_objects(H,1),LF1,FF1,USortOptions,Post0,LocalPost0,TmpUsageM),
    LF2 = fun (Objs)->
        lists:keysort(2,Objs) end,
    FF2 = fun (Objs)->
        file_sort_handle(Objs,{keysort,2},SortOptions,TmpDir,[],Post1,LocalPost1) end,
    {SH,Post,LocalPost} = sort_handle(UH,LF2,FF2,SortOptions,Post1,LocalPost1,TmpUsageM),
    if is_list(SH) ->
        {untag_objects2(SH),Post,LocalPost};true ->
        {fun ()->
            untag_objects(SH) end,Post,LocalPost} end.
...
sort_list_output(L) ->
    fun (close)->
        lists:append(lists:reverse(L));(Terms)
        when is_list(Terms)->
        sort_list_output([Terms| L]) end.
...
file_sort_handle(H,Kp,SortOptions,TmpDir,Compressed,Post,LocalPost) ->
    ...
    Unique = lists:member(unique,SortOptions) orelse lists:keymember(unique,1,SortOptions),
    ...
    case Reply of
        {file,FileName}->
            ...
            P = fun ()->
                _ = file:close(Fd),
                _ = file:delete(FileName) end,
            {F,[P| Post],LocalPost};
        {terms,BTerms}->
            try {[(binary_to_term(B)) || B <- BTerms],Post,LocalPost}
                catch
                    Class:Reason:Stacktrace->
                        post_funs(Post),
                        erlang:raise(Class,Reason,Stacktrace) end
...
do_sort(In,Out,Sort,SortOptions,Post) ->
    try case do_sort(In,Out,Sort,SortOptions) of
        {error,Reason}->
            throw_reason(Reason);
        Reply->
            Reply
            ...
            Class:Term:Stacktrace->
                post_funs(Post),
                erlang:raise(Class,Term,Stacktrace) end.
...
del_table(Ets) ->
    fun ()->
        true = ets:delete(Ets) end.

empty_table(Ets) ->
    fun ()->
        true = ets:delete_all_objects(Ets) end.
...
cache([Object| Cont],MTab,SeqNo,LocalPost) ->
    ...
    [Object| fun ()->
        cache(Cont,MTab,SeqNo + 1,LocalPost) end];
...
ucache3(Object,Cont,Hash,UTab,MTab,SeqNo,LocalPost) ->
    ...
    [Object| fun ()->
        ucache(Cont,UTab,MTab,SeqNo + 1,LocalPost) end].
...
ucache_recall(UTab,MTab,SeqNo) ->
    case ets:lookup(MTab,SeqNo) of
        ...
        [{SeqNo,Hash}]->
            ...
            [Object| fun ()->
                ucache_recall(UTab,MTab,SeqNo + 1) end]
    end.
...
lcache1(H,State,Sz,Acc)
    when Sz < 0->
    {Key,LocalPost,TmpDir,MaxList,TmpUsage} = State,
    GetFile = case get(Key) of
        {file,Fd0,TmpFile,_F}->
            {TmpFile,Fd0};
        undefined
            when TmpUsage =:= not_allowed->
...
lcache2([Object| Cont],State,Sz0,Acc)
    when Sz0 >= 0->
    Sz = decr_list_size(Sz0,Object),
    [Object| lcache2(Cont,State,Sz,[Object| Acc])];
lcache2(Cont,State,Sz,Acc) ->
    fun ()->
        lcache1(Cont,State,Sz,Acc) end.
...
merge_join([E1| L1],C1,L2,C2,Opt) ->
    #qlc_opt{tmpdir = TmpDir,max_list = MaxList,tmpdir_usage = TmpUsage} = Opt,
    M = #m{id = merge_join_id(),tmpdir = TmpDir,max_list = MaxList,tmp_usage = TmpUsage},
...
merge_join1(E2,K2,[E1| L1],C1,L2,C2,M) ->
    K1 = element(C1,E1),
    if K1 == K2 ->
        same_keys2(E1,K1,L1,C1,L2,C2,E2,M);K1 > K2 ->
        merge_join2(E1,K1,L1,C1,L2,C2,M);true ->
        merge_join1(E2,K2,L1,C1,L2,C2,M) end;
...
same_keys2(E1,K1,L1,C1,[],_C2,E2_0,M) ->
    Cont = fun (_L1b)->
        end_merge_join([],M) end,
    loop_same_keys(E1,K1,L1,C1,[E2_0],Cont,M);
same_keys2(E1,K1,L1,C1,[E2| L2] = L2_0,C2,E2_0,M) ->
    K2 = element(C2,E2),
    if K1 == K2 ->
        same_keys1(E1,K1,L1,C1,E2,C2,E2_0,L2,M);K1 < K2 ->
        [[E1| E2_0]| fun ()->
            same_loop1(L1,K1,C1,E2_0,L2_0,C2,M) end] end;
...
same_loop1([E1| L1],K1_0,C1,E2_0,L2,C2,M) ->
    ...
    if K1 == K1_0 ->
        [[E1| E2_0]| fun ()->
            same_loop1(L1,K1_0,C1,E2_0,L2,C2,M) end];K1_0 < K1 ->
        merge_join2(E1,K1,L1,C1,L2,C2,M) end;
...
same_keys1(E1_0,K1_0,[] = L1,C1,E2,C2,E2_0,L2,M) ->
    [[E1_0| E2_0], [E1_0| E2]| fun ()->
        same_keys(K1_0,E1_0,L1,C1,L2,C2,M) end];
same_keys1(E1_0,K1_0,[E1| _] = L1,C1,E2,C2,E2_0,L2,M) ->
    K1 = element(C1,E1),
    if K1_0 == K1 ->
        E2s = [E2, E2_0],
        Sz0 = decr_list_size(M#m.max_list,E2s),
        same_keys_cache(E1_0,K1_0,L1,C1,L2,C2,E2s,Sz0,M);K1_0 < K1 ->
        [[E1_0| E2_0], [E1_0| E2]| fun ()->
            same_keys(K1_0,E1_0,L1,C1,L2,C2,M) end] end;
...
same_keys(K1,E1,L1,C1,[E2| L2],C2,M) ->
    ...
    if K1 == K2 ->
        [[E1| E2]| fun ()->
            same_keys(K1,E1,L1,C1,L2,C2,M) end];K1 < K2 ->
        merge_join1(E2,K2,L1,C1,L2,C2,M) end;
...
same_keys_cache(E1,K1,L1,C1,L2,C2,E2s,Sz0,M)
    when Sz0 < 0->
    case init_merge_join(M) of
        ok->
            Sz = M#m.max_list,
            C = fun ()->
                same_keys_file(E1,K1,L1,C1,L2,C2,[],Sz,M) end,
...
same_keys_cache(E1,K1,L1,C1,[E2| L2],C2,E2s,Sz0,M) ->
    ...
    if K1 == K2 ->
        ...
        same_keys_cache(E1,K1,L1,C1,L2,C2,[E2| E2s],Sz,M);K1 < K2 ->
        Cont = fun (L1b)->
            merge_join1(E2,K2,L1b,C1,L2,C2,M) end,
        loop_same_keys(E1,K1,L1,C1,E2s,Cont,M) end;
...
loop_same_keys(E1,K1,L1,C1,E2s,Cont,M) ->
    loop_same(E1,E2s,fun ()->
        loop_keys(K1,L1,C1,E2s,Cont,M) end).
...
loop_keys(K,[E1| L1] = L1_0,C1,E2s,Cont,M) ->
    K1 = element(C1,E1),
    if K1 == K ->
        loop_same_keys(E1,K1,L1,C1,E2s,Cont,M);K1 > K ->
        Cont(L1_0) end;
...
same_keys_file(E1,K1,L1,C1,L2,C2,E2s,Sz0,M)
    when Sz0 < 0->
    Sz = M#m.max_list,
    C = fun ()->
        same_keys_file(E1,K1,L1,C1,L2,C2,[],Sz,M) end,
...
same_keys_file(E1,K1,L1,C1,[E2| L2],C2,E2s,Sz0,M) ->
    ...
    if K1 == K2 ->
        Sz = decr_list_size(Sz0,E2),
        same_keys_file(E1,K1,L1,C1,L2,C2,[E2| E2s],Sz,M);K1 < K2 ->
        Cont = fun (L1b)->
            merge_join1(E2,K2,L1b,C1,L2,C2,M) end,
        same_keys_file_write(E1,K1,L1,C1,E2s,M,Cont) end;
...
same_keys_file_write(E1,K1,L1,C1,E2s,M,Cont) ->
    C = fun ()->
        loop_keys_file(K1,L1,C1,Cont,M) end,
    write_same_keys(E1,E2s,M,C).
        ...
        none->
            ...
            case file:open(FName,[raw, binary, read, write]) of
                {ok,Fd}->
                    ...
                    put('$_qlc_merge_join_tmpfiles_',[{MergeId,Fd,FName}| TmpFiles]),
                    ok;
                Error->
...
end_merge_join(#m{id = MergeId}) ->
    case tmp_merge_file(MergeId) of
        ...
        {Fd,FileName}->
            ...
            put('$_qlc_merge_join_tmpfiles_',lists:keydelete(MergeId,1,get('$_qlc_merge_join_tmpfiles_')))
    end.

end_all_merge_joins() ->
    lists:foreach(fun (Id)->
        end_merge_join(#m{id = Id}) end,[Id || {Id,_Fd,_FileName} <- lists:flatten([get('$_qlc_merge_join_tmpfiles_')])]),
...
lookup_join([E1| L1],C1,LuF,C2,Rev) ->
    ...
    case LuF(C2,[K1]) of
        ...
        [E2]
            when Rev->
            [[E2| E1]| fun ()->
                lookup_join(L1,C1,LuF,C2,Rev) end];
        [E2]->
            [[E1| E2]| fun ()->
                lookup_join(L1,C1,LuF,C2,Rev) end];
        E2s
...
maybe_error_logger(Name,Why) ->
    [_, _, {qlc,maybe_error_logger,_,_}| Stacktrace] = expand_stacktrace(),
    Trimmer = fun (M,_F,_A)->
        M =:= erl_eval end,
    Formater = fun (Term,I)->
        io_lib:print(Term,I,80,-1) end,
...
expand_stacktrace() ->
    ...
    try expand_stacktrace(max(1,D))
        after erlang:system_flag(backtrace_depth,D) end.

expand_stacktrace(D) ->
    _ = erlang:system_flag(backtrace_depth,D),
    {'EXIT',{foo,Stacktrace}} = ( catch error(foo)),
    L = lists:takewhile(fun ({M,_,_,_})->
        M =/= qlc end,lists:reverse(Stacktrace)),
    if length(L) < 3 andalso length(Stacktrace) =:= D ->
        expand_stacktrace(D + 5);true ->
        Stacktrace end.
