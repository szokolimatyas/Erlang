...
-export([compile_asn/3, compile_asn1/3, compile_py/3, compile/3, vsn/0, get_name_of_def/1, get_pos_of_def/1, unset_pos_mod/1]).

-export([read_config_data/1, get_gen_state_field/1, partial_inc_dec_toptype/1, update_gen_state/2, get_tobe_refed_func/1, reset_gen_state/0, is_function_generated/1, generated_refed_func/1, next_refed_func/0, update_namelist/1, step_in_constructed/0, add_tobe_refed_func/1, add_generated_refed_func/1, maybe_rename_function/3, current_sindex/0, set_current_sindex/1, maybe_saved_sindex/2, parse_and_save/2, verbose/3, warning/3, warning/4, error/3, format_error/1]).
...
-file("/usr/lib/erlang/lib/stdlib-3.14/include/erl_compile.hrl", 1).

-record(options,{includes = []::[file:filename()],outdir = "."::file:filename(),output_type = undefined::atom(),defines = []::[atom()|{atom(),_}],warning = 1::non_neg_integer(),verbose = false::boolean(),optimize = 999,specific = []::[_],outfile = ""::file:filename(),cwd::file:filename()}).
...
compile(File,Options0)
    ...
    try translate_options(Options0) of 
        Options1->
            ...
            in_process(fun ()->
                compile_proc(File,Includes,Options2) end)
        catch
            throw:Error->
                Error end.

-record(st, {file = [],files = [],inputmodules = [],code,opts = [],outfile,dbfile,includes = [],erule,error = none,run}).
...
set_passes() ->
    [{pass,scan_parse,fun set_scan_parse_pass/1}, {pass,merge,fun merge_pass/1}| common_passes()].

single_passes() ->
    [{pass,scan,fun scan_pass/1}, {pass,parse,fun parse_pass/1}| common_passes()].

parse_and_save_passes() ->
    [{pass,scan,fun scan_pass/1}, {pass,parse,fun parse_pass/1}, {pass,save,fun save_pass/1}].

common_passes() ->
    [{iff,parse,{pass,parse_listing,fun parse_listing/1}}, {pass,check,fun check_pass/1}, {iff,abs,{pass,abs_listing,fun abs_listing/1}}, {pass,generate,fun generate_pass/1}, {unless,noobj,{pass,compile,fun compile_pass/1}}].
...
set_scan_parse_pass(#st{files = Files} = St) ->
    try L = set_scan_parse_pass_1(Files,St),
    {ok,St#st{code = L}}
        catch
            throw:Error->
                {error,St#st{error = Error}} end.
...
check_pass(#st{code = M,file = File,includes = Includes,erule = Erule,dbfile = DbFile,opts = Opts,inputmodules = InputModules} = St) ->
    start(Includes),
    ...
    try Run(Name,Pass,St0) of 
        ...
        {error,#st{error = Errors}}->
            ...
            Class:Error:Stk->
                ...
                {error,{internal_error,{Class,Error}}} end;
run_passes_1([],_St) ->
    ok.

clean_errors(Errors)
    when is_list(Errors)->
    F = fun ({structured_error,_,_,_})->
        true;(_)->
        false end,
...
print_structured_errors([_| _] = Errors) ->
    _ = [(io:format("~ts:~w: ~ts\n",[F, L, M:format_error(E)])) || {structured_error,{F,L},M,E} <- Errors],
    ok;
print_structured_errors(_) ->
    ok.
...
compile_set(SetBase,Files,#st{opts = Opts} = St0) ->
    ...
    InputModules = [begin F1 = filename:basename(F0),
    F = filename:rootname(F1),
    list_to_atom(F) end || F0 <- Files],
    St = St0#st{file = SetBase,files = Files,outfile = OutFile,dbfile = DbFile,inputmodules = InputModules},
    Passes = set_passes(),
...
merge_modules(ModuleList,CommonName) ->
    ...
    TypeOrVal = lists:append(lists:map(fun (X)->
        X#module.typeorval end,NewModuleList)),
    InputMNameList = lists:map(fun (X)->
        X#module.name end,NewModuleList),
    CExports = common_exports(NewModuleList),
    ImportsModuleNameList = lists:map(fun (X)->
        {X#module.imports,X#module.name} end,NewModuleList),
...
remove_name_collisions2(ModName,[T| Ts],Ms,Acc) ->
    ...
    case discover_dupl_in_mods(Name,T,Ms,[],0) of
        {_,0}->
...
discover_dupl_in_mods(Name,Def,[M = #module{name = N,typeorval = TorV}| Ms],Acc,AnyRenamed) ->
    ...
    discover_dupl_in_mods(Name,Def,Ms,[M#module{typeorval = lists:flatten(NewTorV)}| Acc],NewAnyRenamed);
discover_dupl_in_mods(_,_,[],Acc,AnyRenamed) ->
    {Acc,AnyRenamed}.

warn_renamed_def(ModName,NewName,OldName) ->
    maybe_first_warn_print(),
    io:format("NOTICE: The ASN.1 definition in module ~p with name ~p h" "as been renamed in generated module. New name is ~p.~n",[ModName, OldName, NewName]).

warn_deleted_def(ModName,DefName) ->
    maybe_first_warn_print(),
    io:format("NOTICE: The ASN.1 definition in module ~p with name ~p h" "as been deleted in generated module.~n",[ModName, DefName]).

warn_kept_def(ModName,DefName) ->
    maybe_first_warn_print(),
    io:format("NOTICE: The ASN.1 definition in module ~p with name ~p h" "as kept its name due to equal definition as duplicate.~n",[ModName, DefName]).

maybe_first_warn_print() ->
    case get(warn_duplicate_defs) of
        undefined->
            put(warn_duplicate_defs,true),
            io:format("~nDue to multiple occurrences of a definition na" "me in multi-file compiled files:~n");
        _->
            ok
    end.
...
unset_pos_mod(Def)
    when is_record(Def,classdef)->
    Def#classdef{pos = undefined};
unset_pos_mod(Def)
    when is_record(Def,valuedef)->
    Def#valuedef{pos = undefined,module = undefined};
unset_pos_mod(Def)
    when is_record(Def,ptypedef)->
    Def#ptypedef{pos = undefined};
unset_pos_mod(Def)
    when is_record(Def,pvaluedef)->
    Def#pvaluedef{pos = undefined};
unset_pos_mod(Def)
    when is_record(Def,pvaluesetdef)->
    Def#pvaluesetdef{pos = undefined};
unset_pos_mod(Def)
    when is_record(Def,pobjectdef)->
    Def#pobjectdef{pos = undefined};
unset_pos_mod(Def)
    when is_record(Def,pobjectsetdef)->
...
set_name_of_def(ModName,Name,OldDef) ->
    ...
    case OldDef of
        #typedef{}->
            OldDef#typedef{name = NewName};
        #classdef{}->
            OldDef#classdef{name = NewName};
        #valuedef{}->
            OldDef#valuedef{name = NewName};
        #ptypedef{}->
            OldDef#ptypedef{name = NewName};
        #pvaluedef{}->
            OldDef#pvaluedef{name = NewName};
        #pvaluesetdef{}->
            OldDef#pvaluesetdef{name = NewName};
        #pobjectdef{}->
            OldDef#pobjectdef{name = NewName};
        #pobjectsetdef{}->
            OldDef#pobjectsetdef{name = NewName}
    end.
...
save_imports(ModuleList) ->
    ...
    case lists:flatten(ImportsList) of
        ...
        ImportsList2->
            ...
            lists:foreach(fun (X)->
                asn1ct_table:insert(original_imports,X) end,ImportsList2)
    end.

common_exports(ModuleList) ->
    case lists:filter(fun (X)->
        element(2,X#module.exports) /= all end,ModuleList) of
        []->
            {exports,all};
        ModsWithExpList->
            CExports1 = lists:append(lists:map(fun (X)->
                element(2,X#module.exports) end,ModsWithExpList)),
        ...
        lists:map(fun (Def)->
            case Def of
                T
                    ...
                    #'Externaltypereference'{pos = 0,module = MName,type = T#typedef.name};
                V
                    when is_record(V,valuedef)->
                    #'Externalvaluereference'{pos = 0,module = MName,value = V#valuedef.name};
                C
                    when is_record(C,classdef)->
                    #'Externaltypereference'{pos = 0,module = MName,type = C#classdef.name};
                P
                    when is_record(P,ptypedef)->
                    #'Externaltypereference'{pos = 0,module = MName,type = P#ptypedef.name};
                PV
                    when is_record(PV,pvaluesetdef)->
                    #'Externaltypereference'{pos = 0,module = MName,type = PV#pvaluesetdef.name};
                PO
                    when is_record(PO,pobjectdef)->
                    #'Externalvaluereference'{pos = 0,module = MName,value = PO#pobjectdef.name}
            end end,TorVL) end,ModuleList),
...
save_automatic_tagged_types([#module{tagdefault = 'AUTOMATIC',typeorval = TorV}| Ms]) ->
    Fun = fun (T)->
        asn1ct_table:insert(automatic_tags,{get_name_of_def(T)}) end,
...
merge_symbols_from_module([Imp| Imps],Acc) ->
    ...
    IfromModName = lists:filter(fun (I)->
        case I#'SymbolsFromModule'.module of
    ...
    NewImp = Imp#'SymbolsFromModule'{symbols = lists:append(lists:map(fun (SL)->
        SL#'SymbolsFromModule'.symbols end,[Imp| IfromModName]))},
...
delete_double_of_symbol1([TRef = {#'Externaltypereference'{type = MRef},#'Externaltypereference'{type = TRef}}| Rest],Acc) ->
    NewRest = lists:filter(fun (S)->
        case S of
            {#'Externaltypereference'{type = MRef},#'Externaltypereference'{type = TRef}}->
                false;
            _->
                true
        end end,Rest),
...
generate({M,CodeTuple},OutFile,EncodingRule,Options) ->
    ...
    Code = #abst{name = M#module.name,types = Types,values = Values,ptypes = Ptypes,classes = Classes,objects = Objects,objsets = ObjectSets},
    ...
    try specialized_decode_prepare(Gen,M)
        catch
            throw:{error,Reason}->
                warning("Error in configuration file: ~n~p~n",[Reason],Options,"Error in configuration file") end,
...
setup_bit_string_format(Opts) ->
    Format = case {lists:member(compact_bit_string,Opts),lists:member(legacy_bit_string,Opts)} of
        {false,false}->
            bitstring;
        {true,false}->
            compact;
        {false,true}->
            legacy;
        {true,true}->
            Message = "Contradicting options given: compact_bit_string an" "d legacy_bit_string",
...
check_maps_option(#gen{pack = map}) ->
    case get_bit_string_format() of
        bitstring->
            ok;
        _->
            Message1 = "The 'maps' option must not be combined with 'compact_b" "it_string' or 'legacy_bit_string'",
    ...
    case use_legacy_types() of
        false->
            ok;
        true->
            Message2 = "The 'maps' option must not be combined with 'legacy_er" "lang_types'",
...
input_file_type(File) ->
    case filename:extension(File) of
        ...
        Asn1SFix->
            ...
            Ret = case filename:extension(Base) of
                []->
                    {single_file,File};
                SetSFix
                    when SetSFix == ".set"->
...
get_file_list1(Stream,Dir,Includes,Acc) ->
    ...
    case Ret of
        eof->
            ...
            lists:reverse(Acc);
        FileName->
            SuffixedNameList = case  catch input_file_type(filename:join([Dir, lists:delete($\n,FileName)]),Includes) of
                {empty_name,[]}->
                    [];
                {single_file,Name}->
                    [Name];
...
get_rule(Options) ->
    case [Rule || Rule <- [ber, per, uper, jer],Opt <- Options,Rule =:= Opt] of
        [Rule]->
            Rule;
        [Rule| _]->
            Rule;
...
translate_options([{inline,_}| _]) ->
    io:format("ERROR: The option {inline,OutputFilename} is no longer s" "upported.\n"),
...
is_asn1_flag(asn1config) ->
    true;
is_asn1_flag(ber) ->
    true;
is_asn1_flag(compact_bit_string) ->
    true;
is_asn1_flag(debug) ->
    true;
is_asn1_flag(der) ->
    true;
is_asn1_flag(legacy_bit_string) ->
    true;
is_asn1_flag({macro_name_prefix,_}) ->
    true;
is_asn1_flag({n2n,_}) ->
    true;
is_asn1_flag(noobj) ->
    true;
is_asn1_flag(no_ok_wrapper) ->
    true;
is_asn1_flag(optimize) ->
    true;
is_asn1_flag(per) ->
    true;
is_asn1_flag({record_name_prefix,_}) ->
    true;
is_asn1_flag(undec_rec) ->
    true;
is_asn1_flag(uper) ->
    true;
is_asn1_flag(verbose) ->
    true;
is_asn1_flag(_) ->
    false.
...
include_append(Dir,Options) ->
    option_add({i,Dir},Options,fun (Opts)->
        Opts ++ [{i,Dir}] end).
...
compile(File,_OutFile,Options) ->
    case compile(File,make_erl_options(Options)) of
        ...
        ok->
            ok;
        ParseRes
            when is_tuple(ParseRes)->
            io:format("~p~n",[ParseRes]),
            ok;
        ScanRes
            when is_list(ScanRes)->
...
make_erl_options(Opts) ->
    ...
    Options = case Verbose of
        ...
        _->
            [{optimize,Optimize}]
    end ++ lists:map(fun ({Name,Value})->
        {d,Name,Value};(Name)->
        {d,Name} end,Defines) ++ case OutputType of
        undefined->
            [ber];
        _->
            [OutputType]
    end,
    Options ++ [errors, {cwd,Cwd}, {outdir,Outdir}| lists:map(fun (Dir)->
        {i,Dir} end,Includes)] ++ Specific.
...
pretty2(Module,AbsFile) ->
    ...
    lists:foreach(fun (T)->
        io:format(F,"~s.\n",[asn1ct_pretty_format:term(asn1_db:dbget(Module,T))]) end,Types),
    io:format(F,"%%%%%%%%%%%%%%%%%%% VALUES in ~p  %%%%%%%%%%%%%%%%%%%~n",[Module]),
    lists:foreach(fun (T)->
        io:format(F,"~s.\n",[asn1ct_pretty_format:term(asn1_db:dbget(Module,T))]) end,Values),
    io:format(F,"%%%%%%%%%%%%%%%%%%% Parameterized Types in ~p  %%%%%%%%%" "%%%%%%%%%%~n",[Module]),
    lists:foreach(fun (T)->
        io:format(F,"~s.\n",[asn1ct_pretty_format:term(asn1_db:dbget(Module,T))]) end,ParameterizedTypes),
    io:format(F,"%%%%%%%%%%%%%%%%%%% Classes in ~p  %%%%%%%%%%%%%%%%%%%~n",[Module]),
    lists:foreach(fun (T)->
        io:format(F,"~s.\n",[asn1ct_pretty_format:term(asn1_db:dbget(Module,T))]) end,Classes),
    io:format(F,"%%%%%%%%%%%%%%%%%%% Objects in ~p  %%%%%%%%%%%%%%%%%%%~n",[Module]),
    lists:foreach(fun (T)->
        io:format(F,"~s.\n",[asn1ct_pretty_format:term(asn1_db:dbget(Module,T))]) end,Objects),
    io:format(F,"%%%%%%%%%%%%%%%%%%% Object Sets in ~p  %%%%%%%%%%%%%%%%%" "%%~n",[Module]),
    lists:foreach(fun (T)->
        io:format(F,"~s.\n",[asn1ct_pretty_format:term(asn1_db:dbget(Module,T))]) end,ObjectSets).

start(Includes)
    when is_list(Includes)->
...
test(Module,Type,[{i,_}| _] = Options) ->
    test_type(Module,Type,Options);
...
check(Module,Includes) ->
    case asn1_db:dbload(Module) of
        ...
        ok->
            M = asn1_db:dbget(Module,'MODULE'),
            TypeOrVal = M#module.typeorval,
            State = #state{mname = M#module.name,module = M#module{typeorval = []},options = Includes},
...
special_decode_prepare_1(#gen{options = Options} = Gen,M) ->
    ...
    case read_config_file(Gen,ModName) of
        ...
        CfgList->
            ...
            CommandList = create_partial_decode_gen_info(M#module.name,SelectedDecode),
            save_config(partial_decode,CommandList),
            save_gen_state(selective_decode,SelectedDecode),
            CommandList2 = create_partial_inc_decode_gen_info(M#module.name,ExclusiveDecode),
...
create_partial_inc_decode_gen_info(ModName,{Mod,[{Name,L}| Ls]})
    when is_list(L)->
...
create_partial_inc_decode_gen_info1(ModName,TopTypeName,{ModName,[_TopType| Rest]}) ->
    case asn1_db:dbget(ModName,TopTypeName) of
        #typedef{typespec = TS}->
            TagCommand = get_tag_command(TS,mandatory,mandatory),
            create_pdec_inc_command(ModName,get_components(TS#type.def),Rest,[TagCommand]);
        _->
            throw({error,{"wrong type list in asn1 config file",TopTypeName}})
    end;
create_partial_inc_decode_gen_info1(M1,_,{M2,_})
    when M1 /= M2->
...
create_pdec_inc_command(ModName,{Comps1,Comps2},TNL,Acc)
    ...
    is_list(Comps2)->
    create_pdec_inc_command(ModName,Comps1 ++ Comps2,TNL,Acc);
create_pdec_inc_command(ModN,Clist,[CL| _Rest],[[]])
    when is_list(CL)->
    create_pdec_inc_command(ModN,Clist,CL,[]);
create_pdec_inc_command(ModN,Clist,[CL| _Rest],Acc)
    when is_list(CL)->
    InnerDirectives = create_pdec_inc_command(ModN,Clist,CL,[]),
    lists:reverse([InnerDirectives| Acc]);
create_pdec_inc_command(ModName,CList = [#'ComponentType'{name = Name,typespec = TS,prop = Prop}| Comps],TNL = [C1| Cs],Acc) ->
    case C1 of
        {Name,undecoded}->
        ...
        {Name,RestPartsList}
            when is_list(RestPartsList)->
            case get_tag_command(TS,mandatory,Prop) of
                mandatory->
                    InnerDirectives = create_pdec_inc_command(ModName,TS#type.def,RestPartsList,[]),
...
create_pdec_inc_command(ModName,{'CHOICE',[#'ComponentType'{name = C1,typespec = TS,prop = Prop}| Comps]},[{C1,Directive}| Rest],Acc) ->
    case Directive of
        List
            when is_list(List)->
            TagCommand = get_tag_command(TS,alt,Prop),
            CompAcc = create_pdec_inc_command(ModName,get_components(TS#type.def),List,[]),
...
create_pdec_inc_command(ModName,{'CHOICE',[#'ComponentType'{typespec = TS,prop = Prop}| Comps]},TNL,Acc) ->
    TagCommand = get_tag_command(TS,alt,Prop),
...
create_pdec_inc_command(M,{'CHOICE',{Cs1,Cs2}},TNL,Acc)
    ...
    is_list(Cs2)->
    create_pdec_inc_command(M,{'CHOICE',Cs1 ++ Cs2},TNL,Acc);
create_pdec_inc_command(ModName,#'Externaltypereference'{module = M,type = Name},TNL,Acc) ->
...
partial_inc_dec_toptype([T| _])
    when is_atom(T)->
    T;
partial_inc_dec_toptype([{T,_}| _])
    when is_atom(T)->
    T;
partial_inc_dec_toptype([L| _])
    when is_list(L)->
        ...
        [TopType| Rest]->
            case asn1_db:dbget(ModName,TopType) of
                #typedef{typespec = TS}->
                    ...
                    Ret = create_pdec_command(ModName,get_components(TS#type.def),Rest,concat_tags(TagCommand,[])),
                    {FuncName,Ret};
                _->
...
create_pdec_command(_ModName,_,[],Acc) ->
    Remove_empty_lists = fun ([[]| L],Res,Fun)->
        Fun(L,Res,Fun);([],Res,_)->
        Res;([H| L],Res,Fun)->
        Fun(L,[H| Res],Fun) end,
    Remove_empty_lists(Acc,[],Remove_empty_lists);
create_pdec_command(ModName,[#'ComponentType'{name = C1,typespec = TS}| _Comps],[C1| Cs],Acc) ->
    TagCommand = get_tag_command(TS,choosen),
    create_pdec_command(ModName,get_components(TS#type.def),Cs,concat_tags(TagCommand,Acc));
create_pdec_command(ModName,[#'ComponentType'{typespec = TS,prop = Prop}| Comps],[C2| Cs],Acc) ->
...
create_pdec_command(ModName,{'CHOICE',{Cs1,Cs2}},TNL,Acc)
    ...
    is_list(Cs2)->
    create_pdec_command(ModName,{'CHOICE',Cs1 ++ Cs2},TNL,Acc);
create_pdec_command(ModName,#'Externaltypereference'{module = M,type = C1},TypeNameList,Acc) ->
...
create_pdec_command(ModName,TS = #type{def = Def},[C1| Cs],Acc) ->
    case C1 of
        ...
        Err->
            throw({error,{"unexpected error when creating partial decode comm" "and",Err}})
    end;
create_pdec_command(_,_,TNL,_) ->
    throw({error,{"unexpected error when creating partial decode command",TNL}}).

get_components(#'SEQUENCE'{components = {C1,C2}})
    when is_list(C1),
    is_list(C2)->
    C1 ++ C2;
get_components(#'SEQUENCE'{components = Components}) ->
    Components;
get_components(#'SET'{components = {C1,C2}})
    when is_list(C1),
    is_list(C2)->
    C1 ++ C2;
...
concat_sequential(L = [A, B],Acc)
    when is_atom(A),
    is_binary(B)->
    [L| Acc];
concat_sequential(L,Acc)
    when is_list(L)->
...
concat_sequential1([[]],Acc) ->
    Acc;
concat_sequential1([El| RestEl],Acc)
    when is_list(El)->
...
get_tag_command(#type{tag = [Tag]},Command) ->
    [Command, encode_tag_val(decode_class(Tag#tag.class),Tag#tag.form,Tag#tag.number)];
...
get_tag_command(#type{tag = Tag},Command,Prop)
    when is_record(Tag,tag)->
    get_tag_command(#type{tag = [Tag]},Command,Prop);
get_tag_command(T = #type{tag = [Tag| Tags]},Command,Prop) ->
    [get_tag_command(T#type{tag = [Tag]},Command,Prop), get_tag_command(T#type{tag = Tags},Command,Prop)].

anonymous_dec_command(undec,'OPTIONAL') ->
    opt_undec;
        ...
        #typedef{typespec = TS}->
            case TS of
                ...
                #type{}->
                    TS;
                _->
                    throw({error,{"unexpected element when fetching reference" "d type",TS}})
            end;
        T->
...
read_config_file_info(ModuleName,InfoType)
    when is_atom(InfoType)->
...
save_gen_state(exclusive_decode,{_,ConfList},PartIncTlvTagList) ->
    State = case get_gen_state() of
        S
            when is_record(S,gen_state)->
            S;
        _->
            #gen_state{}
    end,
    StateRec = State#gen_state{inc_tag_pattern = PartIncTlvTagList,inc_type_pattern = ConfList},
...
save_gen_state(GenState)
    when is_record(GenState,gen_state)->
...
update_gen_state(Field,Data) ->
    case get_gen_state() of
        State
            when is_record(State,gen_state)->
...
generated_functions_member(_M,Name,[{Name,_,_}| _]) ->
    true;
generated_functions_member(M,#'Externaltypereference'{module = M,type = T},[{#'Externaltypereference'{module = M,type = T},_,_}| _]) ->
    true;
generated_functions_member(M,#'Externaltypereference'{module = M,type = Name},[{Name,_,_}| _]) ->
    true;
...
generated_functions_filter(_,Name,L)
    when is_atom(Name);
    is_list(Name)->
    lists:filter(fun ({N,_,_})
        when N == Name->
        true;(_)->
        false end,L);
generated_functions_filter(M,#'Externaltypereference'{module = M,type = Name},L) ->
    RemoveTType = fun ({N,I,[N, P]})
        when N == Name->
        {N,I,P};({#'Externaltypereference'{module = M1,type = N},I,P})
        when M1 == M->
        {N,I,P};(P)->
        P end,
...
in_process(Fun) ->
    ...
    {Pid,Class,Reason,Stack}->
        ST = try throw(x)
                ...
                throw:x:Stk->
                    Stk end,
        erlang:raise(Class,Reason,Stack ++ ST) end.

process(Parent,Fun) ->
    try Parent ! {self(),Fun()}
        catch
            Class:Reason:Stack->
                Parent ! {self(),Class,Reason,Stack} end.