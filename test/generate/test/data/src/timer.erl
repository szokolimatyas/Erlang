...
-export([apply_after/4, send_after/3, send_after/2, exit_after/3, exit_after/2, kill_after/2, kill_after/1, apply_interval/4, send_interval/3, send_interval/2, cancel/1, sleep/1, tc/1, tc/2, tc/3, now_diff/2, seconds/1, minutes/1, hours/1, hms/3]).

-export([start_link/0, start/0, handle_call/3, handle_info/2, init/1, code_change/3, handle_cast/2, terminate/2]).
...
-spec(send_after(Time,Pid,Message) -> {ok,TRef}|{error,Reason} when Time::time(),Pid::pid()|(RegName::atom()),Message::term(),TRef::tref(),Reason::term()).
...
-spec(exit_after(Time,Pid,Reason1) -> {ok,TRef}|{error,Reason2} when Time::time(),Pid::pid()|(RegName::atom()),TRef::tref(),Reason1::term(),Reason2::term()).
...
-spec(kill_after(Time,Pid) -> {ok,TRef}|{error,Reason2} when Time::time(),Pid::pid()|(RegName::atom()),TRef::tref(),Reason2::term()).
...
-spec(send_interval(Time,Pid,Message) -> {ok,TRef}|{error,Reason} when Time::time(),Pid::pid()|(RegName::atom()),Message::term(),TRef::tref(),Reason::term()).
...
-spec(tc(Fun,Arguments) -> {Time,Value} when Fun::function(),Arguments::[term()],Time::integer(),Value::term()).
...
-spec(tc(Module,Function,Arguments) -> {Time,Value} when Module::module(),Function::atom(),Arguments::[term()],Time::integer(),Value::term()).
...
init([]) ->
    ...
    timer_interval_tab = ets:new(timer_interval_tab,[named_table, protected]),
    {ok,[],infinity}.
...
-spec(handle_call(term(),term(),timers()) -> {reply,term(),timers(),timeout()}|{noreply,timers(),timeout()}).
        ...
        Key->
            case ets:lookup(timer_tab,Key) of
                ...
                [{{Time,Ref},Repeat = {repeat,Interv,To},MFA}]->
                    ets:delete(timer_tab,Key),
                    NewTime = Time + Interv,
                    ets:insert(timer_interval_tab,{{interval,Ref},{NewTime,Ref},To}),
                    do_apply(MFA),
...
next_timeout() ->
    case ets:first(timer_tab) of
        '$end_of_table'->
            infinity;
        {Time,_}->
            min(positive((Time - system_time() + 999) div 1000),8388608)
    end.
...
-spec(get_status() -> {{timer_tab,non_neg_integer()},{timer_interval_tab,non_neg_integer()}}).
