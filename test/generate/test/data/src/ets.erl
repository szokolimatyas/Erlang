...
-export([file2tab/1, file2tab/2, filter/3, foldl/3, foldr/3, match_delete/2, tab2file/2, tab2file/3, tabfile_info/1, from_dets/2, to_dets/2, init_table/2, test_ms/2, tab2list/1, table/1, table/2, fun2ms/1, match_spec_run/2, repair_continuation/2]).
...
-type(continuation()::'$end_of_table'|{tab(),integer(),integer(),comp_match_spec(),list(),integer()}|{tab(),_,_,integer(),comp_match_spec(),list(),integer(),integer()}).
...
-export([all/0, delete/1, delete/2, delete_all_objects/1, delete_object/2, first/1, give_away/3, info/1, info/2, insert/2, insert_new/2, is_compiled_ms/1, last/1, lookup/2, lookup_element/3, match/1, match/2, match/3, match_object/1, match_object/2, match_object/3, match_spec_compile/1, match_spec_run_r/3, member/2, new/2, next/2, prev/2, rename/2, safe_fixtable/2, select/1, select/2, select/3, select_count/2, select_delete/2, select_replace/2, select_reverse/1, select_reverse/2, select_reverse/3, setopts/2, slot/2, take/2, update_counter/3, update_counter/4, update_element/3, whereis/1]).

-export([internal_request_all/0, internal_delete_all/2, internal_select_delete/2]).

-spec(all() -> [Tab] when Tab::tab()).
...
receive_all(Ref,N,All) ->
    receive {Ref,SchedAll}->
        receive_all(Ref,N - 1,SchedAll ++ All) end.
...
-spec(delete(Tab) -> true when Tab::tab()).

delete(_) ->
    erlang:nif_error(undef).

-spec(delete(Tab,Key) -> true when Tab::tab(),Key::term()).

delete(_,_) ->
    erlang:nif_error(undef).

-spec(delete_all_objects(Tab) -> true when Tab::tab()).

delete_all_objects(Tab) ->
    _ = ets:internal_delete_all(Tab,undefined),
    true.

-spec(internal_delete_all(Tab,undefined) -> NumDeleted when Tab::tab(),NumDeleted::non_neg_integer()).

internal_delete_all(_,_) ->
    erlang:nif_error(undef).

-spec(delete_object(Tab,Object) -> true when Tab::tab(),Object::tuple()).

delete_object(_,_) ->
    erlang:nif_error(undef).

-spec(first(Tab) -> Key|'$end_of_table' when Tab::tab(),Key::term()).

first(_) ->
    erlang:nif_error(undef).

-spec(give_away(Tab,Pid,GiftData) -> true when Tab::tab(),Pid::pid(),GiftData::term()).

give_away(_,_,_) ->
    erlang:nif_error(undef).

-spec(info(Tab) -> InfoList|undefined when Tab::tab(),InfoList::[InfoTuple],InfoTuple::{compressed,boolean()}|{decentralized_counters,boolean()}|{heir,pid()|none}|{id,tid()}|{keypos,pos_integer()}|{memory,non_neg_integer()}|{name,atom()}|{named_table,boolean()}|{node,node()}|{owner,pid()}|{protection,access()}|{size,non_neg_integer()}|{type,type()}|{write_concurrency,boolean()}|{read_concurrency,boolean()}).

info(_) ->
    erlang:nif_error(undef).

-spec(info(Tab,Item) -> Value|undefined when Tab::tab(),Item::binary|compressed|decentralized_counters|fixed|heir|id|keypos|memory|name|named_table|node|owner|protection|safe_fixed|safe_fixed_monotonic_time|size|stats|type|write_concurrency|read_concurrency,Value::term()).

info(_,_) ->
    erlang:nif_error(undef).

-spec(insert(Tab,ObjectOrObjects) -> true when Tab::tab(),ObjectOrObjects::tuple()|[tuple()]).

insert(_,_) ->
    erlang:nif_error(undef).

-spec(insert_new(Tab,ObjectOrObjects) -> boolean() when Tab::tab(),ObjectOrObjects::tuple()|[tuple()]).
...
-spec(lookup(Tab,Key) -> [Object] when Tab::tab(),Key::term(),Object::tuple()).

lookup(_,_) ->
    erlang:nif_error(undef).

-spec(lookup_element(Tab,Key,Pos) -> Elem when Tab::tab(),Key::term(),Pos::pos_integer(),Elem::term()|[term()]).

lookup_element(_,_,_) ->
    erlang:nif_error(undef).

-spec(match(Tab,Pattern) -> [Match] when Tab::tab(),Pattern::match_pattern(),Match::[term()]).

match(_,_) ->
    erlang:nif_error(undef).

-spec(match(Tab,Pattern,Limit) -> {[Match],Continuation}|'$end_of_table' when Tab::tab(),Pattern::match_pattern(),Limit::pos_integer(),Match::[term()],Continuation::continuation()).
...
-spec(match_object(Tab,Pattern) -> [Object] when Tab::tab(),Pattern::match_pattern(),Object::tuple()).

match_object(_,_) ->
    erlang:nif_error(undef).

-spec(match_object(Tab,Pattern,Limit) -> {[Object],Continuation}|'$end_of_table' when Tab::tab(),Pattern::match_pattern(),Limit::pos_integer(),Object::tuple(),Continuation::continuation()).
...
-spec(match_spec_compile(MatchSpec) -> CompiledMatchSpec when MatchSpec::match_spec(),CompiledMatchSpec::comp_match_spec()).

match_spec_compile(_) ->
    erlang:nif_error(undef).

-spec(match_spec_run_r(List,CompiledMatchSpec,list()) -> list() when List::[term()],CompiledMatchSpec::comp_match_spec()).

match_spec_run_r(_,_,_) ->
    erlang:nif_error(undef).

-spec(member(Tab,Key) -> boolean() when Tab::tab(),Key::term()).

member(_,_) ->
    erlang:nif_error(undef).

-spec(new(Name,Options) -> tid()|atom() when Name::atom(),Options::[Option],Option::Type|Access|named_table|{keypos,Pos}|{heir,Pid::pid(),HeirData}|{heir,none}|Tweaks,Type::type(),Access::access(),Tweaks::{write_concurrency,boolean()}|{read_concurrency,boolean()}|{decentralized_counters,boolean()}|compressed,Pos::pos_integer(),HeirData::term()).

new(_,_) ->
    erlang:nif_error(undef).

-spec(next(Tab,Key1) -> Key2|'$end_of_table' when Tab::tab(),Key1::term(),Key2::term()).
...
-spec(rename(Tab,Name) -> Name when Tab::tab(),Name::atom()).

rename(_,_) ->
    erlang:nif_error(undef).

-spec(safe_fixtable(Tab,Fix) -> true when Tab::tab(),Fix::boolean()).

safe_fixtable(_,_) ->
    erlang:nif_error(undef).

-spec(select(Tab,MatchSpec) -> [Match] when Tab::tab(),MatchSpec::match_spec(),Match::term()).

select(_,_) ->
    erlang:nif_error(undef).

-spec(select(Tab,MatchSpec,Limit) -> {[Match],Continuation}|'$end_of_table' when Tab::tab(),MatchSpec::match_spec(),Limit::pos_integer(),Match::term(),Continuation::continuation()).
...
-spec(select_count(Tab,MatchSpec) -> NumMatched when Tab::tab(),MatchSpec::match_spec(),NumMatched::non_neg_integer()).
...
-spec(internal_select_delete(Tab,MatchSpec) -> NumDeleted when Tab::tab(),MatchSpec::match_spec(),NumDeleted::non_neg_integer()).

internal_select_delete(_,_) ->
    erlang:nif_error(undef).

-spec(select_replace(Tab,MatchSpec) -> NumReplaced when Tab::tab(),MatchSpec::match_spec(),NumReplaced::non_neg_integer()).

select_replace(_,_) ->
    erlang:nif_error(undef).

-spec(select_reverse(Tab,MatchSpec) -> [Match] when Tab::tab(),MatchSpec::match_spec(),Match::term()).

select_reverse(_,_) ->
    erlang:nif_error(undef).

-spec(select_reverse(Tab,MatchSpec,Limit) -> {[Match],Continuation}|'$end_of_table' when Tab::tab(),MatchSpec::match_spec(),Limit::pos_integer(),Match::term(),Continuation::continuation()).
...
-spec(setopts(Tab,Opts) -> true when Tab::tab(),Opts::Opt|[Opt],Opt::{heir,pid(),HeirData}|{heir,none},HeirData::term()).

setopts(_,_) ->
    erlang:nif_error(undef).

-spec(slot(Tab,I) -> [Object]|'$end_of_table' when Tab::tab(),I::non_neg_integer(),Object::tuple()).

slot(_,_) ->
    erlang:nif_error(undef).

-spec(take(Tab,Key) -> [Object] when Tab::tab(),Key::term(),Object::tuple()).

take(_,_) ->
    erlang:nif_error(undef).

-spec(update_counter(Tab,Key,UpdateOp) -> Result when Tab::tab(),Key::term(),UpdateOp::{Pos,Incr}|{Pos,Incr,Threshold,SetValue},Pos::integer(),Incr::integer(),Threshold::integer(),SetValue::integer(),Result::integer();(Tab,Key,[UpdateOp]) -> [Result] when Tab::tab(),Key::term(),UpdateOp::{Pos,Incr}|{Pos,Incr,Threshold,SetValue},Pos::integer(),Incr::integer(),Threshold::integer(),SetValue::integer(),Result::integer();(Tab,Key,Incr) -> Result when Tab::tab(),Key::term(),Incr::integer(),Result::integer()).

update_counter(_,_,_) ->
    erlang:nif_error(undef).

-spec(update_counter(Tab,Key,UpdateOp,Default) -> Result when Tab::tab(),Key::term(),UpdateOp::{Pos,Incr}|{Pos,Incr,Threshold,SetValue},Pos::integer(),Incr::integer(),Threshold::integer(),SetValue::integer(),Result::integer(),Default::tuple();(Tab,Key,[UpdateOp],Default) -> [Result] when Tab::tab(),Key::term(),UpdateOp::{Pos,Incr}|{Pos,Incr,Threshold,SetValue},Pos::integer(),Incr::integer(),Threshold::integer(),SetValue::integer(),Result::integer(),Default::tuple();(Tab,Key,Incr,Default) -> Result when Tab::tab(),Key::term(),Incr::integer(),Result::integer(),Default::tuple()).

update_counter(_,_,_,_) ->
    erlang:nif_error(undef).

-spec(update_element(Tab,Key,ElementSpec::{Pos,Value}) -> boolean() when Tab::tab(),Key::term(),Pos::pos_integer(),Value::term();(Tab,Key,ElementSpec::[{Pos,Value}]) -> boolean() when Tab::tab(),Key::term(),Pos::pos_integer(),Value::term()).
...
-spec(match_spec_run(List,CompiledMatchSpec) -> list() when List::[term()],CompiledMatchSpec::comp_match_spec()).
...
fun2ms(ShellFun)
    when is_function(ShellFun)->
    case erl_eval:fun_data(ShellFun) of
        {fun_data,ImportList,Clauses}->
            case ms_transform:transform_from_shell(ets,Clauses,ImportList) of
...
-spec(foldl(Function,Acc0,Tab) -> Acc1 when Function::fun((Element::term(),AccIn) -> AccOut),Tab::tab(),Acc0::term(),Acc1::term(),AccIn::term(),AccOut::term()).

foldl(F,Accu,T) ->
    ets:safe_fixtable(T,true),
    First = ets:first(T),
    try do_foldl(F,Accu,First,T)
        after ets:safe_fixtable(T,false) end.
...
-spec(foldr(Function,Acc0,Tab) -> Acc1 when Function::fun((Element::term(),AccIn) -> AccOut),Tab::tab(),Acc0::term(),Acc1::term(),AccIn::term(),AccOut::term()).

foldr(F,Accu,T) ->
    ets:safe_fixtable(T,true),
    Last = ets:last(T),
    try do_foldr(F,Accu,Last,T)
        after ets:safe_fixtable(T,false) end.
...
-spec(from_dets(Tab,DetsTab) -> true when Tab::tab(),DetsTab::dets:tab_name()).

from_dets(EtsTable,DetsTable) ->
    case  catch dets:to_ets(DetsTable,EtsTable) of
        {error,Reason}->
            error(Reason,[EtsTable, DetsTable]);
        {'EXIT',{Reason1,_Stack1}}->
            error(Reason1,[EtsTable, DetsTable]);
        {'EXIT',EReason}->
            error(EReason,[EtsTable, DetsTable]);
        EtsTable->
            true;
        Unexpected->
            error(Unexpected,[EtsTable, DetsTable])
    end.
...
-spec(init_table(Tab,InitFun) -> true when Tab::tab(),InitFun::fun((Arg) -> Res),Arg::read|close,Res::end_of_input|{Objects::[term()],InitFun}|term()).
...
init_table_continue(_Table,end_of_input) ->
    true;
init_table_continue(Table,{List,Fun})
    when is_list(List),
    is_function(Fun)->
...
-spec(match_delete(Tab,Pattern) -> true when Tab::tab(),Pattern::match_pattern()).
...
-record(filetab_options,{object_count = false::boolean(),md5sum = false::boolean(),sync = false::boolean()}).

-spec(tab2file(Tab,Filename) -> ok|{error,Reason} when Tab::tab(),Filename::file:name(),Reason::term()).

tab2file(Tab,File) ->
    tab2file(Tab,File,[]).

-spec(tab2file(Tab,Filename,Options) -> ok|{error,Reason} when Tab::tab(),Filename::file:name(),Options::[Option],Option::{extended_info,[ExtInfo]}|{sync,boolean()},ExtInfo::md5sum|object_count,Reason::term()).

tab2file(Tab,File,Options) ->
    try {ok,FtOptions} = parse_ft_options(Options),
    _ = file:delete(File),
    ...
    try Info0 = case ets:info(Tab) of
        undefined->
            throw(badtab);
        I->
            I
    end,
    Info = [list_to_tuple(Info0 ++ [{major_version,1}, {minor_version,0}, {extended_info,ft_options_to_list(FtOptions)}])],
    {LogFun,InitState} = case FtOptions#filetab_options.md5sum of
        true->
            {fun (Oldstate,Termlist)->
                {NewState,BinList} = md5terms(Oldstate,Termlist),
                case disk_log:blog_terms(Name,BinList) of
                    ok->
                        NewState;
                    {error,Reason2}->
                        throw(Reason2)
                end end,erlang:md5_init()};
        false->
            {fun (_,Termlist)->
    ...
    EndInfo = case FtOptions#filetab_options.object_count of
        ...
        false->
            []
    end ++ case FtOptions#filetab_options.md5sum of
        true->
    ...
    disk_log:close(Name)
            ...
            error:ErReason:StackTrace->
                _ = disk_log:close(Name),
                _ = file:delete(File),
                erlang:raise(error,ErReason,StackTrace) end
        catch
            throw:TReason2->
                {error,TReason2};
            exit:ExReason2->
...
-spec(file2tab(Filename) -> {ok,Tab}|{error,Reason} when Filename::file:name(),Tab::tab(),Reason::term()).

file2tab(File) ->
    file2tab(File,[]).

-spec(file2tab(Filename,Options) -> {ok,Tab}|{error,Reason} when Filename::file:name(),Tab::tab(),Options::[Option],Option::{verify,boolean()},Reason::term()).

file2tab(File,Opts) ->
    try {ok,Verify,TabArg} = parse_f2t_opts(Opts,false,[]),
    Name = make_ref(),
    {ok,Name} = case disk_log:open([{name,Name}, {file,File}, {mode,read_only}]) of
        {ok,Name}->
            {ok,Name};
        {repaired,Name,_,_}->
            case Verify of
    ...
    {ok,Major,Minor,FtOptions,MD5State,FullHeader,DLContext} = try get_header_data(Name,Verify)
        catch
            throw:badfile->
                _ = disk_log:close(Name),
                throw(badfile) end,
    try if Major > 1 ->
        throw({unsupported_file_version,{Major,Minor}});true ->
        ok end,
    {ok,Tab,HeadCount} = create_tab(FullHeader,TabArg),
    StrippedOptions = case Verify of
        true->
            FtOptions;
        false->
            #filetab_options{}
    end,
    {ReadFun,InitState} = case StrippedOptions#filetab_options.md5sum of
        true->
            {fun ({OldMD5State,OldCount,_OL,ODLContext} = OS)->
                case wrap_bchunk(Name,ODLContext,100,Verify) of
                    eof->
                        {OS,[]};
                    {NDLContext,Blist}->
                        {Termlist,NewMD5State,NewCount,NewLast} = md5_and_convert(Blist,OldMD5State,OldCount),
                        {{NewMD5State,NewCount,NewLast,NDLContext},Termlist}
                end end,{MD5State,0,[],DLContext}};
        false->
            {fun ({_,OldCount,_OL,ODLContext} = OS)->
                case wrap_chunk(Name,ODLContext,100,Verify) of
                    eof->
                        {OS,[]};
                    {NDLContext,List}->
                        {NewLast,NewCount,NewList} = scan_for_endinfo(List,OldCount),
                        {{false,NewCount,NewLast,NDLContext},NewList}
                end end,{false,0,[],DLContext}}
    end,
    try do_read_and_verify(ReadFun,InitState,Tab,StrippedOptions,HeadCount,Verify)
        catch
            throw:TReason->
                ets:delete(Tab),
                throw(TReason);
            ...
            error:ErReason:StackTrace->
                ets:delete(Tab),
                erlang:raise(error,ErReason,StackTrace) end
        after _ = disk_log:close(Name) end
        catch
            throw:TReason2->
                {error,TReason2};
            exit:ExReason2->
...
do_read_and_verify(ReadFun,InitState,Tab,FtOptions,HeadCount,Verify) ->
    case load_table(ReadFun,InitState,Tab) of
        {ok,{_,FinalCount,[],_}}->
            case {FtOptions#filetab_options.md5sum,FtOptions#filetab_options.object_count} of
                {false,false}->
                    case Verify of
                        false->
                            ok;
                ...
                Tup
                    ...
                    case verify_header_mandatory(L) of
                        ...
                        true->
                            ...
                            FtOptions = case lists:keyfind(extended_info,1,L) of
                                {extended_info,I}
                                    when is_list(I)->
                                    #filetab_options{object_count = lists:member(object_count,I),md5sum = lists:member(md5sum,I)};
                                _->
                                    #filetab_options{}
                            end,
                            MD5Initial = case FtOptions#filetab_options.md5sum of
                                true->
                                    X = erlang:md5_init(),
...
md5_and_convert([],MD5State,Count) ->
    {[],MD5State,Count,[]};
md5_and_convert([H| T],MD5State,Count)
    when is_binary(H)->
...
create_tab(I,TabArg) ->
    ...
    case TabArg of
        []->
            try Tab = ets:new(Name,L5),
            {ok,Tab,Sz}
                catch
                    _:_->
                        throw(cannot_create_table) end;
        _->
            {ok,TabArg,Sz}
    end.
...
tabfile_info(File)
    ...
    {ok,Major,Minor,_FtOptions,_MD5State,FullHeader,_DLContext} = try get_header_data(Name,false)
        catch
            throw:badfile->
                _ = disk_log:close(Name),
                throw(badfile) end,
...
-spec(table(Tab) -> QueryHandle when Tab::tab(),QueryHandle::qlc:query_handle()).

table(Tab) ->
    table(Tab,[]).

-spec(table(Tab,Options) -> QueryHandle when Tab::tab(),QueryHandle::qlc:query_handle(),Options::[Option]|Option,Option::{n_objects,NObjects}|{traverse,TraverseMethod},NObjects::default|pos_integer(),TraverseMethod::first_next|last_prev|select|{select,MatchSpec::match_spec()}).
    ...
    case options(Opts,[traverse, n_objects]) of
        ...
        [[Traverse, NObjs], QlcOptions]->
            TF = case Traverse of
                first_next->
                    fun ()->
                        qlc_next(Tab,ets:first(Tab)) end;
                last_prev->
                    fun ()->
                        qlc_prev(Tab,ets:last(Tab)) end;
                select->
                    fun (MS)->
                        qlc_select(ets:select(Tab,MS,NObjs)) end;
                {select,MS}->
                    fun ()->
                        qlc_select(ets:select(Tab,MS,NObjs)) end
            end,
            PreFun = fun (_)->
                ets:safe_fixtable(Tab,true) end,
            PostFun = fun ()->
                ets:safe_fixtable(Tab,false) end,
            InfoFun = fun (Tag)->
            ...
            LookupFun = case Traverse of
                ...
                _->
                    fun (_Pos,[K])->
                        ets:lookup(Tab,K);(_Pos,Ks)->
                        lists:flatmap(fun (K)->
                            ets:lookup(Tab,K) end,Ks) end
            end,
            FormatFun = fun ({all,_NElements,_ElementFun})->
                As = [Tab| [Opts || _ <- [[]],Opts =/= []]],
                {ets,table,As};({match_spec,MS})->
                {ets,table,[Tab, [{traverse,{select,MS}}| listify(Opts)]]};({lookup,_KeyPos,[Value],_NElements,ElementFun})->
                io_lib:format("~w:lookup(~w, ~w)",[ets, Tab, ElementFun(Value)]);({lookup,_KeyPos,Values,_NElements,ElementFun})->
                Vals = [(ElementFun(V)) || V <- Values],
                io_lib:format("lists:flatmap(fun(V) -> ~w:lookup" "(~w, V) end, ~w)",[ets, Tab, Vals]) end,
            qlc:table(TF,[{pre_fun,PreFun}, {post_fun,PostFun}, {info_fun,InfoFun}, {format_fun,FormatFun}, {key_equality,KeyEquality}, {lookup_fun,LookupFun}] ++ QlcOptions)
    end.
...
qlc_next(_Tab,'$end_of_table') ->
    [];
qlc_next(Tab,Key) ->
    ets:lookup(Tab,Key) ++ fun ()->
        qlc_next(Tab,ets:next(Tab,Key)) end.

qlc_prev(_Tab,'$end_of_table') ->
    [];
qlc_prev(Tab,Key) ->
    ets:lookup(Tab,Key) ++ fun ()->
        qlc_prev(Tab,ets:prev(Tab,Key)) end.

qlc_select('$end_of_table') ->
    [];
qlc_select({Objects,Cont}) ->
    Objects ++ fun ()->
        qlc_select(ets:select(Cont)) end.
...
pad_right(String,Len) ->
    if length(String) >= Len ->
        String;true ->
        [Space] = " ",
        String ++ lists:duplicate(Len - length(String),Space) end.

to_string(X) ->
    lists:flatten(io_lib:format("~p",[X])).

-spec(i(Tab) -> ok when Tab::tab()).
...
print_number(Tab,Key,Num) ->
    ...
    if Num - Len < 1 ->
        O = lists:nth(Num,Os),
        io:format("~p~n",[O]);true ->
        print_number(Tab,ets:next(Tab,Key),Num - Len) end.
...
do_display_item(_Height,Width,I,Opos) ->
    L = to_string(I),
    L2 = if length(L) > Width - 8 ->
        string:slice(L,0,Width - 13) ++ "  ...";true ->
        L end,
...
print_re_num(_,'$end_of_table',_,_) ->
    ok;
print_re_num(Tab,Key,Num,Re) ->
    ...
    if Num - Len < 1 ->
        O = lists:nth(Num,Os),
        io:format("~p~n",[O]);true ->
        print_re_num(Tab,ets:next(Tab,Key),Num - Len,Re) end.
