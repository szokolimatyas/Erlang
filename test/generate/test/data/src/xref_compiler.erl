...
-import(lists, [concat/1, foldl/3, nthtail/2, reverse/1, sort/1, sublist/2]).

-import(sofs, [composite/2, difference/2, empty_set/0, from_term/1, intersection/2, is_empty_set/1, multiple_relative_product/2, projection/2, relation/1, relation_to_family/1, restriction/2, specification/2, substitution/2, to_external/1, union/2, union_of_family/1]).
...
format_error({type_error,Expr}) ->
    io_lib:format("Operator applied to argument(s) of different or inva" "lid type(s): ~ts~n",[Expr]);
...
statements([Stmt = {assign,VarType,Name,E}| Stmts0],Table,L,UV) ->
    case dict:find(Name,Table) of
        ...
        error->
            ...
            Val = #xref_var{name = Name,vtype = VarType,otype = OType,type = Type},
...
check_expr(Expr = {set,SOp,E1,E2},Table) ->
    ...
    case OType of
        ...
        _Else->
            {Type,NewE1,NewE2} = case {type_ord(Type1),type_ord(Type2)} of
                {T1,T2}
                    when T1 =:= T2->
                    {Type1,NE1,NE2};
                {T1,T2}
...
check_expr({regexpr,RExpr,Type0},_Table) ->
    ...
    Call = {call,fun (E,V2)->
        xref_utils:regexpr(E,V2) end,{constants,RExpr},Var},
...
conversions(_OType,{line,LineType},{line,LineType}) ->
    ok;
conversions(edge,{line,_},{line,all_line_call}) ->
    ok;
conversions(edge,From,{line,Line})
    when is_atom(From),
    Line =/= all_line_call->
    ok;
conversions(vertex,From,{line,line})
    when is_atom(From)->
    ok;
conversions(vertex,From,To)
    when is_atom(From),
    is_atom(To)->
    ok;
conversions(edge,From,To)
    when is_atom(From),
    is_atom(To)->
    ok;
conversions(edge,{line,Line},To)
    ...
    Line =/= all_line_call->
    ok;
conversions(vertex,{line,line},To)
    when is_atom(To)->
    ok;
conversions(_OType,_From,_To) ->
    not_ok.

set_op(union,{line,_LineType},edge) ->
    family_union;
set_op(intersection,{line,_LineType},edge) ->
    family_intersection;
set_op(difference,{line,_LineType},edge) ->
    family_difference;
set_op(union,function,vertex) ->
    family_union;
set_op(intersection,function,vertex) ->
    family_intersection;
set_op(difference,function,vertex) ->
    family_difference;
set_op(SOp,_Type,_OType) ->
    SOp.
...
restriction(Column,Type1,VE,Type2,E2)
    when Type1 =:= function->
...
use_of_closure(Op,C) ->
    access_of_closure(C,{call,fun (X)->
        xref_utils:Op(X) end,C}).

use_of_closure(Op,C,E) ->
    access_of_closure(C,{call,fun (X,Y)->
        xref_utils:Op(X,Y) end,C,E}).
...
bifs(Cs) ->
    specification({external,fun ({M,F,A})->
        xref_utils:is_builtin(M,F,A) end},Cs).
...
general(_ObjectType,FromType,ToType,X)
    when FromType =:= ToType->
    X;
general(edge,{line,_LineType},ToType,LEs) ->
    VEs = {projection,{quote,{external,fun ({V1V2,_Ls})->
        V1V2 end}},LEs},
    general(edge,function,ToType,VEs);
general(edge,function,ToType,VEs) ->
    MEs = {projection,{quote,{external,fun ({{M1,_,_},{M2,_,_}})->
        {M1,M2} end}},VEs},
...
special(_ObjectType,FromType,ToType,X)
    when FromType =:= ToType->
    X;
special(edge,{line,_LineType},{line,all_line_call},Calls) ->
    {put,{tmp,mods},{projection,{quote,{external,fun ({{{M1,_,_},{M2,_,_}},_})->
        {M1,M2} end}},Calls},{put,{tmp,def_at},{union,{image,{get,def_at},{union,{domain,{get,{tmp,mods}}},{range,{get,{tmp,mods}}}}}},{fun funs_to_lines/2,{get,{tmp,def_at}},Calls}}};
special(edge,function,{line,LineType},VEs) ->
    Var = if LineType =:= line ->
        call_at;LineType =:= export_call ->
        e_call_at;LineType =:= local_call ->
        l_call_at;LineType =:= external_call ->
        x_call_at end,
    line_edges(VEs,Var);
special(edge,module,ToType,MEs) ->
    VEs = {image,{projection,{quote,{external,fun (FE = {{M1,_,_},{M2,_,_}})->
        {{M1,M2},FE} end}},{union,{image,{get,e},{projection,{quote,{external,fun ({M1,_M2})->
        M1 end}},MEs}}}},MEs},
...
special(vertex,function,{line,_LineType},V) ->
    {restriction,{union_of_family,{restriction,{get,def_at},{domain,V}}},{union_of_family,V}};
...
line_edges(VEs,CallAt) ->
    {put,{tmp,ves},VEs,{put,{tmp,m1},{projection,{quote,{external,fun ({{M1,_,_},_})->
        M1 end}},{get,{tmp,ves}}},{image,{projection,{quote,{external,fun (C = {VV,_L})->
        {VV,C} end}},{union,{image,{get,CallAt},{get,{tmp,m1}}}}},{get,{tmp,ves}}}}}.
...
i(E,Table) ->
    ...
    Fun = fun ({NN,S},D)->
        dict:store(NN,{extra,S,dict:fetch(NN,D)},D) end,
...
evaluate([{get,Var}| P],T,S)
    ...
    evaluate(P,T,[Val| S]);
evaluate([{get,{inverse,Var}}| P],T,S) ->
...
evaluate([{save,{user,Name}}| P],T,S = [Val| _]) ->
    ...
    NewVar = #xref_var{name = Name,value = Val,vtype = user,otype = OType,type = Type},
    T1 = update_graph_counter(Val,+1,T),
...
format_parse_error(["invalid_regexp", String, Error],Line) ->
    io_lib:format("Invalid regular expression \"~ts\"~s: ~ts~n",[String, Line, lists:flatten(Error)]);
