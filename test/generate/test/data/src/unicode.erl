...
-spec(bin_is_7bit(Binary) -> boolean() when Binary::binary()).
...
-spec(characters_to_list(Data,InEncoding) -> Result when Data::latin1_chardata()|chardata()|external_chardata(),InEncoding::encoding(),Result::list()|{error,list(),RestData}|{incomplete,list(),binary()},RestData::latin1_chardata()|chardata()|external_chardata()).

characters_to_list(_,_) ->
    erlang:nif_error(undef).

-spec(characters_to_list(Data) -> Result when Data::latin1_chardata()|chardata()|external_chardata(),Result::list()|{error,list(),RestData}|{incomplete,list(),binary()},RestData::latin1_chardata()|chardata()|external_chardata()).
...
characters_to_binary(ML) ->
    try unicode:characters_to_binary(ML,unicode)
        catch
            error:AnyError->
                ...
                {'EXIT',{new_stacktrace,[{Mod,_,L,_}| Rest]}} = ( catch error(new_stacktrace,[ML])),
                erlang:raise(error,TheError,[{Mod,characters_to_binary,L}| Rest]) end.
...
characters_to_binary(ML,latin1,latin1)
    when is_binary(ML)->
    ML;
characters_to_binary(ML,latin1,Uni)
    when is_binary(ML) and ((Uni =:= utf8) or (Uni =:= unicode))->
    case unicode:bin_is_7bit(ML) of
        true->
            ML;
        false->
            try characters_to_binary_int(ML,latin1,utf8)
                catch
                    error:AnyError->
                        TheError = case AnyError of
                            system_limit->
                        ...
                        {'EXIT',{new_stacktrace,[{Mod,_,L,_}| Rest]}} = ( catch error(new_stacktrace,[ML, latin1, Uni])),
                        erlang:raise(error,TheError,[{Mod,characters_to_binary,L}| Rest]) end
    end;
characters_to_binary(ML,Uni,latin1)
    when is_binary(ML) and ((Uni =:= utf8) or (Uni =:= unicode))->
    case unicode:bin_is_7bit(ML) of
        true->
            ML;
        false->
            try characters_to_binary_int(ML,utf8,latin1)
                    ...
                    error:AnyError->
                        ...
                        {'EXIT',{new_stacktrace,[{Mod,_,L,_}| Rest]}} = ( catch error(new_stacktrace,[ML, Uni, latin1])),
                        erlang:raise(error,TheError,[{Mod,characters_to_binary,L}| Rest]) end
    end;
characters_to_binary(ML,InEncoding,OutEncoding) ->
    try characters_to_binary_int(ML,InEncoding,OutEncoding)
            ...
            error:AnyError->
                ...
                {'EXIT',{new_stacktrace,[{Mod,_,L,_}| Rest]}} = ( catch error(new_stacktrace,[ML, InEncoding, OutEncoding])),
                erlang:raise(error,TheError,[{Mod,characters_to_binary,L}| Rest]) end.
...
characters_to_nfd_binary(CD,N,Row,Acc)
    when N > 0->
    case unicode_util:nfd(CD) of
        [GC| Str]->
...
characters_to_nfkd_binary(CD,N,Row,Acc)
    when N > 0->
    case unicode_util:nfkd(CD) of
        [GC| Str]->
...
characters_to_nfc_binary(CD,N,Row,Acc)
    when N > 0->
    case unicode_util:nfc(CD) of
        [GC| Str]->
...
characters_to_nfkc_binary(CD,N,Row,Acc)
    when N > 0->
    case unicode_util:nfkc(CD) of
        [GC| Str]->
...
characters_to_list_int(ML,Encoding) ->
    try do_characters_to_list(ML,Encoding)
        catch
            error:AnyError->
                TheError = case AnyError of
                    system_limit->
                ...
                {'EXIT',{new_stacktrace,[{Mod,_,L,_}| Rest]}} = ( catch error(new_stacktrace,[ML, Encoding])),
                erlang:raise(error,TheError,[{Mod,characters_to_list,L}| Rest]) end.
...
characters_to_binary_int(ML,InEncoding) ->
    try characters_to_binary_int(ML,InEncoding,unicode)
        catch
            error:AnyError->
                TheError = case AnyError of
                    system_limit->
                ...
                {'EXIT',{new_stacktrace,[{Mod,_,L,_}| Rest]}} = ( catch error(new_stacktrace,[ML, InEncoding])),
                erlang:raise(error,TheError,[{Mod,characters_to_binary,L}| Rest]) end.

characters_to_binary_int(ML,InEncoding,OutEncoding)
    when InEncoding =:= latin1,
    OutEncoding =:= unicode;
    InEncoding =:= latin1,
    OutEncoding =:= utf8;
    InEncoding =:= unicode,
...
characters_to_binary_int(ML,InEncoding,OutEncoding) ->
    {InTrans,Limit} = case OutEncoding of
        ...
        _->
            {i_trans(InEncoding),case InEncoding of
                latin1->
                    255;
                _->
                    1114111
...
cbv({utf32,big},<<0:8>>) ->
    3;
cbv({utf32,big},<<0:8,X:8>>)
    when X =< 16->
    2;
cbv({utf32,big},<<0:8,X:8,Y:8>>)
    when X =< 16,
    (X > 0) or ((Y =< 215) or (Y >= 224))->
    1;
cbv({utf32,big},_) ->
    false;
...
cbv({utf32,little},<<_:8,_:8>>) ->
    2;
cbv({utf32,little},<<X:8,255:8,0:8>>)
    when X =:= 254;
    X =:= 255->
    false;
...
ml_map([],_Fun,Accum) ->
    Accum;
ml_map([Part| _] = Whole,_,{{Incomplete,_},Accum})
    when is_integer(Part)->
    {error,Accum,[Incomplete| Whole]};
ml_map([Part| T],Fun,Accum)
    when is_integer(Part)->
...
ml_map([Part| T],Fun,{{Incomplete,Missing},Accum})
    when is_binary(Part)->
...
ml_map(Bin,Fun,{{Incomplete,Missing},Accum})
    when is_binary(Bin)->
...
ml_map(Part,Fun,Accum)
    when is_binary(Part),
    byte_size(Part) > 8192->
    <<Part1:8192/binary,Part2/binary>> = Part,
    ml_map([Part1, Part2],Fun,Accum);
ml_map(Bin,Fun,Accum)
    when is_binary(Bin)->
    Fun(Bin,Accum).

i_trans(latin1) ->
    fun (Bin)->
        binary_to_list(Bin) end;
i_trans(unicode) ->
    i_trans(utf8);
i_trans(utf8) ->
    fun do_i_utf8/1;
...
o_trans({utf32,little}) ->
    fun (L)->
        do_o_binary(fun (One)->
            <<One/utf32-little>> end,L) end.

do_o_binary(F,L) ->
    case do_o_binary2(F,L) of
        {Tag,List,R}->
            {Tag,iolist_to_binary(List),R};
        List->
...
do_i_utf16_big_chk(<<>>) ->
    [];
do_i_utf16_big_chk(<<U/utf16,R/binary>>)
    when U =< 255->
...
do_i_utf16_little_chk(<<>>) ->
    [];
do_i_utf16_little_chk(<<U/utf16-little,R/binary>>)
    when U =< 255->
...
do_i_utf32_big_chk(<<>>) ->
    [];
do_i_utf32_big_chk(<<U/utf32,R/binary>>)
    when U =< 255->
...
do_i_utf32_little_chk(<<>>) ->
    [];
do_i_utf32_little_chk(<<U/utf32-little,R/binary>>)
    when U =< 255->
