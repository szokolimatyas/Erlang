...
-record(erl_scan,{resword_fun = fun reserved_word/1::resword_fun(),ws = false::boolean(),comment = false::boolean(),text = false::boolean()}).

-spec(format_error(ErrorDescriptor) -> string() when ErrorDescriptor::error_description()).

format_error({string,Quote,Head}) ->
    lists:flatten(["unterminated " ++ string_thing(Quote) ++ " starting with " ++ io_lib:write_string(Head,Quote)]);
...
-type(cont_fun()::fun((char_spec(),#erl_scan{},erl_anno:line(),erl_anno:column(),tokens(),any()) -> any())).

-opaque(return_cont()::{erl_scan_continuation,string(),erl_anno:column(),tokens(),erl_anno:line(),#erl_scan{},any(),cont_fun()}).
...
options(Opts0)
    ...
    [RW_fun] = case opts(Opts,[reserved_word_fun],[]) of
        badarg->
            error(badarg,[Opts0]);
        R->
            R
...
scan1([$\t| Cs],St,Line,Col,Toks) ->
    skip_white_space(Cs,St,Line,Col,Toks,1);
scan1([C| Cs],St,Line,Col,Toks)
    when is_integer(C) andalso (C >= $\\ andalso C =< $  orelse C >= $\\ andalso C =< $ )->
...
scan_dot([$\n = C| Cs],St,Line,Col,Toks,Ncs) ->
    Anno = anno(Line,Col,St,if St#erl_scan.text ->
        Ncs ++ [C];true ->
        [] end),
...
scan_nl_white_space([$\n| Cs],#erl_scan{text = false} = St,Line,no_col = Col,Toks0,Ncs) ->
...
scan_char([C| Cs],St,Line,Col,Toks)
    when is_integer(C) andalso (C >= 0 andalso C < 55296 orelse C > 57343 andalso C < 65534 orelse C > 65535 andalso C =< 1114111)->
    Anno = anno(Line,Col,St,if St#erl_scan.text ->
        [$$, C];true ->
        [] end),
...
scan_string_no_col([C| Cs],Line,Col,Q,Wcs)
    when C =/= $\\,
    is_integer(C) andalso (C >= 0 andalso C < 55296 orelse C > 57343 andalso C < 65534 orelse C > 65535 andalso C =< 1114111)->
...
scan_escape([C0| Cs],Col)
    when is_integer(C0) andalso (C0 >= 0 andalso C0 < 55296 orelse C0 > 57343 andalso C0 < 65534 orelse C0 > 65535 andalso C0 =< 1114111)->
    C = escape_char(C0),
...
scan_hex([C| Cs],no_col = Col,Wcs)
    when C >= $0 andalso C =< $9 orelse C >= $A andalso C =< $F orelse C >= $a andalso C =< $f->
...
scan_esc_end([$}| Cs],Col,Wcs0,B,Str0) ->
    Wcs = lists:reverse(Wcs0),
    case  catch list_to_integer(Wcs,B) of
        Val
            when is_integer(Val) andalso (Val >= 0 andalso Val < 55296 orelse Val > 57343 andalso Val < 65534 orelse Val > 65535 andalso Val =< 1114111)->
...
scan_number([$_, Next| Cs],St,Line,Col,Toks,[Prev| _] = Ncs,_Us)
    when Next >= $0 andalso Next =< $9 andalso Prev >= $0 andalso Prev =< $9->
...
scan_based_int([C| Cs],St,Line,Col,Toks,B,Ncs,Bcs,Us)
    when C >= $0 andalso C =< $9 andalso C < $0 + B orelse C >= $A andalso B > 10 andalso C < $A + B - 10 orelse C >= $a andalso B > 10 andalso C < $a + B - 10->
    scan_based_int(Cs,St,Line,Col,Toks,B,[C| Ncs],Bcs,Us);
scan_based_int([$_, Next| Cs],St,Line,Col,Toks,B,[Prev| _] = Ncs,Bcs,_Us)
    when (Next >= $0 andalso Next =< $9 andalso Next < $0 + B orelse Next >= $A andalso B > 10 andalso Next < $A + B - 10 orelse Next >= $a andalso B > 10 andalso Next < $a + B - 10) andalso (Prev >= $0 andalso Prev =< $9 andalso Prev < $0 + B orelse Prev >= $A andalso B > 10 andalso Prev < $A + B - 10 orelse Prev >= $a andalso B > 10 andalso Prev < $a + B - 10)->
...
scan_based_int(Cs,St,Line,Col,Toks,B,Ncs0,Bcs,Us) ->
    Ncs = lists:reverse(Ncs0),
    case  catch list_to_integer(remove_digit_separators(Ncs,Us),B) of
        N
            when is_integer(N)->
...
scan_fraction([C| Cs],St,Line,Col,Toks,Ncs,Us)
    when C >= $0 andalso C =< $9->
    scan_fraction(Cs,St,Line,Col,Toks,[C| Ncs],Us);
scan_fraction([$_, Next| Cs],St,Line,Col,Toks,[Prev| _] = Ncs,_Us)
    when Next >= $0 andalso Next =< $9 andalso Prev >= $0 andalso Prev =< $9->
...
skip_comment([C| Cs],St,Line,Col,Toks,N)
    ...
    is_integer(C),
    C >= 0->
    case is_integer(C) andalso (C >= 0 andalso C < 55296 orelse C > 57343 andalso C < 65534 orelse C > 65535 andalso C =< 1114111) of
        true->
...
tok3(Cs,#erl_scan{text = false} = St,Line,no_col = Col,Toks,Item,_String,Sym,_Length) ->
    scan1(Cs,St,Line,Col,[{Item,anno(Line),Sym}| Toks]);
...
reserved_word('catch') ->
    true;
reserved_word('andalso') ->
    true;
reserved_word('orelse') ->
    true;
reserved_word('end') ->
    true;
reserved_word('fun') ->
