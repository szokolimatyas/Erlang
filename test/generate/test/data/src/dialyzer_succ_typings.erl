...
-export([find_succ_types_for_scc/2, refine_one_module/2, find_depends_on/2, collect_warnings/2, lookup_names/2]).
...
-record(st,{callgraph::dialyzer_callgraph:callgraph(),codeserver::dialyzer_codeserver:codeserver(),parent = none::parent(),timing_server::dialyzer_timing:timing_server(),solvers::[solver()],plt::dialyzer_plt:plt()}).

-spec(analyze_callgraph(dialyzer_callgraph:callgraph(),dialyzer_plt:plt(),dialyzer_codeserver:codeserver()) -> dialyzer_plt:plt()).

analyze_callgraph(Callgraph,Plt,Codeserver) ->
    analyze_callgraph(Callgraph,Plt,Codeserver,none,[],none).

-spec(analyze_callgraph(dialyzer_callgraph:callgraph(),dialyzer_plt:plt(),dialyzer_codeserver:codeserver(),dialyzer_timing:timing_server(),[solver()],parent()) -> dialyzer_plt:plt()).

analyze_callgraph(Callgraph,Plt,Codeserver,TimingServer,Solvers,Parent) ->
    NewState = init_state_and_get_success_typings(Callgraph,Plt,Codeserver,TimingServer,Solvers,Parent),
    NewState#st.plt.

init_state_and_get_success_typings(Callgraph,Plt,Codeserver,TimingServer,Solvers,Parent) ->
    {SCCs,Callgraph1} = begin dialyzer_timing:start_stamp(TimingServer,"order"),
    _T = dialyzer_callgraph:finalize(Callgraph),
    dialyzer_timing:end_stamp(TimingServer),
    _T end,
    State = #st{callgraph = Callgraph1,plt = Plt,codeserver = Codeserver,parent = Parent,timing_server = TimingServer,solvers = Solvers},
    get_refined_success_typings(SCCs,State).

get_refined_success_typings(SCCs,#st{callgraph = Callgraph,timing_server = TimingServer} = State) ->
    case find_succ_typings(SCCs,State) of
        {fixpoint,State1}->
            State1;
        {not_fixpoint,NotFixpoint1,State1}->
            {ModulePostorder,ModCallgraph} = begin dialyzer_timing:start_stamp(TimingServer,"order"),
            _C1 = dialyzer_callgraph:module_postorder_from_funs(NotFixpoint1,Callgraph),
            dialyzer_timing:end_stamp(TimingServer),
            _C1 end,
            ...
            case refine_succ_typings(ModulePostorder,ModState) of
                ...
                {not_fixpoint,NotFixpoint2,State2}->
                    {NewSCCs,Callgraph2} = begin dialyzer_timing:start_stamp(TimingServer,"order"),
                    _C2 = dialyzer_callgraph:reset_from_funs(NotFixpoint2,ModCallgraph),
                    dialyzer_timing:end_stamp(TimingServer),
                    _C2 end,
...
-spec(get_warnings(dialyzer_callgraph:callgraph(),dialyzer_plt:plt(),doc_plt(),dialyzer_codeserver:codeserver(),dialyzer_timing:timing_server(),[solver()],pid()) -> {[raw_warning()],dialyzer_plt:plt(),doc_plt()}).

get_warnings(Callgraph,Plt,DocPlt,Codeserver,TimingServer,Solvers,Parent) ->
    InitState = init_state_and_get_success_typings(Callgraph,Plt,Codeserver,TimingServer,Solvers,Parent),
    Mods = dialyzer_callgraph:modules(InitState#st.callgraph),
    Plt = InitState#st.plt,
    CWarns = dialyzer_contracts:get_invalid_contract_warnings(Mods,Codeserver,Plt),
    ModWarns = begin dialyzer_timing:start_stamp(TimingServer,"warning"),
...
get_warnings_from_modules(Mods,State,DocPlt) ->
    #st{callgraph = Callgraph,codeserver = Codeserver,plt = Plt,timing_server = TimingServer} = State,
...
collect_warnings(M,{Codeserver,Callgraph,Plt,DocPlt}) ->
    ...
    {Warnings2,FunTypes} = dialyzer_dataflow:get_warnings(ModCode,Plt,Callgraph,Codeserver,Records),
...
postprocess_warnings(RawWarnings,Codeserver) ->
    Pred = fun ({warn_contract_range,_,_})->
        true;(_)->
        false end,
...
postprocess_dataflow_warns([{warn_contract_range,WarningInfo,Msg}| Rest],Codeserver,WAcc,Acc) ->
        ...
        {ok,{{ContrF,ContrL},_C,_X}}->
            case CallF =:= ContrF of
                true->
                    ...
                    Filter = fun ({warn_contract_types,WI,_})
...
refine_succ_typings(Modules,#st{codeserver = Codeserver,callgraph = Callgraph,plt = Plt,timing_server = Timing,solvers = Solvers} = State) ->
    ok,
    Init = {Codeserver,Callgraph,Plt,Solvers},
    NotFixpoint = begin dialyzer_timing:start_stamp(Timing,"refine"),
...
compare_types_1([{X,_Type1}| Left1],[{X,failed_fun}| Left2],Strict,NotFixpoint) ->
    compare_types_1(Left1,Left2,Strict,NotFixpoint);
compare_types_1([{X,failed_fun}| Left1],[{X,_Type2}| Left2],Strict,NotFixpoint) ->
    compare_types_1(Left1,Left2,Strict,NotFixpoint);
...
find_succ_typings(SCCs,#st{codeserver = Codeserver,callgraph = Callgraph,plt = Plt,timing_server = Timing,solvers = Solvers} = State) ->
    Init = {Codeserver,Callgraph,Plt,Solvers},
    NotFixpoint = begin dialyzer_timing:start_stamp(Timing,"typesig"),
...
find_succ_types_for_scc(SCC0,{Codeserver,Callgraph,Plt,Solvers}) ->
    ...
    AllFuns = lists:append([begin {_Var,Fun} = dialyzer_codeserver:lookup_mfa_code(MFA,Codeserver),
    collect_fun_info([Fun]) end || MFA <- SCC]),
    PropTypes = get_fun_types_from_plt(AllFuns,Callgraph,Plt),
    FunTypes = dialyzer_typesig:analyze_scc(SCC,Label,Callgraph,Codeserver,Plt,PropTypes,Solvers),
    AllFunSet = sets:from_list([X || {X,_} <- AllFuns]),
    FilteredFunTypes = orddict:filter(fun (F,_T)->
        sets:is_element(F,AllFunSet) end,FunTypes),
    {FunMFAContracts,ModOpaques} = prepare_decoration(FilteredFunTypes,Callgraph,Codeserver),
    DecoratedFunTypes = decorate_succ_typings(FunMFAContracts,ModOpaques),
    Contracts = orddict:from_list([{MFA,Contract} || {_,{MFA,Contract}} <- FunMFAContracts]),
    PltContracts = dialyzer_contracts:check_contracts(Contracts,Callgraph,DecoratedFunTypes,ModOpaques),
    debug_pp_functions("SCC",FilteredFunTypes,DecoratedFunTypes,Callgraph),
    NewPltContracts = [MC || {MFA,_C} = MC <- PltContracts, not dialyzer_plt:is_contract(Plt,MFA)],
    ...
    case ContractFixpoint andalso reached_fixpoint_strict(PropTypes,DecoratedFunTypes) of
        true->
            [];
        false->
            ok,
...
prepare_decoration(FunTypes,Callgraph,Codeserver) ->
    ...
    ModOpaques = [{M,lookup_opaques(M,Codeserver)} || M <- lists:usort([M || {_LabelType,{{M,_,_},_Con}} <- Contracts])],
