...
-export([start/5, start/6, debug_options/2, hibernate_after/1, name/1, unregister_name/1, get_proc_name/1, get_parent/0, call/3, call/4, reply/2, send_request/3, wait_response/2, check_response/2, stop/1, stop/3]).
...
do_spawn(GenMod,link,Mod,Args,Options) ->
    ...
    proc_lib:start_link(gen,init_it,[GenMod, self(), self(), Mod, Args, Options],Time,spawn_opts(Options));
do_spawn(GenMod,monitor,Mod,Args,Options) ->
    Time = timeout(Options),
    Ret = proc_lib:start_monitor(gen,init_it,[GenMod, self(), self(), Mod, Args, Options],Time,spawn_opts(Options)),
    monitor_return(Ret);
do_spawn(GenMod,_,Mod,Args,Options) ->
    Time = timeout(Options),
    proc_lib:start(gen,init_it,[GenMod, self(), self, Mod, Args, Options],Time,spawn_opts(Options)).

do_spawn(GenMod,link,Name,Mod,Args,Options) ->
    Time = timeout(Options),
    proc_lib:start_link(gen,init_it,[GenMod, self(), self(), Name, Mod, Args, Options],Time,spawn_opts(Options));
do_spawn(GenMod,monitor,Name,Mod,Args,Options) ->
    Time = timeout(Options),
    Ret = proc_lib:start_monitor(gen,init_it,[GenMod, self(), self(), Name, Mod, Args, Options],Time,spawn_opts(Options)),
    monitor_return(Ret);
do_spawn(GenMod,_,Name,Mod,Args,Options) ->
    Time = timeout(Options),
    proc_lib:start(gen,init_it,[GenMod, self(), self, Name, Mod, Args, Options],Time,spawn_opts(Options)).
...
monitor_return({Error,Mon})
    when is_reference(Mon)->
    receive {'DOWN',Mon,process,_Pid,_Reason}->
        ok end,
    Error.
...
send_request(Process,Label,Request) ->
    Fun = fun (Pid)->
        do_send_request(Pid,Label,Request) end,
...
wait_response(Mref,Timeout)
    ...
    receive {{'$gen_request_id',Mref},Reply}->
        demonitor(Mref,[flush]),
        {reply,Reply};
    {'DOWN',Mref,_,Object,Reason}->
        {error,{Reason,Object}} after Timeout->
        timeout end.
...
stop(Process,Reason,Timeout)
    ...
    Timeout >= 0->
    Fun = fun (Pid)->
        proc_lib:stop(Pid,Reason,Timeout) end,
    do_for_proc(Process,Fun).
...
do_for_proc(Process,Fun)
    when tuple_size(Process) == 2 andalso element(1,Process) == global orelse tuple_size(Process) == 3 andalso element(1,Process) == via->
...
do_for_proc({Name,Node},Fun)
    when Node =:= node()->
    do_for_proc(Name,Fun);
do_for_proc({_Name,Node} = Process,Fun)
    when is_atom(Node)->
    if node() =:= nonode@nohost ->
        exit({nodedown,Node});true ->
        Fun(Process) end.
