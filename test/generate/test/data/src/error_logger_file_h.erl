-file("error_logger_file_h.erl", 1).
...
-record(st,{fd,filename,prev_handler,depth = unlimited::unlimited|non_neg_integer()}).
...
init(File,PrevHandler) ->
    ...
    case file:open(File,[write, {encoding,utf8}]) of
        {ok,Fd}->
            ...
            State = #st{fd = Fd,filename = File,prev_handler = PrevHandler,depth = Depth},
            {ok,State};
        Error->
            Error
    end.
...
header({{Y,Mo,D},{H,Mi,S}},Title,UTC) ->
    io_lib:format("~n=~ts==== ~p-~s-~p::~s:~s:~s ~s===~n",[Title, D, month(Mo), Y, t(H), t(Mi), t(S), UTC]).
