...
-file("ssh_subsystem_sup.erl", 30).

-export([start_link/5, connection_supervisor/1, channel_supervisor/1, tcpip_fwd_supervisor/1, start_channel/8]).

-export([init/1]).

start_link(Role,Address,Port,Profile,Options) ->
    supervisor:start_link(ssh_subsystem_sup,[Role, Address, Port, Profile, Options]).
...
child_specs(Role,Address,Port,Profile,Options) ->
    [ssh_channel_child_spec(Role,Address,Port,Profile,Options), ssh_connection_child_spec(Role,Address,Port,Profile,Options), ssh_tcpip_forward_acceptor_child_spec()].

ssh_connection_child_spec(Role,Address,Port,_Profile,Options) ->
    #{id=>id(Role,ssh_connection_sup,Address,Port),start=>{ssh_connection_sup,start_link,[Options]},restart=>temporary,type=>supervisor}.

ssh_channel_child_spec(Role,Address,Port,_Profile,Options) ->
    #{id=>id(Role,ssh_channel_sup,Address,Port),start=>{ssh_channel_sup,start_link,[Options]},restart=>temporary,type=>supervisor}.

ssh_tcpip_forward_acceptor_child_spec() ->
    #{id=>make_ref(),start=>{ssh_tcpip_forward_acceptor_sup,start_link,[]},restart=>temporary,type=>supervisor}.
