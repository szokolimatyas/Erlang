-file("erl_error.erl", 1).

-module(erl_error).

-export([format_exception/6, format_exception/7, format_exception/8, format_stacktrace/4, format_stacktrace/5, format_call/4, format_call/5, format_fun/1, format_fun/2]).
...
format_exception(I,Class,Reason,StackTrace,StackFun,FormatFun,Encoding,CharsLimit)
    when is_integer(I),
    I >= 1,
    is_function(StackFun,3),
    is_function(FormatFun,3),
    is_integer(CharsLimit)->
    S = n_spaces(I - 1),
    {Term,Trace1,Trace} = analyze_exception(Class,Reason,StackTrace),
    StLimit = if CharsLimit < 0 ->
        CharsLimit;true ->
        CharsLimit div 3 end,
...
explain_reason(badarg,error,[],_PF,_S,_Enc,_CL) ->
    <<"bad argument">>;
explain_reason({badarg,V},error = Cl,[],PF,S,_Enc,CL) ->
...
explain_reason({badarity,{Fun,As}},error,[],_PF,_S,Enc,_CL)
    when is_function(Fun)->
    io_lib:fwrite(<<"~ts called with ~s">>,[format_fun(Fun,Enc), argss(length(As))]);
...
explain_reason(function_clause,error = Cl,[{M,F,As,Loc}],PF,S,Enc,CL) ->
    ...
    [format_errstr_call(Str,Cl,{M,F},As,PF,S,Enc,CL), $ | location(Loc)];
...
explain_reason({argument_limit,_Fun},error,[],_PF,_S,_Enc,_CL) ->
    io_lib:fwrite(<<"limit of number of arguments to interpreted functi" "on exceeded">>,[]);
...
explain_reason({restricted_shell_disallowed,{ForMF,As}},exit = Cl,[],PF,S,Enc,CL) ->
...
format_stacktrace1(S0,Stack0,PF,SF,Enc,CL) ->
    Stack1 = lists:dropwhile(fun ({M,F,A,_})->
        SF(M,F,A) end,lists:reverse(Stack0)),
    S = ["  "| S0],
...
format_stacktrace2(_S,_Stack,_N,_PF,_Enc,_CL = 0) ->
    [];
format_stacktrace2(S,[{M,F,A,L}| Fs],N,PF,Enc,CL)
    when is_integer(A)->
    Cs = io_lib:fwrite(<<"~s~s ~ts ~ts">>,[sep(N,S), origin(N,M,F,A), mfa_to_string(M,F,A,Enc), location(L)]),
    CL1 = sub(CL,Cs,Enc),
    [Cs| format_stacktrace2(S,Fs,N + 1,PF,Enc,CL1)];
format_stacktrace2(S,[{M,F,As,_}| Fs],N,PF,Enc,CL)
    when is_list(As)->
    A = length(As),
    CalledAs = [S, <<"   called as ">>],
    C = format_call("",CalledAs,{M,F},As,PF,Enc,CL),
    Cs = io_lib:fwrite(<<"~s~s ~ts\n~s~ts">>,[sep(N,S), origin(N,M,F,A), mfa_to_string(M,F,A,Enc), CalledAs, C]),
    CL1 = sub(CL,Enc,Cs),
    [Cs| format_stacktrace2(S,Fs,N + 1,PF,Enc,CL1)];
...
location(L) ->
    ...
    if File =/= undefined,
    Line =/= undefined ->
        io_lib:format("(~ts, line ~w)",[File, Line]);true ->
        "" end.
...
format_call(ErrStr,Pre1,ForMForFun,As,PF,Enc,CL) ->
    Arity = length(As),
    [ErrStr| case is_op(ForMForFun,Arity) of
        {yes,Op}->
