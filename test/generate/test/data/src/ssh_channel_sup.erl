...
start_child(client,ChannelSup,ConnRef,Callback,Id,Args,Exec,_Opts)
    when is_pid(ConnRef)->
...
start_the_child(ChanMod,ChannelSup,ConnRef,Callback,Id,Args,Exec) ->
    ChildSpec = #{id=>make_ref(),start=>{ChanMod,start_link,[ConnRef, Id, Callback, Args, Exec]},restart=>temporary,type=>worker,modules=>[ChanMod]},
...
max_num_channels_not_exceeded(ChannelSup,Opts) ->
    MaxNumChannels = ssh_options:get_value(user_options,max_channels,Opts,ssh_channel_sup,86),
    NumChannels = length([x || {_,_,worker,[ssh_server_channel]} <- supervisor:which_children(ChannelSup)]),
