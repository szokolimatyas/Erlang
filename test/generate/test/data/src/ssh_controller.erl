...
handle_call({start_system_subsystem,Sup,Address,Port,Profile,Options,ChildSpec},_From,D) ->
    try {ok,SystemSup0} = case ssh_system_sup:system_supervisor(Address,Port,Profile) of
        undefined->
            supervisor:start_child(Sup,ChildSpec);
        Pid->
            {ok,Pid}
    end,
    {SystemSup0,ssh_system_sup:start_subsystem(SystemSup0,D#data.role,Address,Port,Profile,Options)} of 
        {SystemSup,{ok,SubSysSup}}->
            {reply,{ok,{SystemSup,SubSysSup}},D}
        catch
            C:E:S->
                {reply,{error,{failed,C,E,S}},D} end;
