...
read_line(Prompt,Opts) ->
    ...
    ssh_options:get_value(internal_options,user_pid,Opts,ssh_io,32) ! {self(),question},
    receive Answer
        when is_list(Answer) or is_binary(Answer)->
        unicode:characters_to_list(Answer) end.

yes_no(Prompt,Opts) ->
    format("~s [y/n]?",[Prompt]),
    ssh_options:get_value(internal_options,user_pid,Opts,ssh_io,40) ! {self(),question},
    receive y->
        yes;
    n->
        no;
...
read_password(Prompt,Opts) ->
    format("~s",[listify(Prompt)]),
    ssh_options:get_value(internal_options,user_pid,Opts,ssh_io,61) ! {self(),user_password},
    receive Answer
        when is_list(Answer) or is_binary(Answer)->
        case trim(Answer) of
            ""->
