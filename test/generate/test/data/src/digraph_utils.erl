-file("digraph_utils.erl", 1).

-module(digraph_utils).

-export([components/1, strong_components/1, cyclic_strong_components/1, reachable/2, reachable_neighbours/2, reaching/2, reaching_neighbours/2, topsort/1, is_acyclic/1, arborescence_root/1, is_arborescence/1, is_tree/1, loop_vertices/1, subgraph/2, subgraph/3, condensation/1, preorder/1, postorder/1]).
...
is_tree(G) ->
    digraph:no_edges(G) =:= digraph:no_vertices(G) - 1 andalso length(components(G)) =:= 1.
...
forest(G,SF,Vs,HandleFirst) ->
    ...
    F = fun (V,LL)->
        pretraverse(HandleFirst,V,SF,G,T,LL) end,
    LL = lists:foldl(F,[],Vs),
    ets:delete(T),
    LL.
...
subgraph(G,Vs,Type,Keep) ->
    try digraph:new(Type) of 
        SG->
            ...
            EFun = fun (V)->
                lists:foreach(fun (E)->
                    subgraph_edge(E,G,SG,Keep) end,digraph:out_edges(G,V)) end,
...
condense(SC,G,SCG,V2I,I2C) ->
    ...
    NFun = fun (Neighbour)->
        ...
        ets:insert(T,{I}) end,
    VFun = fun (V)->
        lists:foreach(NFun,digraph:out_neighbours(G,V)) end,
