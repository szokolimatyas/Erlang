...
-type(rec_print_fun()::fun((Tag::atom(),NFields::non_neg_integer()) -> no|[FieldName::atom()])).
...
-type(option()::{chars_limit,chars_limit()}|{column,column()}|{depth,depth()}|{encoding,encoding()}|{line_length,line_length()}|{line_max_chars,line_max_chars()}|{record_print_fun,rec_print_fun()}|{strings,boolean()}).

-type(options()::[option()]).

-spec(print(term(),rec_print_fun()) -> chars();(term(),options()) -> chars()).
...
-spec(print(term(),column(),line_length(),depth(),rec_print_fun()) -> chars()).

print(Term,Col,Ll,D,RecDefFun) ->
    print(Term,Col,Ll,D,_M = -1,RecDefFun).

-spec(print(term(),column(),line_length(),depth(),line_max_chars(),rec_print_fun()) -> chars()).
...
print(_,_,_,_D,_M,0,_RF,_Enc,_Str) ->
    "...";
print(Term,Col,Ll,D,M,T,RecDefFun,Enc,Str)
    when Col =< 0->
    print(Term,1,Ll,D,M,T,RecDefFun,Enc,Str);
print(Atom,_Col,_Ll,_D,_M,_T,_RF,Enc,_Str)
    when is_atom(Atom)->
    write_atom(Atom,Enc);
print(Term,Col,Ll,D,M0,T,RecDefFun,Enc,Str)
    when is_tuple(Term);
    is_list(Term);
    is_map(Term);
    is_bitstring(Term)->
    {_,Len,_Dots,_} = If = case T < 0 of
        true->
    ...
    Len =< M ->
        write(If);true ->
        TInd = while_fail([-1, 4],fun (I)->
            cind(If,Col,Ll,M,I,0,0) end,1),
        pp(If,Col,Ll,M,TInd,indent(Col),0,0) end;
...
pp_tag_tuple([{Tag,Tlen,_,_}| L],Col,Ll,M,TInd,Ind,LD,W) ->
    ...
    TagInd > TInd ->
        ...
        [Tag| pp_tail(L,Col1,Tcol,Ll,M,TInd,Indent,LD,S,W + Tlen)];true ->
        Indent = indent(TagInd,Ind),
        [Tag, S| pp_list(L,Tcol,Ll,M,TInd,Indent,LD,S,W + Tlen + 1)] end.
...
pp_pairs_tail([{_,Len,_,_} = P| Ps],Col0,Col,Ll,M,TInd,Ind,LD,W) ->
    ...
    ELen + 1 < Ll - Col,
    W + ELen + 1 =< M,
    is_list(element(1,element(2,element(1,P)))) andalso is_list(element(1,element(3,element(1,P))));
    LD1 > 0,
    ELen < Ll - Col - LD1,
    W + ELen + LD1 =< M,
    is_list(element(1,element(2,element(1,P)))) andalso is_list(element(1,element(3,element(1,P)))) ->
        [$,, write_pair(P)| pp_pairs_tail(Ps,Col0,Col + ELen,Ll,M,TInd,Ind,LD,W + ELen)];true ->
        {PS,PW} = pp_pair(P,Col0,Ll,M,TInd,Ind,LD1,0),
        [$,, $\n, Ind, PS| pp_pairs_tail(Ps,Col0,Col0 + PW,Ll,M,TInd,Ind,LD,PW)] end.

pp_pair({_,Len,_,_} = Pair,Col,Ll,M,_TInd,_Ind,LD,W)
    when Len < Ll - Col - LD,
    Len + W + LD =< M->
    {write_pair(Pair),if is_list(element(1,element(2,element(1,Pair)))) andalso is_list(element(1,element(3,element(1,Pair)))) ->
        Len;true ->
        Ll end};
pp_pair({{map_pair,K,V},_Len,_,_},Col0,Ll,M,TInd,Ind0,LD,W) ->
    I = map_value_indent(TInd),
    Ind = indent(I,Ind0),
    {[pp(K,Col0,Ll,M,TInd,Ind0,LD,W), " =>\n", Ind| pp(V,Col0 + I,Ll,M,TInd,Ind,LD,0)],Ll}.
...
pp_fields_tail([{_,Len,_,_} = F| Fs],Col0,Col,Ll,M,TInd,Ind,LD,W) ->
    ...
    ELen + 1 < Ll - Col,
    W + ELen + 1 =< M,
    is_list(element(1,element(4,element(1,F))));
    LD1 > 0,
    ELen < Ll - Col - LD1,
    W + ELen + LD1 =< M,
    is_list(element(1,element(4,element(1,F)))) ->
        [$,, write_field(F)| pp_fields_tail(Fs,Col0,Col + ELen,Ll,M,TInd,Ind,LD,W + ELen)];true ->
        {FS,FW} = pp_field(F,Col0,Ll,M,TInd,Ind,LD1,0),
        [$,, $\n, Ind, FS| pp_fields_tail(Fs,Col0,Col0 + FW,Ll,M,TInd,Ind,LD,FW)] end.

pp_field({_,Len,_,_} = Fl,Col,Ll,M,_TInd,_Ind,LD,W)
    when Len < Ll - Col - LD,
    Len + W + LD =< M->
    {write_field(Fl),if is_list(element(1,element(4,element(1,Fl)))) ->
        Len;true ->
        Ll end};
...
pp_tail([{_,Len,_,_} = E| Es],Col0,Col,Ll,M,TInd,Ind,LD,S,W) ->
    ...
    W + ELen + LD1 =< M,
    is_list(element(1,E)) ->
        [$,, write(E)| pp_tail(Es,Col0,Col + ELen,Ll,M,TInd,Ind,LD,S,W + ELen)];true ->
        {ES,WE} = pp_element(E,Col0,Ll,M,TInd,Ind,LD1,0),
        [$,, $\n, Ind, ES| pp_tail(Es,Col0,Col0 + WE,Ll,M,TInd,Ind,LD,S,WE)] end;
...
-type(more()::fun((chars_limit(),DeltaDepth::non_neg_integer()) -> intermediate_format())).

-type(if_list()::maybe_improper_list(intermediate_format(),{dots,non_neg_integer(),non_neg_integer(),more()})).

-type(intermediate_format()::{chars()|{bin,chars()}|dots|{field,Name::chars(),NameLen::non_neg_integer(),intermediate_format()}|{list,if_list()}|{map,if_list()}|{map_pair,K::intermediate_format(),V::intermediate_format()}|{record,[{Name::chars(),NameLen::non_neg_integer()}|if_list()]}|{tuple,IsTagged::boolean(),if_list()},Len::non_neg_integer(),NumOfDots::non_neg_integer(),More::more()|no_more}).

-spec(intermediate(term(),depth(),pos_integer(),rec_print_fun(),encoding(),boolean()) -> intermediate_format()).

intermediate(Term,D,T,RF,Enc,Str)
    when T > 0->
    D0 = 1,
...
print_length({},_D,_T,_RF,_Enc,_Str) ->
    {"{}",2,0,no_more};
print_length(#{} = M,_D,_T,_RF,_Enc,_Str)
    when map_size(M) =:= 0->
    {"#{}",3,0,no_more};
print_length(Atom,_D,_T,_RF,Enc,_Str)
    when is_atom(Atom)->
    S = write_atom(Atom,Enc),
    {S,io_lib:chars_length(S),0,no_more};
print_length(List,D,T,RF,Enc,Str)
    when is_list(List)->
        ...
        false->
            case print_length_list(List,D,T,RF,Enc,Str) of
                {What,Len,Dots,_More}
                    when Dots > 0->
                    More = fun (T1,Dd)->
                        print_length(List,D + Dd,T1,RF,Enc,Str) end,
                    {What,Len,Dots,More};
                If->
                    If
            end
    end;
print_length(Fun,_D,_T,_RF,_Enc,_Str)
    when is_function(Fun)->
    S = io_lib:write(Fun),
    {S,iolist_size(S),0,no_more};
print_length(R,D,T,RF,Enc,Str)
    when is_atom(element(1,R)),
    is_function(RF)->
...
print_length(Tuple,D,T,RF,Enc,Str)
    when is_tuple(Tuple)->
    print_length_tuple(Tuple,D,T,RF,Enc,Str);
print_length(Map,D,T,RF,Enc,Str)
    when is_map(Map)->
...
print_length(<<_/bitstring>> = Bin,1,_T,RF,Enc,Str) ->
    More = fun (T1,Dd)->
        print_length(Bin,1 + Dd,T1,RF,Enc,Str) end,
    {"<<...>>",7,3,More};
print_length(<<_/bitstring>> = Bin,D,T,RF,Enc,Str) ->
    D1 = D - 1,
    case Str andalso bit_size(Bin) rem 8 =:= 0 andalso printable_bin0(Bin,D1,tsub(T,6),Enc) of
        {true,List}
            when is_list(List)->
        ...
        {true,true,Prefix}->
            ...
            More = fun (T1,Dd)->
                print_length(Bin,D + Dd,T1,RF,Enc,Str) end,
            {[$<, $<, S| "...>>"],7 + length(S),3,More};
        {false,true,Prefix}->
            S = io_lib:write_string(Prefix,$\"),
            More = fun (T1,Dd)->
...
print_length_map(Map,1,_T,RF,Enc,Str) ->
    More = fun (T1,Dd)->
        print_length_map(Map,1 + Dd,T1,RF,Enc,Str) end,
    {"#{...}",6,3,More};
print_length_map(Map,D,T,RF,Enc,Str)
    when is_map(Map)->
...
print_length_map_pairs(none,_D,_D0,_T,_RF,_Enc,_Str) ->
    [];
print_length_map_pairs(Term,D,D0,T,RF,Enc,Str)
    when D =:= 1;
    T =:= 0->
    More = fun (T1,Dd)->
        print_length_map_pairs(Term,D + Dd,D0,T1,RF,Enc,Str) end,
    {dots,3,3,More};
...
print_length_tuple(Tuple,1,_T,RF,Enc,Str) ->
    More = fun (T1,Dd)->
        print_length_tuple(Tuple,1 + Dd,T1,RF,Enc,Str) end,
    {"{...}",5,3,More};
print_length_tuple(Tuple,D,T,RF,Enc,Str) ->
...
print_length_tuple1(Tuple,I,D,T,RF,Enc,Str)
    ...
    T =:= 0->
    More = fun (T1,Dd)->
        print_length_tuple1(Tuple,I,D + Dd,T1,RF,Enc,Str) end,
    {dots,3,3,More};
print_length_tuple1(Tuple,I,D,T,RF,Enc,Str) ->
    E = element(I,Tuple),
    T1 = tsub(T,1),
...
print_length_record(Tuple,1,_T,RF,RDefs,Enc,Str) ->
    More = fun (T1,Dd)->
        print_length_record(Tuple,1 + Dd,T1,RF,RDefs,Enc,Str) end,
    {"{...}",5,3,More};
...
print_length_fields(Term,D,T,Tuple,I,RF,Enc,Str)
    ...
    T =:= 0->
    More = fun (T1,Dd)->
        print_length_fields(Term,D + Dd,T1,Tuple,I,RF,Enc,Str) end,
    {dots,3,3,More};
...
print_length_list1([],_D,_T,_RF,_Enc,_Str) ->
    [];
print_length_list1(Term,D,T,RF,Enc,Str)
    when D =:= 1;
    T =:= 0->
    More = fun (T1,Dd)->
        print_length_list1(Term,D + Dd,T1,RF,Enc,Str) end,
    {dots,3,3,More};
...
printable_list(L,_D,T,_Unicode)
    when T >= 0->
    N = tsub(T,2),
...
printable_latin1_list([$\n| Cs],N) ->
    printable_latin1_list(Cs,N - 1);
printable_latin1_list([$\r| Cs],N) ->
    printable_latin1_list(Cs,N - 1);
printable_latin1_list([$\t| Cs],N) ->
    printable_latin1_list(Cs,N - 1);
printable_latin1_list([$\v| Cs],N) ->
    printable_latin1_list(Cs,N - 1);
printable_latin1_list([$\b| Cs],N) ->
    printable_latin1_list(Cs,N - 1);
printable_latin1_list([$\f| Cs],N) ->
    printable_latin1_list(Cs,N - 1);
printable_latin1_list([$| Cs],N) ->
    printable_latin1_list(Cs,N - 1);
printable_latin1_list([],_) ->
    all;
printable_latin1_list(_,N) ->
    N.
...
printable_unicode(<<C/utf8,R/binary>> = Bin,I,L,Range)
    when I > 0->
...
printable_char(C,latin1) ->
    C >= $  andalso C =< $~ orelse C >= 160 andalso C =< 255;
printable_char(C,unicode) ->
    C >= $  andalso C =< $~ orelse C >= 160 andalso C < 55296 orelse C > 57343 andalso C < 65534 orelse C > 65535 andalso C =< 1114111.
...
expand({_,_,_Dots = 0,no_more} = If,_T,_Dd) ->
    If;
...
cind({_S,Len,_,_},Col,Ll,M,Ind,LD,W)
    when Len < Ll - Col - LD,
    Len + W + LD =< M->
    Ind;
...
cind_tag_tuple([{_Tag,Tlen,_,_}| L],Col,Ll,M,Ind,LD,W) ->
    ...
    TagInd > Ind ->
        ...
        if M + Col1 =< Ll;
        Col1 =< Ll div 2 ->
            cind_tail(L,Col1,Tcol,Ll,M,Ind,LD,W + Tlen);true ->
            throw(no_good) end;M + Tcol < Ll;
    Tcol < Ll div 2 ->
        cind_list(L,Tcol,Ll,M,Ind,LD,W + Tlen + 1);true ->
        throw(no_good) end.
...
cind_pairs_tail([{_,Len,_,_} = P| Ps],Col0,Col,Ll,M,Ind,LD,W) ->
    ...
    W + ELen + LD1 =< M,
    is_list(element(1,element(2,element(1,P)))) andalso is_list(element(1,element(3,element(1,P)))) ->
        cind_pairs_tail(Ps,Col0,Col + ELen,Ll,M,Ind,LD,W + ELen);true ->
        PW = cind_pair(P,Col0,Ll,M,Ind,LD1,0),
        cind_pairs_tail(Ps,Col0,Col0 + PW,Ll,M,Ind,LD,PW) end;
...
cind_pair({{map_pair,_Key,_Value},Len,_,_} = Pair,Col,Ll,M,_Ind,LD,W)
    when Len < Ll - Col - LD,
    Len + W + LD =< M->
    if is_list(element(1,element(2,element(1,Pair)))) andalso is_list(element(1,element(3,element(1,Pair)))) ->
        Len;true ->
        Ll end;
...
cind_fields_tail([{_,Len,_,_} = F| Fs],Col0,Col,Ll,M,Ind,LD,W) ->
    ...
    W + ELen + LD1 =< M,
    is_list(element(1,element(4,element(1,F)))) ->
        cind_fields_tail(Fs,Col0,Col + ELen,Ll,M,Ind,LD,W + ELen);true ->
        FW = cind_field(F,Col0,Ll,M,Ind,LD1,0),
        cind_fields_tail(Fs,Col0,Col + FW,Ll,M,Ind,LD,FW) end;
...
cind_field({{field,_N,_NL,_F},Len,_,_} = Fl,Col,Ll,M,_Ind,LD,W)
    when Len < Ll - Col - LD,
    Len + W + LD =< M->
    if is_list(element(1,element(4,element(1,Fl)))) ->
        Len;true ->
        Ll end;
...
cind_tail([{_,Len,_,_} = E| Es],Col0,Col,Ll,M,Ind,LD,W) ->
    ...
    W + ELen + LD1 =< M,
    is_list(element(1,E)) ->
        cind_tail(Es,Col0,Col + ELen,Ll,M,Ind,LD,W + ELen);true ->
        WE = cind_element(E,Col0,Ll,M,Ind,LD1,0),
        cind_tail(Es,Col0,Col0 + WE,Ll,M,Ind,LD,WE) end;
