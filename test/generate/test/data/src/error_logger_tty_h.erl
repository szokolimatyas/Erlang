...
-record(st, {user,prev_handler,io_mod = io,depth = unlimited,modifier = ""}).

init({[],{error_logger,Buf}}) ->
    User = set_group_leader(),
    Depth = error_logger:get_format_depth(),
    Modifier = modifier(),
    State = #st{user = User,prev_handler = error_logger,depth = Depth,modifier = Modifier},
...
handle_event({_Type,GL,_Msg},State)
    when node(GL) =/= node()->
    {ok,State};
...
handle_info({'EXIT',User,_Reason},#st{user = User,prev_handler = PrevHandler} = State) ->
    case PrevHandler of
        []->
            remove_handler;
        _->
...
header({{Y,Mo,D},{H,Mi,S}},Title,UTC,M) ->
    io_lib:format("~n=~" ++ M ++ "s==== ~p-~s-~p::~s:~s:~s ~s===~n",[Title, D, month(Mo), Y, t(H), t(Mi), t(S), UTC]).
