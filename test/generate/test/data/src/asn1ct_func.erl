-file("asn1ct_func.erl", 1).

-module(asn1ct_func).

-export([start_link/0, need/1, call/3, call_gen/3, call_gen/4, generate/1, is_used/1]).
...
call_gen(Prefix,Key,Gen,Args)
    when is_function(Gen,2)->
    F = req({gen_func,Prefix,Key,Gen}),
    asn1ct_gen:emit([{asis,F}, "(", call_args(Args,""), ")"]).

call_gen(Prefix,Key,Gen)
    when is_function(Gen,2)->
...
req(Req) ->
    gen_server:call(get(asn1ct_func),Req,infinity).

cast(Req) ->
    gen_server:cast(get(asn1ct_func),Req).

-record(st, {used,gen,gc = 1}).
...
do_generate(Fd) ->
    case req(get_gen) of
        []->
            ok;
        [_| _] = Gen->
            _ = [begin ok = file:write(Fd,"\n"),
            GenFun(Fd,Name) end || {Name,GenFun} <- Gen],
            do_generate(Fd)
    end.