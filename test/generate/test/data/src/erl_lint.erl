...
-import(lists, [all/2, any/2, foldl/3, foldr/3, map/2, mapfoldl/3, member/2, reverse/1]).
...
bool_option(On,Off,Default,Opts) ->
    foldl(fun (Opt,_Def)
        ...
        when Opt =:= Off->
        false;(_Opt,Def)->
        Def end,Default,Opts).

value_option(Flag,Default,Opts) ->
    foldl(fun ({Opt,Val},_Def)
        when Opt =:= Flag->
        Val;(_Opt,Def)->
        Def end,Default,Opts).
...
-file("/usr/lib/erlang/lib/stdlib-3.14/include/erl_bits.hrl", 1).
...
-record(bittype,{type::bt_type()|undefined,unit::bt_unit()|undefined,sign::bt_sign()|undefined,endian::bt_endian()|undefined}).

-file("erl_lint.erl", 78).
...
format_error(disallowed_nowarn_bif_clash) ->
    io_lib:format("compile directive nowarn_bif_clash is no longer allo" "wed,~n - use explicit module names or -compile({no_a" "uto_import, [F/A]})",[]);
...
format_error(nif_inline) ->
    "inlining is enabled - local calls to NIFs may call their Erlang im" "plementation instead";
...
format_error({call_to_redefined_old_bif,{F,A}}) ->
    io_lib:format("ambiguous call of overridden pre R14 auto-imported B" "IF ~w/~w~n - use erlang:~w/~w or \"-compile({no_auto" "_import,[~w/~w]}).\" to resolve name clash",[F, A, F, A, F, A]);
...
format_error({redefine_bif_import,{F,A}}) ->
    io_lib:format("import directive overrides auto-imported BIF ~w/~w~n" " - use \"-compile({no_auto_import,[~w/~w]}).\" to re" "solve name clash",[F, A, F, A]);
format_error({deprecated,MFA,String,Rel}) ->
    io_lib:format("~s is deprecated and will be removed in ~s; use ~s",[format_mfa(MFA), Rel, String]);
format_error({deprecated,MFA,String})
    when is_list(String)->
    io_lib:format("~s is deprecated; ~s",[format_mfa(MFA), String]);
format_error({deprecated_type,{M1,F1,A1},String})
    when is_list(String)->
    io_lib:format("the type ~p:~p~s is deprecated; ~s",[M1, F1, gen_type_paren(A1), String]);
format_error({removed,MFA,ReplacementMFA,Rel}) ->
    io_lib:format("call to ~s will fail, since it was removed in ~s; us" "e ~s",[format_mfa(MFA), Rel, format_mfa(ReplacementMFA)]);
format_error({removed,MFA,String})
    when is_list(String)->
...
format_error({obsolete_guard_overridden,Test}) ->
    io_lib:format("obsolete ~s/1 (meaning is_~s/1) is illegal when ther" "e is a local/imported function named is_~p/1 ",[Test, Test, Test]);
...
format_error(bittype_unit) ->
    "a bit unit size must not be specified unless a size is specified t" "oo";
...
format_error(non_integer_bitsize) ->
    "a size expression in a pattern evaluates to a non-integer value; t" "his pattern cannot possibly match";
format_error(unsized_binary_not_at_end) ->
    "a binary field without size is only allowed at the end of a binary" " pattern";
format_error(typed_literal_string) ->
    "a literal string in a binary pattern must not have a type or a siz" "e";
format_error(utf_bittype_size_or_unit) ->
    "neither size nor unit must be given for segments of type utf8/utf1" "6/utf32";
format_error({bad_bitsize,Type}) ->
    io_lib:format("bad ~s bit size",[Type]);
format_error(unsized_binary_in_bin_gen_pattern) ->
    "binary fields without size are not allowed in patterns of bit stri" "ng generators";
format_error({conflicting_behaviours,{Name,Arity},B,FirstL,FirstB}) ->
    io_lib:format("conflicting behaviours - callback ~tw/~w required by" " both '~p' and '~p' ~s",[Name, Arity, B, FirstB, format_where(FirstL)]);
...
format_error({behaviour_info,{_M,F,A}}) ->
    io_lib:format("cannot define callback attibute for ~tw/~w when beha" "viour_info is defined",[F, A]);
...
format_error({new_builtin_type,{TypeName,Arity}}) ->
    io_lib:format("type ~w~s is a new builtin type; its (re)definition " "is allowed only until the next release",[TypeName, gen_type_paren(Arity)]);
format_error({builtin_type,{TypeName,Arity}}) ->
    io_lib:format("type ~w~s is a builtin type; it cannot be redefined",[TypeName, gen_type_paren(Arity)]);
format_error({renamed_type,OldName,NewName}) ->
    io_lib:format("type ~w() is now called ~w(); please use the new nam" "e instead",[OldName, NewName]);
...
format_error(old_abstract_code) ->
    io_lib:format("abstract code generated before Erlang/OTP 19.0 and h" "aving typed record fields cannot be compiled",[]);
...
format_error({deprecated_builtin_type,{Name,Arity},Replacement,Rel}) ->
    UseS = case Replacement of
        ...
        {Mod,NewName,NewArity}->
            io_lib:format("use ~w:~w/~w or preferably ~w:~w/~w",[Mod, NewName, Arity, Mod, NewName, NewArity])
    end,
...
format_error({underspecified_opaque,{TypeName,Arity}}) ->
    io_lib:format("opaque type ~tw~s is underspecified and therefore me" "aningless",[TypeName, gen_type_paren(Arity)]);
...
exprs_opt(Exprs,BindingsList,Opts) ->
    {St0,Vs} = foldl(fun ({{record,_SequenceNumber,_Name},Attr0},{St1,Vs1})->
        ...
        {attribute_state(Attr,St1),Vs1};({V,_},{St1,Vs1})->
        {St1,[{V,{bound,unused,[]}}| Vs1]} end,{start("nofile",Opts),[]},BindingsList),
    Vt = orddict:from_list(Vs),
    {_Evt,St} = exprs(set_file(Exprs,"nofile"),Vt,St0),
    return_status(St).

used_vars(Exprs,BindingsList) ->
    Vs = foldl(fun ({{record,_SequenceNumber,_Name},_Attr},Vs0)->
        Vs0;({V,_Val},Vs0)->
        [{V,{bound,unused,[]}}| Vs0] end,[],BindingsList),
    Vt = orddict:from_list(Vs),
    {Evt,_St} = exprs(set_file(Exprs,"nofile"),Vt,start()),
    {ok,foldl(fun ({V,{_,used,_}},L)->
        [V| L];(_,L)->
        L end,[],Evt)}.

-spec(module(AbsForms) -> {ok,Warnings}|{error,Errors,Warnings} when AbsForms::[erl_parse:abstract_form()|erl_parse:form_info()],Warnings::[{file:filename(),[ErrorInfo]}],Errors::[{FileName2::file:filename(),[ErrorInfo]}],ErrorInfo::error_info()).
...
-spec(module(AbsForms,FileName) -> {ok,Warnings}|{error,Errors,Warnings} when AbsForms::[erl_parse:abstract_form()|erl_parse:form_info()],FileName::atom()|string(),Warnings::[{file:filename(),[ErrorInfo]}],Errors::[{FileName2::file:filename(),[ErrorInfo]}],ErrorInfo::error_info()).
...
-spec(module(AbsForms,FileName,CompileOptions) -> {ok,Warnings}|{error,Errors,Warnings} when AbsForms::[erl_parse:abstract_form()|erl_parse:form_info()],FileName::atom()|string(),CompileOptions::[compile:option()],Warnings::[{file:filename(),[ErrorInfo]}],Errors::[{FileName2::file:filename(),[ErrorInfo]}],ErrorInfo::error_info()).
...
start(File,Opts) ->
    ...
    #lint{state = start,exports = gb_sets:from_list([{module_info,0}, {module_info,1}]),compile = Opts,defined = gb_sets:from_list(pseudolocals()),called = [{F,0} || F <- pseudolocals()],usage = #usage{calls = Calls},warn_format = value_option(warn_format,1,warn_format,1,nowarn_format,0,Opts),enabled_warnings = Enabled,nowarn_bif_clash = nowarn_function(nowarn_bif_clash,Opts),file = File}.
...
pack_errors(Es) ->
    {Es1,_} = mapfoldl(fun ({File,E},I)->
        {{File,{I,E}},I - 1} end,-1,Es),
    map(fun ({File,EIs})->
        {File,map(fun ({_I,E})->
            E end,EIs)} end,pack_warnings(Es1)).

pack_warnings(Ws) ->
    [{File,lists:sort([W || {F,W} <- Ws,F =:= File])} || File <- lists:usort([F || {F,_} <- Ws])].

add_error(E,St) ->
    add_lint_error(E,St#lint.file,St).
...
add_warning(W,St) ->
    add_lint_warning(W,St#lint.file,St).
...
forms(Forms0,St0) ->
    ...
    St1 = includes_qlc_hrl(Forms,StDeprecated#lint{locals = Locals,no_auto = AutoImportSuppressed}),
...
pre_scan([{attribute,L,compile,C}| Fs],St) ->
    case is_warn_enabled(export_all,St) andalso member(export_all,lists:flatten([C])) of
        true->
...
includes_qlc_hrl(Forms,St) ->
    QH = [File || {attribute,_,file,{File,_line}} <- Forms,filename:basename(File) =:= "qlc.hrl"],
    St#lint{xqlc = QH =/= []}.
...
anno_set_file(T,File) ->
    F = fun (Anno)->
        erl_anno:set_file(File,Anno) end,
...
bif_clashes(Forms,#lint{nowarn_bif_clash = Nowarn} = St) ->
    Clashes0 = [{Name,Arity} || {function,_L,Name,Arity,_Cs} <- Forms,erl_internal:bif(Name,Arity)],
...
not_deprecated(Forms,#lint{compile = Opts} = St0) ->
    MFAsL = [{MFA,L} || {attribute,L,compile,Args} <- Forms,{nowarn_deprecated_function,MFAs0} <- lists:flatten([Args]),MFA <- lists:flatten([MFAs0])],
    ...
    St1 = foldl(fun ({M,L},St2)->
        check_module_name(M,L,St2) end,St0,ML),
    St1#lint{not_deprecated = ordsets:from_list(Nowarn)}.

not_removed(Forms,#lint{compile = Opts} = St0) ->
    MFAsL = [{MFA,L} || {attribute,L,compile,Args} <- Forms,{nowarn_removed,MFAs0} <- lists:flatten([Args]),MFA <- lists:flatten([MFAs0])],
    Nowarn = [MFA || {nowarn_removed,MFAs0} <- Opts,MFA <- lists:flatten([MFAs0])],
    St1 = foldl(fun ({{M,_F,_A},L},St2)->
        check_module_name(M,L,St2);({M,L},St2)->
        check_module_name(M,L,St2) end,St0,MFAsL),
    St1#lint{not_removed = gb_sets:from_list(Nowarn)}.

disallowed_compile_flags(Forms,St0) ->
    Errors0 = [{St0#lint.file,{L,erl_lint,disallowed_nowarn_bif_clash}} || {attribute,A,compile,nowarn_bif_clash} <- Forms,{_,L} <- [loc(A,St0)]],
    Errors1 = [{St0#lint.file,{L,erl_lint,disallowed_nowarn_bif_clash}} || {attribute,A,compile,{nowarn_bif_clash,{_,_}}} <- Forms,{_,L} <- [loc(A,St0)]],
    Disabled =  not is_warn_enabled(bif_clash,St0),
    Errors = if Disabled andalso Errors0 =:= [] ->
        [{St0#lint.file,{erl_lint,disallowed_nowarn_bif_clash}}| St0#lint.errors];Disabled ->
        Errors0 ++ Errors1 ++ St0#lint.errors;true ->
        Errors1 ++ St0#lint.errors end,
...
behaviour_check(Bs,St0) ->
    ...
    Exports = exports(St0),
    F = fun (Bfs,OBfs)->
        [B || B <- Bfs, not lists:member(B,OBfs) orelse gb_sets:is_member(B,Exports)] end,
...
behaviour_conflicting(AllBfs,St) ->
    ...
    R4 = sofs:family_specification(fun (S)->
        sofs:no_elements(S) > 1 end,R3),
...
check_deprecated(Forms,St0) ->
    ...
    #lint{module = Mod} = St0,
    Bad = [{E,L} || {attribute,L,deprecated,Depr} <- Forms,D <- lists:flatten([Depr]),E <- depr_cat(D,X,Mod)],
    foldl(fun ({E,L},St1)->
        add_error(L,E,St1) end,St0,Bad).
...
check_removed(Forms,St0) ->
    ...
    X = ignore_predefined_funcs(gb_sets:to_list(Exports)),
    #lint{module = Mod} = St0,
    Bad = [{E,L} || {attribute,L,removed,Removed} <- Forms,R <- lists:flatten([Removed]),E <- removed_cat(R,X,Mod)],
    foldl(fun ({E,L},St1)->
        add_error(L,E,St1) end,St0,Bad).
...
check_imports(Forms,St0) ->
    case is_warn_enabled(unused_import,St0) of
        ...
        true->
            ...
            Imports = [{{FA,Mod},L} || {attribute,L,import,{Mod,Fs}} <- Forms,FA <- lists:usort(Fs)],
...
check_unused_functions(Forms,St0) ->
    ...
    case member(export_all,Opts) orelse  not is_warn_enabled(unused_function,St1) of
        true->
            St1;
        false->
...
check_undefined_functions(#lint{called = Called0,defined = Def0} = St0) ->
    ...
    foldl(fun ({NA,L},St)->
        add_error(L,{undefined_function,NA},St) end,St0,Undef).

check_undefined_types(#lint{usage = Usage,types = Def} = St0) ->
    Used = Usage#usage.used_types,
    UTAs = maps:keys(Used),
    Undef = [{TA,map_get(TA,Used)} || TA <- UTAs, not is_map_key(TA,Def), not is_default_type(TA)],
    foldl(fun ({TA,L},St)->
        add_error(L,{undefined_type,TA},St) end,St0,Undef).
...
check_option_functions(Forms,Tag0,Type,St0) ->
    FAsL = [{FA,L} || {attribute,L,compile,Args} <- Forms,{Tag,FAs0} <- lists:flatten([Args]),Tag0 =:= Tag,FA <- lists:flatten([FAs0])],
    DefFunctions = (gb_sets:to_list(St0#lint.defined) -- pseudolocals()) ++ [{F,A} || {{F,A},_} <- orddict:to_list(St0#lint.imports)],
...
func_line_warning(Type,Fs,St) ->
    foldl(fun ({F,Line},St0)->
        add_warning(Line,{Type,F},St0) end,St,Fs).

func_line_error(Type,Fs,St) ->
    foldl(fun ({F,Line},St0)->
        add_error(Line,{Type,F},St0) end,St,Fs).

check_untyped_records(Forms,St0) ->
    case is_warn_enabled(untyped_record,St0) of
        true->
            RecNames = maps:keys(St0#lint.records),
            TRecNames = [Name || {attribute,_,record,{Name,Fields}} <- Forms,lists:all(fun ({typed_record_field,_,_})->
                true;(_)->
                false end,Fields)],
            foldl(fun (N,St)->
                ...
                end end,St0,ordsets:subtract(ordsets:from_list(RecNames),ordsets:from_list(TRecNames)));
        false->
            St0
    end.
    ...
    case {is_warn_enabled(unused_record,St0),AttrFiles} of
        {true,[FirstFile| _]}->
            ...
            URecs = gb_sets:fold(fun (Used,Recs)->
                maps:remove(Used,Recs) end,St0#lint.records,UsedRecords),
            Unused = [{Name,FileLine} || {Name,{FileLine,_Fields}} <- maps:to_list(URecs),element(1,loc(FileLine,St0)) =:= FirstFile],
            foldl(fun ({N,L},St)->
                add_warning(L,{unused_record,N},St) end,St0,Unused);
        _->
            St0
    end.

check_callback_information(#lint{callbacks = Callbacks,optional_callbacks = OptionalCbs,defined = Defined} = St0) ->
            ...
            case map_size(Callbacks) of
                ...
                _->
                    FoldFun = fun (Fa,Line,St)->
                        add_error(Line,{behaviour_info,Fa},St) end,
...
export(Line,Es,#lint{exports = Es0,called = Called} = St0) ->
    {Es1,C1,St1} = foldl(fun (NA,{E,C,St2})->
        ...
        {gb_sets:add_element(NA,E),[{NA,Line}| C],St} end,{Es0,Called,St0},Es),
...
export_type(Line,ETs,#lint{usage = Usage,exp_types = ETs0} = St0) ->
    ...
    try foldl(fun ({T,A} = TA,{E,U,St2})
        ...
        {gb_sets:add_element(TA,E),maps:put(TA,Line,U),St} end,{ETs0,UTs0,St0},ETs) of 
        {ETs1,UTs1,St1}->
            St1#lint{usage = Usage#usage{used_types = UTs1},exp_types = ETs1}
        catch
            error:_->
                add_error(Line,{bad_export_type,ETs},St0) end.
...
import(Line,{Mod,Fs},St00) ->
    ...
    case check_imports(Line,Mfs,St#lint.imports) of
        ...
        Efs->
            {Err,St1} = foldl(fun ({bif,{F,A},_},{Err,St0})->
                Warn = is_warn_enabled(bif_clash,St0) andalso  not bif_clash_specifically_disabled(St0,{F,A}),
                AutoImpSup = is_autoimport_suppressed(St0#lint.no_auto,{F,A}),
                OldBif = erl_internal:old_bif(F,A),
                {Err,if Warn and  not AutoImpSup and OldBif ->
                    add_error(Line,{redefine_old_bif_import,{F,A}},St0);Warn and  not AutoImpSup ->
                    add_warning(Line,{redefine_bif_import,{F,A}},St0);true ->
                    St0 end};(Ef,{_Err,St0})->
                {true,add_error(Line,{redefine_import,Ef},St0)} end,{false,St},Efs),
            if  not Err ->
                St1#lint{imports = add_imports(Mod,Mfs,St#lint.imports)};true ->
                St1 end
    end.
...
add_imports(Mod,Fs,Is) ->
    foldl(fun (F,Is0)->
        orddict:store(F,Mod,Is0) end,Is,Fs).
...
call_function(Line,F,A,#lint{usage = Usage0,called = Cd,func = Func,file = File} = St) ->
    ...
    Usage = case Cs of
        undefined->
            Usage0;
        _->
            Usage0#usage{calls = maps_prepend(Func,NA,Cs)}
    end,
...
function_check_max_args(Line,Arity,St)
    when Arity > 255->
...
pattern({record_index,Line,Name,Field},_Vt,_Old,_Bvt,St) ->
    {Vt1,St1} = check_record(Line,Name,St,fun (Dfs,St1)->
        pattern_field(Field,Name,Dfs,St1) end),
    {Vt1,[],St1};
...
pattern_list(Ps,Vt,Old,Bvt0,St) ->
    foldl(fun (P,{Psvt,Bvt,St0})->
        ...
        {vtmerge_pat(Pvt,Psvt),vtmerge_pat(Bvt,Bvt1),St1} end,{[],[],St},Ps).

check_multi_field_init(Fs,Line,Fields,St) ->
    case has_wildcard_field(Fs) andalso init_fields(Fs,Line,Fields) =:= [] of
        true->
...
reject_invalid_alias({map,_Line,Ps1},{map,_,Ps2},Vt,St0) ->
    Fun = fun ({map_field_exact,L,{var,_,K},_V},Sti)->
        ...
        end;({map_field_exact,_L,_K,_V},Sti)->
        Sti end,
...
reject_invalid_alias({record,_,Name1,Pfs1},{record,_,Name2,Pfs2},Vt,#lint{records = Recs} = St) ->
    case Recs of
...
reject_invalid_alias_rec(PfsA0,PfsB0,FieldsA0,FieldsB0,Vt,St) ->
    ...
    C = sofs:join(A,2,B,2),
    D = sofs:projection({external,fun ({_,_,P1,_,P2})->
        {P1,P2} end},C),
...
pattern_map(Ps,Vt,Old,Bvt,St) ->
    foldl(fun ({map_field_assoc,L,_,_},{Psvt,Bvt0,St0})->
        {Psvt,Bvt0,add_error(L,illegal_pattern,St0)};({map_field_exact,_L,K,V},{Psvt,Bvt0,St0})->
        St1 = St0#lint{gexpr_context = map_key},
        {Kvt,St2} = gexpr(K,Vt,St1),
        {Vvt,Bvt2,St3} = pattern(V,Vt,Old,Bvt,St2),
        {vtmerge_pat(vtmerge_pat(Kvt,Vvt),Psvt),vtmerge_pat(Bvt0,Bvt2),St3} end,{[],[],St},Ps).

pattern_bin(Es,Vt,Old,Bvt0,St0) ->
    {_Sz,Esvt,Bvt,St1} = foldl(fun (E,Acc)->
        pattern_element(E,Vt,Old,Acc) end,{0,[],Bvt0,St0},Es),
    {Esvt,Bvt,St1}.

pattern_element({bin_element,Line,{string,_,_},Size,Ts} = Be,Vt,Old,{Sz,Esvt,Bvt,St0} = Acc) ->
...
expr_bin(Es,Vt,St0,Check) ->
    {_Sz,Esvt,St1} = foldl(fun (E,Acc)->
        bin_element(E,Vt,Acc,Check) end,{0,[],St0},Es),
    {Esvt,St1}.
...
elemtype_check(_Line,float,32,St) ->
    St;
elemtype_check(_Line,float,64,St) ->
    St;
elemtype_check(Line,float,_Size,St) ->
    add_warning(Line,{bad_bitsize,"float"},St);
elemtype_check(_Line,_Type,_Size,St) ->
    St.
...
gexpr({record_index,Line,Name,Field},_Vt,St) ->
    check_record(Line,Name,St,fun (Dfs,St1)->
        record_field(Field,Name,Dfs,St1) end);
gexpr({record_field,Line,Rec,Name,Field},Vt,St0) ->
    {Rvt,St1} = gexpr(Rec,Vt,St0),
    {Fvt,St2} = check_record(Line,Name,St1,fun (Dfs,St)->
        record_field(Field,Name,Dfs,St) end),
    {vtmerge(Rvt,Fvt),St2};
gexpr({record,Line,Name,Inits},Vt,St) ->
    check_record(Line,Name,St,fun (Dfs,St1)->
        ginit_fields(Inits,Line,Name,Dfs,Vt,St1) end);
...
gexpr({call,Line,{remote,_Lr,{atom,_Lm,erlang},{atom,Lf,is_record}},[E, A]},Vt,St0) ->
    gexpr({call,Line,{atom,Lf,is_record},[E, A]},Vt,St0);
gexpr({call,Line,{atom,_Lr,is_record},[E0, {atom,_,_Name}, {integer,_,_}]},Vt,St0) ->
...
gexpr({call,Line,{atom,_Lr,is_record},[_, _, _] = Asvt0},Vt,St0) ->
    ...
    {Asvt,add_error(Line,illegal_guard_expr,St1)};
gexpr({call,Line,{remote,_,{atom,_,erlang},{atom,_,is_record} = Isr},[_, _, _] = Args},Vt,St0) ->
...
gexpr({call,Line,{atom,_La,F},As},Vt,St0) ->
    ...
    case erl_internal:guard_bif(F,A) andalso no_guard_bif_clash(St1,{F,A}) of
        true->
            ...
            {Asvt,St1};
        false->
            case is_local_function(St1#lint.locals,{F,A}) orelse is_imported_function(St1#lint.imports,{F,A}) of
                true->
                    {Asvt,add_error(Line,{illegal_guard_local_call,{F,A}},St1)};
                _->
                    {Asvt,add_error(Line,illegal_guard_expr,St1)}
...
gexpr_list(Es,Vt,St) ->
    foldl(fun (E,{Esvt,St0})->
        ...
        {vtmerge(Evt,Esvt),St1} end,{[],St},Es).

-spec(is_guard_test(Expr) -> boolean() when Expr::erl_parse:abstract_expr()).

is_guard_test(E) ->
    is_guard_test2(E,{maps:new(),fun (_)->
        false end}).

is_guard_test(Expression,Forms) ->
    is_guard_test(Expression,Forms,fun (_)->
        false end).

-spec(is_guard_test(Expr,Forms,IsOverridden) -> boolean() when Expr::erl_parse:abstract_expr(),Forms::[erl_parse:abstract_form()|erl_parse:form_info()],IsOverridden::fun((fa()) -> boolean())).

is_guard_test(Expression,Forms,IsOverridden) ->
    RecordAttributes = [A || A = {attribute,_,record,_D} <- Forms],
    St0 = foldl(fun (Attr0,St1)->
        Attr = set_file(Attr0,"none"),
        attribute_state(Attr,St1) end,start(),RecordAttributes),
...
is_guard_test2({call,_Line,{atom,_La,Test},As} = Call,{_,IsOverridden} = Info) ->
    A = length(As),
     not IsOverridden({Test,A}) andalso case erl_internal:type_test(Test,A) of
        true->
...
is_gexpr({bin,_L,Fs},Info) ->
    all(fun ({bin_element,_Line,E,Sz,_Ts})->
        is_gexpr(E,Info) and (Sz =:= default orelse is_gexpr(Sz,Info)) end,Fs);
is_gexpr({call,_L,{atom,_Lf,F},As},{_,IsOverridden} = Info) ->
    A = length(As),
     not IsOverridden({F,A}) andalso erl_internal:guard_bif(F,A) andalso is_gexpr_list(As,Info);
is_gexpr({call,_L,{remote,_Lr,{atom,_Lm,erlang},{atom,_Lf,F}},As},Info) ->
    A = length(As),
    (erl_internal:guard_bif(F,A) orelse is_gexpr_op(F,A)) andalso is_gexpr_list(As,Info);
...
is_gexpr_list(Es,Info) ->
    all(fun (E)->
        is_gexpr(E,Info) end,Es).

is_map_fields([{Tag,_,K,V}| Fs],Info)
    when Tag =:= map_field_assoc;
    Tag =:= map_field_exact->
    is_gexpr(K,Info) andalso is_gexpr(V,Info) andalso is_map_fields(Fs,Info);
is_map_fields([],_Info) ->
    true;
...
is_gexpr_fields(Fs,L,Name,{RDs,_} = Info) ->
    ...
    all(fun ({record_field,_Lf,_Name,V})->
        is_gexpr(V,Info);(_Other)->
        false end,IFs).
...
expr({record,Line,Name,Inits},Vt,St) ->
    check_record(Line,Name,St,fun (Dfs,St1)->
        init_fields(Inits,Line,Name,Dfs,Vt,St1) end);
...
expr({record,Line,Rec,Name,Upds},Vt,St0) ->
    {Rvt,St1} = record_expr(Line,Rec,Vt,St0),
    {Usvt,St2} = check_record(Line,Name,St1,fun (Dfs,St)->
        update_fields(Upds,Name,Dfs,Vt,St) end),
...
expr({'fun',Line,Body},Vt,St) ->
    case Body of
        ...
        {function,F,A}->
            case  not is_local_function(St#lint.locals,{F,A}) andalso erl_internal:bif(F,A) andalso  not is_autoimport_suppressed(St#lint.no_auto,{F,A}) of
                true->
                    {[],St};
                false->
...
expr({call,_Line,{atom,_Lr,is_record},[E, {atom,Ln,Name}]},Vt,St0) ->
    ...
    {Rvt,exist_record(Ln,Name,St1)};
expr({call,Line,{remote,_Lr,{atom,_Lm,erlang},{atom,Lf,is_record}},[E, A]},Vt,St0) ->
...
expr({call,Line,{atom,La,F},As},Vt,St0) ->
    ...
    case  not IsLocal andalso Imported =:= no andalso IsAutoBif andalso  not AutoSuppressed of
        true->
            ...
            {Asvt,case Imported of
                ...
                no->
                    case {F,A} of
                        ...
                        N->
                            St3 = if  not AutoSuppressed andalso IsAutoBif andalso Warn ->
                                case erl_internal:old_bif(F,A) of
                                    true->
                                        add_error(Line,{call_to_redefined_old_bif,{F,A}},St2);
                                    false->
                            ...
                            if N =:= St3#lint.func ->
                                St3;true ->
                                call_function(Line,F,A,St3) end
                    end
            end}
    end;
...
expr_list(Es,Vt,St) ->
    foldl(fun (E,{Esvt,St0})->
        {Evt,St1} = expr(E,Vt,St0),
        {vtmerge_pat(Evt,Esvt),St1} end,{[],St},Es).
...
record_def(Line,Name,Fs0,St0) ->
    case is_map_key(Name,St0#lint.records) of
        ...
        false->
            ...
            St2 = St1#lint{records = maps:put(Name,{Line,Fs1},St1#lint.records)},
    ...
    foldl(fun ({record_field,Lf,{atom,La,F},V},{Fs,St})->
        case exist_field(F,Fs) of
            ...
            false->
                ...
                {_,St2} = expr(V,[],St1),
                St3 = St1#lint{warnings = St2#lint.warnings,errors = St2#lint.errors,called = St2#lint.called,recdef_top = false},
...
normalise_fields(Fs) ->
    map(fun ({record_field,Lf,Field})->
        {record_field,Lf,Field,{atom,Lf,undefined}};({typed_record_field,{record_field,Lf,Field},_Type})->
        {record_field,Lf,Field,{atom,Lf,undefined}};({typed_record_field,Field,_Type})->
        Field;(F)->
        F end,Fs).
...
check_fields(Fs,Name,Fields,Vt,St0,CheckFun) ->
    {_SeenFields,Uvt,St1} = foldl(fun (Field,{Sfsa,Vta,Sta})->
        {Sfsb,{Vtb,Stb}} = check_field(Field,Name,Fields,Vt,Sta,Sfsa,CheckFun),
        {Sfsb,vtmerge_pat(Vta,Vtb),Stb} end,{[],[],St0},Fs),
    {Uvt,St1}.
...
check_field({record_field,Lf,{atom,La,F},Val},Name,Fields,Vt,St,Sfs,CheckFun) ->
    case member(F,Sfs) of
        ...
        false->
            {[F| Sfs],case find_field(F,Fields) of
                {ok,_I}->
                    CheckFun(Val,Vt,St);
                error->
...
pattern_fields(Fs,Name,Fields,Vt0,Old,Bvt,St0) ->
    CheckFun = fun (Val,Vt,St)->
        pattern(Val,Vt,Old,Bvt,St) end,
    {_SeenFields,Uvt,Bvt1,St1} = foldl(fun (Field,{Sfsa,Vta,Bvt1,Sta})->
...
init_fields(Ifs,Line,Dfs) ->
    [{record_field,Lf,{atom,La,F},copy_expr(Di,Line)} || {record_field,Lf,{atom,La,F},Di} <- Dfs, not exist_field(F,Ifs)].

update_fields(Ufs,Name,Dfs,Vt,St) ->
    check_fields(Ufs,Name,Dfs,Vt,St,fun expr/3).

exist_field(F,[{record_field,_Lf,{atom,_La,F},_Val}| _Fs]) ->
    true;
exist_field(F,[_| Fs]) ->
    exist_field(F,Fs);
exist_field(_F,[]) ->
...
find_field(_F,[{record_field,_Lf,{atom,_La,_F},Val}| _Fs]) ->
    {ok,Val};
find_field(F,[_| Fs]) ->
    find_field(F,Fs);
find_field(_F,[]) ->
...
type_def(Attr,Line,TypeName,ProtoType,Args,St0) ->
    ...
    StoreType = fun (St)->
        ...
        check_type(CheckType,St#lint{types = NewDefs}) end,
    case is_default_type(TypePair) of
        ...
        false->
            case is_map_key(TypePair,TypeDefs) of
                ...
                false->
                    St1 = case Attr =:= opaque andalso is_underspecified(ProtoType,Arity) of
                        true->
...
is_underspecified({type,_,term,[]},0) ->
    true;
is_underspecified({type,_,any,[]},0) ->
    true;
is_underspecified(_ProtType,_Arity) ->
    false.
...
check_type(Types,St) ->
    ...
    maps:fold(fun (Var,{seen_once,Line},AccSt)->
        ...
        end;(_Var,seen_multiple,AccSt)->
        AccSt end,St1,SeenVars).
    ...
    case Mod =:= CurrentMod of
        ...
        false->
            lists:foldl(fun (T,{AccSeenVars,AccSt})->
                check_type(T,AccSeenVars,AccSt) end,{SeenVars,St},Args)
    end;
...
check_type({type,L,range,[From, To]},SeenVars,St) ->
    St1 = case {erl_eval:partial_eval(From),erl_eval:partial_eval(To)} of
        {{integer,_,X},{integer,_,Y}}
            when X < Y->
            St;
        _->
...
check_type({type,_L,map,Pairs},SeenVars,St) ->
    lists:foldl(fun (Pair,{AccSeenVars,AccSt})->
        check_type(Pair,AccSeenVars,AccSt) end,{SeenVars,St},Pairs);
...
check_type({type,L,binary,[Base, Unit]},SeenVars,St) ->
    St1 = case {erl_eval:partial_eval(Base),erl_eval:partial_eval(Unit)} of
...
check_type({type,_L,Tag,Args},SeenVars,St)
    when Tag =:= product;
    Tag =:= union;
    Tag =:= tuple->
    lists:foldl(fun (T,{AccSeenVars,AccSt})->
...
check_type({type,La,TypeName,Args},SeenVars,St) ->
    ...
    Arity = length(Args),
    TypePair = {TypeName,Arity},
    Obsolete = is_warn_enabled(deprecated_type,St) andalso obsolete_builtin_type(TypePair),
    St1 = case Obsolete of
        {deprecated,Repl,_}
            when element(1,Repl) =/= Module->
...
check_type({user_type,L,TypeName,Args},SeenVars,St) ->
    ...
    lists:foldl(fun (T,{AccSeenVars,AccSt})->
        check_type(T,AccSeenVars,AccSt) end,{SeenVars,St1},Args);
...
check_record_types(Line,Name,Fields,SeenVars,St) ->
    case maps:find(Name,St#lint.records) of
        {ok,{_L,DefFields}}->
            case lists:all(fun ({type,_,field_type,_})->
                true;(_)->
                false end,Fields) of
                true->
...
check_record_types([{type,_,field_type,[{atom,AL,FName}, Type]}| Left],Name,DefFields,SeenVars,St,SeenFields) ->
...
is_newly_introduced_builtin_type({Name,_})
    when is_atom(Name)->
    false.
...
check_specs([FunType| Left],ETag,Arity,St0) ->
    {FunType1,CTypes} = case FunType of
...
add_missing_spec_warnings(Forms,St0,Type) ->
    ...
    foldl(fun ({FA,L},St)->
        add_warning(L,{missing_spec,FA},St) end,St0,Warns).
...
check_unused_types_1(Forms,#lint{usage = Usage,types = Ts,exp_types = ExpTs} = St) ->
    case [File || {attribute,_L,file,{File,_Line}} <- Forms] of
        [FirstFile| _]->
            ...
            FoldFun = fun ({{record,_} = _Type,0},_,AccSt)->
                AccSt;(Type,#typeinfo{line = FileLine},AccSt)->
...
check_local_opaque_types(St) ->
    ...
    FoldFun = fun (_Type,#typeinfo{attr = type},AccSt)->
        AccSt;(Type,#typeinfo{attr = opaque,line = FileLine},AccSt)->
...
check_dialyzer_attribute(Forms,St0) ->
    Vals = [{L,V} || {attribute,L,dialyzer,Val} <- Forms,V0 <- lists:flatten([Val]),V <- case V0 of
        {O,F}->
            [{A,B} || A <- lists:flatten([O]),B <- lists:flatten([F])];
        T->
            [T]
    end],
    {Wellformed,Bad} = lists:partition(fun ({_,{Option,FA}})
        when is_atom(Option)->
        is_fa(FA);({_,Option})
        when is_atom(Option)->
        true;(_)->
    ...
    St1 = foldl(fun ({L,Term},St)->
        add_error(L,{bad_dialyzer_attribute,Term},St) end,St0,Bad),
...
is_module_dialyzer_option(Option) ->
    lists:member(Option,[no_return, no_unused, no_improper_lists, no_fun_app, no_match, no_opaque, no_fail_call, no_contracts, no_behaviours, no_undefined_callbacks, unmatched_returns, error_handling, race_conditions, no_missing_calls, specdiffs, overspecs, underspecs, unknown]).
...
icrt_clauses(Cs,Vt,St) ->
    mapfoldl(fun (C,St0)->
        icrt_clause(C,Vt,St0) end,St,Cs).
...
icrt_export([{V,{{export,_},_,_}}| Vs0],[{V,{{export,_} = S0,_,Ls}}| Vt],In,I,Acc) ->
    {VVs,Vs} = lists:partition(fun ({K,_})->
        K =:= V end,Vs0),
    S = foldl(fun ({_,{S1,_,_}},AccS)->
        merge_state(AccS,S1) end,S0,VVs),
    icrt_export(Vs,Vt,In,I,[{V,{S,used,Ls}}| Acc]);
icrt_export([{V,_}| Vs0],[{V,{_,_,Ls}}| Vt],In,I,Acc) ->
    Vs = lists:dropwhile(fun ({K,_})->
        K =:= V end,Vs0),
    icrt_export(Vs,Vt,In,I,[{V,{bound,used,Ls}}| Acc]);
icrt_export([{V1,_}| _] = Vs,[{V2,_}| Vt],In,I,Acc)
    when V1 > V2->
...
is_guard_test2_info(#lint{records = RDs,locals = Locals,imports = Imports}) ->
    {RDs,fun (FA)->
        is_local_function(Locals,FA) orelse is_imported_function(Imports,FA) end}.
...
handle_bitstring_gen_pat({bin,_,Segments = [_| _]},St) ->
    case lists:last(Segments) of
        {bin_element,Line,_,default,Flags}
            when is_list(Flags)->
            case member(binary,Flags) orelse member(bytes,Flags) orelse member(bits,Flags) orelse member(bitstring,Flags) of
                true->
...
fun_clauses(Cs,Vt,St) ->
    ...
    {Bvt,St2} = foldl(fun (C,{Bvt0,St0})->
        ...
        {vtmerge(Cvt,Bvt0),St1} end,{[],St#lint{recdef_top = false}},Cs),
    ...
    case orddict:find(V,Bvt) of
        ...
        error->
            case orddict:find(V,Vt) of
                ...
                error
                    when St#lint.recdef_top->
                    {[],[{V,{bound,unused,[Line]}}],add_error(Line,{variable_in_record_def,V},St)};
                error->
...
expr_var(V,Line,Vt0,#lint{bvt = Bvt0} = St0)
    when is_list(Bvt0)->
    ...
    case is_warn_enabled(shadow_vars,St0) of
        true->
            foldl(fun ({V,{_,_,[L| _]}},St)->
                add_warning(L,{shadowed_var,V,In},St);(_,St)->
                St end,St0,vtold(Vt,vt_no_unsafe(Vt0)));
        false->
            St0
    end.
    ...
    St1 = case is_warn_enabled(unused_vars,St0) of
        ...
        true->
            foldl(fun ({V,{_,unused,Ls}},St)->
                foldl(fun (L,St2)->
                    add_warning(L,{unused_var,V},St2) end,St,Ls) end,St0,U)
    end,
    UVt = map(fun ({V,{State,_,Ls}})->
        {V,{State,used,Ls}} end,U),
    {vtmerge(Vt,UVt),St1}.
...
vtupdate(Uvt,Vt0) ->
    orddict:merge(fun (_V,{S,U1,L1},{_S,U2,L2})->
        {S,merge_used(U1,U2),merge_lines(L1,L2)} end,Uvt,Vt0).
...
vtmerge(Vt1,Vt2) ->
    orddict:merge(fun (_V,{S1,U1,L1},{S2,U2,L2})->
        {merge_state(S1,S2),merge_used(U1,U2),merge_lines(L1,L2)} end,Vt1,Vt2).

vtmerge(Vts) ->
    foldl(fun (Vt,Mvts)->
        vtmerge(Vt,Mvts) end,[],Vts).

vtmerge_pat(Vt1,Vt2) ->
    orddict:merge(fun (_V,{S1,_Usage1,L1},{S2,_Usage2,L2})->
        {merge_state(S1,S2),used,merge_lines(L1,L2)} end,Vt1,Vt2).
...
vtnew(New,Old) ->
    orddict:filter(fun (V,_How)->
         not orddict:is_key(V,Old) end,New).
...
vtold(New,Old) ->
    orddict:filter(fun (V,_How)->
        orddict:is_key(V,Old) end,New).

vt_no_unsafe(Vt) ->
    [V || {_,{S,_U,_L}} = V <- Vt,case S of
        {unsafe,_}->
            false;
        _->
            true
...
deprecated_function(Line,M,F,As,St) ->
    ...
    case otp_internal:obsolete(M,F,Arity) of
        {deprecated,String}
            when is_list(String)->
            case  not is_warn_enabled(deprecated_function,St) orelse ordsets:is_element(MFA,St#lint.not_deprecated) of
                true->
                    St;
                false->
...
add_removed_warning(Line,{M,_,_} = MFA,Warning,#lint{not_removed = NotRemoved} = St) ->
    case is_warn_enabled(removed,St) andalso  not gb_sets:is_element(M,NotRemoved) andalso  not gb_sets:is_element(MFA,NotRemoved) of
        true->
...
canonicalize_string({string,Line,Cs}) ->
    foldr(fun (C,T)->
        {cons,Line,{integer,Line,C},T} end,{nil,Line},Cs);
...
check_format_2(Fmt,As) ->
    case Fmt of
        ...
        {atom,_L,A}->
            check_format_2a(atom_to_list(A),As);
        _->
...
no_guard_bif_clash(St,{F,A}) ->
     not is_local_function(St#lint.locals,{F,A}) andalso ( not is_imported_function(St#lint.imports,{F,A}) orelse is_imported_from_erlang(St#lint.imports,{F,A})) andalso ( not is_autoimport_suppressed(St#lint.no_auto,{F,A}) orelse is_imported_from_erlang(St#lint.imports,{F,A})).
