...
start_restricted(RShMod)
    ...
    case code:ensure_loaded(RShMod) of
        ...
        {error,What} = Error->
            error_logger:error_report(lists:flatten(io_lib:fwrite("Restr" "icted" " shel" "l mod" "ule ~" "w not" " foun" "d: ~t" "p\n",[RShMod, What]))),
            Error
    end.
...
server(StartSync) ->
    ...
    RShErr = case application:get_env(stdlib,restricted_shell) of
        {ok,RShMod}
            when is_atom(RShMod)->
    ...
    case RShErr of
        undefined->
            ok;
        {RShMod2,What2}->
            io:fwrite("Warning! Restricted shell module ~w not found: ~" "tp.\nOnly the commands q() and init:stop() will " "be allowed!\n",[RShMod2, What2]),
...
get_command(Prompt,Eval,Bs,RT,Ds) ->
    Parse = fun ()->
        exit(case io:scan_erl_exprs(group_leader(),Prompt,1,[text]) of
            {ok,Toks,_EndPos}->
...
get_command1(Pid,Eval,Bs,RT,Ds) ->
    ...
    {'EXIT',Eval,Reason}->
        ...
        get_command1(Pid,start_eval(Bs,RT,Ds),Bs,RT,Ds) end.
...
expand_expr({record_field,L,R,F},C) ->
...
expand_fields([{record_field,L,F,V}| Fs],C) ->
    [{record_field,L,expand_expr(F,C),expand_expr(V,C)}| expand_fields(Fs,C)];
expand_fields([],_C) ->
    [].
...
shell_rep(Ev,Bs0,RT,Ds0) ->
    receive {shell_rep,Ev,{value,V,Bs,Ds}}->
        {V,Ev,Bs,Ds};
...
eval_loop(Shell,Bs0,RT) ->
    receive {shell_cmd,Shell,{eval,Es},W}->
        Ef = {value,fun (MForFun,As)->
            apply_fun(MForFun,As,Shell) end},
        Lf = local_func_handler(Shell,RT,Ef),
        Bs = eval_exprs(Es,Shell,Bs0,RT,Lf,Ef,W),
        eval_loop(Shell,Bs,RT) end.
...
restricted_eval_loop(Shell,Bs0,RT,RShMod) ->
    receive {shell_cmd,Shell,{eval,Es},W}->
        ...
        restricted_eval_loop(Shell,Bs,RT,RShMod) end.

eval_exprs(Es,Shell,Bs0,RT,Lf,Ef,W) ->
    try {R,Bs2} = exprs(Es,Bs0,RT,Lf,Ef,W),
...
exprs([E0| Es],Bs1,RT,Lf,Ef,Bs0,W) ->
    ...
    case check_command(prep_check([E0]),RBs) of
        ok->
            ...
            Bs = orddict:from_list([VV || {X,_} = VV <- erl_eval:bindings(Bs2), not is_expand_variable(X)]),
            if Es =:= [] ->
                VS = pp(V0,1,RT),
                case W of
                    cmd->
                ...
                {{value,V,Bs,get()},Bs};true ->
                exprs(Es,Bs,RT,Lf,Ef,Bs0,W) end;
        {error,Error}->
...
used_records({record,_,R,Name,Ups}) ->
    {name,Name,[R| Ups]};
used_records({record_field,_,R,F}) ->
    {expr,[R| F]};
used_records({call,_,{atom,_,record},[A, {atom,_,Name}]}) ->
    {name,Name,A};
used_records({call,_,{atom,_,is_record},[A, {atom,_,Name}]}) ->
    {name,Name,A};
used_records({call,_,{remote,_,{atom,_,erlang},{atom,_,is_record}},[A, {atom,_,Name}]}) ->
    {name,Name,A};
...
restrict_handlers(RShMod,Shell,RT) ->
    {fun (F,As,Binds)->
        local_allowed(F,As,RShMod,Binds,Shell,RT) end,fun (MF,As)->
        non_local_allowed(MF,As,RShMod,Shell) end}.

local_allowed(F,As,RShMod,Bs,Shell,RT)
    when is_atom(F)->
    {LFH,NLFH} = restrict_handlers(RShMod,Shell,RT),
    case not_restricted(F,As) of
        ...
        false->
            ...
            case RShMod:local_allowed(F,AsEv,{get(restricted_shell_state),get(restricted_expr_state)}) of
                {Result,{RShShSt,RShExprSt}}->
                    ...
                    if  not Result ->
                        ...
                        exit({restricted_shell_disallowed,{F,AsEv}});true ->
                        non_builtin_local_func(F,AsEv,Bs1) end;
                Unexpected->
                    try error(reason)
                        catch
                            _:_:S->
                                erlang:raise(exit,{restricted_shell_bad_return,Unexpected},[{RShMod,local_allowed,3}| S]) end
            end
    end.

non_local_allowed(MForFun,As,RShMod,Shell) ->
    case RShMod:non_local_allowed(MForFun,As,{get(restricted_shell_state),get(restricted_expr_state)}) of
        {Result,{RShShSt,RShExprSt}}->
            put(restricted_shell_state,RShShSt),
            put(restricted_expr_state,RShExprSt),
            case Result of
        ...
        Unexpected->
            try error(reason)
                    ...
                    _:_:S->
                        erlang:raise(exit,{restricted_shell_bad_return,Unexpected},[{RShMod,non_local_allowed,3}| S]) end
    end.
...
prep_rec(T)
    when is_tuple(T)->
    list_to_tuple(prep_rec(tuple_to_list(T)));
prep_rec([E| Es]) ->
...
local_func(h,[],Bs,Shell,RT,_Lf,_Ef) ->
    ...
    Cs1 = lists:filter(fun ({{command,_},_})->
        true;({{result,_},_})->
        true;(_)->
        false end,Cs),
    Cs2 = lists:map(fun ({{T,N},V})->
        {{N,T},V} end,Cs1),
...
local_func(exit,[],_Bs,Shell,_RT,_Lf,_Ef) ->
    ...
    exit(normal);
local_func(F,As0,Bs0,_Shell,_RT,Lf,Ef)
    when is_atom(F)->
...
shell_undef(F,A) ->
    error({shell_undef,F,A,[]}).

local_func_handler(Shell,RT,Ef) ->
    H = fun (Lf)->
        fun (F,As,Bs)->
            local_func(F,As,Bs,Shell,RT,{eval,Lf(Lf)},Ef) end end,
    {eval,H(H)}.
...
read_records(File,Selected,Options) ->
    case read_records(File,listify(Options)) of
        ...
        RAs->
            ...
            [RA || {attribute,_,_,{Name,_}} = RA <- RAs,lists:member(Name,Sel)]
    end.
...
record_bindings(Recs0,Bs0) ->
    {Recs1,_} = lists:mapfoldl(fun ({Name,Def},I)->
        {{Name,I,Def},I + 1} end,0,Recs0),
...
list_commands([{{N,command},Es0}, {{N,result},V}| Ds],RT) ->
    ...
    I = iolist_size(Ns),
    io:requests([{put_chars,latin1,Ns}, {format,<<"~ts\n">>,[erl_pp:exprs(Es,I,enc())]}, {format,<<"-> ">>,[]}, {put_chars,unicode,VS}, nl]),
    list_commands(Ds,RT);
...
list_records(Records) ->
    lists:foreach(fun ({_Name,Attr})->
        io:fwrite(<<"~ts">>,[erl_pp:attribute(Attr,enc())]) end,Records).

record_defs(RT,Names) ->
    lists:flatmap(fun (Name)->
        ets:lookup(RT,Name) end,Names).

expand_value(E) ->
    substitute_v1(fun ({value,CommandN,V})->
        try_abstract(V,CommandN) end,E).
...
try_abstract(V,CommandN) ->
    try erl_parse:abstract(V)
            ...
            _:_->
                A = a0(),
                {call,A,{atom,A,v},[{integer,A,CommandN}]} end.

prep_list_commands(E) ->
    A = a0(),
    substitute_v1(fun ({value,Anno,_V})->
        CommandN = erl_anno:line(Anno),
        {call,A,{atom,A,v},[{integer,A,CommandN}]} end,E).
...
pp(V,I,D,RT) ->
    ...
    io_lib_pretty:print(V,[{column,I}, {line_length,columns()}, {depth,D}, {line_max_chars,60}, {strings,Strings}, {record_print_fun,record_print_fun(RT)}] ++ enc()).
...
garb(Shell) ->
    garbage_collect(Shell),
     catch garbage_collect(whereis(user)),
     catch garbage_collect(group_leader()),
    garbage_collect().
...
check_env(V) ->
    case application:get_env(stdlib,V) of
        ...
        {ok,Val}
            ...
            ok;
        {ok,Val}->
            Txt = io_lib:fwrite("Invalid value of STDLIB configuration pa" "rameter~tw: ~tp\n",[V, Val]),
