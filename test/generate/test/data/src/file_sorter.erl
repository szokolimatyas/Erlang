-file("file_sorter.erl", 1).

-module(file_sorter).

-compile({no_auto_import,[{error,2}]}).

-export([sort/1, sort/2, sort/3, keysort/2, keysort/3, keysort/4, merge/2, merge/3, keymerge/3, keymerge/4, check/1, check/2, keycheck/2, keycheck/3]).
...
-file("file_sorter.erl", 34).

-record(w, {keypos,runs = [[]],seq = 1,in,out,fun_out,prefix,temp = [],format,runsize,no_files,order,chunksize,wfd,ref,z,unique,hdlen,inout_value}).

-record(opts, {format = binary_term_fun(),size = 524288,no_files = 16,tmpdir = default,order = ascending,compressed = false,unique = false,header = 4}).
...
keysort(KeyPos,Input0,Output0,Options) ->
    R = case {is_keypos(KeyPos),is_input(Input0),maybe_output(Output0),options(Options)} of
        {_,_,_,#opts{format = binary}}->
            {Input0,Output0,[{badarg,format}]};
        {_,_,_,#opts{order = Order}}
...
keymerge(KeyPos,Files0,Output0,Options) ->
    R = case {is_keypos(KeyPos),is_files(Files0),maybe_output(Output0),options(Options)} of
        {_,_,_,#opts{format = binary}}->
...
options([{compressed,Bool}| L],Opts)
    when is_boolean(Bool)->
    options(L,Opts#opts{compressed = Bool});
options([{unique,Bool}| L],Opts)
    when is_boolean(Bool)->
...
do_sort(KeyPos0,Input0,Output0,Opts,Do) ->
    #opts{format = Format0,size = Size,no_files = NoFiles,tmpdir = TmpDir,order = Order,compressed = Compressed,unique = Unique,header = HdLen} = Opts,
    Prefix = tmp_prefix(Output0,TmpDir),
    ChunkSize = 16384,
    Ref = make_ref(),
    KeyPos = case KeyPos0 of
        [Kp]->
            Kp;
        _->
            KeyPos0
    ...
    W = #w{keypos = KeyPos,out = Output,fun_out = FunOut,prefix = Prefix,format = Format,runsize = Size,no_files = NoFiles,order = Order,chunksize = ChunkSize,ref = Ref,z = Z,unique = Unique,hdlen = HdLen,inout_value = no_value},
    try doit(Do,Input,W)
        catch
            throw:{Ref,Error}->
                Error end.
...
wrap_input(term,merge,Files) ->
    Fun = fun (File)->
        merge_terms_fun(file_rterms(no_file,[File])) end,
...
wrap_input(Format,merge,Files) ->
    ...
    {Format,Input};
wrap_input(term,sort,InFun)
    when is_function(InFun,1)->
    ...
    fun (close)->
        ...
        case RFun(read) of
            end_of_input->
                eof;
            {Objs,NRFun}
                when is_function(NRFun,1)->
                {_,[],Ts,_} = fun_objs(Objs,[],0,1 bsl 31,I,W),
...
wrap_output_terms(term,OutFun,_Z)
    when is_function(OutFun,1)->
    {fun_wterms(OutFun),true};
wrap_output_terms(term,File,Z)
    when File =/= undefined->
...
check_objs(IFun,F,FNs,W,L,I,Last,[{T,_BT}| Os])
    when T >= Last->
...
rcheck_objs(IFun,F,FNs,W,L,I,Last,[{T,_BT}| Os])
    when T =< Last->
...
ucheck_objs(IFun,F,FNs,W,L,I,LT,[{T,_BT}| Os])
    when T > LT->
...
rucheck_objs(IFun,F,FNs,W,L,I,LT,[{T,_BT}| Os])
    when T < LT->
...
file_run(InFun,FileNames,I,L,LSz,W)
    when LSz < W#w.runsize->
...
fun_run(I,L,LSz,#w{runsize = Runsize} = W,Objs)
    when LSz < Runsize->
...
internal_sort([] = L,_W) ->
    L;
internal_sort(L,#w{order = CFun,unique = Unique})
    when is_function(CFun)->
    Fun = fun ({T1,_},{T2,_})->
        CFun(T1,T2) end,
    RL = lists:reverse(L),
    lists:reverse(if Unique ->
        lists:usort(Fun,RL);true ->
        lists:sort(Fun,RL) end);
...
init_merge([],_I,Fs0,#w{order = Order} = W)
    when is_function(Order)->
...
merge_files([F1, F2| Fs],L0,LSz,Last0,W)
    when LSz < 16384->
    [Ts0| InEtc] = F1,
    Kind = merge_kind(W),
    ...
    {NInEtc,NFs,NL,NLast} = case Kind of
        umerge->
...
ukmerge_files(L,F2,Fs,InEtc2,Ts2,[{[T| _I],_BT}| Ts],InEtc,T2,Kp,Last)
    when T == Last->
    ukmerge_files(L,F2,Fs,InEtc2,Ts2,Ts,InEtc,T2,Kp,Last);
ukmerge_files(L,F2,Fs,InEtc2,Ts2,[{[T0| _I] = T,BT}| Ts],InEtc,T2,Kp,_Last)
    when T =< T2->
...
rukmerge_files(L,F2,Fs,InEtc2,Ts2,[{[T| _I],BT}| Ts],InEtc,T2,Kp,Last,_LastBT)
    when T == Last->
    rukmerge_files(L,F2,Fs,InEtc2,Ts2,Ts,InEtc,T2,Kp,T,BT);
rukmerge_files(L,F2,Fs,InEtc2,Ts2,[{[T0| _I] = T,BT}| Ts],InEtc,T2,Kp,_Last,LastBT)
    when T >= T2->
...
ucmerge_files(L,F2,Fs,InEtc2,Ts2,I2,[{T,BT}| Ts] = Ts0,I,InEtc,T2,CF,Last)
    when I < I2->
    case CF(T,T2) of
        true->
            case CF(T,Last) of
...
cmerge_files(L,F2,Fs,InEtc2,BT2,Ts2,I2,[{T,BT}| Ts] = Ts0,I,InEtc,T2,CF)
    when I < I2->
    case CF(T,T2) of
        true->
...
ulast_file([{T,_BT}| Ts],Last,L)
    when Last == T->
    last_file(Ts,L);
ulast_file(Ts,_Last,L) ->
    last_file(Ts,L).

kulast_file([{[T| _I],_BT}| Ts],Last,L)
    when Last == T->
    last_file(Ts,L);
kulast_file(Ts,_Last,L) ->
    last_file(Ts,L).

ruklast_file([{[T| _I],BT}| Ts],Last,_LastBT,L)
    when Last == T->
    last_file(Ts,[BT| L]);
...
rinsert(A,[X1, X2, X3, X4| Xs])
    when A < X4->
    [X1, X2, X3, X4| rinsert(A,Xs)];
rinsert(A,[X1, X2, X3| T])
    when A < X3->
    [X1, X2, X3, A| T];
rinsert(A,[X1, X2| Xs])
    when A < X2->
...
read_fun2(Fd,Bin,Size,FileName,Owner) ->
    fun (close)->
        close_read_fun(Fd,FileName,Owner);({I,L,LSz,W})->
...
read_objs(Fd,FileName,I,L,Bin0,Size0,LSz,W) ->
    Max = max(Size0,16384),
    BSz0 = byte_size(Bin0),
    Min = Size0 - BSz0 + W#w.hdlen,
    NoBytes = max(Min,Max),
    case read(Fd,FileName,NoBytes,W) of
        {ok,Bin}->
            BSz = byte_size(Bin),
            NLSz = LSz + BSz,
            case  catch file_loop(L,I,Bin0,Bin,Size0,BSz0,BSz,Min,W) of
                {'EXIT',_R}->
...
file_loop(L,I,B1,B2,Sz,_B1Sz,B2Sz,Min,W)
    when B2Sz > Min->
...
file_loop2(L,I,B,Sz,SzB,Kp,Fun,HdLen)
    when is_integer(I)->
...
fun_binloop([B| Bs],L,LSz,RunSize,HL)
    when LSz < RunSize->
    Size = byte_size(B),
...
fun_loop([B| Bs],L,LSz,RunSize,Fun,HL)
    when LSz < RunSize->
    Size = byte_size(B),
...
cleanup(W) ->
    ...
    W1 = close_input(W),
    F = fun (IFun)
        when is_function(IFun)->
        IFun(close);({Fd,FileName})->
        _ = file:close(Fd),
        _ = file:delete(FileName);(FileName)->
        _ = file:delete(FileName) end,
    lists:foreach(F,W1#w.temp).

close_input(#w{in = In} = W)
    when is_function(In)->
     catch In(close),
    W#w{in = undefined};
...
file_rterms({Fd,FileName},Files) ->
    fun (close)->
        file:close(Fd);(read)->
        file_rterms2(Fd,[],0,FileName,Files) end.
