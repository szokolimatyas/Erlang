-file("asn1ct_constructed_ber_bin_v2.erl", 1).
...
-file("asn1ct_constructed_ber_bin_v2.erl", 34).
...
gen_encode_sequence(Gen,Typename,#type{} = D) ->
    ...
    {SeqOrSet,TableConsInfo,CompList0} = case D#type.def of
    ...
    EncObj = case TableConsInfo of
        #simpletableattributes{usedclassfield = Used,uniqueclassfield = Unique}
            when Used /= Unique->
            false;
        #simpletableattributes{objectsetname = ObjectSetRef,c_name = AttrN,c_index = N,usedclassfield = UniqueFieldName,uniqueclassfield = UniqueFieldName,valueindex = ValueIndex}->
            ...
            case (OSDef#typedef.typespec)#'ObjectSet'.gen of
                true->
                    ...
                    emit([ObjectEncode, " = ", nl, "   ", {asis,ObjSetMod}, ":'getenc_", ObjSetName, "'("]),
    ...
    case SeqOrSet of
        'SET'
            when (D#type.def)#'SET'.sorted == dynamic->
...
enc_match_input(#gen{pack = map},ValName,CompList) ->
    ...
    M = [[{asis,Name}, ":=", Var] || {#'ComponentType'{prop = mandatory,name = Name},Var} <- Zipped],
    case M of
        []->
            ok;
        [_| _]->
    ...
    Os0 = [{Name,Var} || {#'ComponentType'{prop = Prop,name = Name},Var} <- Zipped,Prop =/= mandatory],
    F = fun ({Name,Var})->
...
gen_decode_sequence(Gen,Typename,#type{} = D) ->
    ...
    asn1ct_name:new(tlv),
    asn1ct_name:new(v),
    {DecObjInf,ValueIndex} = case TableConsInfo of
        #simpletableattributes{objectsetname = ObjectSetRef,c_name = AttrN,usedclassfield = UniqueFieldName,uniqueclassfield = UniqueFieldName,valueindex = ValIndex}->
        ...
        {LeadingAttrTerm,PostponedDecArgs}->
            ...
            case {LeadingAttrTerm,PostponedDecArgs} of
                ...
                {[{ObjSetRef,LeadingAttr,Term}],PostponedDecArgs}->
                    ...
                    emit([DecObj, " =", nl, "   ", {asis,ObjSetMod}, ":'getdec_", ObjSetName, "'(", ValueMatch, "),", nl]),
                    gen_dec_postponed_decs(DecObj,PostponedDecArgs)
            end,
            case Ext of
                {ext,_,_}->
                    emit(["case ", {prev,tlv}, " of [] -> true; _ -> true end, % ... extra f" "ields skipped", nl]);
                _->
                    emit(["case ", {prev,tlv}, " of", nl, "[] -> true;", "_ -> exit({error,{asn1, {unexpected,", {prev,tlv}, "}}}) % extra fields not allowed", nl, "end,", nl])
            end,
            asn1ct_name:new(rb),
...
dec_external(#gen{pack = record},RecordName) ->
    ...
    emit(["OldFormat={", lists:join(",",Record), "},", nl, {call,ext,transform_to_EXTERNAL1994,["OldFormat"]}]);
dec_external(#gen{pack = map},_RecordName) ->
    ...
    emit(["OldFormat = #{", MapInit, "}", com, nl, "ASN11994Format =", nl, {call,ext,transform_to_EXTERNAL1994_maps,["OldFormat"]}]).
...
gen_dec_postponed_decs(DecObj,[{_Cname,{FirstPFN,PFNList},Term,TmpTerm,_Tag,OptOrMand}| Rest]) ->
    ...
    emit([indent(N + 3), "case (catch ", DecObj, "(", {asis,FirstPFN}, ", ", TmpTerm, ", ", {asis,PFNList}, ")) of", nl]),
    emit([indent(N + 6), "{'EXIT', ", {curr,reason}, "} ->", nl]),
    emit([indent(N + 9), "exit({'Type not compatible with table constraint',", {curr,reason}, "});", nl]),
...
emit_opt_or_mand_check(Value,TmpTerm) ->
    emit([indent(3), "case ", TmpTerm, " of", nl, indent(6), {asis,Value}, " ->", {asis,Value}, ";", nl, indent(6), "_ ->", nl]).
...
gen_decode_set(Gen,Typename,#type{} = D) ->
    ...
    case CompList of
        ...
        _->
            ...
            emit([indent(6), {curr,else}, " -> ", nl, indent(9), "{", NextNum, ", ", {curr,else}, "}", nl]),
...
gen_encode_sof(Erules,Typename,_InnerTypename,D)
    when is_record(D,type)->
    asn1ct_name:start(),
    ...
    emit(["   {EncBytes,EncLen} = 'enc_", asn1ct_gen:list2name(Typename), "_components'(Val", Objfun, ",[],0),", nl]),
    emit(["   ", {call,ber,encode_tags,["TagIn", "EncBytes", "EncLen"]}, ".", nl, nl]),
    gen_encode_sof_components(Erules,Typename,SeqOrSetOf,Cont).

gen_decode_sof(Erules,TypeName,_InnerTypeName,D)
    when is_record(D,type)->
    asn1ct_name:start(),
    {SeqOrSetOf,_TypeTag,Cont} = case D#type.def of
        {'SET OF',_Cont}->
            {'SET OF','SET',_Cont};
        {'SEQUENCE OF',_Cont}->
...
gen_encode_sof_components(Gen,Typename,SeqOrSetOf,#type{} = Cont) ->
    {Objfun,Objfun_novar,EncObj} = case Cont#type.tablecinf of
        [{objfun,_}| _R]->
            {", ObjFun",", _",{no_attr,"ObjFun"}};
        _->
            {"","",false}
    end,
    emit(["'enc_", asn1ct_gen:list2name(Typename), "_components'([]", Objfun_novar, ", AccBytes, AccLen) -> ", nl]),
    ...
    emit(["'enc_", asn1ct_gen:list2name(Typename), "_components'([H|T]", Objfun, ",AccBytes, AccLen) ->", nl]),
    TypeNameSuffix = asn1ct_gen:constructed_suffix(SeqOrSetOf,Cont#type.def),
...
gen_enc_sequence_call(Erules,TopType,[#'ComponentType'{name = Cname,typespec = Type,prop = Prop,textual_order = Order}| Rest],Pos,Ext,EncObj) ->
    ...
    CindexPos = case Order of
        undefined->
            Pos;
        _->
            Order
    end,
    Element = case TopType of
        ['EXTERNAL']->
...
gen_dec_sequence_call1(Erules,TopType,[#'ComponentType'{name = Cname,typespec = Type,prop = Prop,tags = Tags}| Rest],Num,Ext,DecObjInf,LeadingAttrAcc,ArgsAcc) ->
    {LA,PostponedDec} = gen_dec_component(Erules,TopType,Cname,Tags,Type,Num,Prop,Ext,DecObjInf),
    emit([com, nl]),
    case Rest of
        []->
        ...
        _->
            asn1ct_name:new(bytes),
            gen_dec_sequence_call1(Erules,TopType,Rest,Num + 1,Ext,DecObjInf,LA ++ LeadingAttrAcc,PostponedDec ++ ArgsAcc)
    end;
...
gen_dec_sequence_call2(Erules,TopType,{Root1,EList,Root2},_Ext,DecObjInf) ->
    {LA,ArgsAcc} = case gen_dec_sequence_call1(Erules,TopType,Root1 ++ EList,1,extensible({Root1,EList}),DecObjInf,[],[]) of
        no_terms->
            {[],[]};
        Res->
            Res
    ...
    emit([{curr,tlv}, " = ", {call,ber,skip_ExtensionAdditions,[{prev,tlv}, {asis,TagList}]}, com, nl]),
    asn1ct_name:new(tlv),
    gen_dec_sequence_call1(Erules,TopType,Root2,length(Root1) + length(EList),noext,DecObjInf,LA,ArgsAcc).
...
get_root2_taglist([#'ComponentType'{prop = Prop,typespec = Type}| Rest],Acc) ->
    FirstTag = fun ([])->
        [];([H| _T])->
        asn1ct_gen_ber_bin_v2:decode_class(H#tag.class) bsl 10 + H#tag.number end(Type#type.tag),
    case Prop of
        mandatory->
...
gen_dec_component(Erules,TopType,Cname,CTags,Type,Pos,Prop,Ext,DecObjInf) ->
    InnerType = case Type#type.def of
...
emit_term_tlv(_,type_or_object_field,false) ->
    emit(["[", {curr,v}, "|", {curr,tlv}, "] = ", {prev,tlv}, ", ", nl, {curr,term}, " = "]);
...
gen_dec_set_cases(Erules,TopType,[Comp| RestComps],Pos) ->
    ...
    Tags = case Type#type.tag of
        []->
            [(asn1ct_gen_ber_bin_v2:decode_class(T1class) bsl 10 + T1number) || {T1class,T1number} <- CTags];
        [FirstTag| _]->
            [asn1ct_gen_ber_bin_v2:decode_class(FirstTag#tag.class) bsl 10 + FirstTag#tag.number]
    end,
...
gen_enc_choice2(Erules,TopType,[H1| T])
    when is_record(H1,'ComponentType')->
    Cname = H1#'ComponentType'.name,
    Type = H1#'ComponentType'.typespec,
    emit(["      ", {asis,Cname}, " ->", nl]),
    {Encobj,Assign} = case {Type#type.def,asn1ct_gen:get_constraint(Type#type.constraint,componentrelation)} of
    ...
    case T of
        []->
            emit([indent(6), "Else -> ", nl, indent(9), "exit({error,{asn1,{invalid_choice_type,Else}}})"]);
        _->
            true
    end,
...
gen_dec_choice(Erules,TopType,_ChTag,CompList,Ext) ->
    ...
    asn1ct_name:new(tlv),
    asn1ct_name:new(v),
    emit(["case (case ", {prev,tlv}, " of [Ctemp", {prev,tlv}, "] -> Ctemp", {prev,tlv}, "; _ -> ", {prev,tlv}, " end)", " of", nl]),
...
gen_dec_choice_cases(Erules,TopType,[H| T]) ->
    ...
    Fcases = fun ([{T1class,T1number}| Tail],Fun)->
        emit([indent(4), {curr,v}, " = {", asn1ct_gen_ber_bin_v2:decode_class(T1class) bsl 10 + T1number, ",_} -> ", nl]),
    ...
    case {Tags,asn1ct:get_gen_state_field(namelist)} of
        ...
        {[FirstT| _RestT],[{Cname,undecoded}| Names]}->
            DecTag = asn1ct_gen_ber_bin_v2:decode_class(FirstT#tag.class) bsl 10 + FirstT#tag.number,
            asn1ct:add_generated_refed_func({[Cname| TopType],undecoded,[DecTag],Type}),
            asn1ct:update_gen_state(namelist,Names),
            emit([indent(4), {curr,res}, " = ", match_tag(FirstT#tag.class,FirstT#tag.number), " -> ", nl]),
            emit([indent(8), "{", {asis,Cname}, ", {'", asn1ct_gen:list2name([Cname| TopType]), "',", {curr,res}, "}};", nl, nl]);
        {[FirstT| RestT],_}->
            emit([indent(4), "{", asn1ct_gen_ber_bin_v2:decode_class(FirstT#tag.class) bsl 10 + FirstT#tag.number, ", ", {curr,v}, "} -> ", nl]),
            emit([indent(8), "{", {asis,Cname}, ", "]),
...
match_tag(Class,TagNo)
    when is_integer(TagNo)->
...
mk_object_val(Val)
    ...
    integer_to_list(Val);
mk_object_val(Val) ->
    mk_object_val(Val bsr 7,[integer_to_list(Val band 127)]).

mk_object_val(0,Acc) ->
    Acc;
mk_object_val(Val,Acc) ->
    I = integer_to_list(Val band 127 bor 128),
    mk_object_val(Val bsr 7,[I, ","| Acc]).

gen_enc_line(Erules,TopType,Cname,Type = #type{constraint = C,def = #'ObjectClassFieldType'{type = {typefield,_}}},Element,Indent,OptOrMand = mandatory,EncObj)
    when is_list(Element)->
    case asn1ct_gen:get_constraint(C,componentrelation) of
        {componentrelation,_,_}->
            gen_enc_line(Erules,TopType,Cname,Type,Element,Indent,OptOrMand,["{", {curr,tmpBytes}, ",_} = "],EncObj);
        _->
            gen_enc_line(Erules,TopType,Cname,Type,Element,Indent,OptOrMand,["{", {curr,encBytes}, ",", {curr,encLen}, "} = "],EncObj)
    end;
...
gen_enc_line(Erules,TopType,Cname,Type,Element,Indent,OptOrMand,Assign,EncObj)
    ...
    Tag = lists:reverse([(asn1ct_gen_ber_bin_v2:encode_tag_val(asn1ct_gen_ber_bin_v2:decode_class(X#tag.class),X#tag.form,X#tag.number)) || X <- Type#type.tag]),
    InnerType = asn1ct_gen:get_inner(Type#type.def),
    WhatKind = asn1ct_gen:type(InnerType),
    emit(IndDeep),
    emit(Assign),
    gen_optormand_case(OptOrMand,Erules,TopType,Cname,Type,Element),
    case {Type,asn1ct_gen:get_constraint(Type#type.constraint,componentrelation)} of
        {#type{def = #'ObjectClassFieldType'{type = {typefield,_},fieldname = RefedFieldName}},{componentrelation,_,_}}->
            ...
            case OptOrMand of
                mandatory->
                    emit(["{", {curr,encBytes}, ",", {curr,encLen}, "} = ", {call,ber,encode_open_type,[{curr,tmpBytes}, {asis,Tag}]}, nl]);
                _->
                ...
                'ASN1_OPEN_TYPE'->
                    case Type#type.def of
                        #'ObjectClassFieldType'{}->
                            asn1ct_gen_ber_bin_v2:gen_encode_prim(ber,#type{def = 'ASN1_OPEN_TYPE'},{asis,Tag},Element);
                        _->
                            asn1ct_gen_ber_bin_v2:gen_encode_prim(ber,Type,{asis,Tag},Element)
                    end;
                _->
...
gen_optormand_case({'DEFAULT',DefaultValue},Gen,_TopType,_Cname,Type,Element) ->
    CurrMod = get(currmod),
    case Gen of
        ...
        #gen{erule = ber,der = false,pack = Pack}->
            ...
            emit([" case ", Element, " of", nl, Ind9, {asis,DefMarker}, " ->", nl, Ind9, indent(3), "{", empty_lb(Gen), ",0};", nl, Ind9, "_ when ", Element, " =:= "]),
            Dv = case DefaultValue of
                #'Externalvaluereference'{module = CurrMod,value = V}->
                    ["?", {asis,V}];
                _->
                    [{asis,DefaultValue}]
            end,
            emit(Dv ++ [" ->", nl, Ind9, indent(3), "{", empty_lb(Gen), ",0};", nl, Ind9, "_ ->", nl, indent(12)])
    end.
...
gen_dec_line(Erules,TopType,Cname,CTags,Type,OptOrMand,DecObjInf) ->
    ...
    Tag = [(asn1ct_gen_ber_bin_v2:decode_class(X#tag.class) bsl 10 + X#tag.number) || X <- Type#type.tag],
    ChoiceTags = [(asn1ct_gen_ber_bin_v2:decode_class(Class) bsl 10 + Number) || {Class,Number} <- CTags],
    InnerType = case Type#type.def of
        #'ObjectClassFieldType'{type = OCFTType}->
            OCFTType;
        _->
    ...
    PostpDec = case OptOrMand of
        mandatory->
            gen_dec_call(InnerType,Erules,TopType,Cname,Type,BytesVar,Tag,mandatory,", mandatory, ",DecObjInf,OptOrMand);
        _->
            {FirstTag,RestTag} = case Tag of
                []->
                    {ChoiceTags,[]};
                [Ft| Rt]->
                    {Ft,Rt}
            ...
            PostponedDec = case Tag of
                []
                    when length(ChoiceTags) > 0->
                    Fcases = fun (FirstTag1)->
                        emit(["[", {curr,v}, " = {", {asis,FirstTag1}, ",_}|Temp", {curr,tlv}, "] ->", nl]),
                        emit([indent(4), "{"]),
                        Pdec = gen_dec_call(InnerType,Erules,TopType,Cname,Type,BytesVar,RestTag,mandatory,", mandatory, ",DecObjInf,OptOrMand),
                        emit([", Temp", {curr,tlv}, "}"]),
                        emit([";", nl]),
                        Pdec end,
                ...
                []->
                    ...
                    Pdec;
                _->
                    emit(["[{", {asis,FirstTag}, ",", {curr,v}, "}|Temp", {curr,tlv}, "] ->", nl]),
                    emit([indent(4), "{"]),
                    Pdec = gen_dec_call(InnerType,Erules,TopType,Cname,Type,BytesVar,RestTag,mandatory,", mandatory, ",DecObjInf,OptOrMand),
                    emit([", Temp", {curr,tlv}, "}"]),
                    emit([";", nl]),
...
gen_dec_call({typefield,_},_,_,_Cname,Type,BytesVar,Tag,_,_,false,_) ->
    ...
    asn1ct_name:new(tmpterm),
    asn1ct_name:new(tmptlv),
    {FirstPFName,RestPFName} = (Type#type.def)#'ObjectClassFieldType'.fieldname,
    emit([nl, indent(6), "begin", nl]),
    emit([indent(9), {curr,tmptlv}, " = ", {call,ber,decode_open_type,[BytesVar, {asis,Tag}]}, com, nl]),
    emit([indent(9), "case (catch ObjFun(", {asis,FirstPFName}, ", ", {curr,tmptlv}, ", ", {asis,RestPFName}, ")) of", nl]),
    emit([indent(12), "{'EXIT',", {curr,reason}, "} ->", nl]),
    emit([indent(15), "exit({'Type not ", "compatible with table constraint', ", {curr,reason}, "});", nl]),
...
gen_dec_call({typefield,_},_,_,Cname,Type,BytesVar,Tag,_,_,_DecObjInf,OptOrMandComp) ->
    ...
    [{Cname,RefedFieldName,asn1ct_gen:mk_var(asn1ct_name:curr(term)),asn1ct_gen:mk_var(asn1ct_name:curr(tmpterm)),Tag,OptOrMandComp}];
gen_dec_call(InnerType,Gen,TopType,Cname,Type,BytesVar,Tag,_PrimOptOrMand,_OptOrMand,DecObjInf,_) ->
    WhatKind = asn1ct_gen:type(InnerType),
    ...
    case DecObjInf of
        {Cname,{_,OSet,_UniqueFName,ValIndex}}->
            ...
            emit([",", nl, "ObjFun = ", {asis,ObjSetMod}, ":'getdec_", ObjSetName, "'(", ValueMatch, ")"]);
        _->
            ok
    end,
    [].

gen_dec_call1({primitive,bif},InnerType,TopType,Cname,Type,BytesVar,Tag) ->
...
gen_dec_call1('ASN1_OPEN_TYPE',_InnerType,TopType,Cname,Type,BytesVar,Tag) ->
    case {asn1ct:get_gen_state_field(namelist),Type#type.def} of
        ...
        {_,#'ObjectClassFieldType'{type = OpenType}}->
            asn1ct_gen_ber_bin_v2:gen_dec_prim(#type{def = OpenType},BytesVar,Tag);
        _->
            asn1ct_gen_ber_bin_v2:gen_dec_prim(Type,BytesVar,Tag)
    end;
...
print_attribute_comment(InnerType,Pos,Cname,Prop) ->
    ...
    case InnerType of
        ...
        _
            when is_tuple(InnerType)->
            emit([nl, "%% attribute ", Cname, "(", Pos, ") with type "| tuple_to_list(InnerType)]);
        _->
...
mkfuncname(TopType,Cname,WhatKind,Prefix,Suffix) ->
    ...
    case WhatKind of
        ...
        #'Externaltypereference'{module = Mod,type = EType}->
            {lists:concat(["'", Mod, "':'", Prefix, EType, Suffix, "'"]),Mod,lists:concat(["'", Prefix, EType, "'"])};
        {constructed,bif}->
