...
-record(exprec, {compile = [],vcount = 0,calltype = #{},records = #{},strict_ra = [],checked_ra = []}).

-spec(module(AbsForms,CompileOptions) -> AbsForms2 when AbsForms::[erl_parse:abstract_form()],AbsForms2::[erl_parse:abstract_form()],CompileOptions::[compile:option()]).
...
init_calltype_imports([{attribute,_,import,{Mod,Fs}}| T],Ctype0) ->
    ...
    Ctype = foldl(fun (FA,Acc)->
        Acc#{FA=>{imported,Mod}} end,Ctype0,Fs),
...
guard_test(G0,St0) ->
    in_guard(fun ()->
        ...
        strict_record_access(G1,St1) end).
...
normalise_test(atom,1) ->
    is_atom;
normalise_test(binary,1) ->
    is_binary;
normalise_test(float,1) ->
    is_float;
normalise_test(function,1) ->
    is_function;
normalise_test(integer,1) ->
    is_integer;
normalise_test(list,1) ->
    is_list;
normalise_test(number,1) ->
    is_number;
normalise_test(pid,1) ->
    is_pid;
normalise_test(port,1) ->
    is_port;
normalise_test(record,2) ->
    is_record;
normalise_test(reference,1) ->
    is_reference;
normalise_test(tuple,1) ->
    is_tuple;
normalise_test(Name,_) ->
    Name.
...
record_test_in_guard(Line,Term,Name,St) ->
    case not_a_tuple(Term) of
        ...
        false->
            ...
            expr({call,NLine,{remote,NLine,{atom,NLine,erlang},{atom,NLine,is_record}},[Term, {atom,Line,Name}, {integer,Line,length(Fs) + 1}]},St)
    end.

not_a_tuple({atom,_,_}) ->
    true;
not_a_tuple({integer,_,_}) ->
    true;
not_a_tuple({float,_,_}) ->
    true;
not_a_tuple({nil,_}) ->
    true;
not_a_tuple({cons,_,_,_}) ->
    true;
not_a_tuple({char,_,_}) ->
    true;
not_a_tuple({string,_,_}) ->
    true;
not_a_tuple({record_index,_,_,_}) ->
    true;
not_a_tuple({bin,_,_}) ->
    true;
not_a_tuple({op,_,_,_}) ->
    true;
not_a_tuple({op,_,_,_,_}) ->
    true;
not_a_tuple(_) ->
    false.
...
record_test_in_body(Line,Expr,Name,St0) ->
    ...
    NLine = no_compiler_warning(Line),
    expr({block,Line,[{match,Line,Var,Expr}, {call,NLine,{remote,NLine,{atom,NLine,erlang},{atom,NLine,is_record}},[Var, {atom,Line,Name}, {integer,Line,length(Fs) + 1}]}]},St).
...
expr({record,Line0,Name,Is},St) ->
    ...
    expr({tuple,Line,[{atom,Line0,Name}| record_inits(record_fields(Name,St),Is)]},St);
...
expr({call,Line,{remote,_,{atom,_,erlang},{atom,_,is_record}},[A, {atom,_,Name}]},St) ->
    record_test(Line,A,Name,St);
expr({call,Line,{tuple,_,[{atom,_,erlang}, {atom,_,is_record}]},[A, {atom,_,Name}]},St) ->
    record_test(Line,A,Name,St);
...
conj([{{Name,_Rp},L,R,Sz}| AL],E) ->
    ...
    T1 = {op,NL,'orelse',{call,NL,{remote,NL,{atom,NL,erlang},{atom,NL,is_record}},[R, {atom,NL,Name}, {integer,NL,Sz}]},{atom,NL,fail}},
...
record_fields(R,St) ->
    maps:get(R,St#exprec.records).

find_field(F,[{record_field,_,{atom,_,F},Val}| _]) ->
    {ok,Val};
find_field(F,[_| Fs]) ->
    find_field(F,Fs);
find_field(_,[]) ->
...
field_names(Fs) ->
    map(fun ({record_field,_,Field,_Val})->
        Field end,Fs).

index_expr(Line,{atom,_,F},_Name,Fs) ->
    {integer,Line,index_expr(F,Fs,2)}.

index_expr(F,[{record_field,_,{atom,_,F},_}| _],I) ->
    I;
index_expr(F,[_| Fs],I) ->
    index_expr(F,Fs,I + 1).
...
strict_get_record_field(Line,R,{atom,_,F} = Index,Name,St0) ->
    case is_in_guard() of
        ...
        true->
            ...
            {ExpR,St1} = expr(R,St0),
            A0 = erl_anno:new(0),
            ExpRp = erl_parse:map_anno(fun (_A)->
                A0 end,ExpR),
            RA = {{Name,ExpRp},Line,ExpR,length(Fs) + 1},
            St2 = St1#exprec{strict_ra = [RA| St1#exprec.strict_ra]},
            {{call,Line,{remote,Line,{atom,Line,erlang},{atom,Line,element}},[I, ExpR]},St2}
    end.
...
sloppy_get_record_field(Line,R,Index,Name,St) ->
    Fs = record_fields(Name,St),
    I = index_expr(Line,Index,Name,Fs),
    expr({call,Line,{remote,Line,{atom,Line,erlang},{atom,Line,element}},[I, R]},St).
...
record_update(R,Name,Fs,Us0,St0) ->
    ...
    {Update,St} = if Nu =:= 0 ->
        record_match(Var,Name,Line,Fs,Us,St2);Nu =< Nc,
     not StrictUpdates ->
        {record_setel(Var,Name,Fs,Us),St2};true ->
        record_match(Var,Name,element(2,hd(Us)),Fs,Us,St2) end,
...
record_setel(R,Name,Fs,Us0) ->
    Us1 = foldl(fun ({record_field,Lf,Field,Val},Acc)->
        ...
        [{FieldIndex,{I,Lf,Val}}| Acc] end,[],Us0),
    Us2 = sort(Us1),
    Us = [T || {_,T} <- Us2],
    Lr = element(2,hd(Us)),
...
pattern_bin(Es0,St) ->
    foldr(fun (E,Acc)->
        pattern_element(E,Acc) end,{[],St},Es0).
...
opt_rec_vars_2({call,_,{remote,_,{atom,_,erlang},{atom,_,is_record}},[{var,_,V}, {atom,_,Tag}, {integer,_,Sz}]},Rs) ->
    orddict:store(V,{Tag,Sz},Rs);
opt_rec_vars_2({call,_,{atom,_,is_record},[{var,_,V}, {atom,_,Tag}, {integer,_,Sz}]},Rs) ->
    orddict:store(V,{Tag,Sz},Rs);
opt_rec_vars_2(_,Rs) ->
    Rs.
...
opt_remove_2({call,Line,{remote,_,{atom,_,erlang},{atom,_,is_record}},[{var,_,V}, {atom,_,Tag}, {integer,_,Sz}]} = A,Rs) ->
...
opt_remove_2({call,Line,{atom,_,is_record},[{var,_,V}, {atom,_,Tag}, {integer,_,Sz}]} = A,Rs) ->
    case orddict:find(V,Rs) of
        {ok,{remove,Tag,Sz}}->
            {atom,Line,true};
        _->
