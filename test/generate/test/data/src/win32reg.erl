...
-export([open/1, close/1, current_key/1, change_key/2, change_key_create/2, sub_keys/1, delete_key/1, value/2, values/1, set_value/3, delete_value/2, expand/1, format_error/1]).
...
change_key_create({win32reg,Reg},Key)
    when is_port(Reg)->
...
set_value({win32reg,Reg},Name0,Value)
    when is_port(Reg)->
    Name = case Name0 of
        default->
            [];
...
delete_value({win32reg,Reg},Name0)
    when is_port(Reg)->
    Name = case Name0 of
        default->
            [];
...
-spec(collect_values(port(),[{name(),value()}]) -> {ok,[{name(),value()}]}|{error,ErrorId::atom()}).
...
i32(Int)
    when is_integer(Int)->
    [(Int bsr 24) band 255, (Int bsr 16) band 255, (Int bsr 8) band 255, Int band 255];
i32([X1, X2, X3, X4]) ->
...
parse_root([$\\| Rest],Result) ->
    Root = case lists:reverse(Result) of
        [$h, $k, $e, $y, $_| Root0]->
            Root0;
        Root0->
            Root0
