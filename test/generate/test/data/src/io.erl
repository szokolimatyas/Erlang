...
-export([put_chars/1, put_chars/2, nl/0, nl/1, get_chars/2, get_chars/3, get_line/1, get_line/2, get_password/0, get_password/1, setopts/1, setopts/2, getopts/0, getopts/1]).

-export([write/1, write/2, read/1, read/2, read/3, read/4]).

-export([columns/0, columns/1, rows/0, rows/1]).

-export([fwrite/1, fwrite/2, fwrite/3, fread/2, fread/3, format/1, format/2, format/3]).

-export([scan_erl_exprs/1, scan_erl_exprs/2, scan_erl_exprs/3, scan_erl_exprs/4, scan_erl_form/1, scan_erl_form/2, scan_erl_form/3, scan_erl_form/4, parse_erl_exprs/1, parse_erl_exprs/2, parse_erl_exprs/3, parse_erl_exprs/4, parse_erl_form/1, parse_erl_form/2, parse_erl_form/3, parse_erl_form/4]).
...
o_request(Io,Request,Func) ->
    case request(Io,Request) of
        {error,Reason}->
            ...
            {'EXIT',{get_stacktrace,[_Current| Mfas]}} = ( catch error(get_stacktrace)),
...
-spec(put_chars(IoDevice,Encoding,CharData) -> ok when IoDevice::device(),Encoding::unicode,CharData::unicode:chardata()).
...
-type(opt_pair()::{binary,boolean()}|{echo,boolean()}|{expand_fun,expand_fun()}|{encoding,encoding()}).

-spec(getopts() -> [opt_pair()]|{error,Reason} when Reason::term()).

getopts() ->
    getopts(default_input()).

-spec(getopts(IoDevice) -> [opt_pair()]|{error,Reason} when IoDevice::device(),Reason::term()).
...
-spec(read(IoDevice,Prompt,StartLocation) -> Result when IoDevice::device(),Prompt::prompt(),StartLocation::location(),Result::{ok,Term::term(),EndLocation::location()}|{eof,EndLocation::location()}|server_no_data()|{error,ErrorInfo,ErrorLocation::location()},ErrorInfo::erl_scan:error_info()|erl_parse:error_info()).

read(Io,Prompt,Pos0) ->
    read(Io,Prompt,Pos0,[]).

-spec(read(IoDevice,Prompt,StartLocation,Options) -> Result when IoDevice::device(),Prompt::prompt(),StartLocation::location(),Options::erl_scan:options(),Result::{ok,Term::term(),EndLocation::location()}|{eof,EndLocation::location()}|server_no_data()|{error,ErrorInfo,ErrorLocation::location()},ErrorInfo::erl_scan:error_info()|erl_parse:error_info()).
...
conv_reason(_,arguments) ->
    badarg;
conv_reason(_,terminated) ->
    terminated;
conv_reason(_,{no_translation,_,_}) ->
    no_translation;
conv_reason(_,_Reason) ->
    badarg.
...
-spec(scan_erl_exprs(Device,Prompt,StartLocation) -> Result when Device::device(),Prompt::prompt(),StartLocation::location(),Result::erl_scan:tokens_result()|server_no_data()).

scan_erl_exprs(Io,Prompt,Pos0) ->
    scan_erl_exprs(Io,Prompt,Pos0,[]).

-spec(scan_erl_exprs(Device,Prompt,StartLocation,Options) -> Result when Device::device(),Prompt::prompt(),StartLocation::location(),Options::erl_scan:options(),Result::erl_scan:tokens_result()|server_no_data()).
...
-spec(scan_erl_form(IoDevice,Prompt,StartLocation) -> Result when IoDevice::device(),Prompt::prompt(),StartLocation::location(),Result::erl_scan:tokens_result()|server_no_data()).

scan_erl_form(Io,Prompt,Pos0) ->
    scan_erl_form(Io,Prompt,Pos0,[]).

-spec(scan_erl_form(IoDevice,Prompt,StartLocation,Options) -> Result when IoDevice::device(),Prompt::prompt(),StartLocation::location(),Options::erl_scan:options(),Result::erl_scan:tokens_result()|server_no_data()).

scan_erl_form(Io,Prompt,Pos0,Options) ->
    request(Io,{get_until,unicode,Prompt,erl_scan,tokens,[Pos0, Options]}).

-type(parse_ret()::{ok,ExprList::[erl_parse:abstract_expr()],EndLocation::location()}|{eof,EndLocation::location()}|{error,ErrorInfo::erl_scan:error_info()|erl_parse:error_info(),ErrorLocation::location()}|server_no_data()).
...
-spec(parse_erl_exprs(IoDevice,Prompt,StartLocation) -> Result when IoDevice::device(),Prompt::prompt(),StartLocation::location(),Result::parse_ret()).

parse_erl_exprs(Io,Prompt,Pos0) ->
    parse_erl_exprs(Io,Prompt,Pos0,[]).

-spec(parse_erl_exprs(IoDevice,Prompt,StartLocation,Options) -> Result when IoDevice::device(),Prompt::prompt(),StartLocation::location(),Options::erl_scan:options(),Result::parse_ret()).
...
-type(parse_form_ret()::{ok,AbsForm::erl_parse:abstract_form(),EndLocation::location()}|{eof,EndLocation::location()}|{error,ErrorInfo::erl_scan:error_info()|erl_parse:error_info(),ErrorLocation::location()}|server_no_data()).
...
-spec(parse_erl_form(IoDevice,Prompt,StartLocation) -> Result when IoDevice::device(),Prompt::prompt(),StartLocation::location(),Result::parse_form_ret()).

parse_erl_form(Io,Prompt,Pos0) ->
    parse_erl_form(Io,Prompt,Pos0,[]).

-spec(parse_erl_form(IoDevice,Prompt,StartLocation,Options) -> Result when IoDevice::device(),Prompt::prompt(),StartLocation::location(),Options::erl_scan:options(),Result::parse_form_ret()).
...
execute_request(Pid,{Convert,Converted}) ->
    ...
    receive {io_reply,Mref,Reply}->
        demonitor(Mref,[flush]),
        if Convert ->
            convert_binaries(Reply);true ->
            Reply end;
    {'DOWN',Mref,_,_,_}->
        receive {'EXIT',Pid,_What}->
            true after 0->
            true end,
...
io_request(Pid,{put_chars,Enc,Chars} = Request0)
    when is_list(Chars),
    node(Pid) =:= node()->
    Request = case  catch unicode:characters_to_binary(Chars,Enc) of
        Binary
