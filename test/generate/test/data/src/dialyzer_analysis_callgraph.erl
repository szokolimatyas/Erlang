-file("dialyzer_analysis_callgraph.erl", 1).

-module(dialyzer_analysis_callgraph).

-export([start/3]).

-export([compile_init_result/0, add_to_result/4]).

-export([start_compilation/2, continue_compilation/2]).

-export_type([compile_init_data/0, one_file_mid_error/0, one_file_result_ok/0, compile_result/0]).
...
-file("dialyzer_analysis_callgraph.erl", 40).

-record(analysis_state,{codeserver::dialyzer_codeserver:codeserver(),analysis_type = succ_typings::anal_type(),defines = []::[dial_define()],doc_plt::dialyzer_plt:plt(),include_dirs = []::[file:filename()],parent::pid(),legal_warnings::[dial_warn_tag()],plt::dialyzer_plt:plt(),start_from = byte_code::start_from(),use_contracts = true::boolean(),timing_server::dialyzer_timing:timing_server(),solvers::[solver()]}).

-record(server_state,{parent::pid()}).
...
start(Parent,LegalWarnings,Analysis) ->
    ...
    Analysis0 = Analysis#analysis{race_detection = RacesOn,timing_server = TimingServer},
...
run_analysis(Analysis,LegalWarnings) ->
    Self = self(),
    Fun = fun ()->
        analysis_start(Self,Analysis,LegalWarnings) end,
    Analysis#analysis{analysis_pid = spawn_link(Fun)}.

loop(#server_state{parent = Parent} = State,#analysis{analysis_pid = AnalPid} = Analysis,ExtCalls) ->
    receive {AnalPid,log,LogMsg}->
...
analysis_start(Parent,Analysis,LegalWarnings) ->
    ...
    State = #analysis_state{codeserver = CServer,analysis_type = Analysis#analysis.type,defines = Analysis#analysis.defines,doc_plt = Analysis#analysis.doc_plt,include_dirs = Analysis#analysis.include_dirs,plt = Plt,parent = Parent,legal_warnings = LegalWarnings,start_from = Analysis#analysis.start_from,use_contracts = Analysis#analysis.use_contracts,timing_server = Analysis#analysis.timing_server,solvers = Analysis#analysis.solvers},
    ...
    NewCallgraph = case Analysis#analysis.race_detection of
        true->
    ...
    #analysis_state{plt = Plt2,doc_plt = DocPlt,codeserver = Codeserver0} = State2,
...
remote_type_postproc(TmpCServer0,Args) ->
    ...
    fun ()->
        ...
        #analysis{start_from = StartFrom,timing_server = TimingServer} = Analysis,
        Files = ordsets:from_list(Analysis#analysis.files),
        RemMods = [case StartFrom of
            byte_code->
        ...
        TmpServer4 = begin dialyzer_timing:start_stamp(TimingServer,"remote"),
        _T = begin TmpCServer3 = dialyzer_utils:process_record_remote_types(TmpCServer2),
        dialyzer_contracts:process_contract_remote_types(TmpCServer3) end,
...
skip_ets_transfer(Pid) ->
    receive {'ETS-TRANSFER',_Tid,Pid,_HeriData}->
        skip_ets_transfer(Pid) after 0->
        ok end.
...
analyze_callgraph(Callgraph,#analysis_state{codeserver = Codeserver,doc_plt = DocPlt,plt = Plt,timing_server = TimingServer,parent = Parent,solvers = Solvers} = State) ->
    case State#analysis_state.analysis_type of
        plt_build->
            NewPlt = dialyzer_succ_typings:analyze_callgraph(Callgraph,Plt,Codeserver,TimingServer,Solvers,Parent),
            dialyzer_callgraph:delete(Callgraph),
            State#analysis_state{plt = NewPlt,doc_plt = DocPlt};
        succ_typings->
            {Warnings,NewPlt,NewDocPlt} = dialyzer_succ_typings:get_warnings(Callgraph,Plt,DocPlt,Codeserver,TimingServer,Solvers,Parent),
            dialyzer_callgraph:delete(Callgraph),
...
-record(compile_init,{callgraph::dialyzer_callgraph:callgraph(),codeserver::dialyzer_codeserver:codeserver(),defines = []::[dial_define()],include_dirs = []::[file:filename()],start_from = byte_code::start_from(),use_contracts = true::boolean(),legal_warnings::[dial_warn_tag()]}).

make_compile_init(#analysis_state{codeserver = Codeserver,defines = Defs,include_dirs = Dirs,use_contracts = UseContracts,legal_warnings = LegalWarnings,start_from = StartFrom},Callgraph) ->
    #compile_init{callgraph = Callgraph,codeserver = Codeserver,defines = [{d,Macro,Val} || {Macro,Val} <- Defs],include_dirs = [{i,D} || D <- Dirs],use_contracts = UseContracts,legal_warnings = LegalWarnings,start_from = StartFrom}.

compile_and_store(Files,#analysis_state{codeserver = CServer,timing_server = Timing,parent = Parent} = State) ->
    ...
    {{Failed,Modules},NextLabel} = begin dialyzer_timing:start_stamp(Timing,"compile"),
    _C1 = dialyzer_coordinator:parallel_job(compile,Files,CompileInit,Timing),
    dialyzer_timing:end_stamp(Timing),
    _C1 end,
    CServer2 = dialyzer_codeserver:set_next_core_label(NextLabel,CServer),
    case Failed =:= [] of
        true->
            ModDict = lists:foldl(fun (F,Dict)->
                ModFile = lists:last(filename:split(F)),
                Mod = filename:basename(ModFile,".beam"),
                dict:append(Mod,F,Dict) end,dict:new(),Files),
            check_for_duplicate_modules(ModDict);
        false->
            Msg = io_lib:format("Could not scan the following file(s):~n~" "ts",[[Reason || {_Filename,Reason} <- Failed]]),
            exit({error,Msg})
    end,
    {T2,_} = statistics(wall_clock),
    Msg1 = io_lib:format("done in ~.2f secs\nRemoving edges... ",[(T2 - T1)/1000]),
    send_log(Parent,Msg1),
    Callgraph = begin dialyzer_timing:start_stamp(Timing,"clean"),
...
-opaque(compile_result()::{[{file:filename(),error_reason()}],[module()]}).

-type(one_file_mid_error()::{error,error_reason()}).

-opaque(one_file_result_ok()::{ok,[dialyzer_callgraph:callgraph_edge()],[mfa_or_funlbl()],module()}).

-type(one_file_result()::one_file_mid_error()|one_file_result_ok()).

-type(compile_mid_data()::{module(),cerl:cerl(),dialyzer_callgraph:callgraph(),dialyzer_codeserver:codeserver()}).
...
-spec(add_to_result(file:filename(),one_file_result(),compile_result(),compile_init_data()) -> compile_result()).
...
-spec(start_compilation(file:filename(),compile_init_data()) -> {error,error_reason()}|{ok,integer(),compile_mid_data()}).

start_compilation(File,#compile_init{callgraph = Callgraph,codeserver = Codeserver,defines = Defines,include_dirs = IncludeD,use_contracts = UseContracts,legal_warnings = LegalWarnings,start_from = StartFrom}) ->
    case StartFrom of
        src_code->
...
cleanup_callgraph(#analysis_state{plt = InitPlt,parent = Parent,codeserver = CodeServer},CServer,Callgraph,Modules) ->
    ...
    ExtCalls1 = [Call || Call = {_From,To} <- ExtCalls, not dialyzer_plt:contains_mfa(InitPlt,To)],
    {BadCalls1,RealExtCalls} = if ExtCalls1 =:= [] ->
        {[],[]};true ->
        ...
        Pred = fun ({_From,{M,_F,_A}})->
            sets:is_element(M,AllModules) end,
        lists:partition(Pred,ExtCalls1) end,
    NonLocalCalls = dialyzer_callgraph:non_local_calls(Callgraph1),
    BadCalls2 = [Call || Call = {_From,To} <- NonLocalCalls, not dialyzer_codeserver:is_exported(To,CServer)],
    ...
    if RealExtCalls =:= [] ->
        ok;true ->
        send_ext_calls(Parent,lists:usort([To || {_From,To} <- RealExtCalls])) end,
    Callgraph1.

compile_src(File,Includes,Defines,Callgraph,CServer,UseContracts,LegalWarnings) ->
        ...
        {ok,RecInfo}->
            ...
            case dialyzer_utils:get_fun_meta_info(Mod,Core,LegalWarnings) of
                {error,_} = Error->
                    Error;
                MetaFunInfo->
                    CServer2 = dialyzer_codeserver:insert_fun_meta_info(MetaFunInfo,CServer1),
                        ...
                        true->
                            case dialyzer_utils:get_spec_info(Mod,Core,RecInfo) of
                                ...
                                {ok,SpecInfo,CallbackInfo}->
                                    CServer3 = dialyzer_codeserver:store_temp_contracts(Mod,SpecInfo,CallbackInfo,CServer2),
...
-spec(continue_compilation(integer(),compile_mid_data()) -> one_file_result_ok()).
...
get_exported_types_from_core(Core) ->
    ...
    ExpTypes1 = [(cerl:concrete(L2)) || {L1,L2} <- Attrs,cerl:is_literal(L1),cerl:is_literal(L2),cerl:concrete(L1) =:= export_type],
...
expand_files(Analysis = #analysis{files = Files,start_from = StartFrom}) ->
    ...
    case expand_files(Files,Ext,[]) of
        []->
            Msg = "No " ++ Ext ++ " files to analyze" ++ case StartFrom of
                byte_code->
                    " (no --src specified?)";
                src_code->
                    ""
...
check_for_duplicate_modules(ModDict) ->
    Duplicates = dict:filter(fun (_,[_])->
        false;(_,_Files)->
        true end,ModDict),
...
rcv_ext_types(Self,ExtTypes) ->
    receive {Self,ext_types,ExtType}->
        rcv_ext_types(Self,[ExtType| ExtTypes]);
    {Self,done}->
        lists:usort(ExtTypes) end.
...
filter_warnings(Warnings,Codeserver) ->
    [TWW || {Tag,WarningInfo,_Warning} = TWW <- Warnings,is_ok_fun(WarningInfo,Codeserver),is_ok_tag(Tag,WarningInfo,Codeserver)].

is_ok_fun({_F,_L,Module},_Codeserver)
    when is_atom(Module)->
    true;
                        ...
                        case {cerl:concrete(M),cerl:concrete(F),A} of
                            ...
                            {erlang,make_fun,3}->
                                ...
                                case cerl:is_c_atom(CA1) andalso cerl:is_c_atom(CA2) andalso cerl:is_c_int(CA3) of
                                    true->
                                        case {cerl:concrete(CA1),cerl:concrete(CA2),cerl:concrete(CA3)} of
                                            MFA->
                                                Ann = cerl:get_ann(SubTree),
                                                [{get_file(CodeServer,Module,Ann),get_line(Ann)}| Acc];
                                            _->
...
-spec(dump_callgraph(dialyzer_callgraph:callgraph(),#analysis_state{},#analysis{}) -> ok).

dump_callgraph(_CallGraph,_State,#analysis{callgraph_file = ""}) ->
    ok;
