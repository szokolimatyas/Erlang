...
gen_encode_sequence(Gen,Typename,#type{} = D) ->
    {_SeqOrSet,TableConsInfo,CompList0} = case D#type.def of
    ...
    EncObj = case TableConsInfo of
        #simpletableattributes{usedclassfield = Used,uniqueclassfield = Unique}
            when Used /= Unique->
            false;
        #simpletableattributes{objectsetname = ObjectSetRef,c_name = AttrN,c_index = N,usedclassfield = UniqueFieldName,uniqueclassfield = UniqueFieldName,valueindex = _ValueIndex}->
    ...
    {sequence,list_to_atom(lists:concat([Prefix, asn1ct_gen:list2name(Typename)])),length(CompList1),CompTypes}.
...
gen_encode_choice(Erules,TypeName,D)
    ...
    {choice,maps:from_list([{AltName,AltType} || {AltName,AltType,_OptOrMand} <- gen_enc_comptypes(Erules,TypeName,CompList1,0,0,[])])}.
...
gen_enc_line(Erules,TopType,Cname,Type,Element,Indent,OptOrMand,EncObj)
    when is_list(Element)->
    gen_enc_line(Erules,TopType,Cname,Type,Element,Indent,OptOrMand,[{curr,encV}, " = "],EncObj).
...
gen_encode(Erules,Typename,Type)
    ...
    case asn1ct_gen:type(InnerType) of
        {constructed,bif}->
            ...
            emit([nl, nl, nl, "%%================================", nl, "%%  ", asn1ct_gen:list2name(Typename), nl, "%%================================", nl, Func, "(Val", ObjFun, ") ->", nl, "   "]),
...
gen_encode_user(Erules,#typedef{} = D,_Wrapper) ->
    ...
    TypeInfo = case asn1ct_gen:type(InnerType) of
        {constructed,bif}->
            gen_encode_constructed(Erules,Typename,InnerType,Type);
        {primitive,bif}->
        ...
        'ASN1_OPEN_TYPE'->
            gen_encode_prim(jer,Type#type{def = 'ASN1_OPEN_TYPE'},"Val")
    end,
...
gen_objectset_code(Erules,ObjSet) ->
    ...
    SelectValMap = maps:from_list([{SelectVal,maps:from_list(gen_enc_classtypes(Erules,ObjSetName,[TNameType || TNameType = {_TypeName,#typedef{}} <- TypeList],[]))} || {_,SelectVal,TypeList} <- Set]),
    emit([FuncName, "() ->", nl]),
