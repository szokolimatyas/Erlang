...
m(Module)
    ...
    case xref_utils:find_beam(Module) of
        {ok,File}->
            Fun = fun (S)->
                xref_base:add_module(S,File,{builtins,true}) end,
...
m(File) ->
    case xref_utils:split_filename(File,".beam") of
        ...
        {Dir,BaseName}->
            ...
            Fun = fun (S)->
                xref_base:add_module(S,BeamFile,{builtins,true}) end,
            case  catch do_functions_analysis(Fun) of
                {error,_,{no_debug_info,_}}->
                     catch do_modules_analysis(Fun);
                Result->
...
d(Directory) ->
    Fun = fun (S)->
        xref_base:add_directory(S,Directory,{builtins,true}) end,
    Fun1 = fun (S)->
        case Fun(S) of
            {ok,[],_S}->
                no_modules;
...
stop(Name) ->
    try gen_server:call(Name,stop,infinity)
        after  catch unregister(Name) end.
...
do_functions_analysis(FFun) ->
    ...
    {ok,State2} = xref_base:set_default(State1,[{verbose,false}, {warnings,false}]),
    ...
    [{deprecated,to_external(Deprecated)}, {undefined,to_external(Undef)}, {unused,to_external(Unused)}].
