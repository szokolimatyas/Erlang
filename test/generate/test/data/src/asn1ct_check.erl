...
check(S,{Types,Values,ParameterizedTypes,Classes,Objects,ObjectSets}) ->
    TupleIs = fun ({T,_},T)->
        ...
        false end,
    IsClass = fun (X)->
        TupleIs(X,asn1_class) end,
    IsObjSet = fun (X)->
        TupleIs(X,objectsetdef) end,
    IsPObjSet = fun (X)->
        TupleIs(X,pobjectsetdef) end,
    IsObject = fun (X)->
        TupleIs(X,objectdef) end,
    IsValueSet = fun (X)->
        TupleIs(X,valueset) end,
    Element2 = fun (X)->
        element(2,X) end,
    Element1 = fun (X)->
        element(1,X) end,
    ...
    ParObjectSets = lists:filter(fun ({_OSName,objectset,_})->
        true;(_)->
        false end,ParameterizedElems),
    ParObjectSetNames = lists:map(Element1,ParObjectSets),
    ParTypes = lists:filter(fun ({_TypeName,type,_})->
        true;(_)->
        false end,ParameterizedElems),
    ...
    AllErrors = lists:flatten([ParamError, Terror3, Verror5, Cerror, Oerror, Exporterror, ImportError]),
    case AllErrors of
        []->
            ContextSwitchTs = context_switch_in_spec(),
            InstanceOf = instance_of_in_spec(S#state.mname),
            NewTypes = lists:subtract(Types,AddClasses) ++ ContextSwitchTs ++ InstanceOf ++ ParTypesNames,
            NewValues = lists:subtract(Values,PObjSetNames ++ ObjectNames ++ ValueSetNames),
            {ok,{NewTypes,NewValues,ParameterizedTypes,NewClasses,NewObjects,NewObjectSets},{NewTypes,NewValues,ParameterizedTypes,NewClasses,lists:subtract(NewObjects,ExclO) ++ InlinedObjects,lists:subtract(NewObjectSets,ExclOS) ++ ParObjectSetNames}};
        _->
            {error,AllErrors}
    end.

context_switch_in_spec() ->
    L = [{external,'EXTERNAL'}, {embedded_pdv,'EMBEDDED PDV'}, {character_string,'CHARACTER STRING'}],
    F = fun ({T,TName},Acc)->
        case get(T) of
            generate->
                erase(T),
...
check_exports(S,Module = #module{}) ->
    case Module#module.exports of
        ...
        {exports,all}->
            [];
        {exports,ExportList}
            when is_list(ExportList)->
            IsNotDefined = fun (X)->
                try _ = get_referenced_type(S,X),
                false
                    catch
                        throw:{error,_}->
                            true end end,
            [(return_asn1_error(S,Ext,{undefined_export,Undef})) || Ext = #'Externaltypereference'{type = Undef} <- ExportList,IsNotDefined(Ext)]
    end.
...
check_imports_1(S,[#'SymbolsFromModule'{symbols = Imports,module = ModuleRef}| SFMs],Acc) ->
    ...
    Refs = [{try get_referenced_type(S,Ref)
            ...
            throw:Error->
                Error end,Ref} || Ref <- Imports],
    CreateError = fun (Ref)->
        Error = {undefined_import,name_of_def(Ref),Module},
        return_asn1_error(S,Ref,Error) end,
        ...
        #type{} = Ts->
            case Type0#typedef.checked of
                true->
                    ok;
                _->
                    Type = Type0#typedef{checked = true,typespec = Ts},
                    asn1_db:dbput(NewS#state.mname,Name,Type),
                    ok
            end
        catch
            throw:{error,Reason}->
                Reason;
            throw:{asn1_class,_ClassDef}->
                {asn1_class,Name};
            throw:pobjectsetdef->
                {pobjectsetdef,Name};
            throw:pvalueset->
...
check_contextswitchingtypes(S,Acc) ->
    CSTList = [{external,'EXTERNAL'}, {embedded_pdv,'EMBEDDED PDV'}, {character_string,'CHARACTER STRING'}],
    ...
    try check_value(S,Value) of 
        ...
        V->
            ...
            throw:{objectsetdef}->
                {objectsetdef,Name};
            throw:{asn1_class,_}->
                #valuedef{checked = C,pos = Pos,name = N,type = Type,value = Def} = Value,
...
checko(S0,[Name| Os],Acc,ExclO,ExclOS) ->
    ...
    try checko_1(S,Item,Name,ExclO,ExclOS) of 
        {NewExclO,NewExclOS}->
            ...
            throw:{error,Error}->
                checko(S,Os,[Error| Acc],ExclO,ExclOS) end;
...
check_class(S,ClassName) ->
    ...
    case Def of
        ClassDef
            when is_record(ClassDef,classdef)->
...
check_class_fields(S,[F| Fields],Acc) ->
    NewField = case element(1,F) of
        ...
        object_or_fixedtypevalue_field->
            ...
            Cat = case asn1ct_gen:type(asn1ct_gen:get_inner(Type2#type.def)) of
                Def
                    when is_record(Def,'Externaltypereference')->
            ...
            case Cat of
                Class
                    when is_record(Class,classdef)->
                    ToExt = fun (#type{def = CE = #'Externaltypereference'{}})->
                        CE;(T)->
                        T end,
        ...
        objectset_or_fixedtypevalueset_field->
            {_,Name,Type,OSpec} = F,
            RefType = try check_type(S,#typedef{typespec = Type},Type) of 
                #type{} = CheckedType->
                    CheckedType
                catch
                    throw:{asn1_class,_ClassDef}->
            ...
            if is_record(RefType,'Externaltypereference') ->
                {objectsetfield,Name,Type,OSpec};is_record(RefType,classdef) ->
                {objectsetfield,Name,Type,OSpec};is_record(RefType,objectclass) ->
                {objectsetfield,Name,Type,OSpec};true ->
                {fixedtypevaluesetfield,Name,RefType,OSpec} end;
        typefield->
            case F of
...
if_current_checked_type(S,#type{def = Def}) ->
    ...
    case Def of
        #'Externaltypereference'{module = CurrentModule,type = CurrentCheckedName}->
            true;
        #'Externaltypereference'{module = ModuleName,type = CurrentCheckedName}->
...
check_pobject(_S,PObject)
    when is_record(PObject,pobjectdef)->
    Def = PObject#pobjectdef.def,
    Def.

check_pobjectset(S,PObjSet) ->
    #pvaluesetdef{pos = Pos,name = Name,args = Args,type = Type,valueset = ValueSet} = PObjSet,
    {Mod,Def} = get_referenced_type(S,Type#type.def),
    case Def of
        #classdef{}->
            ClassName = #'Externaltypereference'{module = Mod,type = get_datastr_name(Def)},
            {valueset,Set} = ValueSet,
            ObjectSet = #'ObjectSet'{class = ClassName,set = Set},
            #pobjectsetdef{pos = Pos,name = Name,args = Args,class = Type#type.def,def = ObjectSet};
        _->
            PObjSet
    end.

-record(osi, {st,classref,uniq,ext}).

check_object(_S,ObjDef,ObjSpec)
    when ObjDef#typedef.checked == true->
    ObjSpec;
check_object(S,_ObjDef,#'Object'{classname = ClassRef,def = ObjectDef}) ->
    no_debug,
    _ = check_externaltypereference(S,ClassRef),
    {ClassDef,NewClassRef} = case get_referenced_type(S,ClassRef,true) of
        {MName,#classdef{checked = false,name = CLName} = ClDef}->
            ...
            {ClDef#classdef{checked = true,typespec = ObjClass},#'Externaltypereference'{module = MName,type = CLName}};
        {MName,#classdef{name = CLName} = ClDef}->
            {ClDef,#'Externaltypereference'{module = MName,type = CLName}};
        _->
            asn1_error(S,illegal_object)
    end,
    NewObj = case ObjectDef of
        {object,_,_} = Def->
...
check_object(S,_,#'ObjectSet'{class = ClassRef0,set = Set0} = ObjSet0) ->
    ...
    {UniqueFieldName,UniqueInfo} = case get_unique_fieldname(S,ClassDef) of
        no_unique->
            {{unique,undefined},{unique,undefined}};
        Other->
            {element(1,Other),Other}
    end,
    OSI0 = #osi{st = S,classref = ClassRef,uniq = UniqueInfo,ext = false},
    {Set1,OSI1} = if is_list(Set0) ->
        check_object_set_list(Set0,OSI0);true ->
        check_object_set(Set0,OSI0) end,
    Ext = case Set1 of
        []->
            true;
        [_| _]->
...
check_object_set({object,_,_} = Obj0,OSI) ->
    ...
    #'Object'{def = Def} = check_object(S,#typedef{typespec = Obj0},#'Object'{classname = ClassRef,def = Obj0}),
    ObjList = check_object_set_mk(Def,OSI),
    {ObjList,OSI};
check_object_set(#'ObjectClassFieldType'{classname = ObjName,fieldname = FieldNames},#osi{st = S} = OSI) ->
...
check_object_set({pv,{simpledefinedvalue,DefinedObject},Params} = PV,OSI) ->
    ...
    #'Object'{def = Def} = check_object(S,PV,#'Object'{classname = ClassRef,def = {po,{object,DefinedObject},Args}}),
    ObjList = check_object_set_mk(Def,OSI),
    {ObjList,OSI};
...
remove_duplicate_objects(S,Set0)
    when is_list(Set0)->
...
remove_duplicate_objects_1(S,[{Id,[_| _] = Objs}| T]) ->
    MakeSortable = fun (What)->
        sortable_type(S,What) end,
...
check_referenced_object(S,ObjRef)
    ...
    case get_referenced_type(S,ObjRef) of
        {RefedMod,ObjectDef}
            when is_record(ObjectDef,valuedef)->
            no_debug,
            #type{def = ClassRef} = ObjectDef#valuedef.type,
            Def = ObjectDef#valuedef.value,
            {RefedMod,get_datastr_name(ObjectDef),check_object(update_state(S,RefedMod),ObjectDef,#'Object'{classname = ClassRef,def = Def})};
        {RefedMod,ObjectDef}
            when is_record(ObjectDef,typedef)->
            {RefedMod,get_datastr_name(ObjectDef),check_object(update_state(S,RefedMod),ObjectDef,ObjectDef#typedef.typespec)}
    end.
...
get_fieldname_element(S,Object0,[{_RefType,FieldName}| Fields]) ->
    Object = case Object0 of
        #typedef{typespec = #'Object'{def = Obj}}->
            Obj;
        {_,_,_} = Obj->
            Obj
    end,
...
get_fieldname_set_1(S,[H| T],Fields,Acc) ->
    try get_fieldname_element(S,H,Fields) of 
        ...
        Other->
            ...
            throw:{error,_}->
                get_fieldname_set_1(S,T,Fields,Acc) end;
...
check_fieldname_element_1(S,#valuedef{} = VDef) ->
    try check_value(S,VDef)
        catch
            throw:{asn1_class,_}->
                #valuedef{checked = C,pos = Pos,name = N,type = Type,value = Def} = VDef,
                ClassName = Type#type.def,
                NewSpec = #'Object'{classname = ClassName,def = Def},
                NewDef = #typedef{checked = C,pos = Pos,name = N,typespec = NewSpec},
                check_fieldname_element_1(S,NewDef) end;
...
instantiate_po(S = #state{parameters = _OldArgs},_ClassDef,Object,ArgsList)
    when is_record(Object,pobjectdef)->
    FormalParams = get_pt_args(Object),
    MatchedArgs = match_args(S,FormalParams,ArgsList,[]),
    NewS = S#state{parameters = MatchedArgs},
    check_object(NewS,Object,#'Object'{classname = Object#pobjectdef.class,def = Object#pobjectdef.def}).
...
gen_incl_set(S,Fields,ClassDef) ->
    case get_unique_fieldname(S,ClassDef) of
        no_unique->
            false;
        {_,_}->
            gen_incl_set1(S,Fields,(ClassDef#classdef.typespec)#objectclass.fields)
    end.
...
preprocess_syntax_1(S,[H| T],Cs,Mandatory)
    when is_list(H)->
    [{optional,preprocess_syntax_1(S,H,Cs,false)}| preprocess_syntax_1(S,T,Cs,Mandatory)];
...
preprocess_syntax_1(S,[{typefieldreference,Name}| T],Cs,Mandatory) ->
    F = preprocess_check_field(S,Name,Cs,Mandatory),
    [F| preprocess_syntax_1(S,T,Cs,Mandatory)];
preprocess_syntax_1(S,[{Token,_}| T],Cs,Mandatory)
    when is_atom(Token)->
    [{token,Token}| preprocess_syntax_1(S,T,Cs,Mandatory)];
preprocess_syntax_1(S,[Token| T],Cs,Mandatory)
    when is_atom(Token)->
    [{token,Token}| preprocess_syntax_1(S,T,Cs,Mandatory)];
...
match_syntax_type(_S,_Type,{Atom,Line})
    ...
    is_integer(Line)->
    throw(nomatch);
match_syntax_type(S,{fixedtypevaluefield,Name,#type{} = T,_,_} = Type,#'Externalvaluereference'{} = ValRef0) ->
    try get_referenced_type(S,ValRef0) of 
        {M,#valuedef{} = ValDef}->
            match_syntax_type(update_state(S,M),Type,ValDef)
        catch
            throw:{error,_}->
                ValRef = #valuedef{name = Name,type = T,value = ValRef0,module = S#state.mname},
                match_syntax_type(S,Type,ValRef) end;
match_syntax_type(S,{fixedtypevaluefield,Name,#type{},_,_},#valuedef{} = Val0) ->
    Val = check_value(S,Val0),
    {match,[{Name,Val}]};
match_syntax_type(S,{fixedtypevaluefield,Name,#type{},_,_},{'ValueFromObject',{object,Object},FieldNames}) ->
...
match_syntax_params(S0,#ptypedef{name = Name} = PtDef,#'Externaltypereference'{module = M,type = N} = ERef0,Args) ->
...
match_syntax_external(#state{mname = Mname} = S0,Name,Ref0) ->
    ...
    case T0 of
        #ptypedef{}->
            {params,Name,T0,Ref1};
        #typedef{checked = false} = TDef0
            when Mname =/= M->
            S = S0#state{mname = M,module = load_asn1_module(S0,M),tname = get_datastr_name(TDef0)},
...
match_syntax_objset_1(S,{word_or_setting,_,Set},ClassDef) ->
    match_syntax_objset(S,Set,ClassDef);
match_syntax_objset_1(S,#type{def = {'TypeFromObject',{object,Object},FNs}},ClassDef) ->
...
printable_string_1(#'Externalvaluereference'{value = Type}) ->
    atom_to_list(Type);
printable_string_1({Atom,Line})
    when is_atom(Atom),
    is_integer(Line)->
    q(Atom);
printable_string_1({object,definedsyntax,L}) ->
    Str = lists:join($ ,[(printable_string_1(Item)) || Item <- L]),
    q(lists:flatten(Str));
printable_string_1([_| _] = Def) ->
    case lists:all(fun is_integer/1,Def) of
        ...
        false->
            Str = lists:join($ ,[(printable_string_1(Item)) || Item <- Def]),
            q(lists:flatten(Str))
    end;
printable_string_1(Def) ->
    lists:flatten(io_lib:format("~p",[Def])).
...
get_mandatory_class_fields(ClassFields) ->
    [(element(2,F)) || F <- ClassFields,is_mandatory_class_field(F)].
...
check_value(_OldS,V)
    when is_record(V,pvaluedef)->
    V;
check_value(OldS,V)
    when is_record(V,typedef)->
    no_debug,
    ...
    case TS of
        #'ObjectSet'{class = ClassRef}->
            ...
            case TSDef of
                #classdef{}->
                    throw({objectsetdef});
                #typedef{typespec = #type{def = Eref}}
                    when is_record(Eref,'Externaltypereference')->
...
check_value(S,#valuedef{pos = Pos,name = Name,type = Type,value = {valueset,Constr}}) ->
    NewType = Type#type{constraint = [Constr]},
    {valueset,check_type(S,#typedef{pos = Pos,name = Name,typespec = NewType},NewType)};
        ...
        #'Externaltypereference'{type = RecName} = Ext->
            ...
            case Type of
                #typedef{typespec = TypeSpec0} = TypeDef->
                    ...
                    #valuedef{value = CheckedVal} = check_value(S3,V0#valuedef{type = TypeSpec}),
                    V#valuedef{value = CheckedVal};
                #type{}->
                    #valuedef{value = CheckedVal} = check_value(S2#state{recordtopname = [RecName| TopName]},V#valuedef{type = Type}),
                    V#valuedef{value = CheckedVal}
            end;
        'ASN1_OPEN_TYPE'->
            {opentypefieldvalue,ANYType,ANYValue} = Value,
            CheckedV = check_value(SVal,#valuedef{name = Name,type = ANYType,value = ANYValue,module = ModName}),
...
validate_objectidentifier(S,OidType,{'ValueFromObject',{object,Obj},Fields}) ->
    case extract_field(S,Obj,Fields) of
        #valuedef{checked = true,value = Value,type = Type}
            when is_tuple(Value)->
...
validate_objectidentifier(S,OidType,[{#seqtag{module = Mod,pos = Pos,val = Atom},Val}]) ->
    Rec = #'Externalvaluereference'{pos = Pos,module = Mod,value = Atom},
...
reserved_objectid(a,[0, 0]) ->
    1;
reserved_objectid(b,[0, 0]) ->
    2;
reserved_objectid(c,[0, 0]) ->
    3;
reserved_objectid(d,[0, 0]) ->
    4;
reserved_objectid(e,[0, 0]) ->
    5;
reserved_objectid(f,[0, 0]) ->
    6;
reserved_objectid(g,[0, 0]) ->
    7;
reserved_objectid(h,[0, 0]) ->
    8;
reserved_objectid(i,[0, 0]) ->
    9;
reserved_objectid(j,[0, 0]) ->
    10;
reserved_objectid(k,[0, 0]) ->
    11;
reserved_objectid(l,[0, 0]) ->
    12;
reserved_objectid(m,[0, 0]) ->
    13;
reserved_objectid(n,[0, 0]) ->
    14;
reserved_objectid(o,[0, 0]) ->
    15;
reserved_objectid(p,[0, 0]) ->
    16;
reserved_objectid(q,[0, 0]) ->
    17;
reserved_objectid(r,[0, 0]) ->
    18;
reserved_objectid(s,[0, 0]) ->
    19;
reserved_objectid(t,[0, 0]) ->
    20;
reserved_objectid(u,[0, 0]) ->
    21;
reserved_objectid(v,[0, 0]) ->
    22;
reserved_objectid(w,[0, 0]) ->
    23;
reserved_objectid(x,[0, 0]) ->
    24;
reserved_objectid(y,[0, 0]) ->
    25;
reserved_objectid(z,[0, 0]) ->
    26;
...
validate_oid_path(_,rel_oid,_) ->
    ok;
validate_oid_path(_,o_id,[0, I| _])
    when 0 =< I,
    I =< 9->
    ok;
validate_oid_path(_,o_id,[1, I| _])
    when 0 =< I,
    I =< 3->
    ok;
...
normalize_value(S,Type,{'DEFAULT',Value},NameList) ->
    case  catch get_canonic_type(S,Type,NameList) of
        ...
        Err->
            asn1ct:warning("could not check default value ~p~nType:~n~p" "~nNameList:~n~p~n",[Value, Type, Err],S,"could not check default value"),
            Value
    end;
        ...
        false->
            try get_referenced_value(S,Ref) of 
                ...
                _->
                    ...
                    throw:_->
                        asn1_error(S,illegal_integer_value) end
    end;
...
normalize_sequence(S,Value,Components,NameList)
    ...
    normalize_sequence(S,Value,lists:flatten(tuple_to_list(Components)),NameList);
...
sort_value(Components,Value0)
    when is_list(Value0)->
    {Keys0,_} = lists:mapfoldl(fun (#'ComponentType'{name = N},I)->
        {{N,I},I + 1} end,0,Components),
...
normalized_record(SorS,S,Value,Components,NameList) ->
    NewName = list_to_atom(lists:concat([get_record_prefix_name(S), asn1ct_gen:list2name(NameList)])),
    case is_record_normalized(S,NewName,Value,length(Components)) of
        true->
            Value;
        false->
...
is_record_normalized(_S,Name,Value,NumComps)
    when is_tuple(Value)->
    tuple_size(Value) =:= NumComps + 1 andalso element(1,Value) =:= Name;
is_record_normalized(_S,_Name,Value,_NumComps)
    when is_map(Value)->
    true;
...
create_map_value(Components,ListOfVals) ->
    ...
    L = [{Name,V} || {#'ComponentType'{name = Name},V} <- Zipped,V =/= asn1_NOVALUE],
    maps:from_list(L).

normalize_seq_or_set(SorS,S,[{#seqtag{val = Cname},V}| Vs],[#'ComponentType'{name = Cname,typespec = TS}| Cs],NameList,Acc) ->
    NewNameList = case TS#type.def of
...
normalize_seq_or_set(SorS,S,Values = [{#seqtag{val = Cname0},_V}| _Vs],[#'ComponentType'{prop = 'OPTIONAL'}| Cs],NameList,Acc) ->
    verify_valid_component(S,Cname0,Cs),
    normalize_seq_or_set(SorS,S,Values,Cs,NameList,[asn1_NOVALUE| Acc]);
normalize_seq_or_set(SorS,S,Values = [{#seqtag{val = Cname0},_V}| _Vs],[#'ComponentType'{name = Cname,typespec = TS,prop = {'DEFAULT',Value}}| Cs],NameList,Acc) ->
    verify_valid_component(S,Cname0,Cs),
    NewNameList = case TS#type.def of
        #'Externaltypereference'{type = TName}->
            [TName];
...
normalize_seq_or_set(SorS,S,[],[#'ComponentType'{name = Name,typespec = TS,prop = {'DEFAULT',Value}}| Cs],NameList,Acc) ->
    NewNameList = case TS#type.def of
        #'Externaltypereference'{type = TName}->
            [TName];
        _->
...
normalize_seq_or_set(SorS,S,[],[#'ComponentType'{prop = 'OPTIONAL'}| Cs],NameList,Acc) ->
    normalize_seq_or_set(SorS,S,[],Cs,NameList,[asn1_NOVALUE| Acc]);
normalize_seq_or_set(SorS,S,Value = #'Externalvaluereference'{},Cs,NameList,Acc) ->
    get_normalized_value(S,Value,Cs,fun normalize_seq_or_set/6,[SorS, NameList, Acc]);
...
normalize_s_of(SorS,S,Value,Type,NameList)
    ...
    NormFun = fun (X)->
        normalize_value(S,Type,X,NewNameList) end,
    case  catch lists:map(NormFun,DefValueList) of
        List
            when is_list(List)->
            List;
        _->
            asn1ct:warning("~p could not handle value ~p~n",[SorS, Value],S,"could not handle value"),
            Value
    end;
normalize_s_of(SorS,S,Value,Type,NameList)
    when is_record(Value,'Externalvaluereference')->
    get_normalized_value(S,Value,Type,fun normalize_s_of/5,[SorS, NameList]).

normalize_restrictedstring(S,[H| T],CType)
    when is_list(H);
    is_tuple(H)->
    [normalize_restrictedstring(S,H,CType)| normalize_restrictedstring(S,T,CType)];
normalize_restrictedstring(_S,CString,_)
    when is_list(CString)->
    CString;
normalize_restrictedstring(S,ERef,CType)
    when is_record(ERef,'Externalvaluereference')->
    get_normalized_value(S,ERef,CType,fun normalize_restrictedstring/3,[]).
...
get_normalized_value(S,Val,Type,Func,AddArg) ->
    case  catch get_referenced_type(S,Val) of
        ...
        {ExtM,NewVal}->
            ...
            call_Func(update_state(S,ExtM),V2,Type,Func,AddArg);
        _->
            asn1ct:warning("default value not comparable ~p~n",[Val],S,"default value not comparable"),
            Val
    end.
...
get_canonic_type(S,Type,NameList) ->
    {InnerType,NewType,NewNameList} = case Type#type.def of
        'INTEGER' = Name->
            {Name,[],NameList};
        Name
            when is_atom(Name)->
        ...
        Ref
            ...
            {_,#typedef{name = Name,typespec = RefedType}} = get_referenced_type(S,Ref),
...
check_ptype(S,Type,Ts)
    when is_record(Ts,type)->
    ...
    NewDef = case Def of
        Seq
            ...
            #newt{type = Seq#'SEQUENCE'{pname = get_datastr_name(Type),components = Components}};
        Set
            when is_record(Set,'SET')->
            Components = expand_components(S,Set#'SET'.components),
            #newt{type = Set#'SET'{pname = get_datastr_name(Type),components = Components}};
        _Other->
            #newt{}
    end,
    Ts2 = case NewDef of
    ...
    Ts2;
check_ptype(_S,_PTDef,Ts)
    when is_record(Ts,objectclass)->
...
check_type(_S,Type,Ts)
    when is_record(Type,typedef),
    Type#typedef.checked == idle->
    Ts;
check_type(S = #state{recordtopname = TopName},Type,Ts)
    when is_record(Ts,type)->
    {Def,Tag,Constr,IsInlined} = case match_parameter(S,Ts#type.def) of
    ...
    TempNewDef = #newt{type = Def,tag = Tag,constraint = Constr,inlined = IsInlined},
    TestFun = fun (Tref)->
    ...
    NewDef = case Def of
        Ext
            when is_record(Ext,'Externaltypereference')->
            {RefMod,RefTypeDef,IsParamDef} = case get_referenced_type(S,Ext) of
                {undefined,TmpTDef}->
            ...
            Ct = TestFun(Ext),
            {RefType,ExtRef} = case RefTypeDef#typedef.checked of
                true->
                    {RefTypeDef#typedef.typespec,Ext};
                _->
                    NewRefTypeDef1 = RefTypeDef#typedef{checked = idle},
                    asn1_db:dbput(RefMod,get_datastr_name(NewRefTypeDef1),NewRefTypeDef1),
                    NewS = S#state{mname = RefMod,module = load_asn1_module(S,RefMod),tname = get_datastr_name(NewRefTypeDef1),abscomppath = [],recordtopname = []},
                    RefType1 = check_type(NewS,RefTypeDef,RefTypeDef#typedef.typespec),
                    NewRefTypeDef2 = RefTypeDef#typedef{checked = true,typespec = RefType1},
                    ...
                    case {RefMod == get(top_module),IsParamDef} of
                        {true,true}->
                            ...
                            asn1ct_gen:insert_once(parameterized_objects,Key);
                        _->
                            ok
                    end,
                    Pos = Ext#'Externaltypereference'.pos,
                    {RefType1,#'Externaltypereference'{module = RefMod,pos = Pos,type = TmpName}}
            end,
            case asn1ct_gen:prim_bif(asn1ct_gen:get_inner(RefType#type.def)) of
                true->
                    NewC = check_constraints(S,RefType,Constr ++ RefType#type.constraint),
                    TempNewDef#newt{type = RefType#type.def,tag = merge_tags(Ct,RefType#type.tag),constraint = NewC};
                _->
                    NewExt = ExtRef#'Externaltypereference'{module = merged_mod(S,RefMod,Ext)},
                    TempNewDef#newt{type = check_externaltypereference(S,NewExt),tag = merge_tags(Ct,RefType#type.tag)}
            end;
        'ANY'->
        ...
        {'ANY_DEFINED_BY',_}->
            ...
            TempNewDef#newt{type = 'ASN1_OPEN_TYPE',tag = Ct};
        'INTEGER'->
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 2,type = 'IMPLICIT',form = 0})};
        {'INTEGER',NamedNumberList}->
            TempNewDef#newt{type = {'INTEGER',check_integer(S,NamedNumberList)},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 2,type = 'IMPLICIT',form = 0})};
        'REAL'->
            check_real(S,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 9,type = 'IMPLICIT',form = 0})};
        {'BIT STRING',NamedNumberList}->
            NewL = check_bitstring(S,NamedNumberList),
            TempNewDef#newt{type = {'BIT STRING',NewL},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 3,type = 'IMPLICIT',form = 0})};
        'NULL'->
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 5,type = 'IMPLICIT',form = 0})};
        'OBJECT IDENTIFIER'->
            check_objectidentifier(S,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 6,type = 'IMPLICIT',form = 0})};
        'ObjectDescriptor'->
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 7,type = 'IMPLICIT',form = 0})};
        'EXTERNAL'->
            put_once(external,unchecked),
            TempNewDef#newt{type = #'Externaltypereference'{module = S#state.mname,type = 'EXTERNAL'},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 8,type = 'IMPLICIT',form = 32})};
        {'INSTANCE OF',DefinedObjectClass,Constraint}->
            IOFDef = check_instance_of(S,DefinedObjectClass,Constraint),
            TempNewDef#newt{type = IOFDef,tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 8,type = 'IMPLICIT',form = 32})};
        {'ENUMERATED',NamedNumberList}->
            TempNewDef#newt{type = {'ENUMERATED',check_enumerated(S,NamedNumberList)},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 10,type = 'IMPLICIT',form = 0}),constraint = []};
        'EMBEDDED PDV'->
            put_once(embedded_pdv,unchecked),
            TempNewDef#newt{type = #'Externaltypereference'{module = S#state.mname,type = 'EMBEDDED PDV'},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 11,type = 'IMPLICIT',form = 32})};
        'BOOLEAN'->
            check_boolean(S,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 1,type = 'IMPLICIT',form = 0})};
        'OCTET STRING'->
            check_octetstring(S,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 4,type = 'IMPLICIT',form = 0})};
        'NumericString'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 18,type = 'IMPLICIT',form = 0})};
        TString
            when TString =:= 'TeletexString';
            TString =:= 'T61String'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 20,type = 'IMPLICIT',form = 0})};
        'VideotexString'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 21,type = 'IMPLICIT',form = 0})};
        'UTCTime'->
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 23,type = 'IMPLICIT',form = 0})};
        'GeneralizedTime'->
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 24,type = 'IMPLICIT',form = 0})};
        'GraphicString'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 25,type = 'IMPLICIT',form = 0})};
        'VisibleString'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 26,type = 'IMPLICIT',form = 0})};
        'GeneralString'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 27,type = 'IMPLICIT',form = 0})};
        'PrintableString'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 19,type = 'IMPLICIT',form = 0})};
        'IA5String'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 22,type = 'IMPLICIT',form = 0})};
        'BMPString'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 30,type = 'IMPLICIT',form = 0})};
        'UniversalString'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 28,type = 'IMPLICIT',form = 0})};
        'UTF8String'->
            check_restrictedstring(S,Def,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 12,type = 'IMPLICIT',form = 0})};
        'RELATIVE-OID'->
            check_relative_oid(S,Constr),
            TempNewDef#newt{tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 13,type = 'IMPLICIT',form = 0})};
        'CHARACTER STRING'->
            put_once(character_string,unchecked),
            TempNewDef#newt{type = #'Externaltypereference'{module = S#state.mname,type = 'CHARACTER STRING'},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 29,type = 'IMPLICIT',form = 32})};
        Seq
            when is_record(Seq,'SEQUENCE')->
            RecordName = case TopName of
                []->
                    [get_datastr_name(Type)];
                _->
                    TopName
            end,
            {TableCInf,Components} = check_sequence(S#state{recordtopname = RecordName},Type,Seq#'SEQUENCE'.components),
            TempNewDef#newt{type = Seq#'SEQUENCE'{tablecinf = tablecinf_choose(Seq,TableCInf),components = Components},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 16,type = 'IMPLICIT',form = 32})};
        {'SEQUENCE OF',Components}->
            TempNewDef#newt{type = {'SEQUENCE OF',check_sequenceof(S,Type,Components)},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 16,type = 'IMPLICIT',form = 32})};
        {'CHOICE',_} = Choice->
        ...
        Set
            ...
            {Sorted,TableCInf,Components} = check_set(S#state{recordtopname = RecordName},Type,Set#'SET'.components),
            TempNewDef#newt{type = Set#'SET'{sorted = Sorted,tablecinf = tablecinf_choose(Set,TableCInf),components = Components},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 17,type = 'IMPLICIT',form = 32})};
        {'SET OF',Components}->
            TempNewDef#newt{type = {'SET OF',check_setof(S,Type,Components)},tag = merge_tags(Tag,#tag{class = 'UNIVERSAL',number = 17,type = 'IMPLICIT',form = 32})};
        {pt,Ptype,ParaList}->
            ...
            TempNewDef#newt{type = Instance#type.def,tag = merge_tags(Tag,Instance#type.tag),constraint = Instance#type.constraint,inlined = yes};
        #'ObjectClassFieldType'{classname = ClRef0} = OCFT0->
            ...
            NewTypeDef = maybe_open_type(S,ClassSpec,OCFT#'ObjectClassFieldType'{class = ClassSpec},Constr),
            InnerTag = get_innertag(S,NewTypeDef),
            MergedTag = merge_tags(Tag,InnerTag),
            Ct = case is_open_type(NewTypeDef) of
                true->
            ...
            case TopName of
                []
                    ...
                    #type{constraint = C,def = Simplified} = simplify_type(#type{def = NewTypeDef,constraint = Constr}),
                    TempNewDef#newt{type = Simplified,tag = Ct,constraint = C};
                _->
        ...
        {'TypeFromObject',{object,Object},TypeField}->
            ...
            TempNewDef#newt{tag = merge_tags(Tag,CheckedT#type.tag),type = CheckedT#type.def};
    ...
    Ts#type{def = TDef,inlined = Inlined,constraint = check_constraints(S,#type{def = TDef},NewConstr),tag = lists:map(fun (#tag{type = {default,TTx}} = TempTag)->
        TempTag#tag{type = TTx};(Other)->
        Other end,NewTags)}.
...
get_innertag(_S,#'ObjectClassFieldType'{type = Type}) ->
    case Type of
        {fixedtypevaluefield,_,#type{tag = Tag}}->
            Tag;
        {TypeFieldName,_}
            when is_atom(TypeFieldName)->
            [];
        _->
            []
    end.
...
maybe_illicit_implicit_tag(S,Kind,Tag) ->
    case Tag of
        ...
        [ChTag = #tag{type = {default,_}}| T]->
            case Kind of
                open_type->
                    [ChTag#tag{type = 'EXPLICIT',form = 32}| T];
                choice->
                    [ChTag#tag{type = 'EXPLICIT',form = 32}| T]
            end;
        _->
...
maybe_open_type(_,_,#'ObjectClassFieldType'{fieldname = {_,_}} = OCFT,_) ->
    OCFT;
maybe_open_type(S,#objectclass{fields = Fs} = ClassSpec,#'ObjectClassFieldType'{fieldname = FieldRefList} = OCFT,Constr) ->
    ...
    case lists:last(FieldRefList) of
        {valuefieldreference,_}->
            OCFT#'ObjectClassFieldType'{fieldname = FieldNames,type = Type};
        {typefieldreference,_}->
            case {get_unique_fieldname(S,#classdef{typespec = ClassSpec}),get_componentrelation(Constr)} of
                {no_unique,_}->
                    OCFT#'ObjectClassFieldType'{fieldname = FieldNames,type = 'ASN1_OPEN_TYPE'};
                {_,no}->
                    OCFT#'ObjectClassFieldType'{fieldname = FieldNames,type = 'ASN1_OPEN_TYPE'};
                _->
                    OCFT#'ObjectClassFieldType'{fieldname = FieldNames,type = Type}
...
match_args(S,FA = [FormArg| Ft],AA = [ActArg| At],Acc) ->
    ...
    case categorize_arg(S,FormArg,ActArg) of
        [CategorizedArg]->
            match_args(S#state{parameters = [{FormArg,CategorizedArg}| OldParams]},Ft,At,[{FormArg,CategorizedArg}| Acc]);
        CategorizedArgs->
            match_args(S#state{parameters = CategorizedArgs ++ OldParams},FA,CategorizedArgs ++ AA,Acc)
    end;
match_args(_S,[],[],Acc) ->
    lists:reverse(Acc);
match_args(S,_,_,_) ->
...
categorize({object,_,Type}) ->
    Def = fun (D = #type{})->
        #typedef{name = new_reference_name("type_argument"),typespec = D#type{inlined = yes}};({setting,_,Eref})
        when is_record(Eref,'Externaltypereference')->
        Eref;(D)->
        D end,
    [(Def(X)) || X <- Type];
categorize(#type{} = Def) ->
    [#typedef{name = new_reference_name("type_argument"),typespec = Def#type{inlined = yes}}];
categorize(Def) ->
    [Def].

categorize(S,object_set,Def,ClassRef) ->
    NewObjSetSpec = check_object(S,Def,#'ObjectSet'{class = ClassRef,set = parse_objectset(Def)}),
...
categorize(_S,object,Def,_ClassRef) ->
    [Def];
categorize(_S,value,_Type,Value)
    when is_record(Value,valuedef)->
    [Value];
...
coalesce_constraints({element_set,{Tag,{element_set,A,_}},{Tag,{element_set,B,_}}}) ->
...
remove_extension(Tuple)
    when is_tuple(Tuple)->
    L = [(remove_extension(El)) || El <- tuple_to_list(Tuple)],
    list_to_tuple(L);
remove_extension(Other) ->
    Other.

any_extension({element_set,_,Ext})
    when Ext =/= none->
    true;
any_extension(Tuple)
    when is_tuple(Tuple)->
...
simplify_element_set(_S,_HostType,empty) ->
    {set,[]};
simplify_element_set(S,HostType,{'SingleValue',Vs0})
    when is_list(Vs0)->
        ...
        #'Externalvaluereference'{}->
            ...
            Set = case TorVDef of
                #typedef{typespec = #'Object'{classname = ClassName}}->
                    #'ObjectSet'{class = ClassName,set = {'SingleValue',C}};
                #valuedef{type = #type{def = ClassDef},value = #'Externalvaluereference'{} = Obj}->
                    #'ObjectSet'{class = ClassDef,set = {'SingleValue',Obj}}
            end,
...
range_union_1([{a_range,Ub0}, {range,Lb1,Ub1}| T])
    when Lb1 - 1 =< Ub0->
    range_union_1([{a_range,max(Ub0,Ub1)}| T]);
range_union_1([{a_range,_} = H| T]) ->
    [H| range_union_1(T)];
range_union_1([{range,Lb0,Ub0}, {range,Lb1,Ub1}| T])
    when Lb1 - 1 =< Ub0->
...
finish_constraints_1([{element_set,{Tag,{element_set,_,_} = Set0},none}| T],Collapse0) ->
...
single_value_collapse_1([{range,Lb,Ub}| T])
    when is_integer(Lb),
    is_integer(Ub)->
...
resolve_value1(_,_,#valuedef{checked = true,value = V}) ->
    V;
resolve_value1(S,_,#valuedef{value = {'ValueFromObject',{object,Object},FieldName}}) ->
    get_value_from_object(S,Object,FieldName);
...
resolve_namednumber_1(S,Name,NameList) ->
    try NamedNumberList = check_enumerated(S,NameList),
...
check_imported(S,Imodule,Name,IsParsed) ->
    case asn1_db:dbget(Imodule,'MODULE') of
        undefined
            when IsParsed == true->
            ErrStr = io_lib:format("Type ~s imported from non existing modul" "e ~s~n",[Name, Imodule]),
            error({imported,ErrStr,S});
        undefined->
            parse_and_save(S,Imodule),
            check_imported(S,Imodule,Name,true);
        Im
            when is_record(Im,module)->
            case is_exported(Im,Name) of
                false->
                    ErrStr = io_lib:format("Imported type ~s not exported fr" "om module ~s~n",[Name, Imodule]),
                    error({imported,ErrStr,S});
                _->
                    ok
            end
...
is_exported(Module,Name)
    when is_record(Module,module)->
        ...
        undefined->
            case imported(S,Name) of
                ...
                _->
                    ...
                    #'Externaltypereference'{pos = NewPos,module = M,type = NewName}
            end;
        _->
...
do_get_ref_type_1(S,#'Externaltypereference'{pos = P,module = M,type = T}) ->
    do_get_ref_type_2(S,P,M,T);
do_get_ref_type_1(S,#'Externalvaluereference'{pos = P,module = M,value = V}) ->
    do_get_ref_type_2(S,P,M,V);
do_get_ref_type_1(_,T) ->
    {undefined,T}.

do_get_ref_type_2(#state{mname = Current,inputmodules = Modules} = S,Pos,M,T) ->
            ...
            case asn1_db:dbget(Imodule,'MODULE') of
                ...
                Im
                    ...
                    case is_exported(Im,Name) of
                        ...
                        _->
                            no_debug,
                            get_referenced_type(S,#'Externaltypereference'{module = Imodule,type = Name,pos = Pos})
                    end
            end;
        _->
...
match_parameter(_S,#'Externaltypereference'{type = Name},[{#'Externaltypereference'{type = Name},NewName}| _T]) ->
    NewName;
match_parameter(_S,#'Externaltypereference'{type = Name},[{{_,#'Externaltypereference'{type = Name}},NewName}| _T]) ->
    NewName;
match_parameter(_S,#'Externalvaluereference'{value = Name},[{#'Externalvaluereference'{value = Name},NewName}| _T]) ->
    NewName;
match_parameter(_S,#'Externalvaluereference'{value = Name},[{{_,#'Externalvaluereference'{value = Name}},NewName}| _T]) ->
    NewName;
match_parameter(_S,#type{def = #'Externaltypereference'{module = M,type = Name}},[{#'Externaltypereference'{module = M,type = Name},Type}]) ->
    Type;
match_parameter(_S,{valueset,#type{def = #'Externaltypereference'{type = Name}}},[{{_,#'Externaltypereference'{type = Name}},{valueset,#type{def = NewName}}}| _T]) ->
    NewName;
match_parameter(_S,{valueset,#type{def = #'Externaltypereference'{type = Name}}},[{{_,#'Externaltypereference'{type = Name}},NewName = #type{def = #'Externaltypereference'{}}}| _T]) ->
    NewName#type.def;
match_parameter(_S,{valueset,#type{def = #'Externaltypereference'{type = Name}}},[{{_,#'Externaltypereference'{type = Name}},NewName}| _T]) ->
    NewName;
    ...
    try check_type(S,#typedef{name = S#state.tname,typespec = T},T) of 
        #type{def = Ts}->
            ...
            throw:pobjectsetdef->
                ...
                ClassRef = case ObjDef of
                    #pvaluesetdef{type = TDef}->
                        TDef#type.def;
                    #pobjectsetdef{class = ClRef}->
                        ClRef
                end,
                {HomeMod,_} = get_referenced_type(S,ClassRef),
                RightClassRef = ClassRef#'Externaltypereference'{module = HomeMod},
                ...
                save_object_set_instance(S,Name,ObjSpec) end;
...
imported1(Name,[#'SymbolsFromModule'{symbols = Symlist,module = #'Externaltypereference'{type = ModuleName}}| T]) ->
...
check_bitstring(S,NNL0) ->
    ...
    _ = [(asn1_error(S,{invalid_bit_number,Bit})) || {_,Bit} <- NNL,Bit < 0],
    NNL.
...
iof_associated_type(S,[]) ->
    case get(instance_of) of
        undefined->
            ...
            Tag = [#tag{class = 'UNIVERSAL',number = 8,type = 'IMPLICIT',form = 32}],
            TypeDef = #typedef{checked = true,name = 'INSTANCE OF',typespec = #type{tag = Tag,def = AssociateSeq}},
...
iof_associated_type1(S,C) ->
    ...
    C1TypeTag = [#tag{class = 'UNIVERSAL',number = 6,type = 'IMPLICIT',form = 0}],
...
check_enum_number_root([Id| T] = T0,Used0,Cnt,Acc)
    when is_atom(Id)->
...
check_enum_ext(S,[{'NamedNumber',Id,N}| T],Used0,C,Acc) ->
    Used = check_enum_update_used(S,Id,N,Used0),
    if N < C ->
        asn1_error(S,{enum_not_ascending,Id,N,C - 1});true ->
        ok end,
    check_enum_ext(S,T,Used,N + 1,[{Id,N}| Acc]);
check_enum_ext(S,[Id| T] = T0,Used0,C,Acc)
    when is_atom(Id)->
...
check_sequence(S,Type,Comps) ->
    ...
    case check_unique([C || C <- Components,is_record(C,'ComponentType')],#'ComponentType'.name) of
        []->
...
check_unique_sequence_tags(S,CompList) ->
    TagComps = case complist_as_tuple(CompList) of
        {R1,Ext,R2}->
            R1 ++ [(C#'ComponentType'{prop = 'OPTIONAL'}) || C = #'ComponentType'{} <- Ext] ++ R2;
        {R1,Ext}->
            R1 ++ [(C#'ComponentType'{prop = 'OPTIONAL'}) || C = #'ComponentType'{} <- Ext];
        _->
            CompList
    end,
...
check_unique_sequence_tags1(S,[C| Rest],Acc)
    when is_record(C,'ComponentType')->
...
check_sequenceof(S,Type,Component)
    when is_record(Component,type)->
...
tag_key({'UNIVERSAL',Tag}) ->
    {0,Tag};
tag_key({'APPLICATION',Tag}) ->
    {1,Tag};
tag_key({'CONTEXT',Tag}) ->
    {2,Tag};
tag_key({'PRIVATE',Tag}) ->
...
untagged_choice(S,[#'ComponentType'{typespec = #type{tag = [],def = ExRef}}| Rest])
    ...
    case get_referenced_type(S,ExRef) of
        {_,#typedef{typespec = #type{tag = [],def = {'CHOICE',_}}}}->
            true;
        _->
...
tag_untagged_choice(S,[C = #'ComponentType'{typespec = #type{tag = [],def = ExRef}}| Rest],Acc)
    when is_record(ExRef,'Externaltypereference')->
    case get_referenced_type(S,ExRef) of
        {_,#typedef{typespec = #type{tag = [],def = {'CHOICE',_}}}}->
            TagList = C#'ComponentType'.tags,
...
get_least_tag(TagList) ->
    Pred = fun ({'PRIVATE',_},{'CONTEXT',_})->
        true;({'CONTEXT',_},{'APPLICATION',_})->
        true;({'APPLICATION',_},{'UNIVERSAL',_})->
        true;({A,T1},{A,T2})
        when T1 =< T2->
        true;(_,_)->
        false end,
...
textual_order(Cs) ->
    Fun = fun (C = #'ComponentType'{},Index)->
        {C#'ComponentType'{textual_order = Index},Index + 1};(Other,Index)->
        {Other,Index} end,
    {NewCs,_} = textual_order(Cs,Fun,1),
    NewCs.

textual_order(Cs,Fun,IxIn)
    when is_list(Cs)->
...
extension({Root,ExtList}) ->
    ToOpt = fun (mandatory)->
        'OPTIONAL';(X)->
        X end,
    {Root,[(X#'ComponentType'{prop = ToOpt(Y)}) || X = #'ComponentType'{prop = Y} <- ExtList],[]};
extension({Root1,ExtList,Root2}) ->
    ToOpt = fun (mandatory)->
        'OPTIONAL';(X)->
        X end,
    {Root1,[(X#'ComponentType'{prop = ToOpt(Y)}) || X = #'ComponentType'{prop = Y} <- ExtList],Root2}.

check_setof(S,Type,Component)
    when is_record(Component,type)->
    simplify_type(check_type(S,Type,Component)).
...
check_selectiontype(S,Name,#type{def = Eref})
    ...
    NewS = S#state{module = load_asn1_module(S,RefMod),mname = RefMod,tname = get_datastr_name(TypeDef)},
...
check_selectiontype2(S,Name,TypeDef) ->
    ...
    Components = try CheckedType = check_type(NewS,TypeDef,TypeDef#typedef.typespec),
    get_choice_components(S,CheckedType#type.def)
        catch
            error:_->
                asn1_error(S,{illegal_choice_type,error_value(TypeDef)}) end,
    case lists:keyfind(Name,#'ComponentType'.name,Components) of
        #'ComponentType'{typespec = TS}->
            TS;
        false->
            asn1_error(S,{illegal_id,error_value(Name)})
    end.

get_choice_components(_S,{'CHOICE',Components})
    when is_list(Components)->
    Components;
get_choice_components(_S,{'CHOICE',{C1,C2}})
    when is_list(C1),
    is_list(C2)->
    C1 ++ C2;
...
check_choice(S,Type,Components)
    when is_list(Components)->
    ...
    case check_unique(Components1,#'ComponentType'.name) of
        []->
            ...
            NewComps2 = lists:filter(fun (#'ExtensionAdditionGroup'{})->
...
generate_automatic_tags1([H| T],[TagNo| TagNos])
    when is_record(H,'ComponentType')->
    #'ComponentType'{typespec = Ts} = H,
    NewTs = Ts#type{tag = [#tag{class = 'CONTEXT',number = TagNo,type = {default,'IMPLICIT'},form = 0}]},
...
generate_automatic_tags1([H| T],TagList) ->
...
check_unique2([A, B| T],Pos,Acc)
    when element(Pos,A) == element(Pos,B)->
...
check_each_component2(S = #state{abscomppath = Path,recordtopname = TopName},Type,[C = #'ComponentType'{name = Cname,typespec = Ts,prop = Prop}| Ct],Acc) ->
    NewAbsCPath = case Ts#type.def of
        #'Externaltypereference'{}->
            [];
        _->
            [Cname| Path]
    end,
    CheckedTs = check_type(S#state{abscomppath = NewAbsCPath,recordtopname = [Cname| TopName]},Type,Ts),
    NewTags = get_taglist(S,CheckedTs),
    NewProp = case normalize_value(S,CheckedTs,Prop,[Cname| TopName]) of
        mandatory->
            mandatory;
        'OPTIONAL'->
            'OPTIONAL';
...
check_each_alternative2(S = #state{abscomppath = Path,recordtopname = TopName},Type,[C = #'ComponentType'{name = Cname,typespec = Ts}| Ct],Acc) ->
    NewAbsCPath = case Ts#type.def of
        #'Externaltypereference'{}->
            [];
        _->
...
componentrelation_leadingattr(S,[C = #'ComponentType'{}| Cs],CompList,Acc,CompAcc) ->
    {LAAcc,NewC} = case  catch componentrelation1(S,C#'ComponentType'.typespec,[C#'ComponentType'.name]) of
        {'EXIT',_}->
            {[],C};
        {CRI = [{_A1,_B1,_C1,_D1}| _Rest],NewTSpec}->
            case leading_attr_index(S,CompList,CRI,lists:reverse(S#state.abscomppath),[]) of
                []->
                    {[],C};
                [{ObjSet,Attr,N,ClassDef,_Path,ValueIndex}| _NewRest]->
                    OS = object_set_mod_name(S,ObjSet),
                    UniqFN = get_unique_fieldname(S,#classdef{typespec = ClassDef}),
                    Res = #simpletableattributes{objectsetname = OS,c_name = Attr,c_index = N,usedclassfield = UniqFN,uniqueclassfield = UniqFN,valueindex = ValueIndex},
...
object_set_mod_name(_S,ObjSet)
    when is_atom(ObjSet)->
    ObjSet;
object_set_mod_name(#state{mname = M},#'Externaltypereference'{module = M,type = T}) ->
    {M,T};
...
get_simple_table_info1(S,Cs,[Cname| Cnames],Path) ->
    #'ComponentType'{} = C = lists:keyfind(Cname,#'ComponentType'.name,Cs),
...
simple_table_info(S,#'ObjectClassFieldType'{classname = ClRef,class = ObjectClass,fieldname = FieldName},Path) ->
    ObjectClassFieldName = case FieldName of
        {LastFieldName,[]}->
            LastFieldName;
        {_FirstFieldName,FieldNames}->
            lists:last(FieldNames)
    end,
    ClassDef = case ObjectClass of
        []->
            {_,CDef} = get_referenced_type(S,ClRef),
            CDef;
        _->
            #classdef{typespec = ObjectClass}
    end,
...
any_component_relation(S,[#'ComponentType'{name = CName,typespec = Type}| Cs],CNames,NamePath,Acc) ->
    CRelPath = case lists:keyfind(componentrelation,1,Type#type.constraint) of
        {_,_,AtNotation}->
    ...
    InnerAcc = case {Type#type.inlined,asn1ct_gen:type(asn1ct_gen:get_inner(Type#type.def))} of
        {no,{constructed,bif}}->
            {InnerCs,NewNamePath} = case get_components(Type#type.def) of
                T
                    when is_record(T,type)->
                    {T,NamePath};
                IC->
...
any_component_relation(S,Type,CNames,NamePath,Acc)
...
evaluate_atpath(S = #state{abscomppath = TopPath},NamePath,Cnames,{outermost,AtPath = [_Ref| _Refs]}) ->
    AtPathBelowTop = case TopPath of
        []->
            AtPath;
        _->
...
componentrelation1(S,C = #type{def = Def,constraint = Constraint,tablecinf = TCI},Path) ->
    Ret = case lists:keyfind(componentrelation,1,Constraint) of
        {_,{_,_,ObjectSet},AtList}->
            [{_,AL = [#'Externalvaluereference'{}| _R1]}| _R2] = AtList,
            ClassDef = get_ObjectClassFieldType_classdef(S,Def),
            AtPath = lists:map(fun (#'Externalvaluereference'{value = V})->
                V end,AL),
...
componentlist_comprel(S,[C = #'ComponentType'{name = Name,typespec = Type}| Cs],Acc,Path,NewCL) ->
    case  catch componentrelation1(S,Type,Path ++ [Name]) of
        ...
        nofunobj->
            componentlist_comprel(S,Cs,Acc,Path,[C| NewCL]);
        {CRelInf,NewType}->
            componentlist_comprel(S,Cs,CRelInf ++ Acc,Path,[C#'ComponentType'{typespec = NewType}| NewCL])
    end;
...
innertype_comprel1(S,T = #type{def = Def,constraint = Cons,tablecinf = TCI},Path) ->
    Ret = case lists:keyfind(componentrelation,1,Cons) of
        {_,{_,_,ObjectSet},AtList}->
            [{_,AL = [#'Externalvaluereference'{value = _Attr}| _R1]}| _R2] = AtList,
            ClassDef = get_ObjectClassFieldType_classdef(S,Def),
            AtPath = lists:map(fun (#'Externalvaluereference'{value = V})->
                V end,AL),
...
leading_attr_index(S,Cs,[H = {_,AttrP,_,_}| T],AbsP,Acc) ->
    AttrInfo = case lists:prefix(AbsP,AttrP) of
        true->
...
leading_attr_index1(_,[],_,_,_) ->
    0;
leading_attr_index1(S,[C| Cs],Arg = {ObjectSet,_,CDef,P},AttrInfo = {Attr,SubAttr},N) ->
...
value_match(S,#'ComponentType'{typespec = Type},Name,[At| Ats],Acc) ->
    ...
    Components = case get_atlist_components(Type#type.def) of
        []->
...
get_tableconstraint_info(S,Type,{CheckedTs,EComps,CheckedTs2}) ->
    {get_tableconstraint_info(S,Type,CheckedTs,[]),get_tableconstraint_info(S,Type,EComps,[]),get_tableconstraint_info(S,Type,CheckedTs2,[])};
get_tableconstraint_info(S,Type,{CheckedTs,EComps}) ->
    {get_tableconstraint_info(S,Type,CheckedTs,[]),get_tableconstraint_info(S,Type,EComps,[])};
...
get_tableconstraint_info(S,Type,[C = #'ComponentType'{typespec = CheckedTs}| Cs],Acc) ->
    AccComp = case CheckedTs#type.def of
        OCFT = #'ObjectClassFieldType'{}->
            NewOCFT = OCFT#'ObjectClassFieldType'{class = []},
            C#'ComponentType'{typespec = CheckedTs#type{def = NewOCFT}};
        {'SEQUENCE OF',SOType}
            when is_record(SOType,type),
            ...
            C#'ComponentType'{typespec = CheckedTs#type{def = {'SEQUENCE OF',SOType#type{def = {'CHOICE',NewInnerCList}}}}};
        {'SET OF',SOType}
            when is_record(SOType,type),
            element(1,SOType#type.def) == 'CHOICE'->
            CTypeList = element(2,SOType#type.def),
            NewInnerCList = get_tableconstraint_info(S,Type,CTypeList),
            C#'ComponentType'{typespec = CheckedTs#type{def = {'SET OF',SOType#type{def = {'CHOICE',NewInnerCList}}}}};
        _->
            C
    end,
...
get_OCFType(S,Fields,[PrimFieldName| Rest]) ->
    case lists:keysearch(PrimFieldName,2,Fields) of
        ...
        {value,{objectfield,_,ClassRef,_Unique,_OptSpec}}->
            ...
            NewS = update_state(S#state{tname = get_datastr_name(ClassDef)},MName),
...
findtypes_and_values([H| T],Tacc,Vacc,Pacc,Cacc,Oacc,OSacc)
    ...
    is_record(H#typedef.typespec,'Object')->
    findtypes_and_values(T,Tacc,Vacc,Pacc,Cacc,[H#typedef.name| Oacc],OSacc);
findtypes_and_values([H| T],Tacc,Vacc,Pacc,Cacc,Oacc,OSacc)
    when is_record(H,typedef),
    is_record(H#typedef.typespec,'ObjectSet')->
...
findtypes_and_values([],Tacc,Vacc,Pacc,Cacc,Oacc,OSacc) ->
    {lists:reverse(Tacc),lists:reverse(Vacc),lists:reverse(Pacc),lists:reverse(Cacc),lists:reverse(Oacc),lists:reverse(OSacc)}.
...
format_error({enum_not_ascending,Id,N,Prev}) ->
    io_lib:format("the values for enumerations which follow '...' must " "be in ascending order, but '~p(~p)' is less than the" " previous value '~p'",[Id, N, Prev]);
...
format_error({illegal_class_name,Class}) ->
    io_lib:format("the class name '~s' is illegal (it must start with a" "n uppercase letter and only contain uppercase letter" "s, digits, or hyphens)",[Class]);
...
format_error({illegal_instance_of,Class}) ->
    io_lib:format("using INSTANCE OF on class '~s' is illegal, because " "INSTANCE OF may only be used on the class TYPE-IDENT" "IFIER",[Class]);
...
format_error({implicit_tag_before,Kind}) ->
    "illegal implicit tag before " ++ case Kind of
        choice->
            "'CHOICE'";
        open_type->
            "open type"
...
format_error({missing_table_constraint,Component}) ->
    io_lib:format("the component '~s' is referenced by a component rela" "tion constraint using the '@field-name' notation, bu" "t does not have a table constraint",[Component]);
...
format_error(multiple_uniqs) ->
    "implementation limitation: only one UNIQUE field is allowed in CLA" "SS";
...
format_error({syntax_mandatory_in_optional_group,Name}) ->
    io_lib:format("the field '&~s' must not be within an optional group" " since it is not optional",[Name]);
...
format_error({syntax_nomatch,Actual}) ->
    io_lib:format("~s is not the next item allowed according to the def" "ined syntax",[Actual]);
...
format_error({unique_and_default,Field}) ->
    io_lib:format("the field '&~s' must not have both 'UNIQUE' and 'DEF" "AULT'",[Field]);
...
format_fields([H| T]) ->
    [io_lib:format("fields '&~s'",[H])| [(io_lib:format(", '&~s'",[F])) || F <- T]].
...
include_default_type1(Module,[{Name,TS}| Rest]) ->
    case asn1_db:dbget(Module,Name) of
        undefined->
            T = #typedef{name = Name,typespec = TS},
...
default_type_list() ->
    ...
    Abstract = #'ComponentType'{name = abstract,typespec = #type{def = 'OBJECT IDENTIFIER'},prop = mandatory},
    Transfer = #'ComponentType'{name = transfer,typespec = #type{def = 'OBJECT IDENTIFIER'},prop = mandatory},
    AbstractTrSeq = #'SEQUENCE'{components = [Abstract, Transfer]},
    Syntaxes = #'ComponentType'{name = syntaxes,typespec = #type{def = AbstractTrSeq},prop = mandatory},
    Fixed = #'ComponentType'{name = fixed,typespec = #type{def = 'NULL'},prop = mandatory},
...
include_default_class(S,Module) ->
    _ = [(include_default_class1(S,Module,ClassDef)) || ClassDef <- default_class_list()],
    ok.

include_default_class1(S,Module,{Name,Ts0}) ->
    case asn1_db:dbget(Module,Name) of
        undefined->
            #objectclass{fields = Fields,syntax = {'WITH SYNTAX',Syntax0}} = Ts0,
            Syntax = preprocess_syntax(S,Syntax0,Fields),
            Ts = Ts0#objectclass{syntax = {preprocessed_syntax,Syntax}},
            C = #classdef{checked = true,module = Module,name = Name,typespec = Ts},
...
default_class_list() ->
    [{'TYPE-IDENTIFIER',#objectclass{fields = [{fixedtypevaluefield,id,#type{tag = [#tag{class = 'UNIVERSAL',number = 6,type = 'IMPLICIT',form = 0}],def = 'OBJECT IDENTIFIER'},'UNIQUE','MANDATORY'}, {typefield,'Type','MANDATORY'}],syntax = {'WITH SYNTAX',[{typefieldreference,'Type'}, 'IDENTIFIED', 'BY', {valuefieldreference,id}]}}}, {'ABSTRACT-SYNTAX',#objectclass{fields = [{fixedtypevaluefield,id,#type{tag = [#tag{class = 'UNIVERSAL',number = 6,type = 'IMPLICIT',form = 0}],def = 'OBJECT IDENTIFIER'},'UNIQUE','MANDATORY'}, {typefield,'Type','MANDATORY'}, {fixedtypevaluefield,property,#type{tag = [#tag{class = 'UNIVERSAL',number = 3,type = 'IMPLICIT',form = 0}],def = {'BIT STRING',[]}},undefined,{'DEFAULT',[0, 1, 0]}}],syntax = {'WITH SYNTAX',[{typefieldreference,'Type'}, 'IDENTIFIED', 'BY', {valuefieldreference,id}, ['HAS', 'PROPERTY', {valuefieldreference,property}]]}}}].
...
name_of_def(#'Externaltypereference'{type = N}) ->
    N;
name_of_def(#'Externalvaluereference'{value = N}) ->
    N.