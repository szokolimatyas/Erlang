...
-import(erl_types, [t_has_var/1, t_inf/2, t_is_equal/2, t_is_subtype/2, t_subtract/2, t_subtract_list/2, t_sup/1, t_sup/2, t_unify/2]).

-import(erl_types, [t_any/0, t_atom/0, t_atom_vals/1, t_binary/0, t_bitstr/0, t_bitstr/2, t_bitstr_concat/1, t_boolean/0, t_collect_vars/1, t_cons/2, t_cons_hd/1, t_cons_tl/1, t_float/0, t_from_range/2, t_from_term/1, t_fun/0, t_fun/2, t_fun_args/1, t_fun_range/1, t_integer/0, t_is_any/1, t_is_atom/1, t_is_any_atom/2, t_is_cons/1, t_is_float/1, t_is_fun/1, t_is_integer/1, t_non_neg_integer/0, t_is_list/1, t_is_nil/1, t_is_none/1, t_is_number/1, t_is_singleton/1, t_is_none_or_unit/1, t_limit/2, t_list/0, t_list/1, t_list_elements/1, t_nonempty_list/1, t_maybe_improper_list/0, t_module/0, t_number/0, t_number_vals/1, t_pid/0, t_port/0, t_product/1, t_reference/0, t_subst/2, t_timeout/0, t_tuple/0, t_tuple/1, t_var/1, t_var_name/1, t_none/0, t_unit/0, t_map/0, t_map/1, t_map_get/2, t_map_put/2]).
...
-file("dialyzer_typesig.erl", 58).

-type(dep()::integer()).

-type(deps()::ordsets:ordset(dep())).

-type(type_var()::erl_types:erl_type()).

-record(fun_var,{'fun'::fun((_) -> erl_types:erl_type()),deps::deps(),origin::integer()|undefined}).

-type(constr_op()::eq|sub).

-type(fvar_or_type()::#fun_var{}|erl_types:erl_type()).

-record(constraint,{lhs::erl_types:erl_type(),op::constr_op(),rhs::fvar_or_type(),deps::deps()}).

-type(constraint()::#constraint{}).

-type(mask()::ordsets:ordset(non_neg_integer())).

-record(constraint_list,{type::conj|disj,list::[constr()],deps::deps(),masks::#{dep() => mask()}|undefined,id::{list,dep()}|undefined}).
...
traverse(Tree,DefinedVars,State) ->
    ...
    case cerl:type(Tree) of
        ...
        apply->
            ...
            State4 = state__store_conj(mk_var(Tree),sub,t_fun_range(FunType),State3),
        ...
        binary->
            {State1,SegTypes} = traverse_list(cerl:binary_segments(Tree),DefinedVars,State),
            Type = mk_fun_var(fun (Map)->
                TmpSegTypes = lookup_type_list(SegTypes,Map),
                t_bitstr_concat(TmpSegTypes) end,SegTypes),
        ...
        bitstr->
            ...
            {State2,TypeConstr,BinValTypeConstr} = case cerl:bitstr_bitsize(Tree) of
                all->
                ...
                any->
                    ...
                    T2 = mk_fun_var(bitstr_constr(SizeType,UnitVal,match),[SizeType]),
                    {state__store_conj(SizeType,sub,t_non_neg_integer(),State1),T1,T2}
            end,
            ValTypeConstr = case cerl:concrete(cerl:bitstr_type(Tree)) of
                binary->
                    BinValTypeConstr;
                float->
                ...
                integer->
                    case state__is_in_match(State1) of
                        true->
                            ...
                            mk_fun_var(bitstr_val_constr(SizeType,UnitVal,Flags),[SizeType]);
                        false->
                            t_integer()
                    end;
                utf8->
            ...
            case cerl:is_literal(fold_literal_maybe_match(Tree,State)) of
                ...
                false->
                    ...
                    ConsType = mk_fun_var(fun (Map)->
                        t_cons(lookup_type(HdVar,Map),lookup_type(TlVar,Map)) end,[HdVar, TlVar]),
                    ...
                    State2 = state__store_conj_lists([HdVar, TlVar, ConsVar],sub,[HdType, TlType, ConsType],State1),
                    {State2,ConsVar}
            end;
        'fun'->
            ...
            FunFailType = case state__prop_domain(cerl_trees:get_label(Tree),State0) of
                error->
            ...
            TreeVar = mk_var(Tree),
            State2 = try State1 = case state__add_prop_constrs(Tree,State0) of
                not_called->
                    State0;
                PropState->
                    PropState
            ...
            state__store_conj(TreeVar,eq,t_fun(mk_var_list(Vars),BodyVar),BodyState)
                catch
                    throw:error->
                        state__store_conj(TreeVar,eq,FunFailType,State0) end,
            Cs = state__cs(State2),
        ...
        let->
            ...
            State2 = state__store_conj(t_product(mk_var_list(Vars)),eq,ArgVars,State1),
            DefinedVars1 = add_def_list(Vars,DefinedVars),
        ...
        module->
            ...
            FoldFun = fun (Fun,AccState)->
                {S,_} = traverse(Fun,DefinedVars1,state__new_constraint_context(AccState)),
                S end,
        ...
        primop->
            case cerl:atom_val(cerl:primop_name(Tree)) of
                ...
                recv_wait_timeout->
                    ...
                    case cerl:is_c_atom(Timeout) andalso cerl:atom_val(Timeout) =:= infinity of
                        true->
                            {State,t_none()};
                        false->
        ...
        tuple->
            ...
            {State2,TupleType} = case cerl:is_literal(fold_literal_maybe_match(Tree,State1)) of
                true->
                    {State,t_tuple(EVars)};
                false->
                    Fun = fun (Var,AccState)->
                        case t_has_var(Var) of
                            true->
                                ...
                                {NewVar,state__store_conj(Var,eq,NewVar,AccState1)};
                            false->
                                {Var,AccState}
                        end end,
        ...
        map->
            ...
            MapFoldFun = fun (Entry,AccState)->
                ...
                {AccState2,KeyVar} = traverse(cerl:map_pair_key(Entry),DefinedVars,AccState1),
                AccState3 = state__set_in_match(AccState2,state__is_in_match(AccState)),
                {AccState4,ValVar} = traverse(cerl:map_pair_val(Entry),DefinedVars,AccState3),
                {{KeyVar,ValVar},AccState4} end,
            {Pairs,State1} = lists:mapfoldl(MapFoldFun,State,Entries),
            {State2,ArgVar} = case state__is_in_match(State) of
                false->
                    traverse(cerl:map_arg(Tree),DefinedVars,State1);
                true->
                    {State1,t_map()}
            end,
            MapVar = mk_var(Tree),
            MapType = mk_fun_var(fun (Map)->
                lists:foldl(fun ({K,V},TypeAcc)->
                    t_map_put({lookup_type(K,Map),lookup_type(V,Map)},TypeAcc) end,t_inf(t_map(),lookup_type(ArgVar,Map)),Pairs) end,[ArgVar| lists:append([[K, V] || {K,V} <- Pairs])]),
            Fun = fun ({KeyVar,ValVar},{AccState,ShadowKeys})->
                TypeFun = fun (Map)->
                        ...
                        true->
                            ...
                            case t_is_none_or_unit(MT) of
                                true->
                                    t_none();
                                false->
                                    DisjointFromKeyType = fun (ShadowKey)->
                                        ST = t_inf(lookup_type(ShadowKey,Map),KeyType),
                                        t_is_none_or_unit(ST) end,
                ...
                {state__store_conj(ValVar,sub,ValType,AccState),[KeyVar| ShadowKeys]} end,
            {State3,_} = lists:foldr(Fun,{State2,[]},Pairs),
            State4 = case state__is_in_match(State) of
                true->
                    State3;
                false->
                    ArgFun = fun (Map)->
                        FoldFun = fun ({{KeyVar,_},Entry},{AccType,ShadowedKeys})->
                            OpTree = cerl:map_pair_op(Entry),
                            KeyType = lookup_type(KeyVar,Map),
                            AccType1 = case cerl:is_literal(OpTree) andalso cerl:concrete(OpTree) =:= exact of
                                true->
                            ...
                            {AccType1,t_sup(KeyType,ShadowedKeys)} end,
                        {ResType,_} = lists:foldl(FoldFun,{t_map(),t_none()},lists:zip(Pairs,Entries)),
                        ResType end,
...
handle_try(Tree,DefinedVars,State) ->
    ...
    {ArgBodyState,BodyVar} = try {State2,ArgVar} = traverse(Arg,DefinedVars,State1),
    DefinedVars1 = add_def_list(Vars,DefinedVars),
    {State3,BodyVar1} = traverse(Body,DefinedVars1,State2),
    State4 = state__store_conj(t_product(mk_var_list(Vars)),eq,ArgVar,State3),
    {State4,BodyVar1}
        catch
            throw:error->
                {State1,t_none()} end,
    State6 = state__new_constraint_context(ArgBodyState),
    {HandlerState,HandlerVar} = try DefinedVars2 = add_def_list([X || X <- EVars,cerl:is_c_var(X)],DefinedVars),
    ...
    case state__is_in_guard(State) of
        true->
            Conj1 = mk_conj_constraint_list([ArgBodyCs, mk_constraint(BodyVar,eq,TreeVar)]),
            Disj = mk_disj_constraint_list([Conj1, mk_constraint(HandlerVar,eq,TreeVar)]),
            ...
            {NewState2,TreeVar};
        false->
            {NewCs,ReturnVar} = case {t_is_none(BodyVar),t_is_none(HandlerVar)} of
                {false,false}->
                    Conj1 = mk_conj_constraint_list([ArgBodyCs, mk_constraint(TreeVar,eq,BodyVar)]),
                    Conj2 = mk_conj_constraint_list([HandlerCs, mk_constraint(TreeVar,eq,HandlerVar)]),
                    Disj = mk_disj_constraint_list([Conj1, Conj2]),
                    {Disj,TreeVar};
                {false,true}->
                    {mk_conj_constraint_list([ArgBodyCs, mk_constraint(TreeVar,eq,BodyVar)]),BodyVar};
                {true,false}->
                    {mk_conj_constraint_list([HandlerCs, mk_constraint(TreeVar,eq,HandlerVar)]),HandlerVar};
                {true,true}->
                    ok,
                    throw(error)
            end,
...
get_plt_constr(MFA,Dst,ArgVars,State) ->
    ...
    Contract = case lists:member(MFA,SCCMFAs) of
        true->
            none;
        false->
        ...
        none->
            case PltRes of
                ...
                {value,{PltRetType,PltArgTypes}}->
                    state__store_conj_lists([Dst| ArgVars],sub,[PltRetType| PltArgTypes],State)
            end;
            ...
            {RetType,ArgCs} = case PltRes of
                none->
                    {mk_fun_var(fun (Map)->
                        ...
                        get_contract_return(C,ArgTypes) end,ArgVars),GenArgs};
                {value,{PltRetType,PltArgTypes}}->
                    {mk_fun_var(fun (Map)->
                        ArgTypes = lookup_type_list(ArgVars,Map),
                        CRet = get_contract_return(C,ArgTypes),
                        t_inf(CRet,PltRetType) end,ArgVars),[(t_inf(X,Y)) || {X,Y} <- lists:zip(GenArgs,PltArgTypes)]}
            end,
...
handle_clauses(Clauses,TopVar,Arg,Action,DefinedVars,State) ->
    SubtrTypeList = if length(Clauses) > 15 ->
        overflow;true ->
        [] end,
    {State1,CList} = handle_clauses_1(Clauses,TopVar,Arg,DefinedVars,State,SubtrTypeList,[]),
    {NewCs,NewState} = case Action of
        none->
            if CList =:= [] ->
                throw(error);true ->
                {CList,State1} end;
        _->
            try {State2,ActionVar} = traverse(Action,DefinedVars,State1),
...
handle_clauses_1([Clause| Tail],TopVar,Arg,DefinedVars,State,SubtrTypes,Acc) ->
    State0 = state__new_constraint_context(State),
    Pats = cerl:clause_pats(Clause),
    Guard = cerl:clause_guard(Clause),
    Body = cerl:clause_body(Clause),
    NewSubtrTypes = case SubtrTypes =:= overflow of
        true->
            overflow;
        false->
            ordsets:add_element(get_safe_underapprox(Pats,Guard),SubtrTypes)
    end,
    try DefinedVars1 = add_def_from_tree_list(Pats,DefinedVars),
            ...
            case SubtrTypes =:= overflow of
                ...
                false->
                    SubtrPatVar = mk_fun_var(fun (Map)->
                        ...
                        t_subtract_list(TmpType,SubtrTypes) end,[Arg]),
                    state__store_conj(Arg,sub,SubtrPatVar,S)
            end
    end,
    State4 = handle_guard(Guard,DefinedVars1,State3),
    {State5,BodyVar} = traverse(Body,DefinedVars1,state__set_in_match(State4,false)),
    State6 = state__store_conj(TopVar,eq,BodyVar,State5),
    Cs = state__cs(State6),
    handle_clauses_1(Tail,TopVar,Arg,DefinedVars,State6,NewSubtrTypes,[Cs| Acc])
        catch
            throw:error->
                handle_clauses_1(Tail,TopVar,Arg,DefinedVars,State,NewSubtrTypes,Acc) end;
...
get_safe_underapprox(Pats,Guard) ->
    try Map1 = cerl_trees:fold(fun (X,Acc)->
        case cerl:is_c_var(X) of
            true->
                maps:put(cerl_trees:get_label(X),t_any(),Acc);
            false->
                Acc
        end end,maps:new(),cerl:c_values(Pats)),
        ...
        false->
            case cerl:is_c_var(Guard) of
                false->
                    Map2;
                true->
                    maps:put(cerl_trees:get_label(Guard),t_from_term(true),Map2)
            end
    end,
...
get_underapprox_from_guard(Tree,Map) ->
    ...
    case cerl:type(Tree) of
        call->
            case {cerl:concrete(cerl:call_module(Tree)),cerl:concrete(cerl:call_name(Tree)),length(cerl:call_args(Tree))} of
                        ...
                        error->
                            case MFA of
                                ...
                                {erlang,'and',2}->
                                    ...
                                    case (cerl:is_c_var(Arg1) orelse cerl:is_literal(Arg1)) andalso (cerl:is_c_var(Arg2) orelse cerl:is_literal(Arg2)) of
                                        true->
                                            {Arg1Type,_} = get_underapprox_from_guard(Arg1,Map),
                                            {Arg2Type,_} = get_underapprox_from_guard(Arg2,Map),
                                            case t_is_equal(True,Arg1Type) andalso t_is_equal(True,Arg2Type) of
                                                true->
                                                    {True,Map};
                                                false->
                                                    throw(dont_know)
        ...
        var->
            Type = case maps:find(cerl_trees:get_label(Tree),Map) of
                error->
                    throw(dont_know);
                {ok,T}->
                    T
        ...
        literal->
            case cerl:unfold_literal(Tree) of
                Tree->
                    Type = case cerl:concrete(Tree) of
                        Int
                            when is_integer(Int)->
                            t_from_term(Int);
                        Atom
...
get_type_test({erlang,is_atom,1}) ->
    {ok,t_atom()};
get_type_test({erlang,is_boolean,1}) ->
    {ok,t_boolean()};
get_type_test({erlang,is_binary,1}) ->
    {ok,t_binary()};
get_type_test({erlang,is_bitstring,1}) ->
    {ok,t_bitstr()};
get_type_test({erlang,is_float,1}) ->
    {ok,t_float()};
get_type_test({erlang,is_function,1}) ->
    {ok,t_fun()};
get_type_test({erlang,is_integer,1}) ->
    {ok,t_integer()};
get_type_test({erlang,is_list,1}) ->
    {ok,t_list()};
get_type_test({erlang,is_map,1}) ->
    {ok,t_map()};
get_type_test({erlang,is_number,1}) ->
    {ok,t_number()};
get_type_test({erlang,is_pid,1}) ->
    {ok,t_pid()};
get_type_test({erlang,is_port,1}) ->
    {ok,t_port()};
get_type_test({erlang,is_reference,1}) ->
    {ok,t_reference()};
get_type_test({erlang,is_tuple,1}) ->
    {ok,t_tuple()};
get_type_test({M,F,A})
    when is_atom(M),
    is_atom(F),
            ...
            true->
                case erl_types:number_max(TmpSizeType) of
                    N
                        ...
                        {RangeMin,RangeMax} = case lists:member(signed,Flags) of
                            true->
                                {-(1 bsl (TotalSizeVal - 1)),1 bsl (TotalSizeVal - 1) - 1};
                            false->
                                {0,1 bsl TotalSizeVal - 1}
                        end,
...
get_safe_underapprox_1([Pat0| Left],Acc,Map) ->
    ...
    case cerl:type(Pat) of
        ...
        binary->
            throw(dont_know);
        cons->
            {[Hd, Tl],Map1} = get_safe_underapprox_1([cerl:cons_hd(Pat), cerl:cons_tl(Pat)],[],Map),
            case t_is_any(Tl) of
                true->
                    get_safe_underapprox_1(Left,[t_nonempty_list(Hd)| Acc],Map1);
                false->
                    throw(dont_know)
            end;
        literal->
            case cerl:unfold_literal(Pat) of
                Pat->
                    Type = case cerl:concrete(Pat) of
                        Int
                            when is_integer(Int)->
                            t_from_term(Int);
                        Atom
        ...
        map->
            ...
            true = lists:all(fun (P)->
                cerl:is_literal(Op = cerl:map_pair_op(P)) andalso exact =:= cerl:concrete(Op) end,cerl:map_es(Pat)),
            ...
            SquashDuplicates = fun SquashDuplicates([{K,First}, {K,Second}| List])->
                case t_is_none(Inf = t_inf(First,Second)) of
                    true->
                        throw(dont_know);
                    false->
                        [{K,Inf}| SquashDuplicates(List)]
                end;SquashDuplicates([Good| Rest])->
                [Good| SquashDuplicates(Rest)];SquashDuplicates([])->
                [] end,
...
get_bif_constr({erlang,Op,2},Dst,Args = [Arg1, Arg2],_State)
    ...
    ReturnType = mk_fun_var(fun (Map)->
        ...
        bif_return(erlang,Op,2,TmpArgTypes) end,Args),
    ArgFun = fun (A,Pos)->
        F = fun (Map)->
        ...
        mk_fun_var(F,[Dst, A]) end,
    Arg1FunVar = ArgFun(Arg2,2),
    Arg2FunVar = ArgFun(Arg1,1),
    mk_conj_constraint_list([mk_constraint(Dst,sub,ReturnType), mk_constraint(Arg1,sub,Arg1FunVar), mk_constraint(Arg2,sub,Arg2FunVar)]);
get_bif_constr({erlang,Op,2},Dst,[Arg1, Arg2] = Args,_State)
    when Op =:= '<';
    Op =:= '=<';
    Op =:= '>';
    Op =:= '>='->
    ArgFun = fun (LocalArg1,LocalArg2,LocalOp)->
        fun (Map)->
            DstType = lookup_type(Dst,Map),
                    ...
                    case t_is_integer(Arg1Type) andalso t_is_integer(Arg2Type) of
                        true->
                            ...
                            case LocalOp of
                                '=<'->
                                    if IsTrue ->
                                        t_from_range(Min1,Max2);IsFalse ->
                                        t_from_range(range_inc(Min2),Max1) end;
                                '<'->
                                    if IsTrue ->
                                        t_from_range(Min1,range_dec(Max2));IsFalse ->
                                        t_from_range(Min2,Max1) end;
                                '>='->
                                    if IsTrue ->
                                        t_from_range(Min2,Max1);IsFalse ->
                                        t_from_range(Min1,range_dec(Max2)) end;
                                '>'->
                                    if IsTrue ->
                                        t_from_range(range_inc(Min2),Max1);IsFalse ->
                                        t_from_range(Min1,Max2) end
                            end;
                        false->
                            t_any()
                    end;
    ...
    DstVar = mk_fun_var(fun (Map)->
        TmpArgTypes = lookup_type_list(Args,Map),
        bif_return(erlang,Op,2,TmpArgTypes) end,Args),
    mk_conj_constraint_list([mk_constraint(Dst,sub,DstVar), mk_constraint(Arg1,sub,Arg1Var), mk_constraint(Arg2,sub,Arg2Var)]);
get_bif_constr({erlang,'++',2},Dst,[Hd, Tl] = Args,_State) ->
    ...
    ReturnType = mk_fun_var(fun (Map)->
        TmpArgTypes = lookup_type_list(Args,Map),
        bif_return(erlang,'++',2,TmpArgTypes) end,Args),
    Cs = mk_constraints(Args,sub,ArgTypes),
    mk_conj_constraint_list([mk_constraint(Dst,sub,ReturnType), mk_constraint(Hd,sub,HdVar), mk_constraint(Tl,sub,TlVar)| Cs]);
            ...
            true->
                ...
                case t_number_vals(ArityType) of
                    unknown->
                        t_fun();
                    Vals->
                        t_sup([(t_fun(X,t_any())) || X <- Vals])
                end;
            false->
                t_any()
        end end,
    ArgV = mk_fun_var(ArgFun,[Dst, Arity]),
    mk_conj_constraint_list([mk_constraint(Dst,sub,t_boolean()), mk_constraint(Arity,sub,t_integer()), mk_constraint(Fun,sub,ArgV)]);
...
get_bif_constr({erlang,is_record,2},Dst,[Var, Tag] = Args,_State) ->
    ...
    DstFun = fun (Map)->
        TmpArgTypes = lookup_type_list(Args,Map),
        bif_return(erlang,is_record,2,TmpArgTypes) end,
    DstV = mk_fun_var(DstFun,Args),
    mk_conj_constraint_list([mk_constraint(Dst,sub,DstV), mk_constraint(Tag,sub,t_atom()), mk_constraint(Var,sub,ArgV)]);
get_bif_constr({erlang,is_record,3},Dst,[Var, Tag, Arity] = Args,State) ->
    ...
    DstFun = fun (Map)->
        ...
        bif_return(erlang,is_record,3,TmpArgTypes) end,
    DstV = mk_fun_var(DstFun,Args),
    mk_conj_constraint_list([mk_constraint(Dst,sub,DstV), mk_constraint(Arity,sub,t_integer()), mk_constraint(Tag,sub,t_atom()), mk_constraint(Var,sub,ArgV)]);
...
get_bif_constr({erlang,'and',2},Dst,[Arg1, Arg2] = Args,_State) ->
            ...
            case t_is_any_atom(true,DstType) of
                ...
                false->
                    case t_is_any_atom(false,DstType) of
                        true->
                            case t_is_any_atom(true,lookup_type(Var,Map)) of
                                true->
                                    False;
                                false->
                                    t_boolean()
            ...
            false->
                ...
                case t_is_any_atom(false,Arg2Type) of
                    true->
                        False;
                    false->
                        case t_is_any_atom(true,Arg1Type) andalso t_is_any_atom(true,Arg2Type) of
                            true->
                                True;
                            false->
                                t_boolean()
    ...
    DstV = mk_fun_var(DstFun,Args),
    mk_conj_constraint_list([mk_constraint(Dst,sub,DstV), mk_constraint(Arg1,sub,ArgV1), mk_constraint(Arg2,sub,ArgV2)]);
get_bif_constr({erlang,'or',2},Dst,[Arg1, Arg2] = Args,_State) ->
        ...
        fun (Map)->
            ...
            case t_is_any_atom(false,DstType) of
                ...
                false->
                    case t_is_any_atom(true,DstType) of
                        true->
                            case t_is_any_atom(false,lookup_type(Var,Map)) of
                                true->
                                    True;
                                false->
                                    t_boolean()
            ...
            false->
                ...
                case t_is_any_atom(true,Arg2Type) of
                    true->
                        True;
                    false->
                        case t_is_any_atom(false,Arg1Type) andalso t_is_any_atom(false,Arg2Type) of
                            true->
                                False;
                            false->
                                t_boolean()
    ...
    mk_conj_constraint_list([mk_constraint(Dst,sub,DstV), mk_constraint(Arg1,sub,ArgV1), mk_constraint(Arg2,sub,ArgV2), Disj]);
get_bif_constr({erlang,'not',1},Dst,[Arg] = Args,_State) ->
    ...
    mk_conj_constraint_list([mk_constraint(Arg,sub,ArgV), mk_constraint(Dst,sub,DstV)]);
get_bif_constr({erlang,'=:=',2},Dst,[Arg1, Arg2] = Args,_State) ->
    ArgFun = fun (Self,OtherVar)->
        fun (Map)->
            DstType = lookup_type(Dst,Map),
                    ...
                    case t_is_any_atom(false,DstType) of
                        true->
                            case is_singleton_type(OtherVarType) of
                                true->
                                    t_subtract(lookup_type(Self,Map),OtherVarType);
                                false->
                                    t_any()
                            end;
                        false->
...
get_bif_constr({erlang,'==',2},Dst,[Arg1, Arg2] = Args,_State) ->
    DstFun = fun (Map)->
        TmpArgTypes = lookup_type_list(Args,Map),
        bif_return(erlang,'==',2,TmpArgTypes) end,
    ArgFun = fun (Var,Self)->
        fun (Map)->
    ...
    mk_conj_constraint_list([mk_constraint(Dst,sub,DstV), mk_constraint(Arg1,sub,ArgV1), mk_constraint(Arg2,sub,ArgV2)]);
get_bif_constr({erlang,element,2} = _BIF,Dst,Args,#state{cs = Constrs}) ->
...
get_bif_constr({M,F,A} = _BIF,Dst,Args,_State) ->
    ...
    case t_is_none(GenType) of
        ...
        false->
            ReturnType = mk_fun_var(fun (Map)->
                TmpArgTypes = lookup_type_list(Args,Map),
                bif_return(M,F,A,TmpArgTypes) end,Args),
...
get_bif_test_constr(Dst,Arg,Type,_State) ->
    ...
    mk_conj_constraint_list([mk_constraint(Dst,sub,DstV), mk_constraint(Arg,sub,ArgV)]).
...
comp_users(SCC,State) ->
    ...
    family([{t_var(V),F} || F <- SCC,V <- ordsets:intersection(get_deps(state__get_cs(F,State)),Vars)]).
...
solve_scc(SCC,Map,State,Users,ToSolve,TryingUnit) ->
    ...
    RecTypes = [(t_limit(Type,4)) || Type <- Types],
    CleanMap = lists:foldl(fun (Fun,AccFunMap)->
        erase_type(t_var_name(Fun),AccFunMap) end,Map,ToSolve),
    Map1 = enter_type_lists(Vars,RecTypes,CleanMap),
    ok,
    SolveFun = fun (X,Y)->
        scc_fold_fun(X,Y,State) end,
    ...
    case Updated =:= [] of
        true->
            ...
            case erl_types:any_none([(t_fun_range(T)) || T <- NewTypes]) andalso TryingUnit =:= false of
                true->
                    UnitTypes = [case t_is_none(t_fun_range(T)) of
                        false->
                            T;
                        true->
...
solve_fun(v2,Fun,_Cs,FunMap,State) ->
    fun ()->
        v2_solve_ref(Fun,FunMap,State) end.
...
check_solutions([{S1,Map1,_Time1}| Maps],Fun,S,Map) ->
    ...
    case Map =:= no_map orelse sane_maps(Map,Map1,[Fun],S,S1) of
        ...
        false->
            ...
            ok,
            ok,
            io:format("A bug was found. Please report it, and use the o" "ption `--solver v1' until the bug has been fixed" ".\n"),
            throw(error)
    end.
...
-record(v2_state,{constr_data = maps:new()::map(),state::state()}).
...
v2_solve_reference(Id,Map,V2State0) ->
    ...
    Res = case state__is_self_rec(Id,State) of
        true->
            v2_solve_self_recursive(Cs,Map1,Id,t_none(),V2State0);
        false->
            v2_solve(Cs,Map1,V2State0)
    end,
    {FunType,V2State} = case Res of
        {error,V2State1}->
            ok,
            Arity = state__fun_arity(Id,State),
            FunType0 = case state__prop_domain(t_var_name(Id),State) of
                error->
                    t_fun(Arity,t_none());
                {ok,Dom}->
                    t_fun(Dom,t_none())
    ...
    {NewMap2,U2} = case state__get_rec_var(Id,State) of
        {ok,Var}->
...
v2_solve_disjunct(Disj,Map,V2State0) ->
    ...
    case get_flags(V2State0,Disj) of
        ...
        {V2State1,Flags}
            when Flags =/= []->
            {ok,V2State,Eval,UL,MapL0,Uneval,Failed} = v2_solve_disj(Flags,Cs,1,Map,V2State1,[],[],[],[],false),
            ok,
            if Eval =:= [],
            Uneval =:= [] ->
                {error,failed_list(Disj,V2State0)};true ->
                {Is0,UnIds} = lists:unzip(Uneval),
                MapL = [(restore_local_map(V2State,Id,Map)) || Id <- UnIds] ++ MapL0,
                U0 = case Failed of
                    false->
                        lists:umerge(UL);
                    true->
...
v2_solve_disj([I| Is],[C| Cs],I,Map0,V2State0,UL,MapL,Eval,Uneval,Failed0) ->
    ...
    case v2_solve(C,Map1,V2State0) of
        ...
        {ok,Map,V2State1,U}->
            ...
            ok,
            v2_solve_disj(Is,Cs,I + 1,Map0,V2State,[U| UL],[Map| MapL],[I| Eval],Uneval,Failed0)
    end;
...
v2_solve_disj(Is,[C| Cs],I,Map,V2State,UL,MapL,Eval,Uneval0,Failed) ->
    Uneval = [{I,C#constraint_list.id} ||  not is_failed_list(C,V2State)] ++ Uneval0,
...
save_local_map(#v2_state{constr_data = ConData} = V2State,Id,U,Map) ->
    ...
    Part1 = case maps:find(Id,ConData) of
        error->
            [];
        {ok,{Part2,[]}}->
            Part2
...
restore_local_map(#v2_state{constr_data = ConData},Id,Map0) ->
    case maps:find(Id,ConData) of
        ...
        {ok,{Part0,U}}->
            ...
            ok,
            ok,
            Map = lists:foldl(fun ({K,V},D)->
                maps:put(K,V,D) end,Map0,Part),
            ok,
            Map
    end.
...
v2_solve_conjunct(Conj,Map,V2State0) ->
    ...
    case get_flags(V2State0,Conj) of
        {V2State,failed_list}->
            {error,V2State};
        {V2State,Flags}->
            v2_solve_conj(Flags,Cs,1,Map,Conj,IsFlat,V2State,[],[],[],Map,Flags)
    end.

v2_solve_conj([I| Is],[Cs| Tail],I,Map0,Conj,IsFlat,V2State0,UL,NewFs0,VarsUp,LastMap,LastFlags) ->
    ok,
    ...
    case v2_solve(Cs,Map0,V2State0) of
        ...
        {ok,Map,V2State1,[]}->
            v2_solve_conj(Is,Tail,I + 1,Map,Conj,IsFlat,V2State1,UL,NewFs0,VarsUp,LastMap,LastFlags);
        {ok,Map,V2State1,U}
            when IsFlat->
            Mask = lists:umerge([(get_mask(V,Conj#constraint_list.masks)) || V <- U]),
            {Is1,NewF} = add_mask_to_flags(Is,Mask,I,[]),
            NewFs = [NewF| NewFs0],
            v2_solve_conj(Is1,Tail,I + 1,Map,Conj,IsFlat,V2State1,[U| UL],NewFs,VarsUp,LastMap,LastFlags);
        {ok,Map,V2State1,U}->
            ...
            {NewF,F} = lists:splitwith(fun (J)->
                J < I end,NewF0),
            Is1 = umerge_mask(Is,F),
            NewFs = [NewF| NewFs0],
            v2_solve_conj(Is1,Tail,I + 1,Map,Conj,IsFlat,V2State2,[U| UL],NewFs,VarsUp,LastMap,LastFlags)
    end;
v2_solve_conj([],_Cs,_I,Map,Conj,IsFlat,V2State,UL,NewFs,VarsUp,LastMap,LastFlags) ->
    U = lists:umerge(UL),
    case lists:umerge(NewFs) of
        ...
        NewFlags
            when NewFlags =:= LastFlags,
            Map =:= LastMap->
            report_detected_loop(Conj),
            {ok,Map,V2State,lists:umerge([U| VarsUp])};
        NewFlags->
            #constraint_list{type = conj,list = Cs} = Conj,
            v2_solve_conj(NewFlags,Cs,1,Map,Conj,IsFlat,V2State,[],[],[U| VarsUp],Map,NewFlags)
    end;
v2_solve_conj([every_i],Cs,I,Map,Conj,IsFlat,V2State,UL,NewFs,VarsUp,LastMap,LastFlags) ->
    NewIs = case Cs of
        []->
            [];
        _->
    ...
    v2_solve_conj(NewIs,Cs,I,Map,Conj,IsFlat,V2State,UL,NewFs,VarsUp,LastMap,LastFlags);
v2_solve_conj(Is,[_| Tail],I,Map,Conj,IsFlat,V2State,UL,NewFs,VarsUp,LastMap,LastFlags) ->
    v2_solve_conj(Is,Tail,I + 1,Map,Conj,IsFlat,V2State,UL,NewFs,VarsUp,LastMap,LastFlags).

report_detected_loop(_) ->
    ok.

add_mask_to_flags(Flags,[Im| M],I,L)
    when I > Im->
...
solve_ref_or_list(#constraint_ref{id = Id,deps = Deps},Map,MapDict,State) ->
    {OldLocalMap,Check} = case maps:find(Id,MapDict) of
        error->
            {map_new(),false};
        {ok,M}->
            {M,true}
    ...
    case Check andalso maps_are_equal(OldLocalMap,Map,CheckDeps) of
        ...
        false->
            ...
            Res = case state__is_self_rec(Id,State) of
                true->
                    solve_self_recursive(Cs,Map,MapDict,Id,t_none(),State);
                false->
                    solve_ref_or_list(Cs,Map,MapDict,State)
            end,
            {NewMapDict,FunType} = case Res of
                {error,NewMapDict0}->
                    ok,
                    Arity = state__fun_arity(Id,State),
                    FunType0 = case state__prop_domain(t_var_name(Id),State) of
            ...
            NewMap2 = case state__get_rec_var(Id,State) of
                {ok,Var}->
...
solve_ref_or_list(#constraint_list{type = Type,list = Cs,deps = Deps,id = Id},Map,MapDict,State) ->
    {OldLocalMap,Check} = case maps:find(Id,MapDict) of
        error->
            {map_new(),false};
        {ok,M}->
            ...
            case Cs of
                ...
                _->
                    case maps_are_equal(Map,NewMap,Deps) of
                        ...
                        false->
                            solve_clist(Cs,conj,Id,Deps,NewMapDict,NewMap,State)
                    end
            end
    end;
...
constrained_keys(Maps) ->
    lists:foldl(fun (TmpMap,AccKeys)->
        [Key || Key <- AccKeys,maps:is_key(Key,TmpMap)] end,maps:keys(hd(Maps)),tl(Maps)).
...
new_state(MFAs,NextLabel,CallGraph,CServer,Plt,PropTypes0,Solvers) ->
    List_SCC = [begin {Var,Label} = dialyzer_codeserver:lookup_mfa_var_label(MFA,CServer),
    {{MFA,Var},t_var(Label)} end || MFA <- MFAs],
    ...
    #state{callgraph = CallGraph,name_map = NameMap,next_label = NextLabel,prop_types = PropTypes,plt = Plt,scc = ordsets:from_list(SCC),mfas = MFAs,self_rec = SelfRec,solvers = Solvers,cserver = CServer}.
...
get_apply_constr(FunLabels,Dst,ArgTypes,#state{callgraph = CG} = State) ->
    ...
    case lists:member(error,MFAs) of
        true->
            error;
        false->
            Constrs0 = [begin State1 = state__new_constraint_context(State),
            try get_plt_constr(MFA,Dst,ArgTypes,State1) of 
                State2->
                    state__cs(State2)
                catch
                    throw:error->
                        error end end || {ok,MFA} <- MFAs],
...
strip_of_any_constrs([T1| Left1],[T2| Left2],Acc1,Acc2) ->
    case t_is_any(T1) orelse constraint_opnd_is_any(T2) of
        ...
        false->
            strip_of_any_constrs(Left1,Left2,[T1| Acc1],[T2| Acc2])
    end;
...
state__store_funs(Vars0,Funs0,#state{fun_map = Map} = State) ->
    ...
    NewMap = lists:foldl(fun ({Var,Fun},MP)->
        maps:put(Fun,Var,MP) end,Map,lists:zip(Vars,Funs)),
...
constraint_opnd_is_any(#fun_var{}) ->
    false;
constraint_opnd_is_any(Type) ->
    t_is_any(Type).
...
get_deps(#constraint{deps = D}) ->
    D;
get_deps(#constraint_list{deps = D}) ->
    D;
get_deps(#constraint_ref{deps = D}) ->
    D.
...
mk_constraint_list(Type,List) ->
    ...
    case Type of
        conj->
            List2 = ordsets:filter(fun (X)->
                get_deps(X) =/= [] end,List1),
            mk_constraint_list_cont(Type,List2);
        disj->
            case lists:any(fun (X)->
                get_deps(X) =:= [] end,List1) of
                true->
...
mk_constraint_list_cont(Type,List) ->
    ...
    case Deps =:= [] of
        true->
            #constraint_list{type = conj,list = [mk_constraint_any(eq)],deps = []};
        false->
            #constraint_list{type = Type,list = List,deps = Deps}
    end.
...
mk_disj_norm_form(#constraint_list{} = CL) ->
    try List1 = expand_to_conjunctions(CL),
...
expand_to_conjunctions(#constraint_list{type = disj,list = List}) ->
    if length(List) > 28 ->
        throw(too_many_disj);true ->
        ok end,
    List1 = [C || C <- List,is_simple_constraint(C)],
    [] = [C || #constraint{} = C <- List1],
    Expanded = lists:append([(expand_to_conjunctions(C)) || #constraint_list{} = C <- List]),
    ReturnList = Expanded ++ List1,
    if length(ReturnList) > 28 ->
        throw(too_many_disj);true ->
        ReturnList end.

is_simple_constraint(#constraint{}) ->
    true;
is_simple_constraint(#constraint_ref{}) ->
    true;
is_simple_constraint(#constraint_list{}) ->
    false.

combine_conj_lists([List1, List2| Left],Prefix) ->
    NewList = [(mk_conj_constraint_list([L1, L2])) || L1 <- List1,L2 <- List2],
    if length(NewList) > 28 ->
        throw(too_many_disj);true ->
        ok end,
...
wrap_simple_constr(#constraint{} = C) ->
    mk_conj_constraint_list([C]);
wrap_simple_constr(#constraint_list{} = C) ->
    C;
wrap_simple_constr(#constraint_ref{} = C) ->
    C.

enumerate_constraints(State) ->
    Cs = [(mk_constraint_ref(Id,get_deps(state__get_cs(Id,State)))) || Id <- state__scc(State)],
...
enumerate_constraints([#constraint_list{type = conj,list = List} = C| Tail],N,Acc,State) ->
    {Flat,Deep} = lists:partition(fun (#constraint{})->
        true;(#constraint_list{})->
        false;(#constraint_ref{})->
        false end,List),
    {NewFlat,N1,State1} = enumerate_constraints(Flat,N,[],State),
    {NewDeep,N2,State2} = enumerate_constraints(Deep,N1,[],State1),
    {NewList,N3} = if NewFlat =:= [] ->
        {NewDeep,N2};NewDeep =:= [] ->
        {NewFlat,N2};true ->
        TmpCList = mk_conj_constraint_list(NewFlat),
        {[TmpCList#constraint_list{id = {list,N2}}| NewDeep],N2 + 1} end,
    NewAcc = [C#constraint_list{list = NewList,id = {list,N3}}| Acc],
    enumerate_constraints(Tail,N3 + 1,NewAcc,State2);
enumerate_constraints([#constraint_list{list = List,type = disj} = C| Tail],N,Acc,State) ->
...
order_fun_constraints([#constraint_list{list = List,type = Type,masks = OldMasks} = C| Tail],Funs,Acc,State) ->
    case OldMasks of
        undefined->
            {NewList,NewState} = case Type of
                conj->
                    order_fun_constraints(List,[],[],State);
                disj->
                    FoldFun = fun (X,AccState)->
                        {[NewX],NewAccState} = order_fun_constraints([X],[],[],AccState),
                        {NewX,NewAccState} end,
...
reset_deps(#constraint{} = C) ->
    C#constraint{deps = []};
reset_deps(#constraint_list{} = C) ->
    C#constraint_list{deps = []};
reset_deps(#constraint_ref{} = C) ->
    C#constraint_ref{deps = []}.
...
lookup_record(State,Tag,Arity) ->
    ...
    {State1,Rec} = case lists:keyfind(M,1,ModRecs) of
        {M,Rec0}->
            {State,Rec0};
        false->
    ...
    case erl_types:lookup_record(Tag,Arity,Rec) of
        {ok,Fields}->
            RecType = t_tuple([t_from_term(Tag)| [FieldType || {_FieldName,_Abstr,FieldType} <- Fields]]),
            {ok,RecType,State1};
        error->
            {error,State1}
    end.
