...
wildcard(Pattern)
    ...
    try do_wildcard(Pattern,".",file)
        catch
            error:{badpattern,_} = UnUsUalVaRiAbLeNaMe->
                error(UnUsUalVaRiAbLeNaMe) end.
...
wildcard(Pattern,Cwd)
    when is_list(Pattern),
    is_list(Cwd)->
    try do_wildcard(Pattern,Cwd,file)
        catch
            error:{badpattern,_} = UnUsUalVaRiAbLeNaMe->
                error(UnUsUalVaRiAbLeNaMe) end;
...
wildcard(Pattern,Cwd,Mod)
    ...
    is_list(Cwd),
    is_atom(Mod)->
    try do_wildcard(Pattern,Cwd,Mod)
        catch
            error:{badpattern,_} = UnUsUalVaRiAbLeNaMe->
                error(UnUsUalVaRiAbLeNaMe) end.
...
fold_files(Dir,RegExp,Recursive,Fun,Acc,Mod)
    when is_atom(Mod)->
...
do_fold_files2([File| T],Dir,RegExp,OrigRE,Recursive,Fun,Acc0,Mod) ->
    ...
    case do_is_regular(FullName,Mod) of
        true->
            case  catch re:run(File,if is_binary(File) ->
                OrigRE;true ->
                RegExp end,[{capture,none}]) of
                match->
...
do_wildcard(Pattern,Cwd,Mod) ->
    ...
    Files = if PrefixLen =:= 0 ->
        Files0;true ->
        [(lists:nthtail(PrefixLen,File)) || File <- Files0] end,
    lists:sort(Files).
...
compile_wildcard(Pattern)
    when is_list(Pattern)->
    {compiled_wildcard,try compile_wildcard(Pattern,".")
        catch
            error:{badpattern,_} = UnUsUalVaRiAbLeNaMe->
                error(UnUsUalVaRiAbLeNaMe) end}.
...
compile_charset1([Lower, $-, Upper| Rest],Ordset)
    when Lower =< Upper->
...
compile_range(Lower,Current,Ordset)
    when Lower =< Current->
...
default_search_rules() ->
    [{".beam",".erl",erl_source_search_rules()}, {".erl",".yrl",[]}, {"",".src",erl_source_search_rules()}, {".so",".c",c_source_search_rules()}, {".o",".c",c_source_search_rules()}, {"",".c",c_source_search_rules()}, {"",".in",basic_source_search_rules()}, {"",""}] ++ erl_source_search_rules().
