-file("dialyzer_dataflow.erl", 1).

-module(dialyzer_dataflow).

-export([get_fun_types/5, get_warnings/5, format_args/3]).

-export([state__add_warning/2, state__cleanup/1, state__duplicate/1, dispose_state/1, state__get_callgraph/1, state__get_races/1, state__get_records/1, state__put_callgraph/2, state__put_races/2, state__records_only/1, state__find_function/2]).
...
-record(state,{callgraph::dialyzer_callgraph:callgraph()|undefined,codeserver::dialyzer_codeserver:codeserver()|undefined,envs::env_tab()|undefined,fun_tab::fun_tab()|undefined,fun_homes::dict:dict(label(),mfa())|undefined,reachable_funs::sets:set(label())|undefined,plt::dialyzer_plt:plt()|undefined,opaques::[type()]|undefined,races = dialyzer_races:new()::dialyzer_races:races(),records = dict:new()::types(),tree_map::dict:dict(label(),cerl:cerl())|undefined,warning_mode = false::boolean(),warnings = []::[raw_warning()],work::{[_],[_],sets:set()}|undefined,module::module(),curr_fun::curr_fun()}).

-record(map,{map = maps:new()::type_tab(),subst = maps:new()::subst_tab(),modified = []::[Key::term()],modified_stack = []::[{[Key::term()],reference()}],ref = undefined::reference()|undefined}).

-type(env_tab()::dict:dict(label(),#map{})).

-type(fun_entry()::{Args::[type()],RetType::type()}).

-type(fun_tab()::dict:dict(top|label(),{not_handled,fun_entry()}|fun_entry())).
...
-spec(get_warnings(cerl:c_module(),dialyzer_plt:plt(),dialyzer_callgraph:callgraph(),dialyzer_codeserver:codeserver(),types()) -> {[raw_warning()],fun_types()}).
...
-spec(get_fun_types(cerl:c_module(),dialyzer_plt:plt(),dialyzer_callgraph:callgraph(),dialyzer_codeserver:codeserver(),types()) -> fun_types()).
                ...
                false->
                    case state__fun_env(Fun,NewState1) of
                        ...
                        Map->
                            ...
                            NewState3 = case IsRaceAnalysisEnabled of
                                true->
                                    NewState2 = state__renew_curr_fun(state__lookup_name(FunLabel,NewState1),FunLabel,NewState1),
                            ...
                            ok,
                            NewState5 = case IsRaceAnalysisEnabled of
                                true->
    ...
    case cerl:type(Tree) of
        ...
        letrec->
            ...
            FoldFun = fun ({Var,Fun},{AccState,AccMap})->
                {NewAccState,NewAccMap0,FunType} = traverse(Fun,AccMap,AccState),
                NewAccMap = enter_type(Var,FunType,NewAccMap0),
                {NewAccState,NewAccMap} end,
        ...
        primop->
            case cerl:atom_val(cerl:primop_name(Tree)) of
                ...
                recv_wait_timeout->
                    ...
                    case t_is_atom(TimeoutType,Opaques) andalso t_atom_vals(TimeoutType,Opaques) =:= [infinity] of
                        true->
        ...
        seq->
            ...
            case t_is_none_or_unit(ArgType) of
                true->
                    SMA;
                false->
                    State2 = case t_is_any(ArgType) orelse t_is_simple(ArgType,State) orelse is_call_to_send(Arg) orelse is_lc_simple_list(Arg,ArgType,State) of
                        true->
                            State1;
                        false->
                            state__add_warning(State1,warn_umatched_return,Arg,{unmatched_return,[format_type(ArgType,State1)]})
                    end,
...
handle_apply(Tree,Map,State) ->
    ...
    case any_none(ArgTypes) of
        true->
            {State1,Map2,t_none()};
        false->
            FunList = case state__lookup_call_site(Tree,State) of
                error->
                    [external];
                {ok,List}->
                    List
            ...
            case handle_apply_or_call(FunInfoList,Args,ArgTypes,Map2,Tree,State1) of
                {had_external,State2}->
                    Arity = length(Args),
                    ...
                    case t_is_none(OpType1) of
                        true->
                            ...
                            {State3,Map2,t_none()};
                        false->
                            NewArgs = t_inf_lists(ArgTypes,t_fun_args(OpType1,universe)),
                            case any_none(NewArgs) of
                                true->
                                    EnumNewArgs = lists:zip(lists:seq(1,length(NewArgs)),NewArgs),
                                    ArgNs = [Arg || {Arg,Type} <- EnumNewArgs,t_is_none(Type)],
                                    Msg = {fun_app_args,[ArgNs, format_args(Args,ArgTypes,State), format_type(OpType,State)]},
                                    State3 = state__add_warning(State2,warn_failing_call,Tree,Msg),
                                    {State3,enter_type(Op,OpType1,Map2),t_none()};
                                false->
                                    Map3 = enter_type_lists(Args,NewArgs,Map2),
                                    Range0 = t_fun_range(OpType1,universe),
                                    Range = case t_is_unit(Range0) of
                                        true->
                                            t_none();
                                        false->
                                            Range0
...
handle_apply_or_call(FunInfoList,Args,ArgTypes,Map,Tree,State) ->
    None = t_none(),
    handle_apply_or_call(FunInfoList,Args,ArgTypes,Map,Tree,State,[None || _ <- ArgTypes],None,false,{none,[]}).

handle_apply_or_call([{local,external}| Left],Args,ArgTypes,Map,Tree,State,_AccArgTypes,_AccRet,_HadExternal,Warns) ->
    {HowMany,_} = Warns,
    NewHowMany = case HowMany of
        none->
            one;
        _->
            many
    ...
    NewWarns = {NewHowMany,[]},
    handle_apply_or_call(Left,Args,ArgTypes,Map,Tree,State,ArgTypes,t_any(),true,NewWarns);
handle_apply_or_call([{TypeOfApply,{Fun,Sig,Contr,LocalRet}}| Left],Args,ArgTypes,Map,Tree,#state{opaques = Opaques} = State,AccArgTypes,AccRet,HadExternal,Warns) ->
    Any = t_any(),
    AnyArgs = [Any || _ <- Args],
    GenSig = {AnyArgs,fun (_)->
        t_any() end},
    {CArgs,CRange} = case Contr of
        {value,#contract{args = As} = C}->
            {As,fun (FunArgs)->
                dialyzer_contracts:get_contract_return(C,FunArgs) end};
        none->
            GenSig
    end,
    {BifArgs,BifRange} = case TypeOfApply of
        remote->
            {M,F,A} = Fun,
            case erl_bif_types:is_known(M,F,A) of
                true->
                    ...
                    BRange = fun (FunArgs)->
                        erl_bif_types:type(M,F,A,FunArgs,Opaques) end,
                    {BArgs,BRange};
                false->
                    GenSig
            end;
    ...
    ok,
    ok,
    State1 = case is_race_analysis_enabled(State) of
        true->
    ...
    ok,
    ok,
    State2 = case FailedConj andalso  not (IsFailBif orelse IsFailSig) of
        true->
            case t_is_none(RetWithoutLocal) andalso  not t_is_none(RetWithoutContr) andalso  not any_none(NewArgTypes) of
                true->
                    {value,C1} = Contr,
                ...
                false->
                    ...
                    InfSig = t_inf(t_fun(SigArgs,SigRange),t_fun(BifArgs,BifRange(BifArgs))),
                    FailReason = apply_fail_reason(FailedSig,FailedBif,FailedContract),
                    Msg = get_apply_fail_msg(Fun,Args,ArgTypes,NewArgTypes,InfSig,Contr,CArgs,State1,FailReason,Opaques),
    ...
    State3 = case TypeOfApply of
        local->
            case state__is_escaping(Fun,State2) of
                true->
                    State2;
                false->
                    ForwardArgs = [(t_limit(X,3)) || X <- ArgTypes],
                    forward_args(Fun,ForwardArgs,State2)
            end;
        remote->
            add_bif_warnings(Fun,NewArgTypes,Tree,State2)
    end,
    NewAccArgTypes = case FailedConj of
        true->
            AccArgTypes;
        false->
            [(t_sup(X,Y)) || {X,Y} <- lists:zip(NewArgTypes,AccArgTypes)]
    end,
    TotalRet = case t_is_none(LocalRet) andalso t_is_unit(RetWithoutLocal) of
        true->
            RetWithoutLocal;
        false->
    ...
    handle_apply_or_call(Left,Args,ArgTypes,Map,Tree,State4,NewAccArgTypes,NewAccRet,HadExternal,NewWarns);
handle_apply_or_call([],Args,_ArgTypes,Map,_Tree,State,AccArgTypes,AccRet,HadExternal,{_,Warnings}) ->
...
apply_fail_reason(FailedSig,FailedBif,FailedContract) ->
    if (FailedSig orelse FailedBif) andalso  not FailedContract ->
        only_sig;FailedContract andalso  not (FailedSig orelse FailedBif) ->
        only_contract;true ->
        both end.

get_apply_fail_msg(Fun,Args,ArgTypes,NewArgTypes,Sig,Contract,ContrArgs,State,FailReason,Opaques) ->
    ArgStrings = format_args(Args,ArgTypes,State),
    ContractInfo = case Contract of
        {value,#contract{} = C}->
    ...
    case state__lookup_name(Fun,State) of
        {M,F,A}->
            case is_opaque_type_test_problem(Fun,Args,NewArgTypes,State) of
                {yes,Arg,ArgType}->
                    {opaque_type_test,[atom_to_list(F), ArgStrings, format_arg(Arg), format_type(ArgType,State)]};
                no->
                    ...
                    case lists:keyfind(decl,1,BadOpaque) of
                        {decl,BadArgs}->
                            ExpectedTriples = case FailReason of
                                only_sig->
                        ...
                        false->
                            case lists:keyfind(use,1,BadOpaque) of
                                {use,BadArgs}->
                                    ExpectedArgs = case FailReason of
                                        only_sig->
                                            SigArgs;
                                        _->
                                            ContrArgs
                                ...
                                false->
                                    case erl_bif_types:opaque_args(M,F,A,ArgTypes,Opaques) of
                                        []->
                                            {call,[M, F, ArgStrings, ArgNs, FailReason, format_sig_args(Sig,State), format_type(t_fun_range(Sig),State), ContractInfo]};
                                        Ns->
        ...
        Label
            ...
            {apply,[ArgStrings, ArgNs, FailReason, format_sig_args(Sig,State), format_type(t_fun_range(Sig),State), ContractInfo]}
    end.
...
opaque_problems(ContractOrSigList,ArgTypes,Opaques,ArgNs) ->
    ...
    F = fun (1)->
        decl;(2)->
        use end,
    [{F(ElementI),lists:usort([ArgN || {ArgN,EI} <- ArgElementList,EI =:= ElementI])} || ElementI <- lists:usort([EI || {_,EI} <- ArgElementList])].

find_unknown(ContractOrSigList,ArgTypes,Opaques,NoneArgNs) ->
    ArgNs = lists:seq(1,length(ArgTypes)),
    [{ArgN,ElementI} || ContractOrSig <- ContractOrSigList,{E1,E2,ArgN} <- lists:zip3(ContractOrSig,ArgTypes,ArgNs),lists:member(ArgN,NoneArgNs),ElementI <- erl_types:t_find_unknown_opaque(E1,E2,Opaques)].
...
expected_arg_triples(ArgNs,ArgTypes,State) ->
    [begin Arg = lists:nth(N,ArgTypes),
    {N,Arg,format_type(Arg,State)} end || N <- ArgNs].
...
add_bif_warnings({erlang,Op,2},[T1, T2] = Ts,Tree,State)
    ...
    case t_is_none(Inf) andalso  not any_none(Ts) andalso  not is_int_float_eq_comp(T1,Op,T2,Opaques) of
        true->
...
add_bif_warnings({erlang,Op,2},[T1, T2] = Ts,Tree,State)
    ...
    Op =:= '/='->
    Opaques = State#state.opaques,
    case  not any_none(Ts) andalso  not is_int_float_eq_comp(T1,Op,T2,Opaques) of
        true->
            case erl_types:t_find_unknown_opaque(T1,T2,Opaques) of
                []->
                    State;
...
is_int_float_eq_comp(T1,Op,T2,Opaques) ->
    (Op =:= '==' orelse Op =:= '/=') andalso (erl_types:t_is_float(T1,Opaques) andalso t_is_integer(T2,Opaques) orelse t_is_integer(T1,Opaques) andalso erl_types:t_is_float(T2,Opaques)).
...
handle_bitstr(Tree,Map,State) ->
    ...
    case cerl:bitstr_bitsize(Tree) of
        BitSz
            when BitSz =:= all orelse BitSz =:= utf->
            ValType = case BitSz of
                all->
            ...
            case t_is_none(ValType) of
                true->
                    Msg = {bin_construction,["value", format_cerl(Val), format_cerl(Tree), format_type(ValType0,State2)]},
        ...
        BitSz
            when is_integer(BitSz) orelse BitSz =:= any->
            SizeType = t_inf(SizeType0,t_non_neg_integer()),
            ValType = case BitstrType of
                binary->
                    t_inf(ValType0,t_bitstr());
                float->
            ...
            case any_none([SizeType, ValType]) of
                true->
                    {Msg,Offending} = case t_is_none(SizeType) of
                        true->
                            {{bin_construction,["size", format_cerl(Size), format_cerl(Tree), format_type(SizeType0,State2)]},Size};
                        false->
                            {{bin_construction,["value", format_cerl(Val), format_cerl(Tree), format_type(ValType0,State2)]},Val}
                    end,
                ...
                false->
                    ...
                    {State3,Type} = case t_contains_opaque(SizeType,Opaques) of
                        true->
        ...
        true->
            State2 = if MOpaque ->
                ...
                case t_is_none(t_inf(t_module(),MType0)) of
                    true->
                        Msg = {app_call,[MS, format_cerl(F), format_args(Args,As,State1), MS, format_type(t_module(),State1), format_type(MType0,State1)]},
                        state__add_warning(State1,warn_failing_call,Tree,Msg);
                    false->
                        Msg = {opaque_call,[MS, format_cerl(F), format_args(Args,As,State1), MS, format_type(MType0,State1)]},
                        state__add_warning(State1,warn_failing_call,Tree,Msg)
                end;FOpaque ->
                FS = format_cerl(F),
                case t_is_none(t_inf(t_atom(),FType0)) of
                    true->
                        Msg = {app_call,[format_cerl(M), FS, format_args(Args,As,State1), FS, format_type(t_atom(),State1), format_type(FType0,State1)]},
                        state__add_warning(State1,warn_failing_call,Tree,Msg);
                    false->
                        Msg = {opaque_call,[format_cerl(M), FS, format_args(Args,As,State1), FS, format_type(FType0,State1)]},
                        state__add_warning(State1,warn_failing_call,Tree,Msg)
                end;true ->
                State1 end,
            {State2,Map2,t_none()};
            ...
            case t_is_atom(MType) of
                true->
                    case {t_atom_vals(MType),t_atom_vals(FType)} of
                        {[MAtom],[FAtom]}->
                            FunInfo = [{remote,state__fun_info({MAtom,FAtom,length(Args)},State1)}],
...
handle_case(Tree,Map,State) ->
    ...
    case t_is_none_or_unit(ArgType) of
        true->
            SMA;
        false->
            State2 = case is_race_analysis_enabled(State) of
                true->
            ...
            {MapList,State3,Type,Warns} = handle_clauses(Clauses,Arg,ArgType,ArgType,State2,[],Map2,[],[],[]),
            SupressForced = is_compiler_generated(cerl:get_ann(Tree)) andalso  not t_is_none(Type),
            State4 = lists:foldl(fun ({T,R,M,F},S)->
                state__add_warning(S,T,R,M,F andalso  not SupressForced) end,State3,Warns),
...
handle_cons(Tree,Map,State) ->
    ...
    State3 = case t_is_none(t_inf(TlType,t_list(),State2#state.opaques)) of
        true->
...
handle_let(Tree,Map,State) ->
    ...
    {Map0,State0} = case cerl:is_c_var(Arg) of
        true->
            [Var] = Vars,
            {enter_subst(Var,Arg,Map),case IsRaceAnalysisEnabled of
                true->
                    {RaceList,RaceListSize} = get_race_list_and_size(State),
                    state__renew_race_list([dialyzer_races:let_tag_new(Var,Arg)| RaceList],RaceListSize + 1,State);
                false->
                    State
            end};
        false->
    ...
    State2 = case IsRaceAnalysisEnabled andalso cerl:is_c_call(Arg) of
        true->
            Mod = cerl:call_module(Arg),
            Name = cerl:call_name(Arg),
            case cerl:is_literal(Mod) andalso cerl:concrete(Mod) =:= ets andalso cerl:is_literal(Name) andalso cerl:concrete(Name) =:= new of
                true->
...
handle_module(Tree,Map,State) ->
    ...
    PartFun = fun ({_Var,Fun})->
        state__is_escaping(get_label(Fun),State) end,
    ...
    FoldFun = fun ({_Var,Fun},AccState)->
        state__update_fun_env(Fun,EnvMap,AccState) end,
...
handle_receive(Tree,Map,State) ->
    ...
    {MapList,State2,ReceiveType,Warns} = handle_clauses(Clauses,no_arg,t_any(),t_any(),State1,[],Map,[],[],[]),
    State3 = lists:foldl(fun ({T,R,M,F},S)->
        state__add_warning(S,T,R,M,F) end,State2,Warns),
...
handle_try(Tree,Map,State) ->
    ...
    {SuccState,SuccMap,SuccType} = case bind_pat_vars(Vars,t_to_tlist(ArgType),[],Map2,State1) of
        {error,_,_,_,_}->
            {State1,map__new(),t_none()};
        {SuccMap1,VarTypes}->
            SuccMap2 = case bind_pat_vars_reverse([Arg],[t_product(VarTypes)],[],SuccMap1,State1) of
                {error,_,_,_,_}->
                    SuccMap1;
                {SM,_}->
                    SM
...
handle_map(Tree,Map,State) ->
    ...
    case t_is_none_or_unit(ArgType1) of
        ...
        false->
            {State2,Map2,TypePairs,ExactKeys} = traverse_map_pairs(Pairs,Map1,State1,t_none(),[],[]),
            InsertPair = fun ({KV,assoc,_},Acc)->
                erl_types:t_map_put(KV,Acc);({KV,exact,KVTree},Acc)->
            ...
            try lists:foldl(InsertPair,ArgType1,TypePairs) of 
                ResT->
                    ...
                    throw:{none,MapType,{K,_},KVTree}->
                        Msg2 = {map_update,[format_type(MapType,State2), format_type(K,State2)]},
                        {state__add_warning(State2,warn_map_construction,KVTree,Msg2),Map2,t_none()} end
    end.
...
traverse_map_pairs([Pair| Pairs],Map,State,ShadowKeys,PairAcc,KeyAcc) ->
    ...
    KeyAcc1 = case cerl:is_literal(Op) andalso cerl:concrete(Op) =:= exact andalso t_is_singleton(K,State#state.opaques) andalso t_is_none(t_inf(ShadowKeys,K)) of
        true->
            [K| KeyAcc];
        false->
            KeyAcc
    end,
    traverse_map_pairs(Pairs,Map1,State1,t_sup(K,ShadowKeys),[{{K,V},cerl:concrete(Op),Pair}| PairAcc],KeyAcc1).
                                ...
                                {ok,RecType,FieldNames}->
                                    ...
                                    case t_is_none(InfTupleType) of
                                        true->
                                            ...
                                            {State2,Map1,t_none()};
                                        false->
                                            case bind_pat_vars(Elements,t_tuple_args(RecType),[],Map1,State1) of
                                                {error,bind,ErrorPat,ErrorType,_}->
                                                    Msg = {record_constr,[TagVal, format_patterns(ErrorPat), format_type(ErrorType,State1)]},
                                                    State2 = state__add_warning(State1,warn_matching,Tree,Msg),
                                                    {State2,Map1,t_none()};
                                                {error,opaque,ErrorPat,ErrorType,OpaqueType}->
                                                    ...
                                                    Msg = {opaque_match,["record field" ++ Name ++ " declared to" " be of type " ++ format_type(ErrorType,State1), OpaqueStr, OpaqueStr]},
                                                    State2 = state__add_warning(State1,warn_opaque,Tree,Msg),
                                                    {State2,Map1,t_none()};
                                                {error,record,ErrorPat,ErrorType,_}->
                                                    Msg = {record_match,[format_patterns(ErrorPat), format_type(ErrorType,State1)]},
                                                    State2 = state__add_warning(State1,warn_matching,Tree,Msg),
                                                    {State2,Map1,t_none()};
                                                {Map2,ETypes}->
...
handle_clauses([C| Left],Arg,ArgType,OrigArgType,State,CaseTypes,MapIn,Acc,ClauseAcc,WarnAcc0) ->
    IsRaceAnalysisEnabled = is_race_analysis_enabled(State),
    State1 = case IsRaceAnalysisEnabled of
        true->
            {RaceList,RaceListSize} = get_race_list_and_size(State),
            state__renew_race_list([dialyzer_races:beg_clause_new(Arg,cerl:clause_pats(C),cerl:clause_guard(C))| RaceList],RaceListSize + 1,State);
        false->
            State
    end,
    {State2,ClauseMap,BodyType,NewArgType,WarnAcc} = do_clause(C,Arg,ArgType,OrigArgType,MapIn,State1,WarnAcc0),
    {NewClauseAcc,State3} = case IsRaceAnalysisEnabled of
        true->
            {RaceList1,RaceListSize1} = get_race_list_and_size(State2),
            EndClause = dialyzer_races:end_clause_new(Arg,cerl:clause_pats(C),cerl:clause_guard(C)),
            {[EndClause| ClauseAcc],state__renew_race_list([EndClause| RaceList1],RaceListSize1 + 1,State2)};
        false->
            {ClauseAcc,State2}
    end,
    {NewCaseTypes,NewAcc} = case t_is_none(BodyType) of
        true->
            {CaseTypes,Acc};
        false->
            {[BodyType| CaseTypes],[ClauseMap| Acc]}
    end,
    handle_clauses(Left,Arg,NewArgType,OrigArgType,State3,NewCaseTypes,MapIn,NewAcc,NewClauseAcc,WarnAcc);
handle_clauses([],_Arg,_ArgType,_OrigArgType,State,CaseTypes,_MapIn,Acc,ClauseAcc,WarnAcc) ->
    State1 = case is_race_analysis_enabled(State) of
        true->
            {RaceList,RaceListSize} = get_race_list_and_size(State),
            state__renew_race_list([dialyzer_races:end_case_new(ClauseAcc)| RaceList],RaceListSize + 1,State);
        false->
            State
    end,
...
do_clause(C,Arg,ArgType0,OrigArgType,Map,State,Warns) ->
    ...
    Map1 = if Arg =:= no_arg ->
        Map0;true ->
        bind_subst(Arg,Pats,Map0) end,
    BindRes = case t_is_none(ArgType0) of
        true->
            {error,bind,Pats,ArgType0,ArgType0};
        false->
            ArgTypes = case t_is_any(ArgType0) of
                true->
                    [ArgType0 || _ <- Pats];
                false->
                    t_to_tlist(ArgType0)
    ...
    case BindRes of
        {error,ErrorType,NewPats,Type,OpaqueTerm}->
            ...
            case state__warning_mode(State1) of
                ...
                true->
                    {Msg,Force} = case t_is_none(ArgType0) of
                        true->
                            OrigArgTypes = case t_is_any(OrigArgType) of
                                true->
                                    Any = t_any(),
                                    [Any || _ <- Pats];
                                false->
                            ...
                            PatTypes = case BindResOrig of
                                {error,opaque,_,_,OpaqueType}->
                                    [PatString, ArgTypeString, format_type(OpaqueType,State1)];
                                _->
                                    [PatString, ArgTypeString]
                            end,
                            {{Tag,PatTypes},false};
                        false->
                            Force0 = case is_compiler_generated(cerl:get_ann(C)) of
                                true->
                                    case Pats of
                                        [Pat]->
                                            case cerl:is_c_cons(Pat) of
                                                true->
                                                     not (cerl:is_c_var(cerl:cons_hd(Pat)) andalso cerl:is_c_var(cerl:cons_tl(Pat)) andalso cerl:is_literal(Guard) andalso cerl:concrete(Guard) =:= true);
                                                false->
                                                    true
                                            end;
                                        [Pat0, Pat1]->
                                            case cerl:is_c_cons(Pat0) of
                                                true->
                                                     not (cerl:is_c_var(cerl:cons_hd(Pat0)) andalso cerl:is_c_var(cerl:cons_tl(Pat0)) andalso cerl:is_c_var(Pat1) andalso cerl:is_literal(Guard) andalso cerl:concrete(Guard) =:= true);
                                                false->
                                                    true
                                            end;
                                        _->
                            ...
                            PatTypes = case ErrorType of
                                bind->
                                    [PatString, format_type(ArgType0,State1)];
                                record->
                                    [PatString, format_type(Type,State1)];
                                opaque->
                                    [PatString, format_type(Type,State1), format_type(OpaqueTerm,State1)]
                            end,
        ...
        {Map2,PatTypes}->
            Map3 = case Arg =:= no_arg of
                true->
                    Map2;
                false->
                    case bind_pat_vars_reverse([Arg],[t_product(PatTypes)],[],Map2,State1) of
                        {error,_,_,_,_}->
                            Map2;
                        {NewMap,_}->
                            NewMap
...
bind_subst_list([Arg| ArgLeft],[Pat| PatLeft],Map) ->
    NewMap = case {cerl:type(Arg),cerl:type(Pat)} of
        {var,var}->
            enter_subst(Arg,Pat,Map);
        {var,alias}->
        ...
        {literal,literal}->
            Map;
        {T,T}->
            bind_subst_list(lists:flatten(cerl:subtrees(Arg)),lists:flatten(cerl:subtrees(Pat)),Map);
        _->
            Map
    end,
...
bind_pat_vars(Pats,Types,Acc,Map,State) ->
    try bind_pat_vars(Pats,Types,Acc,Map,State,false)
        catch
            throw:Error->
                Error end.

bind_pat_vars_reverse(Pats,Types,Acc,Map,State) ->
    try bind_pat_vars(Pats,Types,Acc,Map,State,true)
        catch
            throw:Error->
                Error end.
...
bind_pat_vars([Pat| PatLeft],[Type| TypeLeft],Acc,Map,State,Rev) ->
    ok,
    Opaques = State#state.opaques,
    {NewMap,TypeOut} = case cerl:type(Pat) of
        alias->
        ...
        cons->
            ...
            case t_is_none(Cons) of
                ...
                false->
                    {Map1,[HdType, TlType]} = bind_pat_vars([cerl:cons_hd(Pat), cerl:cons_tl(Pat)],[t_cons_hd(Cons,Opaques), t_cons_tl(Cons,Opaques)],[],Map,State,Rev),
                ...
                false->
                    case Rev of
                        true->
                            {Map,MapT};
                        false->
                            FoldFun = fun (Pair,{MapAcc,ListAcc})->
                                exact = cerl:concrete(cerl:map_pair_op(Pair)),
                                Key = cerl:map_pair_key(Pair),
                                KeyType = case cerl:type(Key) of
                                    var->
                                ...
                                {MapAcc1,[ValType]} = bind_pat_vars([cerl:map_pair_val(Pair)],[Bind],[],MapAcc,State,Rev),
        ...
        tuple->
            ...
            {TypedRecord,Prototype} = case Es of
                []->
                    {false,t_tuple([])};
                [Tag| Left]->
            ...
            case t_is_none(Tuple) of
                ...
                false->
                    ...
                    Results = case Rev of
                        true->
                            [(bind_pat_vars_reverse(Es,t_tuple_args(SubTuple,Opaques),[],MapJ,State)) || SubTuple <- SubTuples];
                        false->
                            [(bind_pat_vars(Es,t_tuple_args(SubTuple,Opaques),[],MapJ,State)) || SubTuple <- SubTuples]
                    end,
                        ...
                        false->
                            case [M || {M,_} <- Results,M =/= error] of
                                ...
                                Maps->
                                    ...
                                    TupleType = t_sup([(t_tuple(EsTypes)) || {M,EsTypes} <- Results,M =/= error]),
                                    {Map1,TupleType}
                            end
                    end
            end;
        ...
        var->
            VarType1 = case state__lookup_type_for_letrec(Pat,State) of
                error->
                    lookup_type(Pat,Map);
                {ok,RecType}->
                    RecType
                        ...
                        List->
                            ...
                            case N >= 128 of
                                ...
                                false->
                                    case lists:member(signed,Flags) of
                                        true->
                                            t_from_range(-(1 bsl (N - 1)),1 bsl (N - 1) - 1);
                                        false->
                                            t_from_range(0,1 bsl N - 1)
                                    end
                            end
                    end
            end,
...
-spec(bind_opaque_pats(type(),type(),cerl:c_literal(),state()) -> no_return()).
...
bind_guard(Guard,Map,Env,Eval,State) ->
    ...
    case cerl:type(Guard) of
        ...
        'try'->
            ...
            {{HandlerMap,HandlerType},HandlerE} = try {bind_guard(cerl:try_handler(Guard),Map2,Env,Eval,State),none}
                catch
                    throw:HE->
                        {{Map2,t_none()},HE} end,
            ...
            case t_is_none(t_inf(HandlerType,Wanted)) of
                ...
                false->
                    {{BodyMap,BodyType},BodyE} = try {bind_guard(cerl:try_body(Guard),Map1,BodyEnv,Eval,State),none}
                        catch
                            throw:BE->
                                {{Map1,t_none()},BE} end,
                    ...
                    case t_is_none(Sup = t_sup(BodyType,HandlerType)) of
                        true->
                            ...
                            throw({Fatality,case {BodyE,HandlerE} of
                                {{_,Rsn},_}
                                    when Rsn =/= none->
                                    Rsn;
                                {_,{_,Rsn}}->
...
handle_guard_call(Guard,Map,Env,Eval,State) ->
    MFA = {cerl:atom_val(cerl:call_module(Guard)),cerl:atom_val(cerl:call_name(Guard)),cerl:call_arity(Guard)},
    case MFA of
        {erlang,F,1}
            when F =:= is_atom;
            F =:= is_boolean;
        ...
        MFA
            when (MFA =:= {erlang,internal_is_record,3}) or (MFA =:= {erlang,is_record,3})->
...
handle_guard_comp(Guard,Comp,Map,Env,Eval,State) ->
    ...
    case {type(Arg1),type(Arg2)} of
        ...
        {{literal,Lit1},var}
            when IsInt1 andalso IsInt2 andalso Eval =:= pos->
        ...
        {var,{literal,Lit2}}
            when IsInt1 andalso IsInt2 andalso Eval =:= pos->
            case bind_comp_literal_var(Lit2,Arg1,Type1,invert_comp(Comp),Map1,Opaques) of
                error->
                    signal_guard_fail(Eval,Guard,ArgTypes,State);
                {ok,NewMap}->
                    {NewMap,t_atom(true)}
...
bind_comp_literal_var(Lit,Var,VarType,CompOp,Map,Opaques) ->
    ...
    NewVarType = case t_number_vals(VarType,Opaques) of
        unknown->
            Range = case CompOp of
                '=<'->
...
handle_guard_is_function(Guard,Map,Env,Eval,State) ->
    ...
    case t_is_none(ArityType) of
        ...
        false->
            FunTypeConstr = case t_number_vals(ArityType,State#state.opaques) of
                unknown->
                    t_fun();
                Vals->
...
handle_guard_is_record(Guard,Map,Env,Eval,State) ->
    ...
    case t_is_none(t_inf(Tuple,RecType,Opaques)) of
        ...
        false->
            TupleType = case state__lookup_record(Tag,ArityMin1,State) of
                error->
                    Tuple;
    ...
    case {type(Arg1),type(Arg2)} of
        {{literal,Lit1},{literal,Lit2}}->
            case cerl:concrete(Lit1) =:= cerl:concrete(Lit2) of
                true->
                    if Eval =:= pos ->
                        {Map,t_atom(true)};Eval =:= neg ->
                        ArgTypes = [t_from_term(cerl:concrete(Lit1)), t_from_term(cerl:concrete(Lit2))],
                        signal_guard_fail(Eval,Guard,ArgTypes,State);Eval =:= dont_know ->
                        {Map,t_atom(true)} end;
                false->
                    if Eval =:= neg ->
                        {Map,t_atom(false)};Eval =:= dont_know ->
                        {Map,t_atom(false)};Eval =:= pos ->
                        ArgTypes = [t_from_term(cerl:concrete(Lit1)), t_from_term(cerl:concrete(Lit2))],
                        signal_guard_fail(Eval,Guard,ArgTypes,State) end
            end;
        {{literal,Lit1},_}
            when Eval =:= pos->
...
bind_eq_guard(Guard,Arg1,Arg2,Map,Env,Eval,State) ->
    ...
    Opaques = State#state.opaques,
    case t_is_nil(Type1,Opaques) orelse t_is_nil(Type2,Opaques) orelse t_is_atom(Type1,Opaques) orelse t_is_atom(Type2,Opaques) of
        true->
        ...
        {{literal,Lit1},{literal,Lit2}}->
            case cerl:concrete(Lit1) =:= cerl:concrete(Lit2) of
                true->
                    if Eval =:= neg ->
                        ArgTypes = [t_from_term(cerl:concrete(Lit1)), t_from_term(cerl:concrete(Lit2))],
                        signal_guard_fail(Eval,Guard,ArgTypes,State);Eval =:= pos ->
                        {Map,t_atom(true)};Eval =:= dont_know ->
                        {Map,t_atom(true)} end;
                false->
                    if Eval =:= neg ->
                        {Map,t_atom(false)};Eval =:= dont_know ->
...
handle_guard_and(Guard,Map,Env,Eval,State) ->
    ...
    case Eval of
        ...
        neg->
            MapJ = join_maps_begin(Map),
            {Map1,Type1} = try bind_guard(Arg1,MapJ,Env,neg,State)
                catch
                    throw:{fail,_}->
                        bind_guard(Arg2,MapJ,Env,pos,State) end,
            {Map2,Type2} = try bind_guard(Arg2,MapJ,Env,neg,State)
                catch
                    throw:{fail,_}->
                        bind_guard(Arg1,MapJ,Env,pos,State) end,
            case t_is_any_atom(false,Type1,Opaques) orelse t_is_any_atom(false,Type2,Opaques) of
                true->
        ...
        dont_know->
            ...
            case t_is_none(Bool1) orelse t_is_none(Bool2) of
                ...
                false->
                    ...
                    NewType = case {t_atom_vals(Bool1,Opaques),t_atom_vals(Bool2,Opaques)} of
                        {[true],[true]}->
                            t_atom(true);
                        {[false],_}->
                            t_atom(false);
...
handle_guard_or(Guard,Map,Env,Eval,State) ->
    ...
    case Eval of
        pos->
            MapJ = join_maps_begin(Map),
            {Map1,Bool1} = try bind_guard(Arg1,MapJ,Env,pos,State)
                catch
                    throw:{fail,_}->
                        bind_guard(Arg1,MapJ,Env,dont_know,State) end,
            {Map2,Bool2} = try bind_guard(Arg2,MapJ,Env,pos,State)
                catch
                    throw:{fail,_}->
                        bind_guard(Arg2,MapJ,Env,dont_know,State) end,
            case t_is_any_atom(true,Bool1,Opaques) andalso t_is_boolean(Bool2,Opaques) orelse t_is_any_atom(true,Bool2,Opaques) andalso t_is_boolean(Bool1,Opaques) of
                true->
...
handle_guard_map(Guard,Map,Env,State) ->
    ...
    case t_is_none_or_unit(ArgType1) of
        ...
        false->
            ...
            {Map2,lists:foldl(fun ({KV,assoc},Acc)->
                erl_types:t_map_put(KV,Acc);({KV,exact},Acc)->
                erl_types:t_map_update(KV,Acc) end,ArgType1,TypePairs)}
    end.
...
-spec(signal_guard_fail(eval(),cerl:c_call(),[type()],state()) -> no_return()).

signal_guard_fail(Eval,Guard,ArgTypes,State) ->
    signal_guard_failure(Eval,Guard,ArgTypes,fail,State).

-spec(signal_guard_fatal_fail(eval(),cerl:c_call(),[erl_types:erl_type()],state()) -> no_return()).
...
signal_guard_failure(Eval,Guard,ArgTypes,Tag,State) ->
    ...
    Opaques = State#state.opaques,
    {Kind,XInfo} = case erl_bif_types:opaque_args(M,F,A,ArgTypes,Opaques) of
        []->
            {case Eval of
                neg->
                    neg_guard_fail;
    ...
    FArgs = case is_infix_op(MFA) of
        true->
            [ArgType1, ArgType2] = ArgTypes,
            [Arg1, Arg2] = Args,
            [format_args_1([Arg1],[ArgType1],State), atom_to_list(F), format_args_1([Arg2],[ArgType2],State)] ++ XInfo;
        false->
...
bind_guard_case_clauses(Arg,Clauses,Map0,Env,Eval,State) ->
    ...
    bind_guard_case_clauses(GenArgType,GenMap,Arg,Clauses1,Map,Env,Eval,t_none(),[],[],State).

filter_fail_clauses([Clause| Left]) ->
    case cerl:clause_pats(Clause) =:= [] of
        true->
            Body = cerl:clause_body(Clause),
            case cerl:is_literal(Body) andalso cerl:concrete(Body) =:= fail orelse cerl:is_c_primop(Body) andalso cerl:atom_val(cerl:primop_name(Body)) =:= match_fail of
                true->
...
bind_guard_case_clauses(GenArgType,GenMap,ArgExpr,[Clause| Left],Map,Env,Eval,AccType,AccMaps,Throws,State) ->
    Pats = cerl:clause_pats(Clause),
    {NewMap0,ArgType} = case Pats of
        [Pat]->
            case cerl:is_literal(Pat) of
                true->
                    try case cerl:concrete(Pat) of
                        true->
    ...
    case NewMap1 =:= none orelse t_is_none(GenArgType) of
        true->
            bind_guard_case_clauses(NewGenArgType,GenMap,ArgExpr,Left,Map,Env,Eval,AccType,AccMaps,Throws,State);
        false->
            {NewAccType,NewAccMaps,NewThrows} = try {NewMap2,GuardType} = bind_guard(Guard,NewMap1,Env,pos,State),
            ...
            {NewMap3,CType} = bind_guard(cerl:clause_body(Clause),NewMap2,Env,Eval,State),
            Opaques = State#state.opaques,
            case Eval of
                pos->
            ...
            {t_sup(AccType,CType),[NewMap3| AccMaps],Throws}
                    ...
                    throw:{fail,Reason}->
                        ...
                        {AccType,AccMaps,Throws1} end,
            bind_guard_case_clauses(NewGenArgType,GenMap,ArgExpr,Left,Map,Env,Eval,NewAccType,NewAccMaps,NewThrows,State)
    end;
bind_guard_case_clauses(_GenArgType,_GenMap,_ArgExpr,[],Map,_Env,_Eval,AccType,AccMaps,Throws,_State) ->
...
join_maps_end(Maps,MapOut) ->
    ...
    true = lists:all(fun (M)->
        M#map.ref =:= Ref end,Maps),
    Keys0 = lists:usort(lists:append([(M#map.modified) || M <- Maps])),
    #map{map = Map,subst = Subst} = MapOut,
    Keys = [Key || Key <- Keys0,maps:is_key(Key,Map) orelse maps:is_key(Key,Subst)],
    Out = case Maps of
        []->
    ...
    Out#map{ref = R1,modified = Out#map.modified ++ M1,modified_stack = S}.
...
join_maps(Keys,Maps,MapOut) ->
    ...
    lists:foldl(fun ({K,T},M)->
        enter_type(K,T,M) end,MapOut,KTs).
...
debug_join_check(_Maps,_MapOut,_Out) ->
    ok.
                    ...
                    case maps:find(ValLabel,Subst) of
                        ...
                        error->
                            if KeyLabel =:= ValLabel ->
                                MS;true ->
                                ok,
                                store_subst(KeyLabel,ValLabel,MS) end
                    end
            end
    end.
...
lookup_fun_sig(Fun,Callgraph,Plt) ->
    MFAorLabel = case dialyzer_callgraph:lookup_name(Fun,Callgraph) of
        error->
            Fun;
        {ok,MFA}->
            MFA
...
mark_as_fresh([Tree| Left],Map) ->
    ...
    {SubTrees2,Map1} = case cerl:type(Tree) of
        bitstr->
...
t_is_simple(ArgType,State) ->
    Opaques = State#state.opaques,
    t_is_atom(ArgType,Opaques) orelse t_is_number(ArgType,Opaques) orelse t_is_port(ArgType,Opaques) orelse t_is_pid(ArgType,Opaques) orelse t_is_reference(ArgType,Opaques) orelse t_is_nil(ArgType,Opaques).
...
is_call_to_send(Tree) ->
    case cerl:is_c_call(Tree) of
        ...
        true->
            ...
            cerl:is_c_atom(Mod) andalso cerl:is_c_atom(Name) andalso is_send(cerl:atom_val(Name)) andalso cerl:atom_val(Mod) =:= erlang andalso Arity =:= 2
    end.
...
is_lc_simple_list(Tree,TreeType,State) ->
    Opaques = State#state.opaques,
    Ann = cerl:get_ann(Tree),
    lists:member(list_comprehension,Ann) andalso t_is_list(TreeType) andalso t_is_simple(t_list_elements(TreeType,Opaques),State).
...
state__new(Callgraph,Codeserver,Tree,Plt,Module,Records) ->
    ...
    ExportedFunctions = [Fun || Fun <- Funs -- [top],dialyzer_callgraph:is_escaping(Fun,Callgraph),dialyzer_callgraph:lookup_name(Fun,Callgraph) =/= error],
    Work = init_work(ExportedFunctions),
    Env = lists:foldl(fun (Fun,Env)->
        dict:store(Fun,map__new(),Env) end,dict:new(),Funs),
    #state{callgraph = Callgraph,codeserver = Codeserver,envs = Env,fun_tab = FunTab,fun_homes = FunHomes,opaques = Opaques,plt = Plt,races = dialyzer_races:new(),records = Records,warning_mode = false,warnings = [],work = Work,tree_map = TreeMap,module = Module,reachable_funs = sets:new()}.

state__warning_mode(#state{warning_mode = WM}) ->
    WM.

state__set_warning_mode(#state{tree_map = TreeMap,fun_tab = FunTab,races = Races,callgraph = Callgraph,reachable_funs = ReachableFuns} = State) ->
    ok,
    Funs = dict:fetch_keys(TreeMap),
    Work = [Fun || Fun <- Funs -- [top],dialyzer_callgraph:lookup_name(Fun,Callgraph) =/= error orelse sets:is_element(Fun,ReachableFuns)],
    State#state{work = init_work(Work),fun_tab = FunTab,warning_mode = true,races = dialyzer_races:put_race_analysis(true,Races)}.

state__race_analysis(Analysis,#state{races = Races} = State) ->
    State#state{races = dialyzer_races:put_race_analysis(Analysis,Races)}.

state__renew_curr_fun(CurrFun,CurrFunLabel,#state{races = Races} = State) ->
    State#state{races = dialyzer_races:put_curr_fun(CurrFun,CurrFunLabel,Races)}.
...
state__renew_race_list(RaceList,RaceListSize,#state{races = Races} = State) ->
    State#state{races = dialyzer_races:put_race_list(RaceList,RaceListSize,Races)}.
...
state__add_warning(#state{warning_mode = false} = State,_,_,_,_) ->
    State;
state__add_warning(#state{warnings = Warnings,warning_mode = true} = State,Tag,Tree,Msg,Force) ->
    Ann = cerl:get_ann(Tree),
    case Force of
        true->
            WarningInfo = {get_file(Ann,State),abs(get_line(Ann)),State#state.curr_fun},
        ...
        false->
            case is_compiler_generated(Ann) of
                true->
                    State;
                false->
                    WarningInfo = {get_file(Ann,State),get_line(Ann),State#state.curr_fun},
                    Warn = {Tag,WarningInfo,Msg},
                    case Tag of
                        warn_contract_range->
                            ok;
...
state__get_warnings(#state{tree_map = TreeMap,fun_tab = FunTab,callgraph = Callgraph,plt = Plt,reachable_funs = ReachableFuns} = State) ->
    FoldFun = fun ({top,_},AccState)->
        AccState;({FunLbl,Fun},AccState)->
        AccState1 = state__set_curr_fun(FunLbl,AccState),
        {NotCalled,Ret} = case dict:fetch(get_label(Fun),FunTab) of
        ...
        case NotCalled of
            ...
            false->
                {Name,Contract} = case dialyzer_callgraph:lookup_name(FunLbl,Callgraph) of
                    error->
                        {[],none};
                    {ok,{_M,F,A} = MFA}->
                ...
                case t_is_none(Ret) of
                    true->
                        ...
                        case Warn andalso (dialyzer_callgraph:lookup_name(FunLbl,Callgraph) =/= error orelse sets:is_element(FunLbl,ReachableFuns)) of
                            true->
...
state__lookup_record(Tag,Arity,#state{records = Records}) ->
    case erl_types:lookup_record(Tag,Arity,Records) of
        {ok,Fields}->
            RecType = t_tuple([t_atom(Tag)| [FieldType || {_FieldName,_Abstr,FieldType} <- Fields]]),
...
build_tree_map(Tree,Callgraph) ->
    Fun = fun (T,{Dict,Homes,FunLbls} = Acc)->
...
init_fun_tab([Fun| Left],Dict,TreeMap,Callgraph,Plt) ->
    ...
    FunEntry = case dialyzer_callgraph:is_escaping(Fun,Callgraph) of
        true->
...
state__clean_not_called(#state{fun_tab = FunTab} = State) ->
    NewFunTab = dict:map(fun (top,Entry)->
        Entry;(_Fun,{not_handled,{Args,_}})->
        {Args,t_none()};(_Fun,Entry)->
        Entry end,FunTab),
...
state__all_fun_types(State) ->
    ...
    List = [{Fun,t_fun(Args,Ret)} || {Fun,{Args,Ret}} <- dict:to_list(Tab1)],
    orddict:from_list(List).

state__fun_type(Fun,#state{fun_tab = FunTab}) ->
    Label = if is_integer(Fun) ->
        Fun;true ->
        get_label(Fun) end,
...
state__update_fun_entry(Tree,ArgTypes,Out0,#state{fun_tab = FunTab,callgraph = CG,plt = Plt} = State) ->
    Fun = get_label(Tree),
    Out1 = if Fun =:= top ->
        Out0;true ->
    ...
    SameArgs = lists:all(fun ({A,B})->
        erl_types:t_is_equal(A,B) end,lists:zip(OldArgTypes,ArgTypes)),
    ...
    SameOut ->
        ...
        state__add_work_from_fun(Tree,State1) end.

state__add_work_from_fun(_Tree,#state{warning_mode = true} = State) ->
    State;
state__add_work_from_fun(Tree,#state{callgraph = Callgraph,tree_map = TreeMap} = State) ->
        ...
        Label
            ...
            case dialyzer_callgraph:in_neighbours(Label,Callgraph) of
                none->
                    State;
                MFAList->
                    LabelList = [(dialyzer_callgraph:lookup_label(MFA,Callgraph)) || MFA <- MFAList],
                    FilteredList = [L || {ok,L} <- LabelList,dict:is_key(L,TreeMap)],
                    ok,
                    lists:foldl(fun (L,AccState)->
                        state__add_work(L,AccState) end,State,FilteredList)
            end
    end.
...
state__fun_info(Fun,#state{callgraph = CG,fun_tab = FunTab,plt = PLT}) ->
    {Sig,Contract} = case dialyzer_callgraph:lookup_name(Fun,CG) of
        error->
            {dialyzer_plt:lookup(PLT,Fun),none};
        {ok,MFA}->
            {dialyzer_plt:lookup(PLT,MFA),dialyzer_plt:lookup_contract(PLT,MFA)}
    end,
    LocalRet = case dict:fetch(Fun,FunTab) of
...
forward_args(Fun,ArgTypes,#state{work = Work,fun_tab = FunTab} = State) ->
    {NewArgTypes,OldOut,Fixpoint} = case dict:find(Fun,FunTab) of
        {ok,{not_handled,{_OldArgTypesAreNone,OldOut0}}}->
            {ArgTypes,OldOut0,false};
        {ok,{OldArgTypes0,OldOut0}}->
            NewArgTypes0 = [(t_sup(X,Y)) || {X,Y} <- lists:zip(ArgTypes,OldArgTypes0)],
            {NewArgTypes0,OldOut0,t_is_equal(t_product(NewArgTypes0),t_product(OldArgTypes0))}
    end,
    case Fixpoint of
        true->
            State;
...
state__cleanup(#state{callgraph = Callgraph,races = Races,records = Records}) ->
    #state{callgraph = dialyzer_callgraph:cleanup(Callgraph),races = dialyzer_races:cleanup(Races),records = Records}.
...
-spec(state__put_callgraph(dialyzer_callgraph:callgraph(),state()) -> state()).
...
renew_race_code(#state{races = Races,callgraph = Callgraph,warning_mode = WarningMode} = State) ->
    case WarningMode of
        true->
            State;
        false->
...
renew_race_public_tables([Var],#state{races = Races,callgraph = Callgraph,warning_mode = WarningMode} = State) ->
    case WarningMode of
        true->
            State;
        false->
            ...
            case Table of
                ...
                _Other->
                    ...
                    NewCallgraph = dialyzer_callgraph:renew_race_public_tables(VarLabel,Callgraph),
                    State#state{callgraph = NewCallgraph}
            end
    end.
...
-spec(format_args([cerl:cerl()],[type()],state()) -> nonempty_string()).
...
format_sig_args(Type,#state{opaques = Opaques} = State) ->
    ...
    case SigArgs of
        []->
            "()";
        [SArg| SArgs]->
            lists:flatten("(" ++ format_type(SArg,State) ++ [("," ++ format_type(T,State)) || T <- SArgs] ++ ")")
    end.
...
find_terminals(Tree) ->
    case cerl:type(Tree) of
        ...
        call->
            ...
            case {is_literal(M0),is_literal(F0)} of
                {{yes,LitM},{yes,LitF}}->
                    ...
                    case erl_bif_types:is_known(M,F,A) andalso t_is_none(erl_bif_types:type(M,F,A)) of
                        true->
                            {true,false};
                        false->
                            {false,true}
        ...
        'receive'->
            Timeout = cerl:receive_timeout(Tree),
            Clauses = cerl:receive_clauses(Tree),
            case cerl:is_literal(Timeout) andalso cerl:concrete(Timeout) =:= infinity of
                true->
                    if Clauses =:= [] ->
                        {false,true};true ->
                        find_terminals_list(Clauses) end;
                false->
                    find_terminals_list([cerl:receive_action(Tree)| Clauses])
            end;
        seq->
