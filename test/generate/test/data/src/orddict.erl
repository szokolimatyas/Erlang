...
erase(Key,[{K,_} = E| Dict])
    when Key < K->
    [E| Dict];
erase(Key,[{K,_} = E| Dict])
    when Key > K->
    [E| erase(Key,Dict)];
erase(_Key,[{_K,_Val}| Dict]) ->
    Dict;
erase(_,[]) ->
    [].
...
take_1(Key,[{K,_}| _],_Acc)
    when Key < K->
    error;
take_1(Key,[{K,_} = P| D],Acc)
    when Key > K->
    take_1(Key,D,[P| Acc]);
...
store(Key,New,[{K,_}| _] = Dict)
    when Key < K->
    [{Key,New}| Dict];
store(Key,New,[{K,_} = E| Dict])
    when Key > K->
...
append(Key,New,[{K,_}| _] = Dict)
    when Key < K->
    [{Key,[New]}| Dict];
append(Key,New,[{K,_} = E| Dict])
    when Key > K->
    [E| append(Key,New,Dict)];
append(Key,New,[{_K,Old}| Dict]) ->
    [{Key,Old ++ [New]}| Dict];
append(Key,New,[]) ->
    [{Key,[New]}].
...
append_list(Key,NewList,[{K,_}| _] = Dict)
    when Key < K->
    [{Key,NewList}| Dict];
append_list(Key,NewList,[{K,_} = E| Dict])
    when Key > K->
    [E| append_list(Key,NewList,Dict)];
append_list(Key,NewList,[{_K,Old}| Dict]) ->
...
update(Key,Fun,[{K,_} = E| Dict])
    when Key > K->
    [E| update(Key,Fun,Dict)];
update(Key,Fun,[{K,Val}| Dict])
    when Key == K->
    [{Key,Fun(Val)}| Dict].
...
update(Key,_,Init,[{K,_}| _] = Dict)
    when Key < K->
    [{Key,Init}| Dict];
update(Key,Fun,Init,[{K,_} = E| Dict])
    when Key > K->
    [E| update(Key,Fun,Init,Dict)];
update(Key,Fun,_Init,[{_K,Val}| Dict]) ->
    [{Key,Fun(Val)}| Dict];
update(Key,_,Init,[]) ->
    [{Key,Init}].
...
update_counter(Key,Incr,[{K,_}| _] = Dict)
    when Key < K->
    [{Key,Incr}| Dict];
update_counter(Key,Incr,[{K,_} = E| Dict])
    when Key > K->
    [E| update_counter(Key,Incr,Dict)];
update_counter(Key,Incr,[{_K,Val}| Dict]) ->
    [{Key,Val + Incr}| Dict];
...
merge(F,[{K1,_} = E1| D1],[{K2,_} = E2| D2])
    when K1 < K2->
    [E1| merge(F,D1,[E2| D2])];
merge(F,[{K1,_} = E1| D1],[{K2,_} = E2| D2])
    when K1 > K2->
    [E2| merge(F,[E1| D1],D2)];
merge(F,[{K1,V1}| D1],[{_K2,V2}| D2]) ->
