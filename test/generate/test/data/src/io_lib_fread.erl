...
-spec(fread(Continuation,String,Format) -> Return when Continuation::io_lib:continuation()|[],String::string()|eof,Format::string(),Return::{more,Continuation1::io_lib:continuation()}|{done,Result,LeftOverChars::string()},Result::{ok,InputList::[io_lib:fread_item()]}|eof|{error,{fread,What::io_lib:fread_error()}}).
...
-spec(fread(Format,String) -> Result when Format::string(),String::string(),Result::{ok,InputList::[io_lib:fread_item()],LeftOverChars::string()}|{more,RestFormat::string(),Nchars::non_neg_integer(),InputStack::io_lib:chars()}|{error,{fread,What::io_lib:fread_error()}}).
...
fread([C| Format],Line,N,Results)
    when C =:= $  orelse C =:= $\t orelse C =:= $\r orelse C =:= $\n->
...
fread_field([C| Format],F,Sup,Unic)
    when C >= $0,
    C =< $9->
...
fread1([$#| Format],none,Sup,false,Line0,N0,Res) ->
    case  catch begin {Line1,N1,B1} = fread_base(Line0,N0),
    B = abs(B1),
    true = (B >= 2) and (B =< 1 + $Z - $A + 10),
    {Line2,N2,Cs2} = fread_digits(Line1,N1,B,[]),
    fread_based(reverse(Cs2),B1,Sup,Format,Line2,N2,Res) end of
        {'EXIT',_}->
            fread_error(based);
        Other->
            Other
...
fread1([$#| Format],F,Sup,false,Line0,N,Res) ->
    case  catch begin {Line1,Cs1} = fread_chars(Line0,F,false),
    {Line2,_,B2} = fread_base(reverse(Cs1),N),
    true = (B2 >= 2) and (B2 =< 1 + $Z - $A + 10),
    fread_based(Line2,B2,Sup,Format,Line1,N + F,Res) end of
        {'EXIT',_}->
            fread_error(based);
        Other->
            Other
...
fread_integer(Cs,Base,Sup,Format,Line,N,Res) ->
    case  catch list_to_integer(fread_skip_white(reverse(Cs)),Base) of
        {'EXIT',_}->
            fread_error(integer);
        Integer->
...
fread_skip_latin1_nonwhite([C| Line],N,Cs)
    when C =:= $  orelse C =:= $\t orelse C =:= $\r orelse C =:= $\n->
    {[C| Line],N,Cs};
fread_skip_latin1_nonwhite([C| Line],N,[])
    when C > 255->
    {[C| Line],N,error};
fread_skip_latin1_nonwhite([C| Line],N,Cs)
    when C > 255->
