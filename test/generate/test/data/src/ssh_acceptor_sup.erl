...
start_link(Address,Port,Profile,Options) ->
    supervisor:start_link(ssh_acceptor_sup,[Address, Port, Profile, Options]).
...
child_spec(Address,Port,Profile,Options) ->
    Timeout = ssh_options:get_value(internal_options,timeout,Options,fun ()->
        50000 end,ssh_acceptor_sup,84),
    #{id=>id(Address,Port,Profile),start=>{ssh_acceptor,start_link,[Port, Address, Options, Timeout]},restart=>transient}.
