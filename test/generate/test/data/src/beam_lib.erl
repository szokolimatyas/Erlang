...
-export([info/1, cmp/2, cmp_dirs/2, chunks/2, chunks/3, all_chunks/1, diff_dirs/2, strip/1, strip/2, strip_files/1, strip_files/2, strip_release/1, strip_release/2, significant_chunks/0, build_module/1, version/1, md5/1, format_error/1]).

-export([crypto_key_fun/1, clear_crypto_key_fun/0]).

-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2, code_change/3]).

-export([make_crypto_key/2, get_crypto_key/1]).

-export_type([attrib_entry/0, compinfo_entry/0, labeled_entry/0, label/0]).

-import(lists, [append/1, delete/2, foreach/2, keysort/2, member/2, reverse/1, sort/1, splitwith/2]).
...
-type(chunkname()::abstract_code|debug_info|attributes|compile_info|exports|labeled_exports|imports|indexed_imports|locals|labeled_locals|atoms).
...
-type(chnk_rsn()::{unknown_chunk,file:filename(),atom()}|{key_missing_or_invalid,file:filename(),abstract_code|debug_info}|info_rsn()).

-type(cmp_rsn()::{modules_different,module(),module()}|{chunks_different,chunkid()}|different_chunks|info_rsn()).
...
-spec(all_chunks(beam()) -> {ok,beam_lib,[{chunkid(),dataB()}]}|{error,beam_lib,info_rsn()}).
...
format_error({chunk_too_big,File,ChunkId,Size,Len}) ->
    io_lib:format("~tp: Size of chunk \"~s\" is ~p bytes, but only ~p b" "ytes could be read~n",[File, ChunkId, Size, Len]);
...
format_error({key_missing_or_invalid,File,ChunkId}) ->
    io_lib:format("~tp: Cannot decrypt ~ts because key is missing or in" "valid",[File, ChunkId]);
format_error(badfun) ->
...
-spec(make_crypto_key(mode(),string()) -> {mode(),[binary()],binary(),integer()}).
...
read_info(File) ->
    try {ok,Module,Data} = scan_beam(File,info),
    [if is_binary(File) ->
        {binary,File};true ->
        {file,File} end, {module,Module}, {chunks,Data}]
        catch
            throw:Error->
                Error end.
...
diff_directories(Dir1,Dir2) ->
    ...
    foreach(fun (D)->
        io:format("** different: ~tp~n",[D]) end,Diff),
    ok.
...
cmp_files(File1,File2) ->
    ...
    if M1 =:= M2 ->
        cmp_lists(L1,L2);true ->
        error({modules_different,M1,M2}) end.

cmp_lists([],[]) ->
    ok;
cmp_lists([{Id,C1}| R1],[{Id,C2}| R2]) ->
    if C1 =:= C2 ->
        cmp_lists(R1,R2);true ->
        error({chunks_different,Id}) end;
cmp_lists(_,_) ->
...
strip_fils(Files,AdditionalChunks) ->
    {ok,[begin {ok,Reply} = strip_file(F,AdditionalChunks),
    Reply end || F <- Files]}.
...
read_all_but_useless_chunks(File0)
    ...
    ChunkIds = [Name || {Name,_,_} <- ChunkIds0, not is_useless_chunk(Name)],
    {ok,Module,Chunks} = scan_beam(File,ChunkIds),
    {ok,{Module,lists:reverse(Chunks)}}.

is_useless_chunk("CInf") ->
    true;
is_useless_chunk(_) ->
    false.
...
filter_funtab_1(Tail,_)
    when is_binary(Tail)->
    [Tail].

read_all_chunks(File0)
    when is_atom(File0);
    is_list(File0);
    is_binary(File0)->
    try File = beam_filename(File0),
    {ok,Module,ChunkIds0} = scan_beam(File,info),
...
read_chunk_data(File0,ChunkNames0,Options)
    ...
    try chunks_to_data(Names,Chunks,File,Chunks,Module,T,[])
        after ets:delete(AT) end.

check_chunks([atoms| Ids],File,IL,L,O) ->
    check_chunks(Ids,File,["Atom", "AtU8"| IL],[{atom_chunk,atoms}| L],["Atom", "AtU8"| O]);
check_chunks([abstract_code| Ids],File,IL,L,O) ->
    check_chunks(Ids,File,["Abst", "Dbgi"| IL],[{abst_chunk,abstract_code}| L],["Abst", "Dbgi"| O]);
check_chunks([ChunkName| Ids],File,IL,L,O)
    when is_atom(ChunkName)->
    ChunkId = chunk_name_to_id(ChunkName,File),
    check_chunks(Ids,File,[ChunkId| IL],[{ChunkId,ChunkName}| L],O);
check_chunks([ChunkId| Ids],File,IL,L,O) ->
...
scan_beam(File,What0,AllowMissingChunks,OptionalChunks) ->
    case scan_beam1(File,What0) of
        {missing,_FD,Mod,Data,What}
            when AllowMissingChunks->
        ...
        {missing,FD,Mod,Data,What}->
            case What -- OptionalChunks of
                []->
                    {ok,Mod,Data};
                [Missing| _]->
                    error({missing_chunk,filename(FD),Missing})
            end;
        R->
            R
    end.
...
chunks_to_data([{abst_chunk,Name}| CNs],Chunks,File,Cs,Module,Atoms,L) ->
    ...
    {NewAtoms,Ret} = case  catch chunk_to_data(debug_info,DbgiChunk,File,Cs,Atoms,Module) of
...
chunk_to_data(attributes = Id,Chunk,File,_Cs,AtomTable,_Mod) ->
    try Term = binary_to_term(Chunk),
    {AtomTable,{Id,attributes(Term)}}
        catch
            error:badarg->
                error({invalid_chunk,File,chunk_name_to_id(Id,File)}) end;
chunk_to_data(compile_info = Id,Chunk,File,_Cs,AtomTable,_Mod) ->
    try {AtomTable,{Id,binary_to_term(Chunk)}}
        catch
            error:badarg->
                error({invalid_chunk,File,chunk_name_to_id(Id,File)}) end;
        ...
        _->
            case  catch binary_to_term(Chunk) of
                {'EXIT',_}->
                    error({invalid_chunk,File,chunk_name_to_id(Id,File)});
                Term->
                    try {AtomTable,{Id,old_anno_from_term(Term)}}
                        catch
                            _:_->
                                error({invalid_chunk,File,chunk_name_to_id(Id,File)}) end
            end
...
chunk_to_data(ChunkName,Chunk,File,Cs,AtomTable,_Mod)
    when is_atom(ChunkName)->
...
chunk_to_data(ChunkId,Chunk,_File,_Cs,AtomTable,_Module)
    when is_list(ChunkId)->
    {AtomTable,{ChunkId,Chunk}}.

chunk_name_to_id(indexed_imports,_) ->
    "ImpT";
chunk_name_to_id(imports,_) ->
    "ImpT";
chunk_name_to_id(exports,_) ->
    "ExpT";
chunk_name_to_id(labeled_exports,_) ->
    "ExpT";
chunk_name_to_id(locals,_) ->
    "LocT";
chunk_name_to_id(labeled_locals,_) ->
    "LocT";
chunk_name_to_id(attributes,_) ->
    "Attr";
chunk_name_to_id(abstract_code,_) ->
    "Abst";
chunk_name_to_id(debug_info,_) ->
    "Dbgi";
chunk_name_to_id(compile_info,_) ->
    "CInf";
...
-record(bb,{pos = 0::integer(),bin::binary(),source::binary()|string()}).

open_file(<<"FOR1",_/binary>> = Binary) ->
    #bb{bin = Binary,source = Binary};
open_file(Binary0)
    when is_binary(Binary0)->
...
decrypt_chunk(Type,Module,File,Id,Bin) ->
    try KeyString = get_crypto_key({debug_info,Type,Module,File}),
    ...
    binary_to_term(NewBin)
            ...
            _:_->
                error({key_missing_or_invalid,File,Id}) end.

old_anno_from_term({raw_abstract_v1,Forms}) ->
    {raw_abstract_v1,anno_from_forms(Forms)};
old_anno_from_term({Tag,Forms})
    when Tag =:= abstract_v1;
    Tag =:= abstract_v2->
...
call_crypto_server(Req) ->
    try gen_server:call(beam_lib__crypto_key_server,Req,infinity)
        catch
            exit:{noproc,_}->
                call_crypto_server_1(Req);
            exit:{normal,_}->
                call_crypto_server_1(Req) end.

call_crypto_server_1(Req) ->
    case gen_server:start({local,beam_lib__crypto_key_server},beam_lib,[],[]) of
        {ok,_}->
            ok;
        {error,{already_started,_}}->
            ok
...
-type(calls()::clear_crypto_key_fun|{crypto_key_fun,_}|{get_crypto_key,_}).

-spec(handle_call(calls(),{pid(),term()},#state{}) -> {noreply,#state{}}|{reply,error|{error,badfun|exists},#state{}}|{stop,normal,undefined|{ok,term()},#state{}}).
...
handle_call({crypto_key_fun,F},{_,_} = From,S) ->
    case S#state.crypto_key_f of
        undefined->
            if is_function(F,1) ->
                {Result,Fun,Reply} = case  catch F(init) of
                    ok->
                        {true,F,ok};
                    {ok,F1}
                        when is_function(F1)->
                        if is_function(F1,1) ->
                            {true,F1,ok};true ->
                            {false,undefined,{error,badfun}} end;
                    {error,Reason}->
                ...
                {noreply,NewS};true ->
                {reply,{error,badfun},S} end;
        OtherF
            when is_function(OtherF)->
...
f_p_s(P,F) ->
    case file:path_script(P,F) of
        ...
        {error,{Line,_Mod,_Term} = E}->
            error("file:path_script(~tp,~tp): error on line ~p: ~ts~n",[P, F, Line, file:format_error(E)]),
            ok;
        {error,E}
            when is_atom(E)->
            error("file:path_script(~tp,~tp): ~ts~n",[P, F, file:format_error(E)]),
            ok;
        Other->
            Other
    end.
...
try_load_crypto_fun(KeyInfo)
    when is_list(KeyInfo)->
    T = ets:new(keys,[private, set]),
    foreach(fun ({debug_info,Mode,M,Key})
        when is_atom(M)->
        ets:insert(T,{{debug_info,Mode,M,[]},Key});({debug_info,Mode,[],Key})->
        ets:insert(T,{{debug_info,Mode,[],[]},Key});(Other)->
        error("unknown key: ~p~n",[Other]) end,KeyInfo),
    fun ({debug_info,Mode,M,F})->
        alt_lookup_key([{debug_info,Mode,M,F}, {debug_info,Mode,M,[]}, {debug_info,Mode,[],[]}],T);(clear)->
        ets:delete(T);(_)->
        error end;
try_load_crypto_fun(KeyInfo) ->
